...
function handle_class_input() {
  if { [ -n "$last_class" ] || [ -z "$last_topic" ]; } && [ -f .chemistry_advanced_surveyor ]; then
		while true; do
	    read -rp $'\n\nPlease enter '"${r}5 or 6${t}"' to go to your class or '"${r}x${t}"' to exit'$'\n\n> ' class
      if [ "$class" == "x" ]; then
        exit
      fi
		  # Check if .current_chemistry_advanced_class is accidentally empty
		  if [ ! -s .current_chemistry_advanced_class ]; then
		    # echo 5 to .current_chemistry_advanced_class
		    echo "5" > .current_chemistry_advanced_class
		  fi
		  # Read the value from the file
		  current_chemistry_advanced_class=$(<.current_chemistry_advanced_class) 2>/dev/null
		  # Check if the value is equal to $class
		  if [ "$current_chemistry_advanced_class" -lt "$class" 2>/dev/null ]; then
	      ...
	      rm -f ".chemistry_advanced_topic_selected"
	      # Update the state file with the class
	      if [ "$class" != "x" ]; then
	        echo "$class" > .chemistry_advanced_user_state 2>/dev/null
	      fi
				break
	  	fi
		done
...
function handle_s5_topic_input() {
  ...
  if [ -z "$last_topic" ] || [ -f .chemistry_advanced_topic_selected ]; then
    read -rp $'\n\nChoose either topic '"${g}1 or 2 or 3 or 4 or 5 or 6 or 7${t}"' to learn'$'\nor enter '"${r}z${t}"' for an adventure or '"${r}r${t}"' to revise or '"${r}s${t}"' to get sample_items'$'\nor '"${r}a${t}"' to get an activity of integration or '"${r}q${t}"' to get a short answer question'$'\nor '"${r}n${t}"' to do your final class assignment and if necessary, gain access to the next class'$'\nor '"${r}p${t}"' to track academic progress or '"${r}x${t}"' to exit'$'\n\n1. Moles and equations '"${r}Term1${t}"''$'\n\n2. Atomic and electronic structure '"${r}Term1${t}"''$'\n\n3. Bonding and structure '"${r}Term1${t}"''$'\n\n4. Periodicity I '"${r}Term2${t}"''$'\n\n5. Thermochemistry '"${r}Term2${t}"''$'\n\n6. Organic chemistry I '"${r}Terms 2&3${t}"''$'\n\n7. Equilibria I '"${r}Term3${t}"''$'\n\n> ' topic
    touch .chemistry_advanced_surveyor
    touch .chemistry_advanced_topic_selected
    # Update the state file with the topic
    # Check if the state file exists, and the topic is not "x"
...
process_reminders_from_file() {
  ...
  prev_sentence=""
  echo_next=false
	echo -n > .chemistry_advanced_reminder
    ...
    for sentence in "${sentences[@]}"; do
      if [[ $echo_next == true ]]; then
        ...
        for split_sentence in "${split_sentences[@]}"; do
					echo "$split_sentence" >> .chemistry_advanced_reminder
        done
      echo_next=false
      fi
        ...
        echo "Did you know that $lower_cased_sentence" >> .chemistry_advanced_reminder
        ...
        echo_next=true
  			...
  			echo "Do you recall that $lower_cased_sentence;" >> .chemistry_advanced_reminder

			elif [[ "$sentence" =~ " → " ]]; then
  			echo "Hope you know that: $sentence;" >> .chemistry_advanced_reminder

      elif [[ "$sentence" =~ " ↔ " ]]; then
        echo "Hope you know that: $sentence;" >> .chemistry_advanced_reminder

      elif [[ "$sentence" =~ (Generally|general) ]]; then
        echo "Note: $sentence;" >> .chemistry_advanced_reminder
...
process_random_reminder() {
  ...
  # Check if .chemistry_advanced_reminder exists
  if [ -f .chemistry_advanced_reminder ]; then
		# Remove empty lines from .chemistry_advanced_reminder
  	sed -i '/^[[:space:]]*$/d' .chemistry_advanced_reminder 2>/dev/null
    # Randomly select a non-empty sentence
    local reminder # Declare the variable
		reminder=$(awk -v RS=';' 'BEGIN{srand();}{gsub(/^[[:space:]]+|[[:space:]]+$/, ""); if (length > 0) a[++n]=$0}END{if (n > 0) print a[int(rand()*n)+1]}' .chemistry_advanced_reminder)
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    for sentence in "${sentences[@]}"; do
        ...
        if [[ $sentence == *"Figure"* ]]; then
          ...
          # Change to the "Figures/Chemistry_advanced" directory
          cd Figures/Chemistry_advanced || { echo "Failed to change to Figures/Chemistry_advanced"; return; }
        ...
        if [[ $sentence == *"Table"* ]]; then
          cd Tables/Chemistry_advanced || { echo -e "\nFailed to change to Tables/Chemistry_advanced \c"; return; }
          explorer.exe "$sentence" > /dev/null 2>&1 &
          cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        fi

        if [[ $sentence == *"Video"* ]]; then
          cd Videos/Chemistry_advanced || { echo -e "\nFailed to change to Videos/Chemistry_advanced \c"; return; }
          explorer.exe "$sentence" > /dev/null 2>&1 &
          cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        fi
...
contact_ai() {
  last_topic=$(awk -F' ' '{print $2}' .chemistry_advanced_user_state)
...
while true; do
  ...
  # Prompt the user for input
  if [ -z "$(find ./Notes/Chemistry_advanced -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
  ...
  if [[ -n "$user_input" ]]; then
		if [[ "$user_input" == "sh" ]]; then
		  Response="We are looking forward to receiving your economic support, thoughts, suggestions, and any files you feel should reach out to everyone of our children. Please remember to label the files you are to attach using the format: Your_name_School_Chemistry_advanced_File_content (e.g., Muhumuza_Omega_Kasule_High_School_O_level_Chemistry_Answered_EOC1_Items.pdf). Thanks a lot for your contributions."  
		  chemistry_advanced="$(basename "$0") - $(date +"%Y-%m-%d %H:%M:%S") - Thoughts, suggestions, and contributions"
		  encoded_chemistry_advanced=$(echo "$chemistry_advanced" | sed 's/ /%20/g; s/\n/%0A/g')
		  encoded_body=$(echo "$Response" | sed 's/ /%20/g; s/\n/%0A/g')
		  # Open the email in the browser with the encoded chemistry_advanced and body
		  powershell.exe -Command "Start-Process 'https://mail.google.com/mail/?view=cm&to=2024omd256@gmail.com&su=${encoded_chemistry_advanced}&body=${encoded_body}'"
			return
		fi
     ...
     if [[ "$user_input" == "sr" ]]; then
      ...
      explorer.exe Chemistry_advanced > /dev/null 2>&1 &
    ...
    if [[ -n "$result" ]]; then
      ...
      if echo "$result" | grep "Figure"; then
        echo "$result" | grep -o '\bFigure[0-9]\+.*\.jpg\(\.[0-9]\+\)*\b' > Figures/Chemistry_advanced/figures.txt
        sed -i -e '/^Figure/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Figures/Chemistry_advanced/figures.txt
        # Change to the "Figures/Chemistry_advanced" directory
        cd Figures/Chemistry_advanced || { echo "Failed to change to Figures/Chemistry_advanced"; return; }
        ...
        rm -f Figures/Chemistry_advanced/figures.txt
      fi

      if echo "$result" | grep "Table"; then
        echo "$result" | grep -o '\bTable[0-9]\+\(\.[0-9]\+\)*\b' > Tables/Chemistry_advanced/tables.txt
        sed -i -e '/^Table/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Tables/Chemistry_advanced/tables.txt
        cd Tables/Chemistry_advanced || { echo -e "\nFailed to change to Tables/Chemistry_advanced \c"; return; }
        ...
        # Go back to the original directory
        cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        rm -f Tables/Chemistry_advanced/tables.txt
      fi

      if echo "$result" | grep "Video"; then
        #echo "$result" > Videos/Chemistry_advanced/videos.txt
        echo "$result" | grep -o '\bVideo[0-9]\+\(\.[0-9]\+\)*\b' > Videos/Chemistry_advanced/videos.txt
        # Remove lines not starting with "Video" and any leading/trailing whitespaces
        sed -i -e '/^Video/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Videos/Chemistry_advanced/videos.txt
        # Change to the "Videos/Chemistry_advanced" directory
        cd Videos/Chemistry_advanced || { echo -e "\nFailed to change to Videos/Chemistry_advanced \c"; return; }
        ...
        cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        # Remove the temporary file
        rm -f Videos/Chemistry_advanced/videos.txt
      fi
  ...
  if [ -f "$selected_file" ]; then
    ...
    if [[ -n "$selected_question" && "$selected_question" =~ [[:graph:]] ]]; then
      ...
      if echo "$selected_question" | grep "Figure"; then
        echo "$selected_question" | grep -o '\bFigure[0-9]\+[^;]*\b' > ../../Figures/Chemistry_advanced/figures.txt
        sed -i -e '/^Figure/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' ../../Figures/Chemistry_advanced/figures.txt
        # Change to the "Figures/Chemistry_advanced" directory
        cd ../../Figures/Chemistry_advanced || { echo "Failed to change to Figures/Chemistry_advanced"; return; }
        # Specify the path to the text file containing figure names
        ...
        # Go back to the original directory
        cd ../../Revision/Chemistry_advanced || { echo "Failed to change back to the targeted directory \c"; exit 1; }
        # Remove the temporary file
        rm -f ../../Figures/Chemistry_advanced/figures.txt
      fi
...
process_question_answer() {
  ...
  if [ -f ../../../.school_name ]; then
    ...
    sed -i '/^[[:space:]]*$/d' ../../../."$school_name"_students_file.txt
    existing_class=$(awk '{print $1}' ../../../.chemistry_advanced_user_state)
    existing_topic=$(awk '{print $2}' ../../../.chemistry_advanced_user_state)
    echo ''
    ...
    if grep -q "Chemistry_advanced" ../../../."$school_name"_students_file.txt; then
      sed -i -E 's/;/\n/g' ../../../."$school_name"_students_file.txt
      # Find the line with the word Chemistry_advanced, replace the information below it with the already available information adding a comma existing_class_existing_topic [$percentage]
      if [ "$existing_topic" == "r" ]; then
        sed -i '/Chemistry_advanced/{n;s/\(.*\)/\1, '"$existing_class"'_'"$input"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
      else
        (( existing_topic-- ))
        sed -i '/Chemistry_advanced/{n;s/\(.*\)/\1, '"$existing_class"'_'"$existing_topic"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
      fi
    else
      if [ "$existing_topic" == "r" ]; then
        sed -i -E '/^School/ i\Chemistry_advanced\n'"${existing_class} ${names}[${percentage}%]"'' ../../../."$school_name"_students_file.txt
        echo -e "Chemistry_advanced\n"$existing_class"_'"$input"' "$names"["$percentage%"]" >> ../../../."$school_name"_students_file.txt
      else
        (( existing_topic-- ))
...
function resume_from_last_point() {
  if [ -f .chemistry_advanced_user_state ]; then
    last_class=$(awk -F' ' '{print $1}' .chemistry_advanced_user_state)
    last_topic=$(awk -F' ' '{print $2}' .chemistry_advanced_user_state)
    if [ -n "$last_class" ] && [ -n "$last_topic" ]; then
      echo -e "\n       Resuming from ${r}S$last_class${t} : ${g}Topic '$last_topic'${t} \c"
      rm -f .chemistry_advanced_surveyor
...
process_final_assignment() {
  # Check if user is.chemistry_advanced_ready for the assignment
  # Check if .current_chemistry_advanced_class is accidentally empty
  if [ ! -s .current_chemistry_advanced_class ]; then
    # echo 5 to .current_chemistry_advanced_class
    echo "5" > .current_chemistry_advanced_class
  fi
  # Read the value from the .current_chemistry_advanced_class file
  current_chemistry_advanced_class=$(<.current_chemistry_advanced_class) 2>/dev/null
  # Check if the value in the .chemistry_advanced_ready file is equal to $class
  # Read the value from the .chemistry_advanced_ready file
  if [ ! -s .chemistry_advanced_ready ]; then
    echo "0" > .chemistry_advanced_ready
  fi
  how.chemistry_advanced_ready=$(<.chemistry_advanced_ready) 2>/dev/null
  # Check if the value is equal to $class
  if [ "$how.chemistry_advanced_ready" -lt "$current_chemistry_advanced_class" 2>/dev/null ]; then
    read -rp $'\n\nYou havent done all the topic assignments for your current chemistry_advanced class\n\n'"${r}Proceeding from here will affect your very final score${y}"'. To go back and progress right, enter '"${y}ok${t}"'. Otherwise, press the Enter key to do the final class assignment: ' progress
    if [ "$progress" == "ok" ]; then
      return
    fi
  fi
    ...
    if ! [ "$total_questions" == 100 ]; then
      ...
      if [ "$elapsed_time" -gt 3600 ]; then
        ...
        if [ "$percentage" -lt 95 ]; then
          ...
          cd - > /dev/null 2>&1 || exit
          if ! [ -f .current_chemistry_advanced_class ]; then
            # Echo the result to the file .current_class
            echo "2" > .current_chemistry_advanced_class
            ...
            # Read the value from the file
            current_chemistry_advanced_class=$(<.current_chemistry_advanced_class) 2>/dev/null
            # Check if the new_class value is lt $current_chemistry_advanced_class
            if [ "$new_class" -gt "$current_chemistry_advanced_class" 2>/dev/null ]; then
		          # Echo the result to the file .current_class
		          echo "$new_class" > .current_chemistry_advanced_class
            fi
...
get_sample_items() {
  ...
  # Save the current working directory
  pushd . > /dev/null
  cd Revision/Chemistry_advanced || { echo "Directory not found"; return; }
  ...
  while true; do
      ...
      if [ ! -f "$last_echo_time_file" ]; then
    	...
    	if [ $time_diff -gt 3600 ]; then
      	# Echo the message and update the last echo time
	    	echo -e "\n\n\n${r}You are advised to not make any changes to the provided answers, instead, you can make copies that you can edit${t}\n\n${y}For a teacher willing to join us reach out to everyone of our children, please send us your questions and answers in a file labelled with your name, school, chemistry_advanced, and file content (e.g., Muhumuza_Omega_Kasule_High_School_O_level_Chemistry_Answered_EOC1_Items.pdf) to our contacts${t}\n\n\nEmail: ${g}2024omd256@gmail.com${t} \c"
				echo $current_time > "$last_echo_time_file"
  		fi
      read -rp $'\n\n\nEnter '"${m}any character${t}"' for access to the file of answered items or simply press '"${r}Enter${t}"' to get items to attempt : ' input
     	if [[ -n $input ]]; then
     		explorer.exe .chemistry_advanced_samples* > /dev/null 2>&1 &
      	clear
      	popd > /dev/null || exit
        return
      fi
      if [ -f .e_o_c_chemistry_advanced.txt ]; then
        echo -e "\n\n${y}Below is the list of the elements of construct${t} \n"
        cat .e_o_c_chemistry_advanced.txt
        ...
        if [[ -n $input ]]; then
          ...
          selected_file1=$(ls -a | grep -E "\.e_o_c_chemistry_advanced_${input}\.txt")
          cat "$selected_file1"
          # Remove empty lines from the selected files
          find . -type f -name "*_samples_[0-9].txt" -exec sed -i '/^[[:space:]]*$/d' {} +
          selected_file=$(ls -a | grep -E "\.e_o_c_chemistry_advanced_${input}_samples" | shuf -n 1)
        else
    ...
    rm -f .revise.txt .chemistry_advanced_topic_selected 2>/dev/null
  done
  popd > /dev/null || exit
	return
}
...
while true; do
  ...
  if ls Students | grep -wq "$initials"; then
    ...
    echo -e "                  $initials\n" > "Students/$initials/Exercise/chemistry_advanced_answered_ans.txt"
    echo -e "                  $initials\n" > "Students/$initials/Revision/chemistry_advanced_covered_qns.txt"
    ...
    if [ -f ../../../.school_name ]; then
      ...
      if grep -q "Chemistry_advanced" ../../../."$school_name"_students_file.txt; then
        ...
        if [ "$existing_topic" == "z" ]; then
          sed -i '/Chemistry_advanced/{n;s/\(.*\)/\1, '"$existing_class"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
        else
          (( existing_topic-- ))
          sed -i '/Chemistry_advanced/{n;s/\(.*\)/\1, '"$existing_class"'_'"$existing_topic"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
        fi
...
if [ ! -f .chemistry_advanced_user_state ]; then
	touch .chemistry_advanced_user_state
	touch .chemistry_advanced_surveyor
	get_and_display_pattern
else
  if [ -f .omd_communication.txt ]; then
    ...
    process_random_reminder .chemistry_advanced_reminder
  fi
	handle_resume_input
fi

if [ -z "$class" ] && [ -s ".chemistry_advanced_user_state" ]; then
  get_and_display_pattern
fi
# Check for the presence of specific directories and a file
if ! [ -d "Notes/Chemistry_advanced" ] || ! [ -d "Revision" ] || ! [ -d "Exercise" ] || ! [ -d "Videos" ] || ! [ -d "Figures" ] || ! [ -d "Tables" ]; then
  ...
  mkdir -p Notes Notes/Chemistry_advanced Revision Revision/Chemistry_advanced Revision/Chemistry_advanced/{S5,S6} Exercise Exercise/Chemistry_advanced Exercise/Chemistry_advanced/{S5,S6} Videos Videos/Chemistry_advanced Figures Figures/Chemistry_advanced Tables Tables/Chemistry_advanced
  touch Revision/chemistry_advanced_covered_qns.txt Exercise/chemistry_advanced_answered_ans.txt
  echo -e "\n"
  pwd
...
if [ -z "$(find ./Notes/Chemistry_advanced -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
  ...
  if [[ -z "$user_input" ]]; then
    ...
    curl -O -L https://github.com/Muhumuza7325/OMD/raw/main/CHEM/1.1.moles_and_equations.txt || echo -e "\n\nError fetching material for this tutorial \c"
  ...
  rm -f 1.1.moles_and_equations.txt
fi
...
while true; do
  ...
  handle_class_input
  if [[ "$class" == "5" ]]; then
    if ! find . -maxdepth 1 -name '.s_chemistry_advanced_5*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to S5 Chemistry_advanced class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
    ...
    do
      ...
      do
        ...
        then
          ...
          question_directory="Revision/Chemistry_advanced/S5"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../chemistry_advanced_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Chemistry_advanced/S5"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../chemistry_advanced_answered_ans.txt"
          # Call the function to process a random question
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_chemistry_advanced_5_1" ]; then
                echo -e "\n\nYou chose to explore Moles and equations ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/Chemistry_advanced/5.1.moles_and_equations.txt" . || exit 1
              mv 5.1.moles_and_equations.txt .5.1.moles_and_equations.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .5.1.moles_and_equations.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .5.1.moles_and_equations.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .5.1.moles_and_equations.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .5.1.moles_and_equations.txt
              process_reminders_from_file .5.1.moles_and_equations.txt
              STATE_FILE=".s_chemistry_advanced_5_1"
              process_file .5.1.moles_and_equations.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .5.1.moles_and_equations.txt
              sed -i '/^1$/!d' .s_chemistry_advanced_5_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chemistry_advanced/S5"
              # Define the file extension
              file_extension=".1.moles_and_equations.qns.txt"
              # Define the revision file
              revision_file="../../chemistry_advanced_covered_qns.txt"
              # Call the function to process a random question
              ...
              question_directory="Revision/Chemistry_advanced/S5"
              # Define the file extension
              file_extension_question=".1.moles_and_equations.qns.txt"
              # Define the revision file
              revision_file="../../chemistry_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".chemistry_advanced.5.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chemistry_advanced/S5"
                # Define the file extension
                file_extension_answer=".1.moles_and_equations.ans.txt"
                # Define the exercise file
                exercise_file="../../chemistry_advanced_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .chemistry_advanced.5.1
              fi
              if ! [ -f ".s_chemistry_advanced_5_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Atomic and electronic structure ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Chemistry_advanced/5.2.atomic_and_electronic_structure.txt" . || exit 1
              mv 5.2.atomic_and_electronic_structure.txt .5.2.atomic_and_electronic_structure.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .5.2.atomic_and_electronic_structure.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .5.2.atomic_and_electronic_structure.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .5.2.atomic_and_electronic_structure.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .5.2.atomic_and_electronic_structure.txt
              process_reminders_from_file .5.2.atomic_and_electronic_structure.txt
              STATE_FILE=".s_chemistry_advanced_5_2"
              process_file .5.2.atomic_and_electronic_structure.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .5.2.atomic_and_electronic_structure.txt
              sed -i '/^1$/!d' .s_chemistry_advanced_5_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chemistry_advanced/S5"
              # Define the file extension
              file_extension_question=".2.atomic_and_electronic_structure.qns.txt"
              # Define the revision file
              revision_file="../../chemistry_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".chemistry_advanced.5.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chemistry_advanced/S5"
                # Define the file extension
                file_extension_answer=".2.atomic_and_electronic_structure.ans.txt"
                # Define the exercise file
                exercise_file="../../chemistry_advanced_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .chemistry_advanced.5.2
              fi
              if ! [ -f ".s_chemistry_advanced_5_3" ]; then
                echo -e "\n\nYou have made a choice to cover Bonding and structure ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Chemistry_advanced/5.3.bonding_and_structure.txt" . || exit 1
              mv 5.3.bonding_and_structure.txt .5.3.bonding_and_structure.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .5.3.bonding_and_structure.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .5.3.bonding_and_structure.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .5.3.bonding_and_structure.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .5.3.bonding_and_structure.txt
              process_reminders_from_file .5.3.bonding_and_structure.txt
              STATE_FILE=".s_chemistry_advanced_5_3"
              process_file .5.3.bonding_and_structure.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .5.3.bonding_and_structure.txt
              sed -i '/^1$/!d' .s_chemistry_advanced_5_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chemistry_advanced/S5"
              # Define the file extension
              file_extension_question=".3.bonding_and_structure.qns.txt"
              # Define the revision file
              revision_file="../../chemistry_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".chemistry_advanced.5.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chemistry_advanced/S5"
                # Define the file extension
                file_extension_answer=".3.bonding_and_structure.ans.txt"
                # Define the exercise file
                exercise_file="../../chemistry_advanced_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .chemistry_advanced.5.3
              fi
              if ! [ -f ".s_chemistry_advanced_5_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Periodicity i ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/Chemistry_advanced/5.4.periodicity_i.txt" . || exit 1
              mv 5.4.periodicity_i.txt .5.4.periodicity_i.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .5.4.periodicity_i.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .5.4.periodicity_i.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .5.4.periodicity_i.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .5.4.periodicity_i.txt
              process_reminders_from_file .5.4.periodicity_i.txt
              STATE_FILE=".s_chemistry_advanced_5_4"
              process_file .5.4.periodicity_i.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .5.4.periodicity_i.txt
              sed -i '/^1$/!d' .s_chemistry_advanced_5_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chemistry_advanced/S5"
              # Define the file extension
              file_extension=".4.periodicity_i.qns.txt"
              # Define the revision file
              revision_file="../../chemistry_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Chemistry_advanced/S5"
              # Define the file extension
              file_extension_question=".4.periodicity_i.qns.txt"
              # Define the revision file
              revision_file="../../chemistry_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            5)
              if ! [ -f ".chemistry_advanced.5.4" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chemistry_advanced/S5"
                # Define the file extension
                file_extension_answer=".4.periodicity_i.ans.txt"
                # Define the exercise file
                exercise_file="../../chemistry_advanced_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .chemistry_advanced.5.4
              fi
              if ! [ -f ".s_chemistry_advanced_5_5" ]; then
                echo -e "\n\nHere you are dear one... Stay organised as you explore Thermochemistry ...\n\n${g}Just know we are not going to leave you alone${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Chemistry_advanced/5.5.thermochemistry.txt" . || exit 1
              mv 5.5.thermochemistry.txt .5.5.thermochemistry.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .5.5.thermochemistry.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .5.5.thermochemistry.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .5.5.thermochemistry.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .5.5.thermochemistry.txt
              process_reminders_from_file .5.5.thermochemistry.txt
              STATE_FILE=".s_chemistry_advanced_5_5"
              process_file .5.5.thermochemistry.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .5.5.thermochemistry.txt
              sed -i '/^1$/!d' .s_chemistry_advanced_5_5
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chemistry_advanced/S5"
              # Define the file extension
              file_extension_question=".5.thermochemistry.qns.txt"
              # Define the revision file
              revision_file="../../chemistry_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            6)
              if ! [ -f ".chemistry_advanced.5.5" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chemistry_advanced/S5"
                # Define the file extension
                file_extension_answer=".5.thermochemistry.ans.txt"
                # Define the exercise file
                exercise_file="../../chemistry_advanced_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .chemistry_advanced.5.5
              fi
              if ! [ -f ".s_chemistry_advanced_5_6" ]; then
                echo -e "\n\nYou have managed to make it to Organic chemistry i ...\n\n${g}Remember to pray always${t}\n\nThe fear of the Lord is the beginning of wisdom \c" && wait_for_a_key_press
              fi
              cp "Notes/Chemistry_advanced/5.6.organic_chemistry_i.txt" . || exit 1
              mv 5.6.organic_chemistry_i.txt .5.6.organic_chemistry_i.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .5.6.organic_chemistry_i.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .5.6.organic_chemistry_i.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .5.6.organic_chemistry_i.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .5.6.organic_chemistry_i.txt
              process_reminders_from_file .5.6.organic_chemistry_i.txt
              STATE_FILE=".s_chemistry_advanced_5_6"
              process_file .5.6.organic_chemistry_i.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .5.6.organic_chemistry_i.txt
              sed -i '/^1$/!d' .s_chemistry_advanced_5_6
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chemistry_advanced/S5"
              # Define the file extension
              file_extension_question=".6.organic_chemistry_i.qns.txt"
              # Define the revision file
              revision_file="../../chemistry_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            7)
              if ! [ -f ".chemistry_advanced.5.6" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chemistry_advanced/S5"
                # Define the file extension
                file_extension_answer=".6.organic_chemistry_i.ans.txt"
                # Define the exercise file
                exercise_file="../../chemistry_advanced_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .chemistry_advanced.5.6
              fi
              if ! [ -f ".s_chemistry_advanced_5_7" ]; then
                echo -e "\n\nFrom here, you will be proceeding with Equilibria i ...\n\n${g}Please never ever forget that your education is your future${t}\n\nFocus dear \c" && wait_for_a_key_press
              fi
              cp "Notes/Chemistry_advanced/5.7.equilibria_i.txt" . || exit 1
              mv 5.7.equilibria_i.txt .5.7.equilibria_i.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .5.7.equilibria_i.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .5.7.equilibria_i.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .5.7.equilibria_i.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .5.7.equilibria_i.txt
              process_reminders_from_file .5.7.equilibria_i.txt
              STATE_FILE=".s_chemistry_advanced_5_7"
              process_file .5.7.equilibria_i.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .5.7.equilibria_i.txt
              sed -i '/^1$/!d' .s_chemistry_advanced_5_7
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chemistry_advanced/S5"
              # Define the file extension
              file_extension_question=".7.equilibria_i.qns.txt"
              # Define the revision file
              revision_file="../../chemistry_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".chemistry_advanced.5.7" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/Chemistry_advanced/S5"
	              # Define the file extension
	              file_extension_answer=".7.equilibria_i.ans.txt"
	              # Define the exercise file
	              exercise_file="../../chemistry_advanced_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
								touch .chemistry_advanced.5.7
								echo "5" > .chemistry_advanced_ready
							fi
            ;;
    ...
    if ! find . -maxdepth 1 -name '.s_chemistry_advanced_6*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to S6 Chemistry_advanced class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
    ...
    do
      ...
      do
        ...
        then
          attempts=0
          # Define the targeted directory
          question_directory="Revision/Chemistry_advanced/S6"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../chemistry_advanced_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Chemistry_advanced/S6"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../chemistry_advanced_answered_ans.txt"
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_chemistry_advanced_6_1" ]; then
                echo -e "\n\nYou chose to explore Equilibria ii ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/Chemistry_advanced/6.1.equilibria_ii.txt" . || exit 1
              mv 6.1.equilibria_ii.txt .6.1.equilibria_ii.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .6.1.equilibria_ii.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .6.1.equilibria_ii.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .6.1.equilibria_ii.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .6.1.equilibria_ii.txt
              process_reminders_from_file .6.1.equilibria_ii.txt
              STATE_FILE=".s_chemistry_advanced_6_1"
              process_file .6.1.equilibria_ii.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .6.1.equilibria_ii.txt
              sed -i '/^1$/!d' .s_chemistry_advanced_6_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chemistry_advanced/S6"
              # Define the file extension
              file_extension=".1.equilibria_ii.qns.txt"
              # Define the revision file
              revision_file="../../chemistry_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Chemistry_advanced/S6"
              # Define the file extension
              file_extension_question=".1.equilibria_ii.qns.txt"
              # Define the revision file
              revision_file="../../chemistry_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".chemistry_advanced.6.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chemistry_advanced/S6"
                # Define the file extension
                file_extension_answer=".1.equilibria_ii.ans.txt"
                # Define the exercise file
                exercise_file="../../chemistry_advanced_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .chemistry_advanced.6.1
              fi
              if ! [ -f ".s_chemistry_advanced_6_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Organic chemistry ii ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Chemistry_advanced/6.2.organic_chemistry_ii.txt" . || exit 1
              mv 6.2.organic_chemistry_ii.txt .6.2.organic_chemistry_ii.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .6.2.organic_chemistry_ii.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .6.2.organic_chemistry_ii.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .6.2.organic_chemistry_ii.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .6.2.organic_chemistry_ii.txt
              process_reminders_from_file .6.2.organic_chemistry_ii.txt
              STATE_FILE=".s_chemistry_advanced_6_2"
              process_file .6.2.organic_chemistry_ii.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .6.2.organic_chemistry_ii.txt
              sed -i '/^1$/!d' .s_chemistry_advanced_6_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chemistry_advanced/S6"
              # Define the file extension
              file_extension_question=".2.organic_chemistry_ii.qns.txt"
              # Define the revision file
              revision_file="../../chemistry_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".chemistry_advanced.6.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chemistry_advanced/S6"
                # Define the file extension
                file_extension_answer=".2.organic_chemistry_ii.ans.txt"
                # Define the exercise file
                exercise_file="../../chemistry_advanced_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .chemistry_advanced.6.2
              fi
              if ! [ -f ".s_chemistry_advanced_6_3" ]; then
                echo -e "\n\nYou have made a choice to cover Electrochemistry ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Chemistry_advanced/6.3.electrochemistry.txt" . || exit 1
              mv 6.3.electrochemistry.txt .6.3.electrochemistry.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .6.3.electrochemistry.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .6.3.electrochemistry.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .6.3.electrochemistry.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .6.3.electrochemistry.txt
              process_reminders_from_file .6.3.electrochemistry.txt
              STATE_FILE=".s_chemistry_advanced_6_3"
              process_file .6.3.electrochemistry.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .6.3.electrochemistry.txt
              sed -i '/^1$/!d' .s_chemistry_advanced_6_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chemistry_advanced/S6"
              # Define the file extension
              file_extension_question=".3.electrochemistry.qns.txt"
              # Define the revision file
              revision_file="../../chemistry_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".chemistry_advanced.6.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chemistry_advanced/S6"
                # Define the file extension
                file_extension_answer=".3.electrochemistry.ans.txt"
                # Define the exercise file
                exercise_file="../../chemistry_advanced_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .chemistry_advanced.6.3
              fi
              if ! [ -f ".s_chemistry_advanced_6_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Periodicity ii ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/Chemistry_advanced/6.4.periodicity_ii.txt" . || exit 1
              mv 6.4.periodicity_ii.txt .6.4.periodicity_ii.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .6.4.periodicity_ii.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .6.4.periodicity_ii.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .6.4.periodicity_ii.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .6.4.periodicity_ii.txt
              process_reminders_from_file .6.4.periodicity_ii.txt
              STATE_FILE=".s_chemistry_advanced_6_4"
              process_file .6.4.periodicity_ii.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .6.4.periodicity_ii.txt
              sed -i '/^1$/!d' .s_chemistry_advanced_6_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chemistry_advanced/S6"
              # Define the file extension
              file_extension=".4.periodicity_ii.qns.txt"
              # Define the revision file
              revision_file="../../chemistry_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Chemistry_advanced/S6"
              # Define the file extension
              file_extension_question=".4.periodicity_ii.qns.txt"
              # Define the revision file
              revision_file="../../chemistry_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            5)
              if ! [ -f ".chemistry_advanced.6.4" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chemistry_advanced/S6"
                # Define the file extension
                file_extension_answer=".4.periodicity_ii.ans.txt"
                # Define the exercise file
                exercise_file="../../chemistry_advanced_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .chemistry_advanced.6.4
              fi
              if ! [ -f ".s_chemistry_advanced_6_5" ]; then
                echo -e "\n\nHere you are dear one... Stay organised as you explore Organic chemistry iii ...\n\n${g}Just know we are not going to leave you alone${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Chemistry_advanced/6.5.organic_chemistry_iii.txt" . || exit 1
              mv 6.5.organic_chemistry_iii.txt .6.5.organic_chemistry_iii.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .6.5.organic_chemistry_iii.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .6.5.organic_chemistry_iii.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .6.5.organic_chemistry_iii.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .6.5.organic_chemistry_iii.txt
              process_reminders_from_file .6.5.organic_chemistry_iii.txt
              STATE_FILE=".s_chemistry_advanced_6_5"
              process_file .6.5.organic_chemistry_iii.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .6.5.organic_chemistry_iii.txt
              sed -i '/^1$/!d' .s_chemistry_advanced_6_5
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chemistry_advanced/S6"
              # Define the file extension
              file_extension_question=".5.organic_chemistry_iii.qns.txt"
              # Define the revision file
              revision_file="../../chemistry_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            6)
              if ! [ -f ".chemistry_advanced.6.5" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chemistry_advanced/S6"
                # Define the file extension
                file_extension_answer=".5.organic_chemistry_iii.ans.txt"
                # Define the exercise file
                exercise_file="../../chemistry_advanced_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .chemistry_advanced.6.5
              fi
              if ! [ -f ".s_chemistry_advanced_6_6" ]; then
                echo -e "\n\nYou have managed to make it to Reaction kinetics ...\n\n${g}Remember to pray always${t}\n\nThe fear of the Lord is the beginning of wisdom \c" && wait_for_a_key_press
              fi
              cp "Notes/Chemistry_advanced/6.6.reaction_kinetics.txt" . || exit 1
              mv 6.6.reaction_kinetics.txt .6.6.reaction_kinetics.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .6.6.reaction_kinetics.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .6.6.reaction_kinetics.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .6.6.reaction_kinetics.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .6.6.reaction_kinetics.txt
              process_reminders_from_file .6.6.reaction_kinetics.txt
              STATE_FILE=".s_chemistry_advanced_6_6"
              process_file .6.6.reaction_kinetics.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .6.6.reaction_kinetics.txt
              sed -i '/^1$/!d' .s_chemistry_advanced_6_6
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chemistry_advanced/S6"
              # Define the file extension
              file_extension_question=".6.reaction_kinetics.qns.txt"
              # Define the revision file
              revision_file="../../chemistry_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".chemistry_advanced.6.6" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/Chemistry_advanced/S6"
	              # Define the file extension
	              file_extension_answer=".6.reaction_kinetics.ans.txt"
	              # Define the exercise file
	              exercise_file="../../chemistry_advanced_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
								touch .chemistry_advanced.6.6
								echo "6" > .chemistry_advanced_ready
							fi
            ;;
