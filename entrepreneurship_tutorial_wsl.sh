...
function handle_class_input() {
  if { [ -n "$last_class" ] || [ -z "$last_topic" ]; } && [ -f .entrepreneurship_surveyor ]; then
		while true; do
	    read -rp $'\n\nPlease enter '"${r}1 or 2 or 3 or 4${t}"' to go to your class or '"${r}x${t}"' to exit'$'\n\n> ' class
      if [ "$class" == "x" ]; then
        exit
      fi
		  # Check if .current_entrepreneurship_class is accidentally empty
		  if [ ! -s .current_entrepreneurship_class ]; then
		    # Echo 1 to .current_entrepreneurship_class
		    echo "1" > .current_entrepreneurship_class
		  fi
		  # Read the value from the file
		  current_entrepreneurship_class=$(<.current_entrepreneurship_class) 2>/dev/null
		  # Check if the value is equal to $class
		  if [ "$current_entrepreneurship_class" -lt "$class" 2>/dev/null ]; then
	      ...
	      rm -f ".entrepreneurship_topic_selected"
	      # Update the state file with the class
	      if [ "$class" != "x" ]; then
	        echo "$class" > .entrepreneurship_user_state 2>/dev/null
	      fi
				break
	  	fi
		done
...
function handle_s3_topic_input() {
  ...
  if [ -z "$last_topic" ] || [ -f .entrepreneurship_topic_selected ]; then
    read -rp $'\n\nChoose either topic '"${g}1 or 2 or 3 or 4${t}"' to learn'$'\nor enter '"${r}z${t}"' for an adventure or '"${r}r${t}"' to revise or '"${r}s${t}"' to get sample_items'$'\nor '"${r}a${t}"' to get an activity of integration or '"${r}q${t}"' to get a short answer question'$'\nor '"${r}n${t}"' to do your final class assignment and if necessary, gain access to the next class'$'\nor '"${r}p${t}"' to track academic progress or '"${r}x${t}"' to exit'$'\n\n1. Business planning '"${r}Term1${t}"''$'\n\n2. Introduction to principles of accounting '"${r}Term2${t}"''$'\n\n3. Introduction to principles of accounting '"${r}Term3${t}"''$'\n\n4. Tax administration '"${r}Term3${t}"''$'\n\n> ' topic
    touch .entrepreneurship_surveyor
    touch .entrepreneurship_topic_selected
    # Update the state file with the topic
    # Check if the state file exists, and the topic is not "x"
...
function handle_s2_topic_input() {
  ...
  if [ -z "$last_topic" ] || [ -f .entrepreneurship_topic_selected ]; then
    read -rp $'\n\nChoose either topic '"${g}1 or 2 or 3 or 4 or 5${t}"' to learn'$'\nor enter '"${r}z${t}"' for an adventure or '"${r}r${t}"' to revise or '"${r}s${t}"' to get sample_items'$'\nor '"${r}a${t}"' to get an activity of integration or '"${r}q${t}"' to get a short answer question'$'\nor '"${r}n${t}"' to do your final class assignment and if necessary, gain access to the next class'$'\nor '"${r}p${t}"' to track academic progress or '"${r}x${t}"' to exit'$'\n\n1. Legal forms of business ownership '"${r}Term1${t}"''$'\n\n2. Production in business '"${r}Term1${t}"''$'\n\n3. Marketing in a small business enterprise '"${r}Term2${t}"''$'\n\n4. Money and financial institutions '"${r}Term3${t}"''$'\n\n5. Taxation '"${r}Term3${t}"''$'\n\n> ' topic
    touch .entrepreneurship_surveyor
    touch .entrepreneurship_topic_selected
    # Update the state file with the topic
    # Check if the state file exists, and the topic is not "x"
...
function handle_s1_topic_input() {
  ...
  if [ -z "$last_topic" ] || [ -f .entrepreneurship_topic_selected ]; then
    read -rp $'\n\nChoose either topic '"${g}1 or 2 or 3 or 4 or 5${t}"' to learn'$'\nor enter '"${r}z${t}"' for an adventure or '"${r}r${t}"' to revise or '"${r}s${t}"' to get sample_items'$'\nor '"${r}a${t}"' to get an activity of integration or '"${r}q${t}"' to get a short answer question'$'\nor '"${r}n${t}"' to do your final class assignment and if necessary, gain access to the next class'$'\nor '"${r}p${t}"' to track academic progress or '"${r}x${t}"' to exit'$'\n\n1. Introduction to entrepreneurship education '"${r}Term1${t}"''$'\n\n2. Business in Uganda '"${r}Term2${t}"''$'\n\n3. Business ideas and business opportunities '"${r}Term2${t}"''$'\n\n4. Business start-up process '"${r}Term3${t}"''$'\n\n5. Introduction to government revenue '"${r}Term3${t}"''$'\n\n> ' topic
    touch .entrepreneurship_surveyor
    touch .entrepreneurship_topic_selected
    # Update the state file with the topic
    # Check if the state file exists, and the topic is not "x"
...
process_reminders_from_file() {
  ...
  prev_sentence=""
  echo_next=false
	echo -n > .entrepreneurship_reminder
    ...
    for sentence in "${sentences[@]}"; do
      if [[ $echo_next == true ]]; then
        ...
        for split_sentence in "${split_sentences[@]}"; do
					echo "$split_sentence" >> .entrepreneurship_reminder
        done
      echo_next=false
      fi
        ...
        echo "Did you know that $lower_cased_sentence" >> .entrepreneurship_reminder
        ...
        echo_next=true
  			...
  			echo "Do you recall that $lower_cased_sentence;" >> .entrepreneurship_reminder

			elif [[ "$sentence" =~ " → " ]]; then
  			echo "Hope you know that: $sentence;" >> .entrepreneurship_reminder

      elif [[ "$sentence" =~ " ↔ " ]]; then
        echo "Hope you know that: $sentence;" >> .entrepreneurship_reminder

      elif [[ "$sentence" =~ (Generally|general) ]]; then
        echo "Note: $sentence;" >> .entrepreneurship_reminder
...
process_random_reminder() {
  ...
  # Check if .entrepreneurship_reminder exists
  if [ -f .entrepreneurship_reminder ]; then
		# Remove empty lines from .entrepreneurship_reminder
  	sed -i '/^[[:space:]]*$/d' .entrepreneurship_reminder 2>/dev/null
    # Randomly select a non-empty sentence
    local reminder # Declare the variable
		reminder=$(awk -v RS=';' 'BEGIN{srand();}{gsub(/^[[:space:]]+|[[:space:]]+$/, ""); if (length > 0) a[++n]=$0}END{if (n > 0) print a[int(rand()*n)+1]}' .entrepreneurship_reminder)
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    for sentence in "${sentences[@]}"; do
        ...
        if [[ $sentence == *"Figure"* ]]; then
          ...
          # Change to the "Figures/Entrepreneurship" directory
          cd Figures/Entrepreneurship || { echo "Failed to change to Figures/Entrepreneurship"; return; }
        ...
        if [[ $sentence == *"Table"* ]]; then
          cd Tables/Entrepreneurship || { echo -e "\nFailed to change to Tables/Entrepreneurship \c"; return; }
          explorer.exe "$sentence" > /dev/null 2>&1 &
          cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        fi

        if [[ $sentence == *"Video"* ]]; then
          cd Videos/Entrepreneurship || { echo -e "\nFailed to change to Videos/Entrepreneurship \c"; return; }
          explorer.exe "$sentence" > /dev/null 2>&1 &
          cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        fi
...
contact_ai() {
  last_topic=$(awk -F' ' '{print $2}' .entrepreneurship_user_state)
...
while true; do
  ...
  # Prompt the user for input
  if [ -z "$(find ./Notes/Entrepreneurship -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
  ...
  if [[ -n "$user_input" ]]; then
		if [[ "$user_input" == "sh" ]]; then
		  Response="We are looking forward to receiving your economic support, thoughts, suggestions, and any files you feel should reach out to everyone of our children. Please remember to label the files you are to attach using the format: Your_name_School_Entrepreneurship_File_content (e.g., Muhumuza_Omega_Kasule_High_School_O_level_Chemistry_Answered_EOC1_Items.pdf). Thanks a lot for your contributions."  
		  entrepreneurship="$(basename "$0") - $(date +"%Y-%m-%d %H:%M:%S") - Thoughts, suggestions, and contributions"
		  encoded_entrepreneurship=$(echo "$entrepreneurship" | sed 's/ /%20/g; s/\n/%0A/g')
		  encoded_body=$(echo "$Response" | sed 's/ /%20/g; s/\n/%0A/g')
		  # Open the email in the browser with the encoded entrepreneurship and body
		  powershell.exe -Command "Start-Process 'https://mail.google.com/mail/?view=cm&to=2024omd256@gmail.com&su=${encoded_entrepreneurship}&body=${encoded_body}'"
			return
		fi
     ...
     if [[ "$user_input" == "sr" ]]; then
      ...
      explorer.exe Entrepreneurship > /dev/null 2>&1 &
    ...
    if [[ "$user_input" == "zz" ]]; then
      ...
      TEMP_FILE=$(mktemp) && curl -o "$TEMP_FILE" -L "https://github.com/Muhumuza7325/OMD/raw/main/entrepreneurship_tutorial_wsl.sh" && mv "$TEMP_FILE" entrepreneurship_tutorial_wsl.sh && chmod +x entrepreneurship_tutorial_wsl.sh && echo -e "\n\n${y}Code successfully updated.. You will have to restart a new session${t} \c" && sleep 4 && exit || (echo -e "\n\n${m}Error updating code!... Please check your internet connection and try again!${t} \c" && rm -f "$TEMP_FILE" && return)
    fi
    if [[ "$user_input" == "xx" ]]; then
      ...
      if [[ "$input" == "y" || "$input" == "Y" ]]; then
        ...
        curl -O -L "https://github.com/Muhumuza7325/OMD/raw/main/update_entrepreneurship.sh" || { echo -e "\n\n${m}Check your internet connection and try again!${t}" >&2; return; }
        mv update_entrepreneurship.sh .update_entrepreneurship.sh
        bash .update_entrepreneurship.sh
        return
      else
        return
      fi
    ...
    if [[ -n "$result" ]]; then
      ...
      if echo "$result" | grep "Figure"; then
        echo "$result" | grep -o '\bFigure[0-9]\+.*\.jpg\(\.[0-9]\+\)*\b' > Figures/Entrepreneurship/figures.txt
        sed -i -e '/^Figure/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Figures/Entrepreneurship/figures.txt
        # Change to the "Figures/Entrepreneurship" directory
        cd Figures/Entrepreneurship || { echo "Failed to change to Figures/Entrepreneurship"; return; }
        ...
        rm -f Figures/Entrepreneurship/figures.txt
      fi

      if echo "$result" | grep "Table"; then
        echo "$result" | grep -o '\bTable[0-9]\+\(\.[0-9]\+\)*\b' > Tables/Entrepreneurship/tables.txt
        sed -i -e '/^Table/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Tables/Entrepreneurship/tables.txt
        cd Tables/Entrepreneurship || { echo -e "\nFailed to change to Tables/Entrepreneurship \c"; return; }
        ...
        # Go back to the original directory
        cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        rm -f Tables/Entrepreneurship/tables.txt
      fi

      if echo "$result" | grep "Video"; then
        #echo "$result" > Videos/Entrepreneurship/videos.txt
        echo "$result" | grep -o '\bVideo[0-9]\+\(\.[0-9]\+\)*\b' > Videos/Entrepreneurship/videos.txt
        # Remove lines not starting with "Video" and any leading/trailing whitespaces
        sed -i -e '/^Video/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Videos/Entrepreneurship/videos.txt
        # Change to the "Videos/Entrepreneurship" directory
        cd Videos/Entrepreneurship || { echo -e "\nFailed to change to Videos/Entrepreneurship \c"; return; }
        ...
        cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        # Remove the temporary file
        rm -f Videos/Entrepreneurship/videos.txt
      fi
  ...
  if [ -f "$selected_file" ]; then
    ...
    if [[ -n "$selected_question" && "$selected_question" =~ [[:graph:]] ]]; then
      ...
      if echo "$selected_question" | grep "Figure"; then
        echo "$selected_question" | grep -o '\bFigure[0-9]\+[^;]*\b' > ../../Figures/Entrepreneurship/figures.txt
        sed -i -e '/^Figure/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' ../../Figures/Entrepreneurship/figures.txt
        # Change to the "Figures/Entrepreneurship" directory
        cd ../../Figures/Entrepreneurship || { echo "Failed to change to Figures/Entrepreneurship"; return; }
        # Specify the path to the text file containing figure names
        ...
        # Go back to the original directory
        cd ../../Revision/Entrepreneurship || { echo "Failed to change back to the targeted directory \c"; exit 1; }
        # Remove the temporary file
        rm -f ../../Figures/Entrepreneurship/figures.txt
      fi
...
process_question_answer() {
  ...
  if [ -f ../../../.school_name ]; then
    ...
    sed -i '/^[[:space:]]*$/d' ../../../."$school_name"_students_file.txt
    existing_class=$(awk '{print $1}' ../../../.entrepreneurship_user_state)
    existing_topic=$(awk '{print $2}' ../../../.entrepreneurship_user_state)
    echo ''
    ...
    if grep -q "Entrepreneurship" ../../../."$school_name"_students_file.txt; then
      sed -i -E 's/;/\n/g' ../../../."$school_name"_students_file.txt
      # Find the line with the word Entrepreneurship, replace the information below it with the already available information adding a comma existing_class_existing_topic [$percentage]
      if [ "$existing_topic" == "r" ]; then
        sed -i '/Entrepreneurship/{n;s/\(.*\)/\1, '"$existing_class"'_'"$input"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
      else
        (( existing_topic-- ))
        sed -i '/Entrepreneurship/{n;s/\(.*\)/\1, '"$existing_class"'_'"$existing_topic"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
      fi
    else
      if [ "$existing_topic" == "r" ]; then
        sed -i -E '/^School/ i\Entrepreneurship\n'"${existing_class} ${names}[${percentage}%]"'' ../../../."$school_name"_students_file.txt
        echo -e "Entrepreneurship\n"$existing_class"_'"$input"' "$names"["$percentage%"]" >> ../../../."$school_name"_students_file.txt
      else
        (( existing_topic-- ))
...
function resume_from_last_point() {
  if [ -f .entrepreneurship_user_state ]; then
    last_class=$(awk -F' ' '{print $1}' .entrepreneurship_user_state)
    last_topic=$(awk -F' ' '{print $2}' .entrepreneurship_user_state)
    if [ -n "$last_class" ] && [ -n "$last_topic" ]; then
      echo -e "\n       Resuming from ${r}S$last_class${t} : ${g}Topic '$last_topic'${t} \c"
      rm -f .entrepreneurship_surveyor
...
process_final_assignment() {
  # Check if user is.entrepreneurship_ready for the assignment
  # Check if .current_entrepreneurship_class is accidentally empty
  if [ ! -s .current_entrepreneurship_class ]; then
    # Echo 1 to .current_entrepreneurship_class
    echo "1" > .current_entrepreneurship_class
  fi
  # Read the value from the .current_entrepreneurship_class file
  current_entrepreneurship_class=$(<.current_entrepreneurship_class) 2>/dev/null
  # Check if the value in the .entrepreneurship_ready file is equal to $class
  # Read the value from the .entrepreneurship_ready file
  if [ ! -s .entrepreneurship_ready ]; then
    echo "0" > .entrepreneurship_ready
  fi
  how.entrepreneurship_ready=$(<.entrepreneurship_ready) 2>/dev/null
  # Check if the value is equal to $class
  if [ "$how.entrepreneurship_ready" -lt "$current_entrepreneurship_class" 2>/dev/null ]; then
    read -rp $'\n\nYou havent done all the topic assignments for your current entrepreneurship class\n\n'"${r}Proceeding from here will affect your very final score${y}"'. To go back and progress right, enter '"${y}ok${t}"'. Otherwise, press the Enter key to do the final class assignment: ' progress
    if [ "$progress" == "ok" ]; then
      return
    fi
  fi
    ...
    if ! [ "$total_questions" == 100 ]; then
      ...
      if [ "$elapsed_time" -gt 3600 ]; then
        ...
        if [ "$percentage" -lt 95 ]; then
          ...
          cd - > /dev/null 2>&1 || exit
          if ! [ -f .current_entrepreneurship_class ]; then
            # Echo the result to the file .current_class
            echo "2" > .current_entrepreneurship_class
            ...
            # Read the value from the file
            current_entrepreneurship_class=$(<.current_entrepreneurship_class) 2>/dev/null
            # Check if the new_class value is lt $current_entrepreneurship_class
            if [ "$new_class" -gt "$current_entrepreneurship_class" 2>/dev/null ]; then
		          # Echo the result to the file .current_class
		          echo "$new_class" > .current_entrepreneurship_class
            fi
...
get_sample_items() {
  ...
  # Save the current working directory
  pushd . > /dev/null
  cd Revision/Entrepreneurship || { echo "Directory not found"; return; }
  ...
  while true; do
      ...
      if [ ! -f "$last_echo_time_file" ]; then
    	...
    	if [ $time_diff -gt 3600 ]; then
      	# Echo the message and update the last echo time
	    	echo -e "\n\n\n${r}You are advised to not make any changes to the provided answers, instead, you can make copies that you can edit${t}\n\n${y}For a teacher willing to join us reach out to everyone of our children, please send us your questions and answers in a file labelled with your name, school, entrepreneurship, and file content (e.g., Muhumuza_Omega_Kasule_High_School_O_level_Chemistry_Answered_EOC1_Items.pdf) to our contacts${t}\n\n\nEmail: ${g}2024omd256@gmail.com${t} \c"
				echo $current_time > "$last_echo_time_file"
  		fi
      read -rp $'\n\n\nEnter '"${m}any character${t}"' for access to the file of answered items or simply press '"${r}Enter${t}"' to get items to attempt : ' input
     	if [[ -n $input ]]; then
     		explorer.exe .entrepreneurship_samples* > /dev/null 2>&1 &
      	clear
      	popd > /dev/null || exit
        return
      fi
      if [ -f .e_o_c_entrepreneurship.txt ]; then
        echo -e "\n\n${y}Below is the list of the elements of construct${t} \n"
        cat .e_o_c_entrepreneurship.txt
        ...
        if [[ -n $input ]]; then
          ...
          selected_file1=$(ls -a | grep -E "\.e_o_c_entrepreneurship_${input}\.txt")
          cat "$selected_file1"
          # Remove empty lines from the selected files
          find . -type f -name "*_samples_[0-9].txt" -exec sed -i '/^[[:space:]]*$/d' {} +
          selected_file=$(ls -a | grep -E "\.e_o_c_entrepreneurship_${input}_samples" | shuf -n 1)
        else
    ...
    rm -f .revise.txt .entrepreneurship_topic_selected 2>/dev/null
  done
  popd > /dev/null || exit
	return
}
...
while true; do
  ...
  if ls Students | grep -wq "$initials"; then
    ...
    echo -e "                  $initials\n" > "Students/$initials/Exercise/entrepreneurship_answered_ans.txt"
    echo -e "                  $initials\n" > "Students/$initials/Revision/entrepreneurship_covered_qns.txt"
    ...
    if [ -f ../../../.school_name ]; then
      ...
      if grep -q "Entrepreneurship" ../../../."$school_name"_students_file.txt; then
        ...
        if [ "$existing_topic" == "z" ]; then
          sed -i '/Entrepreneurship/{n;s/\(.*\)/\1, '"$existing_class"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
        else
          (( existing_topic-- ))
          sed -i '/Entrepreneurship/{n;s/\(.*\)/\1, '"$existing_class"'_'"$existing_topic"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
        fi
...
if [ ! -f .entrepreneurship_user_state ]; then
	touch .entrepreneurship_user_state
	touch .entrepreneurship_surveyor
	get_and_display_pattern
else
  if [ -f .omd_communication.txt ]; then
    ...
    process_random_reminder .entrepreneurship_reminder
  fi
	handle_resume_input
fi

if [ -z "$class" ] && [ -s ".entrepreneurship_user_state" ]; then
  get_and_display_pattern
fi
# Check for the presence of specific directories and a file
if ! [ -d "Notes/Entrepreneurship" ] || ! [ -d "Revision" ] || ! [ -d "Exercise" ] || ! [ -d "Videos" ] || ! [ -d "Figures" ] || ! [ -d "Tables" ]; then
  ...
  mkdir -p Notes Notes/Entrepreneurship Revision Revision/Entrepreneurship Revision/Entrepreneurship/{S1,S2,S3,S4} Exercise Exercise/Entrepreneurship Exercise/Entrepreneurship/{S1,S2,S3,S4} Videos Videos/Entrepreneurship Figures Figures/Entrepreneurship Tables Tables/Entrepreneurship
  touch Revision/entrepreneurship_covered_qns.txt Exercise/entrepreneurship_answered_ans.txt
  echo -e "\n"
  pwd
...
if [ -z "$(find ./Notes/Entrepreneurship -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
  ...
  if [[ -z "$user_input" ]]; then
    ...
    curl -O -L https://github.com/Muhumuza7325/OMD/raw/main/ENT/1.1.introduction_to_entrepreneurship_education.txt || echo -e "\n\nError fetching material for this tutorial \c"
    ...
    cp 1.1.introduction_to_entrepreneurship_education.txt Notes/Entrepreneurship || echo -e "\n\nError copying 1.1.introduction_to_entrepreneurship_education.txt to the Entrepreneurship directory in the Notes directory \c"
    wait_for_a_key_press
  else
  ...
  rm -f 1.1.introduction_to_entrepreneurship_education.txt
fi
...
while true; do
  ...
  handle_class_input
  if [[ "$class" == "1" ]]; then
    if ! find . -maxdepth 1 -name '.s_entrepreneurship_1*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to S1 Entrepreneurship class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
    ...
    do
      ...
      do
        ...
        then
          ...
          question_directory="Revision/Entrepreneurship/S1"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../entrepreneurship_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Entrepreneurship/S1"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../entrepreneurship_answered_ans.txt"
          # Call the function to process a random question
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_entrepreneurship_1_1" ]; then
                echo -e "\n\nYou chose to explore Introduction to entrepreneurship education ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship/1.1.introduction_to_entrepreneurship_education.txt" . || exit 1
              mv 1.1.introduction_to_entrepreneurship_education.txt .1.1.introduction_to_entrepreneurship_education.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.1.introduction_to_entrepreneurship_education.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.1.introduction_to_entrepreneurship_education.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.1.introduction_to_entrepreneurship_education.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.1.introduction_to_entrepreneurship_education.txt
              process_reminders_from_file .1.1.introduction_to_entrepreneurship_education.txt
              STATE_FILE=".s_entrepreneurship_1_1"
              process_file .1.1.introduction_to_entrepreneurship_education.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.1.introduction_to_entrepreneurship_education.txt
              sed -i '/^1$/!d' .s_entrepreneurship_1_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship/S1"
              # Define the file extension
              file_extension=".1.introduction_to_entrepreneurship_education.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_covered_qns.txt"
              # Call the function to process a random question
              ...
              question_directory="Revision/Entrepreneurship/S1"
              # Define the file extension
              file_extension_question=".1.introduction_to_entrepreneurship_education.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".entrepreneurship.1.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Entrepreneurship/S1"
                # Define the file extension
                file_extension_answer=".1.introduction_to_entrepreneurship_education.ans.txt"
                # Define the exercise file
                exercise_file="../../entrepreneurship_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .entrepreneurship.1.1
              fi
              if ! [ -f ".s_entrepreneurship_1_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Business in Uganda ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship/1.2.business_in_uganda.txt" . || exit 1
              mv 1.2.business_in_uganda.txt .1.2.business_in_uganda.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.2.business_in_uganda.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.2.business_in_uganda.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.2.business_in_uganda.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.2.business_in_uganda.txt
              process_reminders_from_file .1.2.business_in_uganda.txt
              STATE_FILE=".s_entrepreneurship_1_2"
              process_file .1.2.business_in_uganda.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.2.business_in_uganda.txt
              sed -i '/^1$/!d' .s_entrepreneurship_1_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship/S1"
              # Define the file extension
              file_extension_question=".2.business_in_uganda.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".entrepreneurship.1.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Entrepreneurship/S1"
                # Define the file extension
                file_extension_answer=".2.business_in_uganda.ans.txt"
                # Define the exercise file
                exercise_file="../../entrepreneurship_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .entrepreneurship.1.2
              fi
              if ! [ -f ".s_entrepreneurship_1_3" ]; then
                echo -e "\n\nYou have made a choice to cover Business ideas and business opportunities ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship/1.3.business_ideas_and_business_opportunities.txt" . || exit 1
              mv 1.3.business_ideas_and_business_opportunities.txt .1.3.business_ideas_and_business_opportunities.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.3.business_ideas_and_business_opportunities.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.3.business_ideas_and_business_opportunities.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.3.business_ideas_and_business_opportunities.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.3.business_ideas_and_business_opportunities.txt
              process_reminders_from_file .1.3.business_ideas_and_business_opportunities.txt
              STATE_FILE=".s_entrepreneurship_1_3"
              process_file .1.3.business_ideas_and_business_opportunities.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.3.business_ideas_and_business_opportunities.txt
              sed -i '/^1$/!d' .s_entrepreneurship_1_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship/S1"
              # Define the file extension
              file_extension_question=".3.business_ideas_and_business_opportunities.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".entrepreneurship.1.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Entrepreneurship/S1"
                # Define the file extension
                file_extension_answer=".3.business_ideas_and_business_opportunities.ans.txt"
                # Define the exercise file
                exercise_file="../../entrepreneurship_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .entrepreneurship.1.3
              fi
              if ! [ -f ".s_entrepreneurship_1_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Business start-up process ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship/1.4.business_start-up_process.txt" . || exit 1
              mv 1.4.business_start-up_process.txt .1.4.business_start-up_process.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.4.business_start-up_process.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.4.business_start-up_process.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.4.business_start-up_process.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.4.business_start-up_process.txt
              process_reminders_from_file .1.4.business_start-up_process.txt
              STATE_FILE=".s_entrepreneurship_1_4"
              process_file .1.4.business_start-up_process.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.4.business_start-up_process.txt
              sed -i '/^1$/!d' .s_entrepreneurship_1_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship/S1"
              # Define the file extension
              file_extension=".4.business_start-up_process.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Entrepreneurship/S1"
              # Define the file extension
              file_extension_question=".4.business_start-up_process.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            5)
              if ! [ -f ".entrepreneurship.1.4" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Entrepreneurship/S1"
                # Define the file extension
                file_extension_answer=".4.business_start-up_process.ans.txt"
                # Define the exercise file
                exercise_file="../../entrepreneurship_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .entrepreneurship.1.4
              fi
              if ! [ -f ".s_entrepreneurship_1_5" ]; then
                echo -e "\n\nHere you are dear one... Stay organised as you explore Introduction to government revenue ...\n\n${g}Just know we are not going to leave you alone${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship/1.5.introduction_to_government_revenue.txt" . || exit 1
              mv 1.5.introduction_to_government_revenue.txt .1.5.introduction_to_government_revenue.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.5.introduction_to_government_revenue.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.5.introduction_to_government_revenue.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.5.introduction_to_government_revenue.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.5.introduction_to_government_revenue.txt
              process_reminders_from_file .1.5.introduction_to_government_revenue.txt
              STATE_FILE=".s_entrepreneurship_1_5"
              process_file .1.5.introduction_to_government_revenue.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.5.introduction_to_government_revenue.txt
              sed -i '/^1$/!d' .s_entrepreneurship_1_5
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship/S1"
              # Define the file extension
              file_extension_question=".5.introduction_to_government_revenue.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".entrepreneurship.1.5" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/Entrepreneurship/S1"
	              # Define the file extension
	              file_extension_answer=".5.introduction_to_government_revenue.ans.txt"
	              # Define the exercise file
	              exercise_file="../../entrepreneurship_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
								touch .entrepreneurship.1.5
								echo "1" > .entrepreneurship_ready
							fi
            ;;
    ...
    if ! find . -maxdepth 1 -name '.s_entrepreneurship_2*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to S2 Entrepreneurship class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
      clear_and_center "There are ${r}5${t} topics to be covered. Your tasks will always expand or shrink to fit in the time you give them. For that reason, never procrastinate darling!"
    fi
    attempts=0
    ...
    do
      ...
      do
        ...
        then
          attempts=0
          # Define the targeted directory
          question_directory="Revision/Entrepreneurship/S2"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../entrepreneurship_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Entrepreneurship/S2"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../entrepreneurship_answered_ans.txt"
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_entrepreneurship_2_1" ]; then
                echo -e "\n\nYou chose to explore Legal forms of business ownership ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship/2.1.legal_forms_of_business_ownership.txt" . || exit 1
              mv 2.1.legal_forms_of_business_ownership.txt .2.1.legal_forms_of_business_ownership.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.1.legal_forms_of_business_ownership.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.1.legal_forms_of_business_ownership.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.1.legal_forms_of_business_ownership.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.1.legal_forms_of_business_ownership.txt
              process_reminders_from_file .2.1.legal_forms_of_business_ownership.txt
              STATE_FILE=".s_entrepreneurship_2_1"
              process_file .2.1.legal_forms_of_business_ownership.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.1.legal_forms_of_business_ownership.txt
              sed -i '/^1$/!d' .s_entrepreneurship_2_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship/S2"
              # Define the file extension
              file_extension=".1.legal_forms_of_business_ownership.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Entrepreneurship/S2"
              # Define the file extension
              file_extension_question=".1.legal_forms_of_business_ownership.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".entrepreneurship.2.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Entrepreneurship/S2"
                # Define the file extension
                file_extension_answer=".1.legal_forms_of_business_ownership.ans.txt"
                # Define the exercise file
                exercise_file="../../entrepreneurship_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .entrepreneurship.2.1
              fi
              if ! [ -f ".s_entrepreneurship_2_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Production in business ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship/2.2.production_in_business.txt" . || exit 1
              mv 2.2.production_in_business.txt .2.2.production_in_business.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.2.production_in_business.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.2.production_in_business.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.2.production_in_business.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.2.production_in_business.txt
              process_reminders_from_file .2.2.production_in_business.txt
              STATE_FILE=".s_entrepreneurship_2_2"
              process_file .2.2.production_in_business.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.2.production_in_business.txt
              sed -i '/^1$/!d' .s_entrepreneurship_2_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship/S2"
              # Define the file extension
              file_extension_question=".2.production_in_business.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".entrepreneurship.2.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Entrepreneurship/S2"
                # Define the file extension
                file_extension_answer=".2.production_in_business.ans.txt"
                # Define the exercise file
                exercise_file="../../entrepreneurship_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .entrepreneurship.2.2
              fi
              if ! [ -f ".s_entrepreneurship_2_3" ]; then
                echo -e "\n\nYou have made a choice to cover Marketing in a small business enterprise ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship/2.3.marketing_in_a_small_business_enterprise.txt" . || exit 1
              mv 2.3.marketing_in_a_small_business_enterprise.txt .2.3.marketing_in_a_small_business_enterprise.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.3.marketing_in_a_small_business_enterprise.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.3.marketing_in_a_small_business_enterprise.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.3.marketing_in_a_small_business_enterprise.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.3.marketing_in_a_small_business_enterprise.txt
              process_reminders_from_file .2.3.marketing_in_a_small_business_enterprise.txt
              STATE_FILE=".s_entrepreneurship_2_3"
              process_file .2.3.marketing_in_a_small_business_enterprise.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.3.marketing_in_a_small_business_enterprise.txt
              sed -i '/^1$/!d' .s_entrepreneurship_2_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship/S2"
              # Define the file extension
              file_extension_question=".3.marketing_in_a_small_business_enterprise.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".entrepreneurship.2.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Entrepreneurship/S2"
                # Define the file extension
                file_extension_answer=".3.marketing_in_a_small_business_enterprise.ans.txt"
                # Define the exercise file
                exercise_file="../../entrepreneurship_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .entrepreneurship.2.3
              fi
              if ! [ -f ".s_entrepreneurship_2_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Money and financial institutions ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship/2.4.money_and_financial_institutions.txt" . || exit 1
              mv 2.4.money_and_financial_institutions.txt .2.4.money_and_financial_institutions.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.4.money_and_financial_institutions.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.4.money_and_financial_institutions.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.4.money_and_financial_institutions.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.4.money_and_financial_institutions.txt
              process_reminders_from_file .2.4.money_and_financial_institutions.txt
              STATE_FILE=".s_entrepreneurship_2_4"
              process_file .2.4.money_and_financial_institutions.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.4.money_and_financial_institutions.txt
              sed -i '/^1$/!d' .s_entrepreneurship_2_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship/S2"
              # Define the file extension
              file_extension=".4.money_and_financial_institutions.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Entrepreneurship/S2"
              # Define the file extension
              file_extension_question=".4.money_and_financial_institutions.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            5)
              if ! [ -f ".entrepreneurship.2.4" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Entrepreneurship/S2"
                # Define the file extension
                file_extension_answer=".4.money_and_financial_institutions.ans.txt"
                # Define the exercise file
                exercise_file="../../entrepreneurship_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .entrepreneurship.2.4
              fi
              if ! [ -f ".s_entrepreneurship_2_5" ]; then
                echo -e "\n\nHere you are dear one... Stay organised as you explore Taxation ...\n\n${g}Just know we are not going to leave you alone${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship/2.5.taxation.txt" . || exit 1
              mv 2.5.taxation.txt .2.5.taxation.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.5.taxation.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.5.taxation.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.5.taxation.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.5.taxation.txt
              process_reminders_from_file .2.5.taxation.txt
              STATE_FILE=".s_entrepreneurship_2_5"
              process_file .2.5.taxation.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.5.taxation.txt
              sed -i '/^1$/!d' .s_entrepreneurship_2_5
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship/S2"
              # Define the file extension
              file_extension_question=".5.taxation.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".entrepreneurship.2.5" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/Entrepreneurship/S2"
	              # Define the file extension
	              file_extension_answer=".5.taxation.ans.txt"
	              # Define the exercise file
	              exercise_file="../../entrepreneurship_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
								touch .entrepreneurship.2.5
								echo "2" > .entrepreneurship_ready
							fi
            ;;
    ...
    if ! find . -maxdepth 1 -name '.s_entrepreneurship_3*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to S3 Entrepreneurship class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
    ...
    do
      ...
      do
        ...
        then
          attempts=0
          # Define the targeted directory
          question_directory="Revision/Entrepreneurship/S3"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../entrepreneurship_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Entrepreneurship/S3"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../entrepreneurship_answered_ans.txt"
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_entrepreneurship_3_1" ]; then
                echo -e "\n\nYou chose to explore Business planning ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship/3.1.business_planning.txt" . || exit 1
              mv 3.1.business_planning.txt .3.1.business_planning.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.1.business_planning.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.1.business_planning.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.1.business_planning.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.1.business_planning.txt
              process_reminders_from_file .3.1.business_planning.txt
              STATE_FILE=".s_entrepreneurship_3_1"
              process_file .3.1.business_planning.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.1.business_planning.txt
              sed -i '/^1$/!d' .s_entrepreneurship_3_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship/S3"
              # Define the file extension
              file_extension=".1.business_planning.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Entrepreneurship/S3"
              # Define the file extension
              file_extension_question=".1.business_planning.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".entrepreneurship.3.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Entrepreneurship/S3"
                # Define the file extension
                file_extension_answer=".1.business_planning.ans.txt"
                # Define the exercise file
                exercise_file="../../entrepreneurship_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .entrepreneurship.3.1
              fi
              if ! [ -f ".s_entrepreneurship_3_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Introduction to principles of accounting ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship/3.2.introduction_to_principles_of_accounting.txt" . || exit 1
              mv 3.2.introduction_to_principles_of_accounting.txt .3.2.introduction_to_principles_of_accounting.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.2.introduction_to_principles_of_accounting.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.2.introduction_to_principles_of_accounting.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.2.introduction_to_principles_of_accounting.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.2.introduction_to_principles_of_accounting.txt
              process_reminders_from_file .3.2.introduction_to_principles_of_accounting.txt
              STATE_FILE=".s_entrepreneurship_3_2"
              process_file .3.2.introduction_to_principles_of_accounting.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.2.introduction_to_principles_of_accounting.txt
              sed -i '/^1$/!d' .s_entrepreneurship_3_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship/S3"
              # Define the file extension
              file_extension_question=".2.introduction_to_principles_of_accounting.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".entrepreneurship.3.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Entrepreneurship/S3"
                # Define the file extension
                file_extension_answer=".2.introduction_to_principles_of_accounting.ans.txt"
                # Define the exercise file
                exercise_file="../../entrepreneurship_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .entrepreneurship.3.2
              fi
              if ! [ -f ".s_entrepreneurship_3_3" ]; then
                echo -e "\n\nYou have made a choice to cover Introduction to principles of accounting ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship/3.3.introduction_to_principles_of_accounting.txt" . || exit 1
              mv 3.3.introduction_to_principles_of_accounting.txt .3.3.introduction_to_principles_of_accounting.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.3.introduction_to_principles_of_accounting.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.3.introduction_to_principles_of_accounting.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.3.introduction_to_principles_of_accounting.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.3.introduction_to_principles_of_accounting.txt
              process_reminders_from_file .3.3.introduction_to_principles_of_accounting.txt
              STATE_FILE=".s_entrepreneurship_3_3"
              process_file .3.3.introduction_to_principles_of_accounting.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.3.introduction_to_principles_of_accounting.txt
              sed -i '/^1$/!d' .s_entrepreneurship_3_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship/S3"
              # Define the file extension
              file_extension_question=".3.introduction_to_principles_of_accounting.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".entrepreneurship.3.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Entrepreneurship/S3"
                # Define the file extension
                file_extension_answer=".3.introduction_to_principles_of_accounting.ans.txt"
                # Define the exercise file
                exercise_file="../../entrepreneurship_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .entrepreneurship.3.3
              fi
              if ! [ -f ".s_entrepreneurship_3_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Tax administration ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship/3.4.tax_administration.txt" . || exit 1
              mv 3.4.tax_administration.txt .3.4.tax_administration.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.4.tax_administration.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.4.tax_administration.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.4.tax_administration.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.4.tax_administration.txt
              process_reminders_from_file .3.4.tax_administration.txt
              STATE_FILE=".s_entrepreneurship_3_4"
              process_file .3.4.tax_administration.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.4.tax_administration.txt
              sed -i '/^1$/!d' .s_entrepreneurship_3_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship/S3"
              # Define the file extension
              file_extension=".4.tax_administration.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Entrepreneurship/S3"
              # Define the file extension
              file_extension_question=".4.tax_administration.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".entrepreneurship.3.4" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/Entrepreneurship/S3"
	              # Define the file extension
	              file_extension_answer=".4.tax_administration.ans.txt"
	              # Define the exercise file
	              exercise_file="../../entrepreneurship_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
								touch .entrepreneurship.3.4
								echo "3" > .entrepreneurship_ready
							fi
            ;;
    ...
    if ! find . -maxdepth 1 -name '.s_entrepreneurship_4*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to S4 Entrepreneurship class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
      clear_and_center "There are ${r}4${t} topics to be covered. Your tasks will always expand or shrink to fit in the time you give them. For that reason, never procrastinate darling!"
    fi
    attempts=0
    ...
    do
      ...
      do
        ...
        then
          attempts=0
          # Define the targeted directory
          question_directory="Revision/Entrepreneurship/S4"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../entrepreneurship_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Entrepreneurship/S4"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../entrepreneurship_answered_ans.txt"
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_entrepreneurship_4_1" ]; then
                echo -e "\n\nYou chose to explore Insurance ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship/4.1.insurance.txt" . || exit 1
              mv 4.1.insurance.txt .4.1.insurance.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.1.insurance.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.1.insurance.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.1.insurance.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.1.insurance.txt
              process_reminders_from_file .4.1.insurance.txt
              STATE_FILE=".s_entrepreneurship_4_1"
              process_file .4.1.insurance.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.1.insurance.txt
              sed -i '/^1$/!d' .s_entrepreneurship_4_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship/S4"
              # Define the file extension
              file_extension=".1.insurance.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Entrepreneurship/S4"
              # Define the file extension
              file_extension_question=".1.insurance.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".entrepreneurship.4.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Entrepreneurship/S4"
                # Define the file extension
                file_extension_answer=".1.insurance.ans.txt"
                # Define the exercise file
                exercise_file="../../entrepreneurship_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .entrepreneurship.4.1
              fi
              if ! [ -f ".s_entrepreneurship_4_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into International trade ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship/4.2.international_trade.txt" . || exit 1
              mv 4.2.international_trade.txt .4.2.international_trade.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.2.international_trade.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.2.international_trade.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.2.international_trade.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.2.international_trade.txt
              process_reminders_from_file .4.2.international_trade.txt
              STATE_FILE=".s_entrepreneurship_4_2"
              process_file .4.2.international_trade.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.2.international_trade.txt
              sed -i '/^1$/!d' .s_entrepreneurship_4_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship/S4"
              # Define the file extension
              file_extension_question=".2.international_trade.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".entrepreneurship.4.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Entrepreneurship/S4"
                # Define the file extension
                file_extension_answer=".2.international_trade.ans.txt"
                # Define the exercise file
                exercise_file="../../entrepreneurship_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .entrepreneurship.4.2
              fi
              if ! [ -f ".s_entrepreneurship_4_3" ]; then
                echo -e "\n\nYou have made a choice to cover Tax compliance ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship/4.3.tax_compliance.txt" . || exit 1
              mv 4.3.tax_compliance.txt .4.3.tax_compliance.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.3.tax_compliance.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.3.tax_compliance.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.3.tax_compliance.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.3.tax_compliance.txt
              process_reminders_from_file .4.3.tax_compliance.txt
              STATE_FILE=".s_entrepreneurship_4_3"
              process_file .4.3.tax_compliance.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.3.tax_compliance.txt
              sed -i '/^1$/!d' .s_entrepreneurship_4_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship/S4"
              # Define the file extension
              file_extension_question=".3.tax_compliance.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".entrepreneurship.4.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Entrepreneurship/S4"
                # Define the file extension
                file_extension_answer=".3.tax_compliance.ans.txt"
                # Define the exercise file
                exercise_file="../../entrepreneurship_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .entrepreneurship.4.3
              fi
              if ! [ -f ".s_entrepreneurship_4_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Basic tax computations ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship/4.4.basic_tax_computations.txt" . || exit 1
              mv 4.4.basic_tax_computations.txt .4.4.basic_tax_computations.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.4.basic_tax_computations.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.4.basic_tax_computations.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.4.basic_tax_computations.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.4.basic_tax_computations.txt
              process_reminders_from_file .4.4.basic_tax_computations.txt
              STATE_FILE=".s_entrepreneurship_4_4"
              process_file .4.4.basic_tax_computations.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.4.basic_tax_computations.txt
              sed -i '/^1$/!d' .s_entrepreneurship_4_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship/S4"
              # Define the file extension
              file_extension=".4.basic_tax_computations.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Entrepreneurship/S4"
              # Define the file extension
              file_extension_question=".4.basic_tax_computations.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".entrepreneurship.4.4" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/Entrepreneurship/S4"
	              # Define the file extension
	              file_extension_answer=".4.basic_tax_computations.ans.txt"
	              # Define the exercise file
	              exercise_file="../../entrepreneurship_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
								touch .entrepreneurship.4.4
								echo "4" > .entrepreneurship_ready
							fi
            ;;
