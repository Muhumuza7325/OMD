...
while true; do
    ...
    if [[ -n "$user_input" ]]; then
        ...
        if [[ "$user_input" == "zz" ]]; then
            ...
            TEMP_FILE=$(mktemp) && curl -o "$TEMP_FILE" -L "https://github.com/Muhumuza7325/Muhumuza7325/raw/main/entrepreneurship_tutorial_wsl.sh" && mv "$TEMP_FILE" entrepreneurship_tutorial_wsl.sh && chmod +x entrepreneurship_tutorial_wsl.sh && echo -e "\n\n${y}Code successfully updated.. You will have to restart a new session${t} \c" && sleep 4 && exit || (echo -e "\n\n${m}Error updating code!... Please check your internet connection and try again!${t} \c" && rm -f "$TEMP_FILE" && return)
        fi
        if [[ "$user_input" == "xx" ]]; then
            ...
            if [[ "$input" == "y" || "$input" == "Y" ]]; then
                ...
                curl -O -L "https://github.com/Muhumuza7325/Muhumuza7325/raw/main/update_entrepreneurship.sh" || { echo -e "\n\n${m}Check your internet connection and try again!${t}" >&2; return; }
...
if ! [ -d "Notes" ] || ! [ -d "Revision" ] || ! [ -d "Exercise" ] || ! [ -d "Videos" ] || ! [ -d "Figures" ] || ! [ -d "Tables" ]; then
    ...
    cp ../entrepreneurship_tutorial .
    clear_and_center
    echo
fi

files=(Notes/Entrepreneurship/*.txt)
if [ ${#files[@]} -eq 0 ]; then
    ...
    if [[ -z "$user_input" ]]; then
        ...
        curl -O -L https://github.com/Muhumuza7325/Muhumuza7325/raw/main/1.1.introduction_to_entrepreneurship_education.txt || echo -e "\n\nError fetching material for this tutorial \c"
    ...
    rm -f ../entrepreneurship_tutorial 1.1.introduction_to_entrepreneurship_education.txt
fi
