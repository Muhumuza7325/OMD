...
function handle_class_input() {
  if { [ -n "$last_class" ] || [ -z "$last_topic" ]; } && [ -f .general_paper_surveyor ]; then
		while true; do
	    read -rp $'\n\nPlease enter '"${r}5 or 6${t}"' to go to your class or '"${r}x${t}"' to exit'$'\n\n> ' class
      if [ "$class" == "x" ]; then
        exit
      fi
		  # Check if .current_general_paper_class is accidentally empty
		  if [ ! -s .current_general_paper_class ]; then
		    # echo 5 to .current_general_paper_class
		    echo "5" > .current_general_paper_class
		  fi
		  # Read the value from the file
		  current_general_paper_class=$(<.current_general_paper_class) 2>/dev/null
		  # Check if the value is equal to $class
		  if [ "$current_general_paper_class" -lt "$class" 2>/dev/null ]; then
	      ...
	      if [ "$class" != "x" ]; then
	        echo "$class" > .general_paper_user_state 2>/dev/null
	      fi
				break
	  	fi
		done
    ...
    for sentence in "${sentences[@]}"; do
      if [[ $echo_next == true ]]; then
        ...
        for split_sentence in "${split_sentences[@]}"; do
					echo "$split_sentence" >> .general_paper_reminder
        done
      echo_next=false
      fi
        ...
        echo "Did you know that $lower_cased_sentence" >> .general_paper_reminder
        ...
        echo_next=true
  			...
  			echo "Do you recall that $lower_cased_sentence;" >> .general_paper_reminder

			elif [[ "$sentence" =~ " → " ]]; then
  			echo "Hope you know that: $sentence;" >> .general_paper_reminder

      elif [[ "$sentence" =~ " ↔ " ]]; then
        echo "Hope you know that: $sentence;" >> .general_paper_reminder

      elif [[ "$sentence" =~ (Generally|general) ]]; then
        echo "Note: $sentence;" >> .general_paper_reminder
...
process_random_reminder() {
  ...
  # Check if .general_paper_reminder exists
  if [ -f .general_paper_reminder ]; then
		# Remove empty lines from .general_paper_reminder
  	sed -i '/^[[:space:]]*$/d' .general_paper_reminder 2>/dev/null
    # Randomly select a non-empty sentence
    local reminder # Declare the variable
		reminder=$(awk -v RS=';' 'BEGIN{srand();}{gsub(/^[[:space:]]+|[[:space:]]+$/, ""); if (length > 0) a[++n]=$0}END{if (n > 0) print a[int(rand()*n)+1]}' .general_paper_reminder)
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    for sentence in "${sentences[@]}"; do
        ...
        if [[ $sentence == *"Figure"* ]]; then
          ...
          # Change to the "Figures/General_paper" directory
          cd Figures/General_paper || { echo "Failed to change to Figures/General_paper"; return; }
        ...
        if [[ $sentence == *"Table"* ]]; then
          cd Tables/General_paper || { echo -e "\nFailed to change to Tables/General_paper \c"; return; }
          explorer.exe "$sentence" > /dev/null 2>&1 &
          cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        fi

        if [[ $sentence == *"Video"* ]]; then
          cd Videos/General_paper || { echo -e "\nFailed to change to Videos/General_paper \c"; return; }
          explorer.exe "$sentence" > /dev/null 2>&1 &
          cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        fi
...
contact_ai() {
  last_topic=$(awk -F' ' '{print $2}' .general_paper_user_state)
...
while true; do
  ...
  # Prompt the user for input
  if [ -z "$(find ./Notes/General_paper -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
  ...
  if [[ -n "$user_input" ]]; then
    ...
    if [[ "$user_input" == "zz" ]]; then
      ...
      TEMP_FILE=$(mktemp) && curl -o "$TEMP_FILE" -L "https://github.com/Muhumuza7325/OMD/raw/main/general_paper_tutorial_wsl.sh" && mv "$TEMP_FILE" general_paper_tutorial_wsl.sh && chmod +x general_paper_tutorial_wsl.sh && echo -e "\n\n${y}Code successfully updated.. You will have to restart a new session${t} \c" && sleep 4 && exit || (echo -e "\n\n${m}Error updating code!... Please check your internet connection and try again!${t} \c" && rm -f "$TEMP_FILE" && return)
    fi
    ...
    if [[ -n "$result" ]]; then
      ...
      if echo "$result" | grep "Figure"; then
        echo "$result" | grep -o '\bFigure[0-9]\+.*\.jpg\(\.[0-9]\+\)*\b' > Figures/General_paper/figures.txt
        sed -i -e '/^Figure/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Figures/General_paper/figures.txt
        # Change to the "Figures/General_paper" directory
        cd Figures/General_paper || { echo "Failed to change to Figures/General_paper"; return; }
        ...
        rm -f Figures/General_paper/figures.txt
      fi

      if echo "$result" | grep "Table"; then
        echo "$result" | grep -o '\bTable[0-9]\+\(\.[0-9]\+\)*\b' > Tables/General_paper/tables.txt
        sed -i -e '/^Table/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Tables/General_paper/tables.txt
        cd Tables/General_paper || { echo -e "\nFailed to change to Tables/General_paper \c"; return; }
        ...
        # Go back to the original directory
        cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        rm -f Tables/General_paper/tables.txt
      fi

      if echo "$result" | grep "Video"; then
        #echo "$result" > Videos/General_paper/videos.txt
        echo "$result" | grep -o '\bVideo[0-9]\+\(\.[0-9]\+\)*\b' > Videos/General_paper/videos.txt
        # Remove lines not starting with "Video" and any leading/trailing whitespaces
        sed -i -e '/^Video/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Videos/General_paper/videos.txt
        # Change to the "Videos/General_paper" directory
        cd Videos/General_paper || { echo -e "\nFailed to change to Videos/General_paper \c"; return; }
        ...
        cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        # Remove the temporary file
        rm -f Videos/General_paper/videos.txt
      fi
  ...
  if [ -f "$selected_file" ]; then
    ...
    if [[ -n "$selected_question" && "$selected_question" =~ [[:graph:]] ]]; then
      ...
      if echo "$selected_question" | grep "Figure"; then
        echo "$selected_question" | grep -o '\bFigure[0-9]\+[^;]*\b' > ../../Figures/General_paper/figures.txt
        sed -i -e '/^Figure/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' ../../Figures/General_paper/figures.txt
        # Change to the "Figures/General_paper" directory
        cd ../../Figures/General_paper || { echo "Failed to change to Figures/General_paper"; return; }
        # Specify the path to the text file containing figure names
        ...
        # Go back to the original directory
        cd ../../Revision/General_paper || { echo "Failed to change back to the targeted directory \c"; exit 1; }
        # Remove the temporary file
        rm -f ../../Figures/General_paper/figures.txt
      fi
...
process_question_answer() {
  ...
  if [ -f ../../../.school_name ]; then
    ...
    sed -i '/^[[:space:]]*$/d' ../../../."$school_name"_students_file.txt
    existing_class=$(awk '{print $1}' ../../../.general_paper_user_state)
    existing_topic=$(awk '{print $2}' ../../../.general_paper_user_state)
    echo ''
    ...
    if grep -q "General_paper" ../../../."$school_name"_students_file.txt; then
      sed -i -E 's/;/\n/g' ../../../."$school_name"_students_file.txt
      # Find the line with the word General_paper, replace the information below it with the already available information adding a comma existing_class_existing_topic [$percentage]
      if [ "$existing_topic" == "r" ]; then
        sed -i '/General_paper/{n;s/\(.*\)/\1, '"$existing_class"'_'"$input"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
      else
        (( existing_topic-- ))
        sed -i '/General_paper/{n;s/\(.*\)/\1, '"$existing_class"'_'"$existing_topic"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
      fi
    else
      if [ "$existing_topic" == "r" ]; then
        sed -i -E '/^School/ i\General_paper\n'"${existing_class} ${names}[${percentage}%]"'' ../../../."$school_name"_students_file.txt
        echo -e "General_paper\n"$existing_class"_'"$input"' "$names"["$percentage%"]" >> ../../../."$school_name"_students_file.txt
      else
        (( existing_topic-- ))
...
function resume_from_last_point() {
  if [ -f .general_paper_user_state ]; then
    last_class=$(awk -F' ' '{print $1}' .general_paper_user_state)
    last_topic=$(awk -F' ' '{print $2}' .general_paper_user_state)
    if [ -n "$last_class" ] && [ -n "$last_topic" ]; then
      echo -e "\n       Resuming from ${r}S$last_class${t} : ${g}Topic '$last_topic'${t} \c"
      rm -f .general_paper_surveyor
...
process_final_assignment() {
  # Check if user is.general_paper_ready for the assignment
  # Check if .current_general_paper_class is accidentally empty
  if [ ! -s .current_general_paper_class ]; then
    # echo 5 to .current_general_paper_class
    echo "5" > .current_general_paper_class
  fi
  # Read the value from the .current_general_paper_class file
  current_general_paper_class=$(<.current_general_paper_class) 2>/dev/null
  # Check if the value in the .general_paper_ready file is equal to $class
  # Read the value from the .general_paper_ready file
  if [ ! -s .general_paper_ready ]; then
    echo "0" > .general_paper_ready
  fi
  how.general_paper_ready=$(<.general_paper_ready) 2>/dev/null
  # Check if the value is equal to $class
  if [ "$how.general_paper_ready" -lt "$current_general_paper_class" 2>/dev/null ]; then
    read -rp $'\n\nYou havent done all the topic assignments for your current general_paper class\n\n'"${r}Proceeding from here will affect your very final score${y}"'. To go back and progress right, enter '"${y}ok${t}"'. Otherwise, press the Enter key to do the final class assignment: ' progress
    if [ "$progress" == "ok" ]; then
      return
    fi
  fi
    ...
    if ! [ "$total_questions" == 100 ]; then
      ...
      if [ "$elapsed_time" -gt 3600 ]; then
        ...
        if [ "$percentage" -lt 95 ]; then
          ...
          cd - > /dev/null 2>&1 || exit
          if ! [ -f .current_general_paper_class ]; then
            # Echo the result to the file .current_class
            echo "2" > .current_general_paper_class
            ...
            # Read the value from the file
            current_general_paper_class=$(<.current_general_paper_class) 2>/dev/null
            # Check if the new_class value is lt $current_general_paper_class
            if [ "$new_class" -gt "$current_general_paper_class" 2>/dev/null ]; then
		          # Echo the result to the file .current_class
		          echo "$new_class" > .current_general_paper_class
            fi
...
get_sample_items() {
  ...
  # Save the current working directory
  pushd . > /dev/null
  cd Revision/General_paper || { echo "Directory not found"; return; }
  ...
  while true; do
      ...
      read -rp $'\n\n\nEnter '"${m}any character${t}"' for access to the file of answered items or simply press '"${r}Enter${t}"' to get items to attempt : ' input
     	if [[ -n $input ]]; then
     		explorer.exe .general_paper_samples* > /dev/null 2>&1 &
      	clear
      	popd > /dev/null || exit
        return
      fi
      if [ -f .e_o_c_general_paper.txt ]; then
        echo -e "\n\n${y}Below is the list of the elements of construct${t} \n"
        cat .e_o_c_general_paper.txt
        ...
        if [[ -n $input ]]; then
          ...
          selected_file1=$(ls -a | grep -E "\.e_o_c_general_paper_${input}\.txt")
          cat "$selected_file1"
          # Remove empty lines from the selected files
          find . -type f -name "*_samples_[0-9].txt" -exec sed -i '/^[[:space:]]*$/d' {} +
          selected_file=$(ls -a | grep -E "\.e_o_c_general_paper_${input}_samples" | shuf -n 1)
        else
    ...
    rm -f .revise.txt .general_paper_topic_selected 2>/dev/null
  done
  popd > /dev/null || exit
	return
}
...
while true; do
  ...
  if ls Students | grep -wq "$initials"; then
    ...
    echo -e "                  $initials\n" > "Students/$initials/Exercise/general_paper_answered_ans.txt"
    echo -e "                  $initials\n" > "Students/$initials/Revision/general_paper_covered_qns.txt"
    ...
    if [ -f ../../../.school_name ]; then
      ...
      if grep -q "General_paper" ../../../."$school_name"_students_file.txt; then
        ...
        if [ "$existing_topic" == "z" ]; then
          sed -i '/General_paper/{n;s/\(.*\)/\1, '"$existing_class"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
        else
          (( existing_topic-- ))
          sed -i '/General_paper/{n;s/\(.*\)/\1, '"$existing_class"'_'"$existing_topic"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
        fi
...
if [ ! -f .general_paper_user_state ]; then
	touch .general_paper_user_state
	touch .general_paper_surveyor
	get_and_display_pattern
else
...
if [ -z "$class" ] && [ -s ".general_paper_user_state" ]; then
  get_and_display_pattern
fi
# Check for the presence of specific directories and a file
if ! [ -d "Notes/General_paper" ] || ! [ -d "Revision" ] || ! [ -d "Exercise" ] || ! [ -d "Videos" ] || ! [ -d "Figures" ] || ! [ -d "Tables" ]; then
  ...
  touch Revision/general_paper_covered_qns.txt Exercise/general_paper_answered_ans.txt
  echo -e "\n"
  pwd
...
if [ -z "$(find ./Notes/General_paper -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
  ...
  if [[ -z "$user_input" ]]; then
    ...
    curl -O -L https://github.com/Muhumuza7325/OMD/raw/main/GP/1.1.communication.txt || echo -e "\n\nError fetching material for this tutorial \c"
    ...
    do
      ...
      do
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_general_paper_5_1" ]; then
                echo -e "\n\nYou chose to explore Communication ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/General_paper/5.1.communication.txt" . || exit 1
              mv 5.1.communication.txt .5.1.communication.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .5.1.communication.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .5.1.communication.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .5.1.communication.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .5.1.communication.txt
              process_reminders_from_file .5.1.communication.txt
              STATE_FILE=".s_general_paper_5_1"
              process_file .5.1.communication.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .5.1.communication.txt
              sed -i '/^1$/!d' .s_general_paper_5_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/General_paper/S5"
              # Define the file extension
              file_extension=".1.communication.qns.txt"
              # Define the revision file
              revision_file="../../general_paper_covered_qns.txt"
              # Call the function to process a random question
              ...
              question_directory="Revision/General_paper/S5"
              # Define the file extension
              file_extension_question=".1.communication.qns.txt"
              # Define the revision file
              revision_file="../../general_paper_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".general_paper.5.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/General_paper/S5"
                # Define the file extension
                file_extension_answer=".1.communication.ans.txt"
                # Define the exercise file
                exercise_file="../../general_paper_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .general_paper.5.1
              fi
              if ! [ -f ".s_general_paper_5_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Language and literature ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/General_paper/5.2.language_and_literature.txt" . || exit 1
              mv 5.2.language_and_literature.txt .5.2.language_and_literature.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .5.2.language_and_literature.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .5.2.language_and_literature.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .5.2.language_and_literature.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .5.2.language_and_literature.txt
              process_reminders_from_file .5.2.language_and_literature.txt
              STATE_FILE=".s_general_paper_5_2"
              process_file .5.2.language_and_literature.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .5.2.language_and_literature.txt
              sed -i '/^1$/!d' .s_general_paper_5_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/General_paper/S5"
              # Define the file extension
              file_extension_question=".2.language_and_literature.qns.txt"
              # Define the revision file
              revision_file="../../general_paper_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".general_paper.5.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/General_paper/S5"
                # Define the file extension
                file_extension_answer=".2.language_and_literature.ans.txt"
                # Define the exercise file
                exercise_file="../../general_paper_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .general_paper.5.2
              fi
              if ! [ -f ".s_general_paper_5_3" ]; then
                echo -e "\n\nYou have made a choice to cover Extended essay writing ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/General_paper/5.3.extended_essay_writing.txt" . || exit 1
              mv 5.3.extended_essay_writing.txt .5.3.extended_essay_writing.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .5.3.extended_essay_writing.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .5.3.extended_essay_writing.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .5.3.extended_essay_writing.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .5.3.extended_essay_writing.txt
              process_reminders_from_file .5.3.extended_essay_writing.txt
              STATE_FILE=".s_general_paper_5_3"
              process_file .5.3.extended_essay_writing.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .5.3.extended_essay_writing.txt
              sed -i '/^1$/!d' .s_general_paper_5_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/General_paper/S5"
              # Define the file extension
              file_extension_question=".3.extended_essay_writing.qns.txt"
              # Define the revision file
              revision_file="../../general_paper_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
