...
if ! [ -d "Notes" ] || ! [ -d "Revision" ] || ! [ -d "Exercise" ] || ! [ -d "Videos" ] || ! [ -d "Figures" ] || ! [ -d "Tables" ]; then
    ...
    cp ../geography_tutorial .
    clear_and_center
    echo
fi

files=(Notes/Geography/*.txt)
if [ ${#files[@]} -eq 0 ]; then
    ...
    if [[ -z "$user_input" ]]; then
        ...
        curl -O -L https://github.com/Muhumuza7325/OMD/raw/main/1.1.introduction.txt || echo -e "\n\nError fetching material for this tutorial \c"
    ...
    rm -f ../geography_tutorial 1.1.introduction.txt
fi
