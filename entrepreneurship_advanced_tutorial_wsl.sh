...
if ! [ -d "Notes" ] || ! [ -d "Revision" ] || ! [ -d "Exercise" ] || ! [ -d "Videos" ] || ! [ -d "Figures" ] || ! [ -d "Tables" ]; then
    ...
    cp ../entrepreneurship_advanced_tutorial .
    clear_and_center
    echo
fi

files=(Notes/Entrepreneurship_advanced/*.txt)
if [ ${#files[@]} -eq 0 ]; then
    ...
    rm -f ../entrepreneurship_advanced_tutorial 1.1.introduction_to_entrepreneurship_education.txt
fi
