...
function handle_class_input() {
  if { [ -n "$last_class" ] || [ -z "$last_topic" ]; } && [ -f .entrepreneurship_advanced_surveyor ]; then
		while true; do
	    read -rp $'\n\nPlease enter '"${r}5 or 6${t}"' to go to your class or '"${r}x${t}"' to exit'$'\n\n> ' class
      if [ "$class" == "x" ]; then
        exit
      fi
		  # Check if .current_entrepreneurship_advanced_class is accidentally empty
		  if [ ! -s .current_entrepreneurship_advanced_class ]; then
		    # echo 5 to .current_entrepreneurship_advanced_class
		    echo "5" > .current_entrepreneurship_advanced_class
		  fi
		  # Read the value from the file
		  current_entrepreneurship_advanced_class=$(<.current_entrepreneurship_advanced_class) 2>/dev/null
		  # Check if the value is equal to $class
		  if [ "$current_entrepreneurship_advanced_class" -lt "$class" 2>/dev/null ]; then
	      ...
	      rm -f ".entrepreneurship_advanced_topic_selected"
	      # Update the state file with the class
	      if [ "$class" != "x" ]; then
	        echo "$class" > .entrepreneurship_advanced_user_state 2>/dev/null
	      fi
				break
	  	fi
		done
...
function handle_s6_topic_input() {
  ...
  if [ -z "$last_topic" ] || [ -f .entrepreneurship_advanced_topic_selected ]; then
    read -rp $'\n\nChoose either topic '"${g}1 or 2 or 3 or 4 or 5 or 6 or 7${t}"' to learn'$'\nor enter '"${r}z${t}"' for an adventure or '"${r}r${t}"' to revise or '"${r}s${t}"' to get sample_items'$'\nor '"${r}a${t}"' to get an activity of integration or '"${r}q${t}"' to get a short answer question'$'\nor '"${r}n${t}"' to do your final class assignment and if necessary, gain access to the next class'$'\nor '"${r}p${t}"' to track academic progress or '"${r}x${t}"' to exit'$'\n\n1. Small and medium enterprises (SMEs) '"${r}Term1${t}"''$'\n\n2. Insurance in business '"${r}Term1${t}"''$'\n\n3. Capital markets '"${r}Term1${t}"''$'\n\n4. Human resources in an enterprise '"${r}Term2${t}"''$'\n\n5. Finance in an enterprise '"${r}Term2${t}"''$'\n\n6. Taxation '"${r}Term3${t}"''$'\n\n7. Business competition '"${r}Term3${t}"''$'\n\n> ' topic
    touch .entrepreneurship_advanced_surveyor
    touch .entrepreneurship_advanced_topic_selected
    # Update the state file with the topic
    # Check if the state file exists, and the topic is not "x"
    if [ -f .entrepreneurship_advanced_user_state ] && [ "$topic" != "x" ]; then
      # Get the current class value from the state file
      existing_class=$(awk '{print $1}' .entrepreneurship_advanced_user_state)
      # Update the state file with the topic, preserving the existing class value
      echo "$existing_class $topic" > .entrepreneurship_advanced_user_state 2>/dev/null
    fi
  fi
}
...
function handle_s5_topic_input() {
  ...
  if [ -z "$last_topic" ] || [ -f .entrepreneurship_advanced_topic_selected ]; then
    read -rp $'\n\nChoose either topic '"${g}1 or 2 or 3 or 4 or 5 or 6 or 7${t}"' to learn'$'\nor enter '"${r}z${t}"' for an adventure or '"${r}r${t}"' to revise or '"${r}s${t}"' to get sample_items'$'\nor '"${r}a${t}"' to get an activity of integration or '"${r}q${t}"' to get a short answer question'$'\nor '"${r}n${t}"' to do your final class assignment and if necessary, gain access to the next class'$'\nor '"${r}p${t}"' to track academic progress or '"${r}x${t}"' to exit'$'\n\n1. Introduction to entrepreneurship education '"${r}Term1${t}"''$'\n\n2. The entrepreneurial environment '"${r}Term1${t}"''$'\n\n3. Business ideas and opportunities '"${r}Term2${t}"''$'\n\n4. Business planning '"${r}Term2${t}"''$'\n\n5. Production in an enterprise '"${r}Term2${t}"''$'\n\n6. Marketing in an enterprise '"${r}Term3${t}"''$'\n\n7. Social entrepreneurship '"${r}Term3${t}"''$'\n\n> ' topic
    touch .entrepreneurship_advanced_surveyor
    touch .entrepreneurship_advanced_topic_selected
    # Update the state file with the topic
    # Check if the state file exists, and the topic is not "x"
...
process_reminders_from_file() {
  ...
  prev_sentence=""
  echo_next=false
	echo -n > .entrepreneurship_advanced_reminder
    ...
    for sentence in "${sentences[@]}"; do
      if [[ $echo_next == true ]]; then
        ...
        for split_sentence in "${split_sentences[@]}"; do
					echo "$split_sentence" >> .entrepreneurship_advanced_reminder
        done
      echo_next=false
      fi
        ...
        echo "Did you know that $lower_cased_sentence" >> .entrepreneurship_advanced_reminder
        ...
        echo_next=true
  			...
  			echo "Do you recall that $lower_cased_sentence;" >> .entrepreneurship_advanced_reminder

			elif [[ "$sentence" =~ " → " ]]; then
  			echo "Hope you know that: $sentence;" >> .entrepreneurship_advanced_reminder

      elif [[ "$sentence" =~ " ↔ " ]]; then
        echo "Hope you know that: $sentence;" >> .entrepreneurship_advanced_reminder

      elif [[ "$sentence" =~ (Generally|general) ]]; then
        echo "Note: $sentence;" >> .entrepreneurship_advanced_reminder
...
process_random_reminder() {
  ...
  # Check if .entrepreneurship_advanced_reminder exists
  if [ -f .entrepreneurship_advanced_reminder ]; then
		# Remove empty lines from .entrepreneurship_advanced_reminder
  	sed -i '/^[[:space:]]*$/d' .entrepreneurship_advanced_reminder 2>/dev/null
    # Randomly select a non-empty sentence
    local reminder # Declare the variable
		reminder=$(awk -v RS=';' 'BEGIN{srand();}{gsub(/^[[:space:]]+|[[:space:]]+$/, ""); if (length > 0) a[++n]=$0}END{if (n > 0) print a[int(rand()*n)+1]}' .entrepreneurship_advanced_reminder)
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    for sentence in "${sentences[@]}"; do
        ...
        if [[ $sentence == *"Figure"* ]]; then
          ...
          # Change to the "Figures/Entrepreneurship_advanced" directory
          cd Figures/Entrepreneurship_advanced || { echo "Failed to change to Figures/Entrepreneurship_advanced"; return; }
        ...
        if [[ $sentence == *"Table"* ]]; then
          cd Tables/Entrepreneurship_advanced || { echo -e "\nFailed to change to Tables/Entrepreneurship_advanced \c"; return; }
          explorer.exe "$sentence" > /dev/null 2>&1 &
          cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        fi

        if [[ $sentence == *"Video"* ]]; then
          cd Videos/Entrepreneurship_advanced || { echo -e "\nFailed to change to Videos/Entrepreneurship_advanced \c"; return; }
          explorer.exe "$sentence" > /dev/null 2>&1 &
          cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        fi
...
contact_ai() {
  last_topic=$(awk -F' ' '{print $2}' .entrepreneurship_advanced_user_state)
...
while true; do
  ...
  # Prompt the user for input
  if [ -z "$(find ./Notes/Entrepreneurship_advanced -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
  ...
  if [[ -n "$user_input" ]]; then
		if [[ "$user_input" == "sh" ]]; then
		  Response="We are looking forward to receiving your economic support, thoughts, suggestions, and any files you feel should reach out to everyone of our children. Please remember to label the files you are to attach using the format: Your_name_School_Entrepreneurship_advanced_File_content (e.g., Muhumuza_Omega_Kasule_High_School_O_level_Chemistry_Answered_EOC1_Items.pdf). Thanks a lot for your contributions."  
		  entrepreneurship_advanced="$(basename "$0") - $(date +"%Y-%m-%d %H:%M:%S") - Thoughts, suggestions, and contributions"
		  encoded_entrepreneurship_advanced=$(echo "$entrepreneurship_advanced" | sed 's/ /%20/g; s/\n/%0A/g')
		  encoded_body=$(echo "$Response" | sed 's/ /%20/g; s/\n/%0A/g')
		  # Open the email in the browser with the encoded entrepreneurship_advanced and body
		  powershell.exe -Command "Start-Process 'https://mail.google.com/mail/?view=cm&to=2024omd256@gmail.com&su=${encoded_entrepreneurship_advanced}&body=${encoded_body}'"
			return
		fi
     ...
     if [[ "$user_input" == "sr" ]]; then
      ...
      explorer.exe Entrepreneurship_advanced > /dev/null 2>&1 &
    ...
    if [[ "$user_input" == "zz" ]]; then
      ...
      TEMP_FILE=$(mktemp) && curl -o "$TEMP_FILE" -L "https://github.com/Muhumuza7325/OMD/raw/main/entrepreneurship_advanced_tutorial_wsl.sh" && mv "$TEMP_FILE" entrepreneurship_advanced_tutorial_wsl.sh && chmod +x entrepreneurship_advanced_tutorial_wsl.sh && echo -e "\n\n${y}Code successfully updated.. You will have to restart a new session${t} \c" && sleep 4 && exit || (echo -e "\n\n${m}Error updating code!... Please check your internet connection and try again!${t} \c" && rm -f "$TEMP_FILE" && return)
    fi
    if [[ "$user_input" == "xx" ]]; then
      ...
      if [[ "$input" == "y" || "$input" == "Y" ]]; then
        ...
        curl -O -L "https://github.com/Muhumuza7325/OMD/raw/main/update_entrepreneurship_advanced.sh" || { echo -e "\n\n${m}Check your internet connection and try again!${t}" >&2; return; }
        mv update_entrepreneurship_advanced.sh .update_entrepreneurship_advanced.sh
        bash .update_entrepreneurship_advanced.sh
        return
      else
        return
      fi
    ...
    if [[ -n "$result" ]]; then
      ...
      if echo "$result" | grep "Figure"; then
        echo "$result" | grep -o '\bFigure[0-9]\+.*\.jpg\(\.[0-9]\+\)*\b' > Figures/Entrepreneurship_advanced/figures.txt
        sed -i -e '/^Figure/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Figures/Entrepreneurship_advanced/figures.txt
        # Change to the "Figures/Entrepreneurship_advanced" directory
        cd Figures/Entrepreneurship_advanced || { echo "Failed to change to Figures/Entrepreneurship_advanced"; return; }
        ...
        rm -f Figures/Entrepreneurship_advanced/figures.txt
      fi

      if echo "$result" | grep "Table"; then
        echo "$result" | grep -o '\bTable[0-9]\+\(\.[0-9]\+\)*\b' > Tables/Entrepreneurship_advanced/tables.txt
        sed -i -e '/^Table/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Tables/Entrepreneurship_advanced/tables.txt
        cd Tables/Entrepreneurship_advanced || { echo -e "\nFailed to change to Tables/Entrepreneurship_advanced \c"; return; }
        ...
        # Go back to the original directory
        cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        rm -f Tables/Entrepreneurship_advanced/tables.txt
      fi

      if echo "$result" | grep "Video"; then
        #echo "$result" > Videos/Entrepreneurship_advanced/videos.txt
        echo "$result" | grep -o '\bVideo[0-9]\+\(\.[0-9]\+\)*\b' > Videos/Entrepreneurship_advanced/videos.txt
        # Remove lines not starting with "Video" and any leading/trailing whitespaces
        sed -i -e '/^Video/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Videos/Entrepreneurship_advanced/videos.txt
        # Change to the "Videos/Entrepreneurship_advanced" directory
        cd Videos/Entrepreneurship_advanced || { echo -e "\nFailed to change to Videos/Entrepreneurship_advanced \c"; return; }
        ...
        cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        # Remove the temporary file
        rm -f Videos/Entrepreneurship_advanced/videos.txt
      fi
  ...
  if [ -f "$selected_file" ]; then
    ...
    if [[ -n "$selected_question" && "$selected_question" =~ [[:graph:]] ]]; then
      ...
      if echo "$selected_question" | grep "Figure"; then
        echo "$selected_question" | grep -o '\bFigure[0-9]\+[^;]*\b' > ../../Figures/Entrepreneurship_advanced/figures.txt
        sed -i -e '/^Figure/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' ../../Figures/Entrepreneurship_advanced/figures.txt
        # Change to the "Figures/Entrepreneurship_advanced" directory
        cd ../../Figures/Entrepreneurship_advanced || { echo "Failed to change to Figures/Entrepreneurship_advanced"; return; }
        # Specify the path to the text file containing figure names
        ...
        # Go back to the original directory
        cd ../../Revision/Entrepreneurship_advanced || { echo "Failed to change back to the targeted directory \c"; exit 1; }
        # Remove the temporary file
        rm -f ../../Figures/Entrepreneurship_advanced/figures.txt
      fi
...
process_question_answer() {
  ...
  if [ -f ../../../.school_name ]; then
    ...
    sed -i '/^[[:space:]]*$/d' ../../../."$school_name"_students_file.txt
    existing_class=$(awk '{print $1}' ../../../.entrepreneurship_advanced_user_state)
    existing_topic=$(awk '{print $2}' ../../../.entrepreneurship_advanced_user_state)
    echo ''
    ...
    if grep -q "Entrepreneurship_advanced" ../../../."$school_name"_students_file.txt; then
      sed -i -E 's/;/\n/g' ../../../."$school_name"_students_file.txt
      # Find the line with the word Entrepreneurship_advanced, replace the information below it with the already available information adding a comma existing_class_existing_topic [$percentage]
      if [ "$existing_topic" == "r" ]; then
        sed -i '/Entrepreneurship_advanced/{n;s/\(.*\)/\1, '"$existing_class"'_'"$input"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
      else
        (( existing_topic-- ))
        sed -i '/Entrepreneurship_advanced/{n;s/\(.*\)/\1, '"$existing_class"'_'"$existing_topic"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
      fi
    else
      if [ "$existing_topic" == "r" ]; then
        sed -i -E '/^School/ i\Entrepreneurship_advanced\n'"${existing_class} ${names}[${percentage}%]"'' ../../../."$school_name"_students_file.txt
        echo -e "Entrepreneurship_advanced\n"$existing_class"_'"$input"' "$names"["$percentage%"]" >> ../../../."$school_name"_students_file.txt
      else
        (( existing_topic-- ))
...
function resume_from_last_point() {
  if [ -f .entrepreneurship_advanced_user_state ]; then
    last_class=$(awk -F' ' '{print $1}' .entrepreneurship_advanced_user_state)
    last_topic=$(awk -F' ' '{print $2}' .entrepreneurship_advanced_user_state)
    if [ -n "$last_class" ] && [ -n "$last_topic" ]; then
      echo -e "\n       Resuming from ${r}S$last_class${t} : ${g}Topic '$last_topic'${t} \c"
      rm -f .entrepreneurship_advanced_surveyor
...
process_final_assignment() {
  # Check if user is.entrepreneurship_advanced_ready for the assignment
  # Check if .current_entrepreneurship_advanced_class is accidentally empty
  if [ ! -s .current_entrepreneurship_advanced_class ]; then
    # echo 5 to .current_entrepreneurship_advanced_class
    echo "5" > .current_entrepreneurship_advanced_class
  fi
  # Read the value from the .current_entrepreneurship_advanced_class file
  current_entrepreneurship_advanced_class=$(<.current_entrepreneurship_advanced_class) 2>/dev/null
  # Check if the value in the .entrepreneurship_advanced_ready file is equal to $class
  # Read the value from the .entrepreneurship_advanced_ready file
  if [ ! -s .entrepreneurship_advanced_ready ]; then
    echo "0" > .entrepreneurship_advanced_ready
  fi
  how.entrepreneurship_advanced_ready=$(<.entrepreneurship_advanced_ready) 2>/dev/null
  # Check if the value is equal to $class
  if [ "$how.entrepreneurship_advanced_ready" -lt "$current_entrepreneurship_advanced_class" 2>/dev/null ]; then
    read -rp $'\n\nYou havent done all the topic assignments for your current entrepreneurship_advanced class\n\n'"${r}Proceeding from here will affect your very final score${y}"'. To go back and progress right, enter '"${y}ok${t}"'. Otherwise, press the Enter key to do the final class assignment: ' progress
    if [ "$progress" == "ok" ]; then
      return
    fi
  fi
    ...
    if ! [ "$total_questions" == 100 ]; then
      ...
      if [ "$elapsed_time" -gt 3600 ]; then
        ...
        if [ "$percentage" -lt 95 ]; then
          ...
          cd - > /dev/null 2>&1 || exit
          if ! [ -f .current_entrepreneurship_advanced_class ]; then
            # Echo the result to the file .current_class
            echo "2" > .current_entrepreneurship_advanced_class
            ...
            # Read the value from the file
            current_entrepreneurship_advanced_class=$(<.current_entrepreneurship_advanced_class) 2>/dev/null
            # Check if the new_class value is lt $current_entrepreneurship_advanced_class
            if [ "$new_class" -gt "$current_entrepreneurship_advanced_class" 2>/dev/null ]; then
		          # Echo the result to the file .current_class
		          echo "$new_class" > .current_entrepreneurship_advanced_class
            fi
...
get_sample_items() {
  ...
  # Save the current working directory
  pushd . > /dev/null
  cd Revision/Entrepreneurship_advanced || { echo "Directory not found"; return; }
  ...
  while true; do
      ...
      if [ ! -f "$last_echo_time_file" ]; then
    	...
    	if [ $time_diff -gt 3600 ]; then
      	# Echo the message and update the last echo time
	    	echo -e "\n\n\n${r}You are advised to not make any changes to the provided answers, instead, you can make copies that you can edit${t}\n\n${y}For a teacher willing to join us reach out to everyone of our children, please send us your questions and answers in a file labelled with your name, school, entrepreneurship_advanced, and file content (e.g., Muhumuza_Omega_Kasule_High_School_O_level_Chemistry_Answered_EOC1_Items.pdf) to our contacts${t}\n\n\nEmail: ${g}2024omd256@gmail.com${t} \c"
				echo $current_time > "$last_echo_time_file"
  		fi
      read -rp $'\n\n\nEnter '"${m}any character${t}"' for access to the file of answered items or simply press '"${r}Enter${t}"' to get items to attempt : ' input
     	if [[ -n $input ]]; then
     		explorer.exe .entrepreneurship_advanced_samples* > /dev/null 2>&1 &
      	clear
      	popd > /dev/null || exit
        return
      fi
      if [ -f .e_o_c_entrepreneurship_advanced.txt ]; then
        echo -e "\n\n${y}Below is the list of the elements of construct${t} \n"
        cat .e_o_c_entrepreneurship_advanced.txt
        ...
        if [[ -n $input ]]; then
          ...
          selected_file1=$(ls -a | grep -E "\.e_o_c_entrepreneurship_advanced_${input}\.txt")
          cat "$selected_file1"
          # Remove empty lines from the selected files
          find . -type f -name "*_samples_[0-9].txt" -exec sed -i '/^[[:space:]]*$/d' {} +
          selected_file=$(ls -a | grep -E "\.e_o_c_entrepreneurship_advanced_${input}_samples" | shuf -n 1)
        else
    ...
    rm -f .revise.txt .entrepreneurship_advanced_topic_selected 2>/dev/null
  done
  popd > /dev/null || exit
	return
}
...
while true; do
  ...
  if ls Students | grep -wq "$initials"; then
    ...
    echo -e "                  $initials\n" > "Students/$initials/Exercise/entrepreneurship_advanced_answered_ans.txt"
    echo -e "                  $initials\n" > "Students/$initials/Revision/entrepreneurship_advanced_covered_qns.txt"
    ...
    if [ -f ../../../.school_name ]; then
      ...
      if grep -q "Entrepreneurship_advanced" ../../../."$school_name"_students_file.txt; then
        ...
        if [ "$existing_topic" == "z" ]; then
          sed -i '/Entrepreneurship_advanced/{n;s/\(.*\)/\1, '"$existing_class"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
        else
          (( existing_topic-- ))
          sed -i '/Entrepreneurship_advanced/{n;s/\(.*\)/\1, '"$existing_class"'_'"$existing_topic"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
        fi
...
if [ ! -f .entrepreneurship_advanced_user_state ]; then
	touch .entrepreneurship_advanced_user_state
	touch .entrepreneurship_advanced_surveyor
	get_and_display_pattern
else
  if [ -f .omd_communication.txt ]; then
    ...
    process_random_reminder .entrepreneurship_advanced_reminder
  fi
	handle_resume_input
fi

if [ -z "$class" ] && [ -s ".entrepreneurship_advanced_user_state" ]; then
  get_and_display_pattern
fi
# Check for the presence of specific directories and a file
if ! [ -d "Notes/Entrepreneurship_advanced" ] || ! [ -d "Revision" ] || ! [ -d "Exercise" ] || ! [ -d "Videos" ] || ! [ -d "Figures" ] || ! [ -d "Tables" ]; then
  ...
  mkdir -p Notes Notes/Entrepreneurship_advanced Revision Revision/Entrepreneurship_advanced Revision/Entrepreneurship_advanced/{S5,S6} Exercise Exercise/Entrepreneurship_advanced Exercise/Entrepreneurship_advanced/{S5,S6} Videos Videos/Entrepreneurship_advanced Figures Figures/Entrepreneurship_advanced Tables Tables/Entrepreneurship_advanced
  touch Revision/entrepreneurship_advanced_covered_qns.txt Exercise/entrepreneurship_advanced_answered_ans.txt
  echo -e "\n"
  pwd
...
if [ -z "$(find ./Notes/Entrepreneurship_advanced -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
  ...
  if [[ -z "$user_input" ]]; then
    ...
    cp 1.1.introduction_to_entrepreneurship_education.txt Notes/Entrepreneurship_advanced || echo -e "\n\nError copying 1.1.introduction_to_entrepreneurship_education.txt to the Entrepreneurship_advanced directory in the Notes directory \c"
    wait_for_a_key_press
  else
...
while true; do
  ...
  handle_class_input
  if [[ "$class" == "5" ]]; then
    if ! find . -maxdepth 1 -name '.s_entrepreneurship_advanced_5*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to the S5 Entrepreneurship class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
    ...
    do
      ...
      do
        ...
        then
          ...
          question_directory="Revision/Entrepreneurship_advanced/S5"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../entrepreneurship_advanced_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Entrepreneurship_advanced/S5"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../entrepreneurship_advanced_answered_ans.txt"
          # Call the function to process a random question
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_entrepreneurship_advanced_5_1" ]; then
              ...
              cp "Notes/Entrepreneurship_advanced/5.1.introduction_to_entrepreneurship_education.txt" . || exit 1
              mv 5.1.introduction_to_entrepreneurship_education.txt .5.1.introduction_to_entrepreneurship_education.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .5.1.introduction_to_entrepreneurship_education.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .5.1.introduction_to_entrepreneurship_education.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .5.1.introduction_to_entrepreneurship_education.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .5.1.introduction_to_entrepreneurship_education.txt
              process_reminders_from_file .5.1.introduction_to_entrepreneurship_education.txt
              STATE_FILE=".s_entrepreneurship_advanced_5_1"
              process_file .5.1.introduction_to_entrepreneurship_education.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .5.1.introduction_to_entrepreneurship_education.txt
              sed -i '/^1$/!d' .s_entrepreneurship_advanced_5_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship_advanced/S5"
              # Define the file extension
            ...
            ;;
            2)
              if ! [ -f ".entrepreneurship_advanced.5.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Entrepreneurship_advanced/S5"
                # Define the file extension
                ...
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .entrepreneurship_advanced.5.1
              fi
              if ! [ -f ".s_entrepreneurship_advanced_5_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into The entrepreneurial environment ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship_advanced/5.2.the_entrepreneurial_environment.txt" . || exit 1
              mv 5.2.the_entrepreneurial_environment.txt .5.2.the_entrepreneurial_environment.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .5.2.the_entrepreneurial_environment.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .5.2.the_entrepreneurial_environment.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .5.2.the_entrepreneurial_environment.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .5.2.the_entrepreneurial_environment.txt
              process_reminders_from_file .5.2.the_entrepreneurial_environment.txt
              STATE_FILE=".s_entrepreneurship_advanced_5_2"
              process_file .5.2.the_entrepreneurial_environment.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .5.2.the_entrepreneurial_environment.txt
              sed -i '/^1$/!d' .s_entrepreneurship_advanced_5_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship_advanced/S5"
              # Define the file extension
              file_extension_question=".2.the_entrepreneurial_environment.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".entrepreneurship_advanced.5.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Entrepreneurship_advanced/S5"
                # Define the file extension
                file_extension_answer=".2.the_entrepreneurial_environment.ans.txt"
                # Define the exercise file
                exercise_file="../../entrepreneurship_advanced_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .entrepreneurship_advanced.5.2
              fi
              if ! [ -f ".s_entrepreneurship_advanced_5_3" ]; then
                echo -e "\n\nYou have made a choice to cover Business ideas and opportunities ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship_advanced/5.3.business_ideas_and_opportunities.txt" . || exit 1
              mv 5.3.business_ideas_and_opportunities.txt .5.3.business_ideas_and_opportunities.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .5.3.business_ideas_and_opportunities.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .5.3.business_ideas_and_opportunities.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .5.3.business_ideas_and_opportunities.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .5.3.business_ideas_and_opportunities.txt
              process_reminders_from_file .5.3.business_ideas_and_opportunities.txt
              STATE_FILE=".s_entrepreneurship_advanced_5_3"
              process_file .5.3.business_ideas_and_opportunities.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .5.3.business_ideas_and_opportunities.txt
              sed -i '/^1$/!d' .s_entrepreneurship_advanced_5_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship_advanced/S5"
              # Define the file extension
              file_extension_question=".3.business_ideas_and_opportunities.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".entrepreneurship_advanced.5.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Entrepreneurship_advanced/S5"
                # Define the file extension
                file_extension_answer=".3.business_ideas_and_opportunities.ans.txt"
                # Define the exercise file
                exercise_file="../../entrepreneurship_advanced_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .entrepreneurship_advanced.5.3
              fi
              if ! [ -f ".s_entrepreneurship_advanced_5_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Business planning ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship_advanced/5.4.business_planning.txt" . || exit 1
              mv 5.4.business_planning.txt .5.4.business_planning.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .5.4.business_planning.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .5.4.business_planning.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .5.4.business_planning.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .5.4.business_planning.txt
              process_reminders_from_file .5.4.business_planning.txt
              STATE_FILE=".s_entrepreneurship_advanced_5_4"
              process_file .5.4.business_planning.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .5.4.business_planning.txt
              sed -i '/^1$/!d' .s_entrepreneurship_advanced_5_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship_advanced/S5"
              # Define the file extension
              file_extension=".4.business_planning.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Entrepreneurship_advanced/S5"
              # Define the file extension
              file_extension_question=".4.business_planning.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            5)
              if ! [ -f ".entrepreneurship_advanced.5.4" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Entrepreneurship_advanced/S5"
                # Define the file extension
                file_extension_answer=".4.business_planning.ans.txt"
                # Define the exercise file
                exercise_file="../../entrepreneurship_advanced_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .entrepreneurship_advanced.5.4
              fi
              if ! [ -f ".s_entrepreneurship_advanced_5_5" ]; then
                echo -e "\n\nHere you are dear one... Stay organised as you explore Production in an enterprise ...\n\n${g}Just know we are not going to leave you alone${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship_advanced/5.5.production_in_an_enterprise.txt" . || exit 1
              mv 5.5.production_in_an_enterprise.txt .5.5.production_in_an_enterprise.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .5.5.production_in_an_enterprise.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .5.5.production_in_an_enterprise.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .5.5.production_in_an_enterprise.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .5.5.production_in_an_enterprise.txt
              process_reminders_from_file .5.5.production_in_an_enterprise.txt
              STATE_FILE=".s_entrepreneurship_advanced_5_5"
              process_file .5.5.production_in_an_enterprise.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .5.5.production_in_an_enterprise.txt
              sed -i '/^1$/!d' .s_entrepreneurship_advanced_5_5
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship_advanced/S5"
              # Define the file extension
              file_extension_question=".5.production_in_an_enterprise.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            6)
              if ! [ -f ".entrepreneurship_advanced.5.5" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Entrepreneurship_advanced/S5"
                # Define the file extension
                file_extension_answer=".5.production_in_an_enterprise.ans.txt"
                # Define the exercise file
                exercise_file="../../entrepreneurship_advanced_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .entrepreneurship_advanced.5.5
              fi
              if ! [ -f ".s_entrepreneurship_advanced_5_6" ]; then
                echo -e "\n\nYou have managed to make it to Marketing in an enterprise ...\n\n${g}Remember to pray always${t}\n\nThe fear of the Lord is the beginning of wisdom \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship_advanced/5.6.marketing_in_an_enterprise.txt" . || exit 1
              mv 5.6.marketing_in_an_enterprise.txt .5.6.marketing_in_an_enterprise.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .5.6.marketing_in_an_enterprise.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .5.6.marketing_in_an_enterprise.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .5.6.marketing_in_an_enterprise.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .5.6.marketing_in_an_enterprise.txt
              process_reminders_from_file .5.6.marketing_in_an_enterprise.txt
              STATE_FILE=".s_entrepreneurship_advanced_5_6"
              process_file .5.6.marketing_in_an_enterprise.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .5.6.marketing_in_an_enterprise.txt
              sed -i '/^1$/!d' .s_entrepreneurship_advanced_5_6
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship_advanced/S5"
              # Define the file extension
              file_extension_question=".6.marketing_in_an_enterprise.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            7)
              if ! [ -f ".entrepreneurship_advanced.5.6" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Entrepreneurship_advanced/S5"
                # Define the file extension
                file_extension_answer=".6.marketing_in_an_enterprise.ans.txt"
                # Define the exercise file
                exercise_file="../../entrepreneurship_advanced_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .entrepreneurship_advanced.5.6
              fi
              if ! [ -f ".s_entrepreneurship_advanced_5_7" ]; then
                echo -e "\n\nFrom here, you will be proceeding with Social entrepreneurship ...\n\n${g}Please never ever forget that your education is your future${t}\n\nFocus dear \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship_advanced/5.7.social_entrepreneurship.txt" . || exit 1
              mv 5.7.social_entrepreneurship.txt .5.7.social_entrepreneurship.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .5.7.social_entrepreneurship.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .5.7.social_entrepreneurship.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .5.7.social_entrepreneurship.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .5.7.social_entrepreneurship.txt
              process_reminders_from_file .5.7.social_entrepreneurship.txt
              STATE_FILE=".s_entrepreneurship_advanced_5_7"
              process_file .5.7.social_entrepreneurship.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .5.7.social_entrepreneurship.txt
              sed -i '/^1$/!d' .s_entrepreneurship_advanced_5_7
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship_advanced/S5"
              # Define the file extension
              file_extension_question=".7.social_entrepreneurship.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".entrepreneurship_advanced.5.7" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/Entrepreneurship_advanced/S5"
	              # Define the file extension
	              file_extension_answer=".7.social_entrepreneurship.ans.txt"
	              # Define the exercise file
	              exercise_file="../../entrepreneurship_advanced_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
								touch .entrepreneurship_advanced.5.7
								echo "5" > .entrepreneurship_advanced_ready
							fi
            ;;
    ...
    if ! find . -maxdepth 1 -name '.s_entrepreneurship_advanced_6*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to the S6 Entrepreneurship_advanced class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
      clear_and_center "There are ${r}7${t} topics to be covered. Your tasks will always expand or shrink to fit in the time you give them. For that reason, never procrastinate darling!"
    fi
    attempts=0
    ...
    do
      ...
      do
        ...
        then
          attempts=0
          # Define the targeted directory
          question_directory="Revision/Entrepreneurship_advanced/S6"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../entrepreneurship_advanced_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Entrepreneurship_advanced/S6"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../entrepreneurship_advanced_answered_ans.txt"
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_entrepreneurship_advanced_6_1" ]; then
                echo -e "\n\nYou chose to explore Small and medium enterprises (SMEs) ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship_advanced/6.1.small_and_medium_enterprises_smes.txt" . || exit 1
              mv 6.1.small_and_medium_enterprises_smes.txt .6.1.small_and_medium_enterprises_smes.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .6.1.small_and_medium_enterprises_smes.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .6.1.small_and_medium_enterprises_smes.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .6.1.small_and_medium_enterprises_smes.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .6.1.small_and_medium_enterprises_smes.txt
              process_reminders_from_file .6.1.small_and_medium_enterprises_smes.txt
              STATE_FILE=".s_entrepreneurship_advanced_6_1"
              process_file .6.1.small_and_medium_enterprises_smes.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .6.1.small_and_medium_enterprises_smes.txt
              sed -i '/^1$/!d' .s_entrepreneurship_advanced_6_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship_advanced/S6"
              # Define the file extension
              file_extension=".1.small_and_medium_enterprises_smes.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Entrepreneurship_advanced/S6"
              # Define the file extension
              file_extension_question=".1.small_and_medium_enterprises_smes.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".entrepreneurship_advanced.6.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Entrepreneurship_advanced/S6"
                # Define the file extension
                file_extension_answer=".1.small_and_medium_enterprises_smes.ans.txt"
                # Define the exercise file
                exercise_file="../../entrepreneurship_advanced_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .entrepreneurship_advanced.6.1
              fi
              if ! [ -f ".s_entrepreneurship_advanced_6_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Insurance in business ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship_advanced/6.2.insurance_in_business.txt" . || exit 1
              mv 6.2.insurance_in_business.txt .6.2.insurance_in_business.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .6.2.insurance_in_business.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .6.2.insurance_in_business.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .6.2.insurance_in_business.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .6.2.insurance_in_business.txt
              process_reminders_from_file .6.2.insurance_in_business.txt
              STATE_FILE=".s_entrepreneurship_advanced_6_2"
              process_file .6.2.insurance_in_business.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .6.2.insurance_in_business.txt
              sed -i '/^1$/!d' .s_entrepreneurship_advanced_6_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship_advanced/S6"
              # Define the file extension
              file_extension_question=".2.insurance_in_business.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".entrepreneurship_advanced.6.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Entrepreneurship_advanced/S6"
                # Define the file extension
                file_extension_answer=".2.insurance_in_business.ans.txt"
                # Define the exercise file
                exercise_file="../../entrepreneurship_advanced_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .entrepreneurship_advanced.6.2
              fi
              if ! [ -f ".s_entrepreneurship_advanced_6_3" ]; then
                echo -e "\n\nYou have made a choice to cover Capital markets ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship_advanced/6.3.capital_markets.txt" . || exit 1
              mv 6.3.capital_markets.txt .6.3.capital_markets.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .6.3.capital_markets.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .6.3.capital_markets.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .6.3.capital_markets.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .6.3.capital_markets.txt
              process_reminders_from_file .6.3.capital_markets.txt
              STATE_FILE=".s_entrepreneurship_advanced_6_3"
              process_file .6.3.capital_markets.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .6.3.capital_markets.txt
              sed -i '/^1$/!d' .s_entrepreneurship_advanced_6_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship_advanced/S6"
              # Define the file extension
              file_extension_question=".3.capital_markets.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".entrepreneurship_advanced.6.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Entrepreneurship_advanced/S6"
                # Define the file extension
                file_extension_answer=".3.capital_markets.ans.txt"
                # Define the exercise file
                exercise_file="../../entrepreneurship_advanced_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .entrepreneurship_advanced.6.3
              fi
              if ! [ -f ".s_entrepreneurship_advanced_6_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Human resources in an enterprise ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship_advanced/6.4.human_resources_in_an_enterprise.txt" . || exit 1
              mv 6.4.human_resources_in_an_enterprise.txt .6.4.human_resources_in_an_enterprise.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .6.4.human_resources_in_an_enterprise.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .6.4.human_resources_in_an_enterprise.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .6.4.human_resources_in_an_enterprise.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .6.4.human_resources_in_an_enterprise.txt
              process_reminders_from_file .6.4.human_resources_in_an_enterprise.txt
              STATE_FILE=".s_entrepreneurship_advanced_6_4"
              process_file .6.4.human_resources_in_an_enterprise.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .6.4.human_resources_in_an_enterprise.txt
              sed -i '/^1$/!d' .s_entrepreneurship_advanced_6_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship_advanced/S6"
              # Define the file extension
              file_extension=".4.human_resources_in_an_enterprise.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Entrepreneurship_advanced/S6"
              # Define the file extension
              file_extension_question=".4.human_resources_in_an_enterprise.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            5)
              if ! [ -f ".entrepreneurship_advanced.6.4" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Entrepreneurship_advanced/S6"
                # Define the file extension
                file_extension_answer=".4.human_resources_in_an_enterprise.ans.txt"
                # Define the exercise file
                exercise_file="../../entrepreneurship_advanced_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .entrepreneurship_advanced.6.4
              fi
              if ! [ -f ".s_entrepreneurship_advanced_6_5" ]; then
                echo -e "\n\nHere you are dear one... Stay organised as you explore Finance in an enterprise ...\n\n${g}Just know we are not going to leave you alone${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship_advanced/6.5.finance_in_an_enterprise.txt" . || exit 1
              mv 6.5.finance_in_an_enterprise.txt .6.5.finance_in_an_enterprise.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .6.5.finance_in_an_enterprise.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .6.5.finance_in_an_enterprise.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .6.5.finance_in_an_enterprise.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .6.5.finance_in_an_enterprise.txt
              process_reminders_from_file .6.5.finance_in_an_enterprise.txt
              STATE_FILE=".s_entrepreneurship_advanced_6_5"
              process_file .6.5.finance_in_an_enterprise.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .6.5.finance_in_an_enterprise.txt
              sed -i '/^1$/!d' .s_entrepreneurship_advanced_6_5
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship_advanced/S6"
              # Define the file extension
              file_extension_question=".5.finance_in_an_enterprise.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            6)
              if ! [ -f ".entrepreneurship_advanced.6.5" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Entrepreneurship_advanced/S6"
                # Define the file extension
                file_extension_answer=".5.finance_in_an_enterprise.ans.txt"
                # Define the exercise file
                exercise_file="../../entrepreneurship_advanced_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .entrepreneurship_advanced.6.5
              fi
              if ! [ -f ".s_entrepreneurship_advanced_6_6" ]; then
                echo -e "\n\nYou have managed to make it to Taxation ...\n\n${g}Remember to pray always${t}\n\nThe fear of the Lord is the beginning of wisdom \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship_advanced/6.6.taxation.txt" . || exit 1
              mv 6.6.taxation.txt .6.6.taxation.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .6.6.taxation.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .6.6.taxation.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .6.6.taxation.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .6.6.taxation.txt
              process_reminders_from_file .6.6.taxation.txt
              STATE_FILE=".s_entrepreneurship_advanced_6_6"
              process_file .6.6.taxation.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .6.6.taxation.txt
              sed -i '/^1$/!d' .s_entrepreneurship_advanced_6_6
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship_advanced/S6"
              # Define the file extension
              file_extension_question=".6.taxation.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            7)
              if ! [ -f ".entrepreneurship_advanced.6.6" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Entrepreneurship_advanced/S6"
                # Define the file extension
                file_extension_answer=".6.taxation.ans.txt"
                # Define the exercise file
                exercise_file="../../entrepreneurship_advanced_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .entrepreneurship_advanced.6.6
              fi
              if ! [ -f ".s_entrepreneurship_advanced_6_7" ]; then
                echo -e "\n\nFrom here, you will be proceeding with Business competition ...\n\n${g}Please never ever forget that your education is your future${t}\n\nFocus dear \c" && wait_for_a_key_press
              fi
              cp "Notes/Entrepreneurship_advanced/6.7.business_competition.txt" . || exit 1
              mv 6.7.business_competition.txt .6.7.business_competition.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .6.7.business_competition.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .6.7.business_competition.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .6.7.business_competition.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .6.7.business_competition.txt
              process_reminders_from_file .6.7.business_competition.txt
              STATE_FILE=".s_entrepreneurship_advanced_6_7"
              process_file .6.7.business_competition.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .6.7.business_competition.txt
              sed -i '/^1$/!d' .s_entrepreneurship_advanced_6_7
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Entrepreneurship_advanced/S6"
              # Define the file extension
              file_extension_question=".7.business_competition.qns.txt"
              # Define the revision file
              revision_file="../../entrepreneurship_advanced_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".entrepreneurship_advanced.6.7" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/Entrepreneurship_advanced/S6"
	              # Define the file extension
	              file_extension_answer=".7.business_competition.ans.txt"
	              # Define the exercise file
	              exercise_file="../../entrepreneurship_advanced_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
								touch .entrepreneurship_advanced.6.7
								echo "6" > .entrepreneurship_advanced_ready
							fi
            ;;
