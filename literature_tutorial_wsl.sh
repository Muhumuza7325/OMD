...
while true; do
  ...
  # Prompt the user for input
  if [ -z "$(find ./Notes/Literature -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
...
if ! [ -d "Notes/Literature" ] || ! [ -d "Revision" ] || ! [ -d "Exercise" ] || ! [ -d "Videos" ] || ! [ -d "Figures" ] || ! [ -d "Tables" ]; then
