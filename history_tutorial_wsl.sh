...
function handle_class_input() {
  if { [ -n "$last_class" ] || [ -z "$last_topic" ]; } && [ -f .history_surveyor ]; then
		while true; do
	    read -rp $'\n\nPlease enter '"${r}1 or 2 or 3 or 4${t}"' to go to your class or '"${r}x${t}"' to exit'$'\n\n> ' class
      if [ "$class" == "x" ]; then
        exit
      fi
		  # Check if .current_history_class is accidentally empty
		  if [ ! -s .current_history_class ]; then
		    # Echo 1 to .current_history_class
		    echo "1" > .current_history_class
		  fi
		  # Read the value from the file
		  current_history_class=$(<.current_history_class) 2>/dev/null
		  # Check if the value is equal to $class
		  if [ "$current_history_class" -lt "$class" 2>/dev/null ]; then
	      ...
	      if [ "$class" != "x" ]; then
	        echo "$class" > .history_user_state 2>/dev/null
	      fi
				break
	  	fi
		done
...
function handle_s3_topic_input() {
  ...
  if [ -z "$last_topic" ] || [ -f .history_topic_selected ]; then
    read -rp $'\n\nChoose either topic '"${g}1 or 2 or 3 or 4 or 5 or 6 or 7 or 8 or 9${t}"' to learn'$'\nor enter '"${r}z${t}"' for an adventure or '"${r}r${t}"' to revise or '"${r}s${t}"' to get sample_items'$'\nor '"${r}a${t}"' to get an activity of integration or '"${r}q${t}"' to get a short answer question'$'\nor '"${r}n${t}"' to do your final class assignment and if necessary, gain access to the next class'$'\nor '"${r}p${t}"' to track academic progress or '"${r}x${t}"' to exit'$'\n\n1. The structure of Government '"${r}Term1${t}"''$'\n\n2. The Local Government systems in Uganda '"${r}Term1${t}"''$'\n\n3. Constitutionalism in Uganda '"${r}Term1${t}"''$'\n\n4. Democracy and leadership in East Africa '"${r}Term1${t}"''$'\n\n5. The role of the United Nations in development and its impact on Uganda '"${r}Term2${t}"''$'\n\n6. The evolution of human rights in Uganda '"${r}Term2${t}"''$'\n\n7. The post-independence liberation struggles in Uganda '"${r}Term2${t}"''$'\n\n8. Patriotism in Uganda '"${r}Term3${t}"''$'\n\n9. Key contributors to nation building in the post-colonial Uganda '"${r}Term3${t}"''$'\n\n> ' topic
    touch .history_surveyor
    touch .history_topic_selected
    # Update the state file with the topic
    # Check if the state file exists, and the topic is not "x"
...
function handle_s2_topic_input() {
  ...
  if [ -z "$last_topic" ] || [ -f .history_topic_selected ]; then
    read -rp $'\n\nChoose either topic '"${g}1 or 2 or 3 or 4 or 5 or 6 or 7 or 8${t}"' to learn'$'\nor enter '"${r}z${t}"' for an adventure or '"${r}r${t}"' to revise or '"${r}s${t}"' to get sample_items'$'\nor '"${r}a${t}"' to get an activity of integration or '"${r}q${t}"' to get a short answer question'$'\nor '"${r}n${t}"' to do your final class assignment and if necessary, gain access to the next class'$'\nor '"${r}p${t}"' to track academic progress or '"${r}x${t}"' to exit'$'\n\n1. The colonial administrative systems in East Africa '"${r}Term1${t}"''$'\n\n2. The colonial economy in East Africa '"${r}Term1${t}"''$'\n\n3. World wars and their impact in East Africa '"${r}Term2${t}"''$'\n\n4. Struggle for indepedence in East Africa '"${r}Term2${t}"''$'\n\n5. Post indepedence socio-economic challenges in East Africa '"${r}Term2${t}"''$'\n\n6. Civil society and non-government organisations in East Africa '"${r}Term3${t}"''$'\n\n7. Changing land tenure system in East Africa '"${r}Term3${t}"''$'\n\n8. Key personalities in the history of East Africa before independence '"${r}Term3${t}"''$'\n\n> ' topic
    touch .history_surveyor
    touch .history_topic_selected
    # Update the state file with the topic
    # Check if the state file exists, and the topic is not "x"
...
function handle_s1_topic_input() {
  ...
  if [ -z "$last_topic" ] || [ -f .history_topic_selected ]; then
    read -rp $'\n\nChoose either topic '"${g}1 or 2 or 3 or 4 or 5 or 6 or 7 or 8 or 9${t}"' to learn'$'\nor enter '"${r}z${t}"' for an adventure or '"${r}r${t}"' to revise or '"${r}s${t}"' to get sample_items'$'\nor '"${r}a${t}"' to get an activity of integration or '"${r}q${t}"' to get a short answer question'$'\nor '"${r}n${t}"' to do your final class assignment and if necessary, gain access to the next class'$'\nor '"${r}p${t}"' to track academic progress or '"${r}x${t}"' to exit'$'\n\n1. Finding out about our past '"${r}Term1${t}"''$'\n\n2. The origin of man '"${r}Term1${t}"''$'\n\n3. Migration and settlement into East Africa since 1000 AD '"${r}Term1${t}"''$'\n\n4. Culture and key ethnic groups in East Africa '"${r}Term2${t}"''$'\n\n5. State formation in east africa '"${r}Term2${t}"''$'\n\n6. Religions in East Africa '"${r}Term2${t}"''$'\n\n7. Local and external trade contacts in East Africa before 1880 '"${r}Term2${t}"''$'\n\n8. Scramble, partition and colonisation of East Africa '"${r}Term3${t}"''$'\n\n9. Response to the establishment of colonial rule in East Africa '"${r}Term3${t}"''$'\n\n> ' topic
    touch .history_surveyor
    touch .history_topic_selected
    # Update the state file with the topic
    # Check if the state file exists, and the topic is not "x"
    ...
    for sentence in "${sentences[@]}"; do
      if [[ $echo_next == true ]]; then
        ...
        for split_sentence in "${split_sentences[@]}"; do
					echo "$split_sentence" >> .history_reminder
        done
      echo_next=false
      fi
        ...
        echo "Did you know that $lower_cased_sentence" >> .history_reminder
        ...
        echo_next=true
  			...
  			echo "Do you recall that $lower_cased_sentence;" >> .history_reminder

			elif [[ "$sentence" =~ " → " ]]; then
  			echo "Hope you know that: $sentence;" >> .history_reminder

      elif [[ "$sentence" =~ " ↔ " ]]; then
        echo "Hope you know that: $sentence;" >> .history_reminder

      elif [[ "$sentence" =~ (Generally|general) ]]; then
        echo "Note: $sentence;" >> .history_reminder
...
process_random_reminder() {
  ...
  # Check if .history_reminder exists
  if [ -f .history_reminder ]; then
		# Remove empty lines from .history_reminder
  	sed -i '/^[[:space:]]*$/d' .history_reminder 2>/dev/null
    # Randomly select a non-empty sentence
    local reminder # Declare the variable
		reminder=$(awk -v RS=';' 'BEGIN{srand();}{gsub(/^[[:space:]]+|[[:space:]]+$/, ""); if (length > 0) a[++n]=$0}END{if (n > 0) print a[int(rand()*n)+1]}' .history_reminder)
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    for sentence in "${sentences[@]}"; do
        ...
        if [[ $sentence == *"Figure"* ]]; then
          ...
          # Change to the "Figures/History" directory
          cd Figures/History || { echo "Failed to change to Figures/History"; return; }
        ...
        if [[ $sentence == *"Table"* ]]; then
          cd Tables/History || { echo -e "\nFailed to change to Tables/History \c"; return; }
          explorer.exe "$sentence" > /dev/null 2>&1 &
          cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        fi

        if [[ $sentence == *"Video"* ]]; then
          cd Videos/History || { echo -e "\nFailed to change to Videos/History \c"; return; }
          explorer.exe "$sentence" > /dev/null 2>&1 &
          cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        fi
...
contact_ai() {
  last_topic=$(awk -F' ' '{print $2}' .history_user_state)
...
while true; do
  ...
  # Prompt the user for input
  if [ -z "$(find ./Notes/History -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
  ...
  if [[ -n "$user_input" ]]; then
     ...
     if [[ "$user_input" == "sr" ]]; then
      ...
      explorer.exe History > /dev/null 2>&1 &
    ...
    if [[ "$user_input" == "zz" ]]; then
      ...
      TEMP_FILE=$(mktemp) && curl -o "$TEMP_FILE" -L "https://github.com/Muhumuza7325/OMD/raw/main/history_tutorial_wsl.sh" && mv "$TEMP_FILE" history_tutorial_wsl.sh && chmod +x history_tutorial_wsl.sh && cp history_tutorial_wsl.sh Students/Omd && echo -e "\n\n${y}Code successfully updated.. You will have to restart a new session${t} \c" && sleep 4 && exit || (echo -e "\n\n${m}Error updating code!... Please check your internet connection and try again!${t} \c" && rm -f "$TEMP_FILE" && return)
    fi
    ...
    if [[ -n "$result" ]]; then
      ...
      if echo "$result" | grep "Figure"; then
        echo "$result" | grep -o '\bFigure[0-9]\+.*\.jpg\(\.[0-9]\+\)*\b' > Figures/History/figures.txt
        sed -i -e '/^Figure/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Figures/History/figures.txt
        # Change to the "Figures/History" directory
        cd Figures/History || { echo "Failed to change to Figures/History"; return; }
        ...
        rm -f Figures/History/figures.txt
      fi

      if echo "$result" | grep "Table"; then
        echo "$result" | grep -o '\bTable[0-9]\+\(\.[0-9]\+\)*\b' > Tables/History/tables.txt
        sed -i -e '/^Table/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Tables/History/tables.txt
        cd Tables/History || { echo -e "\nFailed to change to Tables/History \c"; return; }
        ...
        # Go back to the original directory
        cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        rm -f Tables/History/tables.txt
      fi

      if echo "$result" | grep "Video"; then
        #echo "$result" > Videos/History/videos.txt
        echo "$result" | grep -o '\bVideo[0-9]\+\(\.[0-9]\+\)*\b' > Videos/History/videos.txt
        # Remove lines not starting with "Video" and any leading/trailing whitespaces
        sed -i -e '/^Video/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Videos/History/videos.txt
        # Change to the "Videos/History" directory
        cd Videos/History || { echo -e "\nFailed to change to Videos/History \c"; return; }
        ...
        cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        # Remove the temporary file
        rm -f Videos/History/videos.txt
      fi
  ...
  if [ -f "$selected_file" ]; then
    ...
    if [[ -n "$selected_question" && "$selected_question" =~ [[:graph:]] ]]; then
      ...
      if echo "$selected_question" | grep "Figure"; then
        echo "$selected_question" | grep -o '\bFigure[0-9]\+[^;]*\b' > ../../Figures/History/figures.txt
        sed -i -e '/^Figure/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' ../../Figures/History/figures.txt
        # Change to the "Figures/History" directory
        cd ../../Figures/History || { echo "Failed to change to Figures/History"; return; }
        # Specify the path to the text file containing figure names
        ...
        # Go back to the original directory
        cd ../../Revision/History || { echo "Failed to change back to the targeted directory \c"; exit 1; }
        # Remove the temporary file
        rm -f ../../Figures/History/figures.txt
      fi
...
process_question_answer() {
  ...
  if [ -f ../../../.school_name ]; then
    ...
    sed -i '/^[[:space:]]*$/d' ../../../."$school_name"_students_file.txt
    existing_class=$(awk '{print $1}' ../../../.history_user_state)
    existing_topic=$(awk '{print $2}' ../../../.history_user_state)
    echo ''
    ...
    if grep -q "History" ../../../."$school_name"_students_file.txt; then
      sed -i -E 's/;/\n/g' ../../../."$school_name"_students_file.txt
      # Find the line with the word History, replace the information below it with the already available information adding a comma existing_class_existing_topic [$percentage]
      if [ "$existing_topic" == "r" ]; then
        sed -i '/History/{n;s/\(.*\)/\1, '"$existing_class"'_'"$input"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
      else
        (( existing_topic-- ))
        sed -i '/History/{n;s/\(.*\)/\1, '"$existing_class"'_'"$existing_topic"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
      fi
    else
      if [ "$existing_topic" == "r" ]; then
        sed -i -E '/^School/ i\History\n'"${existing_class} ${names}[${percentage}%]"'' ../../../."$school_name"_students_file.txt
...
function resume_from_last_point() {
  if [ -f .history_user_state ]; then
    last_class=$(awk -F' ' '{print $1}' .history_user_state)
    last_topic=$(awk -F' ' '{print $2}' .history_user_state)
    if [ -n "$last_class" ] && [ -n "$last_topic" ]; then
      echo -e "\n       Resuming from ${r}S$last_class${t} : ${g}Topic '$last_topic'${t} \c"
      rm -f .history_surveyor
...
process_final_assignment() {
  # Check if user is.history_ready for the assignment
  # Check if .current_history_class is accidentally empty
  if [ ! -s .current_history_class ]; then
    # Echo 1 to .current_history_class
    echo "1" > .current_history_class
  fi
  # Read the value from the .current_history_class file
  current_history_class=$(<.current_history_class) 2>/dev/null
  # Check if the value in the .history_ready file is equal to $class
  # Read the value from the .history_ready file
  if [ ! -s .history_ready ]; then
    echo "0" > .history_ready
  fi
  how.history_ready=$(<.history_ready) 2>/dev/null
  # Check if the value is equal to $class
  if [ "$how.history_ready" -lt "$current_history_class" 2>/dev/null ]; then
    read -rp $'\n\nYou havent done all the topic assignments for your current history class\n\n'"${r}Proceeding from here will affect your very final score${y}"'. To go back and progress right, enter '"${y}ok${t}"'. Otherwise, press the Enter key to do the final class assignment: ' progress
    if [ "$progress" == "ok" ]; then
      return
    fi
  fi
    ...
    if ! [ "$total_questions" == 100 ]; then
      ...
      if [ "$elapsed_time" -gt 3600 ]; then
        ...
        if [ "$percentage" -lt 95 ]; then
          ...
          cd - > /dev/null 2>&1 || exit
          if ! [ -f .current_history_class ]; then
            # Echo the result to the file .current_class
            echo "2" > .current_history_class
            ...
            # Read the value from the file
            current_history_class=$(<.current_history_class) 2>/dev/null
            # Check if the new_class value is lt $current_history_class
            if [ "$new_class" -gt "$current_history_class" 2>/dev/null ]; then
		          # Echo the result to the file .current_class
		          echo "$new_class" > .current_history_class
            fi
...
get_sample_items() {
	...
	# Save the current working directory
	pushd . > /dev/null
	cd Revision/History || { echo "Directory not found"; return; }
	...
	while true; do
		if [ ! -f .revise.txt ]; then
			...
			if [[ -n $input ]]; then
				explorer.exe .history_samples* > /dev/null 2>&1 &
				clear
				popd > /dev/null || exit
				return
			fi
...
while true; do
  ...
  if ls Students | grep -wq "$initials"; then
    ...
    echo -e "                  $initials\n" > "Students/$initials/Exercise/history_answered_ans.txt"
    echo -e "                  $initials\n" > "Students/$initials/Revision/history_covered_qns.txt"
    ...
    if [ -f ../../../.school_name ]; then
      ...
      if grep -q "History" ../../../."$school_name"_students_file.txt; then
        ...
        if [ "$existing_topic" == "z" ]; then
          sed -i '/History/{n;s/\(.*\)/\1, '"$existing_class"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
        else
          (( existing_topic-- ))
          sed -i '/History/{n;s/\(.*\)/\1, '"$existing_class"'_'"$existing_topic"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
        fi
...
if [ ! -f .history_user_state ]; then
  ...
  if [ -f .omd_communication.txt ]; then
    ...
    process_random_reminder .history_reminder
  fi
	handle_resume_input
fi

if [ -z "$class" ] && [ -s ".history_user_state" ]; then
  get_and_display_pattern
fi
# Check for the presence of specific directories and a file
if ! [ -d "Notes/History" ] || ! [ -d "Revision" ] || ! [ -d "Exercise" ] || ! [ -d "Videos" ] || ! [ -d "Figures" ] || ! [ -d "Tables" ]; then
  ...
  mkdir -p Notes Notes/History Revision Revision/History Revision/History/{S1,S2,S3,S4} Exercise Exercise/History Exercise/History/{S1,S2,S3,S4} Videos Videos/History Figures Figures/History Tables Tables/History
  touch Revision/history_covered_qns.txt Exercise/history_answered_ans.txt
  echo -e "\n"
  pwd
...
if [ -z "$(find ./Notes/History -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
  ...
  if [[ -z "$user_input" ]]; then
    ...
    cp 1.1.finding_out_about_our_past.txt Notes/History || echo -e "\n\nError copying 1.1.finding_out_about_our_past.txt to the History directory in the Notes directory \c"
    wait_for_a_key_press
  else
  ...
  rm -f 1.1.finding_out_about_our_past.txt
fi
...
while true; do
  ...
  handle_class_input
  if [[ "$class" == "1" ]]; then
    if ! find . -maxdepth 1 -name '.s_history_1*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to S1 History class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
    ...
    do
      ...
      do
        ...
        then
          ...
          question_directory="Revision/History/S1"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../history_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/History/S1"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../history_answered_ans.txt"
          # Call the function to process a random question
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_history_1_1" ]; then
                echo -e "\n\nYou chose to explore Finding out about our past ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/History/1.1.finding_out_about_our_past.txt" . || exit 1
              mv 1.1.finding_out_about_our_past.txt .1.1.finding_out_about_our_past.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.1.finding_out_about_our_past.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.1.finding_out_about_our_past.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.1.finding_out_about_our_past.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.1.finding_out_about_our_past.txt
              process_reminders_from_file .1.1.finding_out_about_our_past.txt
              STATE_FILE=".s_history_1_1"
              process_file .1.1.finding_out_about_our_past.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.1.finding_out_about_our_past.txt
              sed -i '/^1$/!d' .s_history_1_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S1"
              # Define the file extension
              file_extension=".1.finding_out_about_our_past.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              ...
              question_directory="Revision/History/S1"
              # Define the file extension
              file_extension_question=".1.finding_out_about_our_past.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".history.1.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/History/S1"
                # Define the file extension
                file_extension_answer=".1.finding_out_about_our_past.ans.txt"
                # Define the exercise file
                exercise_file="../../history_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_history_1_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into The origin of man ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/History/1.2.the_origin_of_man.txt" . || exit 1
              mv 1.2.the_origin_of_man.txt .1.2.the_origin_of_man.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.2.the_origin_of_man.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.2.the_origin_of_man.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.2.the_origin_of_man.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.2.the_origin_of_man.txt
              process_reminders_from_file .1.2.the_origin_of_man.txt
              STATE_FILE=".s_history_1_2"
              process_file .1.2.the_origin_of_man.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.2.the_origin_of_man.txt
              sed -i '/^1$/!d' .s_history_1_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S1"
              # Define the file extension
              file_extension_question=".2.the_origin_of_man.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".history.1.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/History/S1"
                # Define the file extension
                file_extension_answer=".2.the_origin_of_man.ans.txt"
                # Define the exercise file
                exercise_file="../../history_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_history_1_3" ]; then
                echo -e "\n\nYou have made a choice to cover Migration and settlement into East Africa since 1000 AD ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/History/1.3.migration_and_settlement_into_east_africa_since_1000_ad.txt" . || exit 1
              mv 1.3.migration_and_settlement_into_east_africa_since_1000_ad.txt .1.3.migration_and_settlement_into_east_africa_since_1000_ad.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.3.migration_and_settlement_into_east_africa_since_1000_ad.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.3.migration_and_settlement_into_east_africa_since_1000_ad.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.3.migration_and_settlement_into_east_africa_since_1000_ad.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.3.migration_and_settlement_into_east_africa_since_1000_ad.txt
              process_reminders_from_file .1.3.migration_and_settlement_into_east_africa_since_1000_ad.txt
              STATE_FILE=".s_history_1_3"
              process_file .1.3.migration_and_settlement_into_east_africa_since_1000_ad.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.3.migration_and_settlement_into_east_africa_since_1000_ad.txt
              sed -i '/^1$/!d' .s_history_1_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S1"
              # Define the file extension
              file_extension_question=".3.migration_and_settlement_into_east_africa_since_1000_ad.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".history.1.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/History/S1"
                # Define the file extension
                file_extension_answer=".3.migration_and_settlement_into_east_africa_since_1000_ad.ans.txt"
                # Define the exercise file
                exercise_file="../../history_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_history_1_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Culture and key ethnic groups in East Africa ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/History/1.4.culture_and_key_ethnic_groups_in_east_africa.txt" . || exit 1
              mv 1.4.culture_and_key_ethnic_groups_in_east_africa.txt .1.4.culture_and_key_ethnic_groups_in_east_africa.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.4.culture_and_key_ethnic_groups_in_east_africa.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.4.culture_and_key_ethnic_groups_in_east_africa.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.4.culture_and_key_ethnic_groups_in_east_africa.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.4.culture_and_key_ethnic_groups_in_east_africa.txt
              process_reminders_from_file .1.4.culture_and_key_ethnic_groups_in_east_africa.txt
              STATE_FILE=".s_history_1_4"
              process_file .1.4.culture_and_key_ethnic_groups_in_east_africa.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.4.culture_and_key_ethnic_groups_in_east_africa.txt
              sed -i '/^1$/!d' .s_history_1_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S1"
              # Define the file extension
              file_extension=".4.culture_and_key_ethnic_groups_in_east_africa.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/History/S1"
              # Define the file extension
              file_extension_question=".4.culture_and_key_ethnic_groups_in_east_africa.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            5)
              if ! [ -f ".history.1.4" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/History/S1"
                # Define the file extension
                file_extension_answer=".4.culture_and_key_ethnic_groups_in_east_africa.ans.txt"
                # Define the exercise file
                exercise_file="../../history_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_history_1_5" ]; then
                echo -e "\n\nHere you are dear one... Stay organised as you explore State formation in east africa ...\n\n${g}Just know we are not going to leave you alone${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/History/1.5.state_formation_in_east_africa.txt" . || exit 1
              mv 1.5.state_formation_in_east_africa.txt .1.5.state_formation_in_east_africa.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.5.state_formation_in_east_africa.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.5.state_formation_in_east_africa.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.5.state_formation_in_east_africa.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.5.state_formation_in_east_africa.txt
              process_reminders_from_file .1.5.state_formation_in_east_africa.txt
              STATE_FILE=".s_history_1_5"
              process_file .1.5.state_formation_in_east_africa.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.5.state_formation_in_east_africa.txt
              sed -i '/^1$/!d' .s_history_1_5
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S1"
              # Define the file extension
              file_extension_question=".5.state_formation_in_east_africa.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            6)
              if ! [ -f ".history.1.5" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/History/S1"
                # Define the file extension
                file_extension_answer=".5.state_formation_in_east_africa.ans.txt"
                # Define the exercise file
                exercise_file="../../history_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_history_1_6" ]; then
                echo -e "\n\nYou have managed to make it to Religions in East Africa ...\n\n${g}Remember to pray always${t}\n\nThe fear of the Lord is the beginning of wisdom \c" && wait_for_a_key_press
              fi
              cp "Notes/History/1.6.religions_in_east_africa.txt" . || exit 1
              mv 1.6.religions_in_east_africa.txt .1.6.religions_in_east_africa.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.6.religions_in_east_africa.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.6.religions_in_east_africa.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.6.religions_in_east_africa.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.6.religions_in_east_africa.txt
              process_reminders_from_file .1.6.religions_in_east_africa.txt
              STATE_FILE=".s_history_1_6"
              process_file .1.6.religions_in_east_africa.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.6.religions_in_east_africa.txt
              sed -i '/^1$/!d' .s_history_1_6
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S1"
              # Define the file extension
              file_extension_question=".6.religions_in_east_africa.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            7)
              if ! [ -f ".history.1.6" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/History/S1"
                # Define the file extension
                file_extension_answer=".6.religions_in_east_africa.ans.txt"
                # Define the exercise file
                exercise_file="../../history_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_history_1_7" ]; then
                echo -e "\n\nFrom here, you will be proceeding with Local and external trade contacts in East Africa before 1880 ...\n\n${g}Please never ever forget that your education is your future${t}\n\nFocus dear \c" && wait_for_a_key_press
              fi
              cp "Notes/History/1.7.local_and_external_trade_contacts_in_east_africa_before_1880.txt" . || exit 1
              mv 1.7.local_and_external_trade_contacts_in_east_africa_before_1880.txt .1.7.local_and_external_trade_contacts_in_east_africa_before_1880.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.7.local_and_external_trade_contacts_in_east_africa_before_1880.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.7.local_and_external_trade_contacts_in_east_africa_before_1880.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.7.local_and_external_trade_contacts_in_east_africa_before_1880.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.7.local_and_external_trade_contacts_in_east_africa_before_1880.txt
              process_reminders_from_file .1.7.local_and_external_trade_contacts_in_east_africa_before_1880.txt
              STATE_FILE=".s_history_1_7"
              process_file .1.7.local_and_external_trade_contacts_in_east_africa_before_1880.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.7.local_and_external_trade_contacts_in_east_africa_before_1880.txt
              sed -i '/^1$/!d' .s_history_1_7
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S1"
              # Define the file extension
              file_extension_question=".7.local_and_external_trade_contacts_in_east_africa_before_1880.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            8)
              if ! [ -f ".history.1.7" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/History/S1"
                # Define the file extension
                file_extension_answer=".7.local_and_external_trade_contacts_in_east_africa_before_1880.ans.txt"
                # Define the exercise file
                exercise_file="../../history_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_history_1_8" ]; then
                echo -e "\n\nYou are to cover Scramble partition and colonisation of East Africa ...\n\n${g}Please never ever settle for less${t}\n\nPromise yourself that you wont give up \c" && wait_for_a_key_press
              fi
              cp "Notes/History/1.8.scramble_partition_and_colonisation_of_east_africa.txt" . || exit 1
              mv 1.8.scramble_partition_and_colonisation_of_east_africa.txt .1.8.scramble_partition_and_colonisation_of_east_africa.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.8.scramble_partition_and_colonisation_of_east_africa.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.8.scramble_partition_and_colonisation_of_east_africa.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.8.scramble_partition_and_colonisation_of_east_africa.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.8.scramble_partition_and_colonisation_of_east_africa.txt
              process_reminders_from_file .1.8.scramble_partition_and_colonisation_of_east_africa.txt
              STATE_FILE=".s_history_1_8"
              process_file .1.8.scramble_partition_and_colonisation_of_east_africa.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.8.scramble_partition_and_colonisation_of_east_africa.txt
              sed -i '/^1$/!d' .s_history_1_8
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S1"
              # Define the file extension
              file_extension_question=".8.scramble_partition_and_colonisation_of_east_africa.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            9)
              if ! [ -f ".history.1.8" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/History/S1"
                # Define the file extension
                file_extension_answer=".8.scramble_partition_and_colonisation_of_east_africa.ans.txt"
                # Define the exercise file
                exercise_file="../../history_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_history_1_9" ]; then
                echo -e "\n\nI am so happy for you dear one. You are here to cover the the 9th topic (Response to the establishment of colonial rule in East Africa )...\n\n${g}Just never underestimate the value of a single second${t}\n\nThat extra one second maybe all you need to fully understand a given concept \c" && wait_for_a_key_press
              fi
              cp "Notes/History/1.9.response_to_the_establishment_of_colonial_rule_in_east_africa.txt" . || exit 1
              mv 1.9.response_to_the_establishment_of_colonial_rule_in_east_africa.txt .1.9.response_to_the_establishment_of_colonial_rule_in_east_africa.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.9.response_to_the_establishment_of_colonial_rule_in_east_africa.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.9.response_to_the_establishment_of_colonial_rule_in_east_africa.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.9.response_to_the_establishment_of_colonial_rule_in_east_africa.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.9.response_to_the_establishment_of_colonial_rule_in_east_africa.txt
              process_reminders_from_file .1.9.response_to_the_establishment_of_colonial_rule_in_east_africa.txt
              STATE_FILE=".s_history_1_9"
              process_file .1.9.response_to_the_establishment_of_colonial_rule_in_east_africa.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.9.response_to_the_establishment_of_colonial_rule_in_east_africa.txt
              sed -i '/^1$/!d' .s_history_1_9
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S1"
              # Define the file extension
              file_extension_question=".9.response_to_the_establishment_of_colonial_rule_in_east_africa.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".history.1.9" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/History/S1"
	              # Define the file extension
	              file_extension_answer=".9.response_to_the_establishment_of_colonial_rule_in_east_africa.ans.txt"
	              # Define the exercise file
	              exercise_file="../../history_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
								touch .history.1.9
								echo "1" > .history_ready
							fi
            ;;
    ...
    if ! find . -maxdepth 1 -name '.s_history_2*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to S2 History class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
    ...
    do
      ...
      do
        ...
        then
          attempts=0
          # Define the targeted directory
          question_directory="Revision/History/S2"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../history_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/History/S2"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../history_answered_ans.txt"
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_history_2_1" ]; then
                echo -e "\n\nYou chose to explore The colonial administrative systems in East Africa ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/History/2.1.the_colonial_administrative_systems_in_east_africa.txt" . || exit 1
              mv 2.1.the_colonial_administrative_systems_in_east_africa.txt .2.1.the_colonial_administrative_systems_in_east_africa.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.1.the_colonial_administrative_systems_in_east_africa.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.1.the_colonial_administrative_systems_in_east_africa.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.1.the_colonial_administrative_systems_in_east_africa.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.1.the_colonial_administrative_systems_in_east_africa.txt
              process_reminders_from_file .2.1.the_colonial_administrative_systems_in_east_africa.txt
              STATE_FILE=".s_history_2_1"
              process_file .2.1.the_colonial_administrative_systems_in_east_africa.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.1.the_colonial_administrative_systems_in_east_africa.txt
              sed -i '/^1$/!d' .s_history_2_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S2"
              # Define the file extension
              file_extension=".1.the_colonial_administrative_systems_in_east_africa.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/History/S2"
              # Define the file extension
              file_extension_question=".1.the_colonial_administrative_systems_in_east_africa.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".history.2.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/History/S2"
                # Define the file extension
                file_extension_answer=".1.the_colonial_administrative_systems_in_east_africa.ans.txt"
                # Define the exercise file
                exercise_file="../../history_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_history_2_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into The colonial economy in East Africa ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/History/2.2.the_colonial_economy_in_east_africa.txt" . || exit 1
              mv 2.2.the_colonial_economy_in_east_africa.txt .2.2.the_colonial_economy_in_east_africa.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.2.the_colonial_economy_in_east_africa.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.2.the_colonial_economy_in_east_africa.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.2.the_colonial_economy_in_east_africa.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.2.the_colonial_economy_in_east_africa.txt
              process_reminders_from_file .2.2.the_colonial_economy_in_east_africa.txt
              STATE_FILE=".s_history_2_2"
              process_file .2.2.the_colonial_economy_in_east_africa.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.2.the_colonial_economy_in_east_africa.txt
              sed -i '/^1$/!d' .s_history_2_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S2"
              # Define the file extension
              file_extension_question=".2.the_colonial_economy_in_east_africa.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".history.2.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/History/S2"
                # Define the file extension
                file_extension_answer=".2.the_colonial_economy_in_east_africa.ans.txt"
                # Define the exercise file
                exercise_file="../../history_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_history_2_3" ]; then
                echo -e "\n\nYou have made a choice to cover World wars and their impact in East Africa ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/History/2.3.world_wars_and_their_impact_in_east_africa.txt" . || exit 1
              mv 2.3.world_wars_and_their_impact_in_east_africa.txt .2.3.world_wars_and_their_impact_in_east_africa.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.3.world_wars_and_their_impact_in_east_africa.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.3.world_wars_and_their_impact_in_east_africa.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.3.world_wars_and_their_impact_in_east_africa.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.3.world_wars_and_their_impact_in_east_africa.txt
              process_reminders_from_file .2.3.world_wars_and_their_impact_in_east_africa.txt
              STATE_FILE=".s_history_2_3"
              process_file .2.3.world_wars_and_their_impact_in_east_africa.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.3.world_wars_and_their_impact_in_east_africa.txt
              sed -i '/^1$/!d' .s_history_2_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S2"
              # Define the file extension
              file_extension_question=".3.world_wars_and_their_impact_in_east_africa.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".history.2.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/History/S2"
                # Define the file extension
                file_extension_answer=".3.world_wars_and_their_impact_in_east_africa.ans.txt"
                # Define the exercise file
                exercise_file="../../history_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_history_2_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Struggle for indepedence in East Africa ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/History/2.4.struggle_for_indepedence_in_east_africa.txt" . || exit 1
              mv 2.4.struggle_for_indepedence_in_east_africa.txt .2.4.struggle_for_indepedence_in_east_africa.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.4.struggle_for_indepedence_in_east_africa.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.4.struggle_for_indepedence_in_east_africa.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.4.struggle_for_indepedence_in_east_africa.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.4.struggle_for_indepedence_in_east_africa.txt
              process_reminders_from_file .2.4.struggle_for_indepedence_in_east_africa.txt
              STATE_FILE=".s_history_2_4"
              process_file .2.4.struggle_for_indepedence_in_east_africa.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.4.struggle_for_indepedence_in_east_africa.txt
              sed -i '/^1$/!d' .s_history_2_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S2"
              # Define the file extension
              file_extension=".4.struggle_for_indepedence_in_east_africa.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/History/S2"
              # Define the file extension
              file_extension_question=".4.struggle_for_indepedence_in_east_africa.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            5)
              if ! [ -f ".history.2.4" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/History/S2"
                # Define the file extension
                file_extension_answer=".4.struggle_for_indepedence_in_east_africa.ans.txt"
                # Define the exercise file
                exercise_file="../../history_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_history_2_5" ]; then
                echo -e "\n\nHere you are dear one... Stay organised as you explore Post indepedence socio-economic challenges in East Africa ...\n\n${g}Just know we are not going to leave you alone${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/History/2.5.post_indepedence_socio-economic_challenges_in_east_africa.txt" . || exit 1
              mv 2.5.post_indepedence_socio-economic_challenges_in_east_africa.txt .2.5.post_indepedence_socio-economic_challenges_in_east_africa.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.5.post_indepedence_socio-economic_challenges_in_east_africa.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.5.post_indepedence_socio-economic_challenges_in_east_africa.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.5.post_indepedence_socio-economic_challenges_in_east_africa.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.5.post_indepedence_socio-economic_challenges_in_east_africa.txt
              process_reminders_from_file .2.5.post_indepedence_socio-economic_challenges_in_east_africa.txt
              STATE_FILE=".s_history_2_5"
              process_file .2.5.post_indepedence_socio-economic_challenges_in_east_africa.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.5.post_indepedence_socio-economic_challenges_in_east_africa.txt
              sed -i '/^1$/!d' .s_history_2_5
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S2"
              # Define the file extension
              file_extension_question=".5.post_indepedence_socio-economic_challenges_in_east_africa.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            6)
              if ! [ -f ".history.2.5" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/History/S2"
                # Define the file extension
                file_extension_answer=".5.post_indepedence_socio-economic_challenges_in_east_africa.ans.txt"
                # Define the exercise file
                exercise_file="../../history_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_history_2_6" ]; then
                echo -e "\n\nYou have managed to make it to Civil society and non-government organisations in East Africa ...\n\n${g}Remember to pray always${t}\n\nThe fear of the Lord is the beginning of wisdom \c" && wait_for_a_key_press
              fi
              cp "Notes/History/2.6.civil_society_and_non-government_organisations_in_east_africa.txt" . || exit 1
              mv 2.6.civil_society_and_non-government_organisations_in_east_africa.txt .2.6.civil_society_and_non-government_organisations_in_east_africa.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.6.civil_society_and_non-government_organisations_in_east_africa.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.6.civil_society_and_non-government_organisations_in_east_africa.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.6.civil_society_and_non-government_organisations_in_east_africa.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.6.civil_society_and_non-government_organisations_in_east_africa.txt
              process_reminders_from_file .2.6.civil_society_and_non-government_organisations_in_east_africa.txt
              STATE_FILE=".s_history_2_6"
              process_file .2.6.civil_society_and_non-government_organisations_in_east_africa.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.6.civil_society_and_non-government_organisations_in_east_africa.txt
              sed -i '/^1$/!d' .s_history_2_6
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S2"
              # Define the file extension
              file_extension_question=".6.civil_society_and_non-government_organisations_in_east_africa.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            7)
              if ! [ -f ".history.2.6" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/History/S2"
                # Define the file extension
                file_extension_answer=".6.civil_society_and_non-government_organisations_in_east_africa.ans.txt"
                # Define the exercise file
                exercise_file="../../history_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_history_2_7" ]; then
                echo -e "\n\nFrom here, you will be proceeding with Changing land tenure system in East Africa ...\n\n${g}Please never ever forget that your education is your future${t}\n\nFocus dear \c" && wait_for_a_key_press
              fi
              cp "Notes/History/2.7.changing_land_tenure_system_in_east_africa.txt" . || exit 1
              mv 2.7.changing_land_tenure_system_in_east_africa.txt .2.7.changing_land_tenure_system_in_east_africa.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.7.changing_land_tenure_system_in_east_africa.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.7.changing_land_tenure_system_in_east_africa.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.7.changing_land_tenure_system_in_east_africa.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.7.changing_land_tenure_system_in_east_africa.txt
              process_reminders_from_file .2.7.changing_land_tenure_system_in_east_africa.txt
              STATE_FILE=".s_history_2_7"
              process_file .2.7.changing_land_tenure_system_in_east_africa.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.7.changing_land_tenure_system_in_east_africa.txt
              sed -i '/^1$/!d' .s_history_2_7
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S2"
              # Define the file extension
              file_extension_question=".7.changing_land_tenure_system_in_east_africa.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            8)
              if ! [ -f ".history.2.7" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/History/S2"
                # Define the file extension
                file_extension_answer=".7.changing_land_tenure_system_in_east_africa.ans.txt"
                # Define the exercise file
                exercise_file="../../history_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_history_2_8" ]; then
                echo -e "\n\nYou are to cover Key personalities in the history of East Africa before independence ...\n\n${g}Please never ever settle for less${t}\n\nPromise yourself that you wont give up \c" && wait_for_a_key_press
              fi
              cp "Notes/History/2.8.key_personalities_in_the_history_of_east_africa_before_independence.txt" . || exit 1
              mv 2.8.key_personalities_in_the_history_of_east_africa_before_independence.txt .2.8.key_personalities_in_the_history_of_east_africa_before_independence.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.8.key_personalities_in_the_history_of_east_africa_before_independence.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.8.key_personalities_in_the_history_of_east_africa_before_independence.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.8.key_personalities_in_the_history_of_east_africa_before_independence.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.8.key_personalities_in_the_history_of_east_africa_before_independence.txt
              process_reminders_from_file .2.8.key_personalities_in_the_history_of_east_africa_before_independence.txt
              STATE_FILE=".s_history_2_8"
              process_file .2.8.key_personalities_in_the_history_of_east_africa_before_independence.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.8.key_personalities_in_the_history_of_east_africa_before_independence.txt
              sed -i '/^1$/!d' .s_history_2_8
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S2"
              # Define the file extension
              file_extension_question=".8.key_personalities_in_the_history_of_east_africa_before_independence.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".history.2.8" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/History/S2"
	              # Define the file extension
	              file_extension_answer=".8.key_personalities_in_the_history_of_east_africa_before_independence.ans.txt"
	              # Define the exercise file
	              exercise_file="../../history_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
								touch .history.2.8
								echo "2" > .history_ready
							fi
            ;;
    ...
    if ! find . -maxdepth 1 -name '.s_history_3*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to S3 History class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
      clear_and_center "There are ${r}9${t} topics to be covered. Your tasks will always expand or shrink to fit in the time you give them. For that reason, never procrastinate darling!"
    fi
    attempts=0
    ...
    do
      ...
      do
        ...
        then
          attempts=0
          # Define the targeted directory
          question_directory="Revision/History/S3"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../history_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/History/S3"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../history_answered_ans.txt"
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_history_3_1" ]; then
                echo -e "\n\nYou chose to explore The structure of Government ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/History/3.1.the_structure_of_government.txt" . || exit 1
              mv 3.1.the_structure_of_government.txt .3.1.the_structure_of_government.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.1.the_structure_of_government.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.1.the_structure_of_government.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.1.the_structure_of_government.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.1.the_structure_of_government.txt
              process_reminders_from_file .3.1.the_structure_of_government.txt
              STATE_FILE=".s_history_3_1"
              process_file .3.1.the_structure_of_government.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.1.the_structure_of_government.txt
              sed -i '/^1$/!d' .s_history_3_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S3"
              # Define the file extension
              file_extension=".1.the_structure_of_government.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/History/S3"
              # Define the file extension
              file_extension_question=".1.the_structure_of_government.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".history.3.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/History/S3"
                # Define the file extension
                file_extension_answer=".1.the_structure_of_government.ans.txt"
                # Define the exercise file
                exercise_file="../../history_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_history_3_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into The Local Government systems in Uganda ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/History/3.2.the_local_government_systems_in_uganda.txt" . || exit 1
              mv 3.2.the_local_government_systems_in_uganda.txt .3.2.the_local_government_systems_in_uganda.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.2.the_local_government_systems_in_uganda.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.2.the_local_government_systems_in_uganda.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.2.the_local_government_systems_in_uganda.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.2.the_local_government_systems_in_uganda.txt
              process_reminders_from_file .3.2.the_local_government_systems_in_uganda.txt
              STATE_FILE=".s_history_3_2"
              process_file .3.2.the_local_government_systems_in_uganda.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.2.the_local_government_systems_in_uganda.txt
              sed -i '/^1$/!d' .s_history_3_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S3"
              # Define the file extension
              file_extension_question=".2.the_local_government_systems_in_uganda.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".history.3.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/History/S3"
                # Define the file extension
                file_extension_answer=".2.the_local_government_systems_in_uganda.ans.txt"
                # Define the exercise file
                exercise_file="../../history_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_history_3_3" ]; then
                echo -e "\n\nYou have made a choice to cover Constitutionalism in Uganda ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/History/3.3.constitutionalism_in_uganda.txt" . || exit 1
              mv 3.3.constitutionalism_in_uganda.txt .3.3.constitutionalism_in_uganda.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.3.constitutionalism_in_uganda.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.3.constitutionalism_in_uganda.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.3.constitutionalism_in_uganda.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.3.constitutionalism_in_uganda.txt
              process_reminders_from_file .3.3.constitutionalism_in_uganda.txt
              STATE_FILE=".s_history_3_3"
              process_file .3.3.constitutionalism_in_uganda.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.3.constitutionalism_in_uganda.txt
              sed -i '/^1$/!d' .s_history_3_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S3"
              # Define the file extension
              file_extension_question=".3.constitutionalism_in_uganda.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".history.3.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/History/S3"
                # Define the file extension
                file_extension_answer=".3.constitutionalism_in_uganda.ans.txt"
                # Define the exercise file
                exercise_file="../../history_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_history_3_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Democracy and leadership in East Africa ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/History/3.4.democracy_and_leadership_in_east_africa.txt" . || exit 1
              mv 3.4.democracy_and_leadership_in_east_africa.txt .3.4.democracy_and_leadership_in_east_africa.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.4.democracy_and_leadership_in_east_africa.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.4.democracy_and_leadership_in_east_africa.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.4.democracy_and_leadership_in_east_africa.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.4.democracy_and_leadership_in_east_africa.txt
              process_reminders_from_file .3.4.democracy_and_leadership_in_east_africa.txt
              STATE_FILE=".s_history_3_4"
              process_file .3.4.democracy_and_leadership_in_east_africa.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.4.democracy_and_leadership_in_east_africa.txt
              sed -i '/^1$/!d' .s_history_3_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S3"
              # Define the file extension
              file_extension=".4.democracy_and_leadership_in_east_africa.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/History/S3"
              # Define the file extension
              file_extension_question=".4.democracy_and_leadership_in_east_africa.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            5)
              if ! [ -f ".history.3.4" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/History/S3"
                # Define the file extension
                file_extension_answer=".4.democracy_and_leadership_in_east_africa.ans.txt"
                # Define the exercise file
                exercise_file="../../history_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_history_3_5" ]; then
                echo -e "\n\nHere you are dear one... Stay organised as you explore The, role, of, the, United, nations, in, development and its, impact, on, Uganda ...\n\n${g}Just know we are not going to leave you alone${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/History/3.5.the_role_of_the_united_nations_in_development_and_its_impact_on_uganda.txt" . || exit 1
              mv 3.5.the_role_of_the_united_nations_in_development_and_its_impact_on_uganda.txt .3.5.the_role_of_the_united_nations_in_development_and_its_impact_on_uganda.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.5.the_role_of_the_united_nations_in_development_and_its_impact_on_uganda.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.5.the_role_of_the_united_nations_in_development_and_its_impact_on_uganda.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.5.the_role_of_the_united_nations_in_development_and_its_impact_on_uganda.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.5.the_role_of_the_united_nations_in_development_and_its_impact_on_uganda.txt
              process_reminders_from_file .3.5.the_role_of_the_united_nations_in_development_and_its_impact_on_uganda.txt
              STATE_FILE=".s_history_3_5"
              process_file .3.5.the_role_of_the_united_nations_in_development_and_its_impact_on_uganda.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.5.the_role_of_the_united_nations_in_development_and_its_impact_on_uganda.txt
              sed -i '/^1$/!d' .s_history_3_5
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S3"
              # Define the file extension
              file_extension_question=".5.the_role_of_the_united_nations_in_development_and_its_impact_on_uganda.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            6)
              if ! [ -f ".history.3.5" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/History/S3"
                # Define the file extension
                file_extension_answer=".5.the_role_of_the_united_nations_in_development_and_its_impact_on_uganda.ans.txt"
                # Define the exercise file
                exercise_file="../../history_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_history_3_6" ]; then
                echo -e "\n\nYou have managed to make it to The evolution of human rights in Uganda ...\n\n${g}Remember to pray always${t}\n\nThe fear of the Lord is the beginning of wisdom \c" && wait_for_a_key_press
              fi
              cp "Notes/History/3.6.the_evolution_of_human_rights_in_uganda.txt" . || exit 1
              mv 3.6.the_evolution_of_human_rights_in_uganda.txt .3.6.the_evolution_of_human_rights_in_uganda.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.6.the_evolution_of_human_rights_in_uganda.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.6.the_evolution_of_human_rights_in_uganda.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.6.the_evolution_of_human_rights_in_uganda.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.6.the_evolution_of_human_rights_in_uganda.txt
              process_reminders_from_file .3.6.the_evolution_of_human_rights_in_uganda.txt
              STATE_FILE=".s_history_3_6"
              process_file .3.6.the_evolution_of_human_rights_in_uganda.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.6.the_evolution_of_human_rights_in_uganda.txt
              sed -i '/^1$/!d' .s_history_3_6
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S3"
              # Define the file extension
              file_extension_question=".6.the_evolution_of_human_rights_in_uganda.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            7)
              if ! [ -f ".history.3.6" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/History/S3"
                # Define the file extension
                file_extension_answer=".6.the_evolution_of_human_rights_in_uganda.ans.txt"
                # Define the exercise file
                exercise_file="../../history_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_history_3_7" ]; then
                echo -e "\n\nFrom here, you will be proceeding with The post-independence liberation struggles in Uganda ...\n\n${g}Please never ever forget that your education is your future${t}\n\nFocus dear \c" && wait_for_a_key_press
              fi
              cp "Notes/History/3.7.the_post-independence_liberation_struggles_in_uganda.txt" . || exit 1
              mv 3.7.the_post-independence_liberation_struggles_in_uganda.txt .3.7.the_post-independence_liberation_struggles_in_uganda.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.7.the_post-independence_liberation_struggles_in_uganda.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.7.the_post-independence_liberation_struggles_in_uganda.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.7.the_post-independence_liberation_struggles_in_uganda.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.7.the_post-independence_liberation_struggles_in_uganda.txt
              process_reminders_from_file .3.7.the_post-independence_liberation_struggles_in_uganda.txt
              STATE_FILE=".s_history_3_7"
              process_file .3.7.the_post-independence_liberation_struggles_in_uganda.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.7.the_post-independence_liberation_struggles_in_uganda.txt
              sed -i '/^1$/!d' .s_history_3_7
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S3"
              # Define the file extension
              file_extension_question=".7.the_post-independence_liberation_struggles_in_uganda.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            8)
              if ! [ -f ".history.3.7" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/History/S3"
                # Define the file extension
                file_extension_answer=".7.the_post-independence_liberation_struggles_in_uganda.ans.txt"
                # Define the exercise file
                exercise_file="../../history_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_history_3_8" ]; then
                echo -e "\n\nYou are to cover Patriotism in Uganda ...\n\n${g}Please never ever settle for less${t}\n\nPromise yourself that you wont give up \c" && wait_for_a_key_press
              fi
              cp "Notes/History/3.8.patriotism_in_uganda.txt" . || exit 1
              mv 3.8.patriotism_in_uganda.txt .3.8.patriotism_in_uganda.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.8.patriotism_in_uganda.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.8.patriotism_in_uganda.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.8.patriotism_in_uganda.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.8.patriotism_in_uganda.txt
              process_reminders_from_file .3.8.patriotism_in_uganda.txt
              STATE_FILE=".s_history_3_8"
              process_file .3.8.patriotism_in_uganda.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.8.patriotism_in_uganda.txt
              sed -i '/^1$/!d' .s_history_3_8
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S3"
              # Define the file extension
              file_extension_question=".8.patriotism_in_uganda.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            9)
              if ! [ -f ".history.3.8" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/History/S3"
                # Define the file extension
                file_extension_answer=".8.patriotism_in_uganda.ans.txt"
                # Define the exercise file
                exercise_file="../../history_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_history_3_9" ]; then
                echo -e "\n\nI am so happy for you dear one. You are here to cover the the 9th topic (Key contributors to nation building in the post-colonial Uganda )...\n\n${g}Just never underestimate the value of a single second${t}\n\nThat extra one second maybe all you need to fully understand a given concept \c" && wait_for_a_key_press
              fi
              cp "Notes/History/3.9.key_contributors_to_nation_building_in_the_post-colonial_uganda.txt" . || exit 1
              mv 3.9.key_contributors_to_nation_building_in_the_post-colonial_uganda.txt .3.9.key_contributors_to_nation_building_in_the_post-colonial_uganda.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.9.key_contributors_to_nation_building_in_the_post-colonial_uganda.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.9.key_contributors_to_nation_building_in_the_post-colonial_uganda.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.9.key_contributors_to_nation_building_in_the_post-colonial_uganda.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.9.key_contributors_to_nation_building_in_the_post-colonial_uganda.txt
              process_reminders_from_file .3.9.key_contributors_to_nation_building_in_the_post-colonial_uganda.txt
              STATE_FILE=".s_history_3_9"
              process_file .3.9.key_contributors_to_nation_building_in_the_post-colonial_uganda.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.9.key_contributors_to_nation_building_in_the_post-colonial_uganda.txt
              sed -i '/^1$/!d' .s_history_3_9
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S3"
              # Define the file extension
              file_extension_question=".9.key_contributors_to_nation_building_in_the_post-colonial_uganda.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".history.3.9" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/History/S3"
	              # Define the file extension
	              file_extension_answer=".9.key_contributors_to_nation_building_in_the_post-colonial_uganda.ans.txt"
	              # Define the exercise file
	              exercise_file="../../history_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
								touch .history.3.9
								echo "3" > .history_ready
							fi
            ;;
    ...
    if ! find . -maxdepth 1 -name '.s_history_4*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to S4 History class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
    ...
    do
      ...
      do
        ...
        then
          attempts=0
          # Define the targeted directory
          question_directory="Revision/History/S4"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../history_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/History/S4"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../history_answered_ans.txt"
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_history_4_1" ]; then
                echo -e "\n\nYou chose to explore Lessons from world economic transformations ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/History/4.1.lessons_from_world_economic_transformations.txt" . || exit 1
              mv 4.1.lessons_from_world_economic_transformations.txt .4.1.lessons_from_world_economic_transformations.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.1.lessons_from_world_economic_transformations.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.1.lessons_from_world_economic_transformations.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.1.lessons_from_world_economic_transformations.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.1.lessons_from_world_economic_transformations.txt
              process_reminders_from_file .4.1.lessons_from_world_economic_transformations.txt
              STATE_FILE=".s_history_4_1"
              process_file .4.1.lessons_from_world_economic_transformations.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.1.lessons_from_world_economic_transformations.txt
              sed -i '/^1$/!d' .s_history_4_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S4"
              # Define the file extension
              file_extension=".1.lessons_from_world_economic_transformations.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/History/S4"
              # Define the file extension
              file_extension_question=".1.lessons_from_world_economic_transformations.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".history.4.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/History/S4"
                # Define the file extension
                file_extension_answer=".1.lessons_from_world_economic_transformations.ans.txt"
                # Define the exercise file
                exercise_file="../../history_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_history_4_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Evolution of pan-Africanism ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/History/4.2.evolution_of_pan-africanism.txt" . || exit 1
              mv 4.2.evolution_of_pan-africanism.txt .4.2.evolution_of_pan-africanism.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.2.evolution_of_pan-africanism.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.2.evolution_of_pan-africanism.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.2.evolution_of_pan-africanism.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.2.evolution_of_pan-africanism.txt
              process_reminders_from_file .4.2.evolution_of_pan-africanism.txt
              STATE_FILE=".s_history_4_2"
              process_file .4.2.evolution_of_pan-africanism.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.2.evolution_of_pan-africanism.txt
              sed -i '/^1$/!d' .s_history_4_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S4"
              # Define the file extension
              file_extension_question=".2.evolution_of_pan-africanism.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".history.4.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/History/S4"
                # Define the file extension
                file_extension_answer=".2.evolution_of_pan-africanism.ans.txt"
                # Define the exercise file
                exercise_file="../../history_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_history_4_3" ]; then
                echo -e "\n\nYou have made a choice to cover Neo-colonialism in East Africa ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/History/4.3.neo-colonialism_in_east_africa.txt" . || exit 1
              mv 4.3.neo-colonialism_in_east_africa.txt .4.3.neo-colonialism_in_east_africa.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.3.neo-colonialism_in_east_africa.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.3.neo-colonialism_in_east_africa.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.3.neo-colonialism_in_east_africa.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.3.neo-colonialism_in_east_africa.txt
              process_reminders_from_file .4.3.neo-colonialism_in_east_africa.txt
              STATE_FILE=".s_history_4_3"
              process_file .4.3.neo-colonialism_in_east_africa.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.3.neo-colonialism_in_east_africa.txt
              sed -i '/^1$/!d' .s_history_4_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S4"
              # Define the file extension
              file_extension_question=".3.neo-colonialism_in_east_africa.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".history.4.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/History/S4"
                # Define the file extension
                file_extension_answer=".3.neo-colonialism_in_east_africa.ans.txt"
                # Define the exercise file
                exercise_file="../../history_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_history_4_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Globalisation ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/History/4.4.globalisation.txt" . || exit 1
              mv 4.4.globalisation.txt .4.4.globalisation.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.4.globalisation.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.4.globalisation.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.4.globalisation.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.4.globalisation.txt
              process_reminders_from_file .4.4.globalisation.txt
              STATE_FILE=".s_history_4_4"
              process_file .4.4.globalisation.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.4.globalisation.txt
              sed -i '/^1$/!d' .s_history_4_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S4"
              # Define the file extension
              file_extension=".4.globalisation.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/History/S4"
              # Define the file extension
              file_extension_question=".4.globalisation.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            5)
              if ! [ -f ".history.4.4" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/History/S4"
                # Define the file extension
                file_extension_answer=".4.globalisation.ans.txt"
                # Define the exercise file
                exercise_file="../../history_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_history_4_5" ]; then
                echo -e "\n\nHere you are dear one... Stay organised as you explore The struggle and liberation of South Africa ...\n\n${g}Just know we are not going to leave you alone${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/History/4.5.the_struggle_and_liberation_of_south_africa.txt" . || exit 1
              mv 4.5.the_struggle_and_liberation_of_south_africa.txt .4.5.the_struggle_and_liberation_of_south_africa.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.5.the_struggle_and_liberation_of_south_africa.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.5.the_struggle_and_liberation_of_south_africa.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.5.the_struggle_and_liberation_of_south_africa.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.5.the_struggle_and_liberation_of_south_africa.txt
              process_reminders_from_file .4.5.the_struggle_and_liberation_of_south_africa.txt
              STATE_FILE=".s_history_4_5"
              process_file .4.5.the_struggle_and_liberation_of_south_africa.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.5.the_struggle_and_liberation_of_south_africa.txt
              sed -i '/^1$/!d' .s_history_4_5
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S4"
              # Define the file extension
              file_extension_question=".5.the_struggle_and_liberation_of_south_africa.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            6)
              if ! [ -f ".history.4.5" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/History/S4"
                # Define the file extension
                file_extension_answer=".5.the_struggle_and_liberation_of_south_africa.ans.txt"
                # Define the exercise file
                exercise_file="../../history_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_history_4_6" ]; then
                echo -e "\n\nYou have managed to make it to Peace and conflict resolution in East Africa ...\n\n${g}Remember to pray always${t}\n\nThe fear of the Lord is the beginning of wisdom \c" && wait_for_a_key_press
              fi
              cp "Notes/History/4.6.peace_and_conflict_resolution_in_east_africa.txt" . || exit 1
              mv 4.6.peace_and_conflict_resolution_in_east_africa.txt .4.6.peace_and_conflict_resolution_in_east_africa.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.6.peace_and_conflict_resolution_in_east_africa.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.6.peace_and_conflict_resolution_in_east_africa.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.6.peace_and_conflict_resolution_in_east_africa.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.6.peace_and_conflict_resolution_in_east_africa.txt
              process_reminders_from_file .4.6.peace_and_conflict_resolution_in_east_africa.txt
              STATE_FILE=".s_history_4_6"
              process_file .4.6.peace_and_conflict_resolution_in_east_africa.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.6.peace_and_conflict_resolution_in_east_africa.txt
              sed -i '/^1$/!d' .s_history_4_6
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S4"
              # Define the file extension
              file_extension_question=".6.peace_and_conflict_resolution_in_east_africa.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            7)
              if ! [ -f ".history.4.6" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/History/S4"
                # Define the file extension
                file_extension_answer=".6.peace_and_conflict_resolution_in_east_africa.ans.txt"
                # Define the exercise file
                exercise_file="../../history_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_history_4_7" ]; then
                echo -e "\n\nFrom here, you will be proceeding with Topical reviews ...\n\n${g}Please never ever forget that your education is your future${t}\n\nFocus dear \c" && wait_for_a_key_press
              fi
              cp "Notes/History/4.7.topical_reviews.txt" . || exit 1
              mv 4.7.topical_reviews.txt .4.7.topical_reviews.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.7.topical_reviews.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.7.topical_reviews.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.7.topical_reviews.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.7.topical_reviews.txt
              process_reminders_from_file .4.7.topical_reviews.txt
              STATE_FILE=".s_history_4_7"
              process_file .4.7.topical_reviews.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.7.topical_reviews.txt
              sed -i '/^1$/!d' .s_history_4_7
              attempts=0
              # Define the targeted directory
              question_directory="Revision/History/S4"
              # Define the file extension
              file_extension_question=".7.topical_reviews.qns.txt"
              # Define the revision file
              revision_file="../../history_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".history.4.7" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/History/S4"
	              # Define the file extension
	              file_extension_answer=".7.topical_reviews.ans.txt"
	              # Define the exercise file
	              exercise_file="../../history_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
								touch .history.4.7
								echo "4" > .history_ready
							fi
            ;;
