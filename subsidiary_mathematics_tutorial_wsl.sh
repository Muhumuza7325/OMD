...
while true; do
  ...
  # Prompt the user for input
  if [ -z "$(find ./Notes/Subsidiary_mathematics -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
	...
	while true; do
		if [ ! -f .revise.txt ]; then
			...
			if [ $time_diff -gt 3600 ]; then
				...
				echo -e "\n\n\n${r}You are advised to not make any changes to the provided answers, instead, you can make copies that you can edit${t}\n\n${y}For a teacher willing to join us reach out to everyone of our children, please send us your questions and answers in a file labelled with your name, school, subsidiary_mathematics, and file content (e.g., Muhumuza_Omega_Kasule_High_School_O_level_Chemistry_Answered_EOC1_Items.pdf) to our contacts${t}\n\n\nEmail: ${g}2024omd256@gmail.com${t} \c"
				echo $current_time > "$last_echo_time_file"
			fi
...
if ! [ -d "Notes/Subsidiary_mathematics" ] || ! [ -d "Revision" ] || ! [ -d "Exercise" ] || ! [ -d "Videos" ] || ! [ -d "Figures" ] || ! [ -d "Tables" ]; then
...
if [ -z "$(find ./Notes/Subsidiary_mathematics -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
  ...
  if [[ -z "$user_input" ]]; then
    ...
    curl -O -L https://github.com/Muhumuza7325/OMD/raw/main/SUB_MTC/1.1.matrices.txt || echo -e "\n\nError fetching material for this tutorial \c"
