...
while true; do
  ...
  # Prompt the user for input
  if [ -z "$(find ./Notes/Luganda -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
...
if ! [ -d "Notes/Luganda" ] || ! [ -d "Revision" ] || ! [ -d "Exercise" ] || ! [ -d "Videos" ] || ! [ -d "Figures" ] || ! [ -d "Tables" ]; then
