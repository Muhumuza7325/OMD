...
if ! [ -d "Notes" ] || ! [ -d "Revision" ] || ! [ -d "Exercise" ] || ! [ -d "Videos" ] || ! [ -d "Figures" ] || ! [ -d "Tables" ]; then
    ...
    cp ../ugandan_sign_language_tutorial .
    clear_and_center
    echo
fi

files=(Notes/Ugandan_sign_language/*.txt)
if [ ${#files[@]} -eq 0 ]; then
    ...
    rm -f ../ugandan_sign_language_tutorial 1.1.introduction_to_ugandan_sign_language.txt
fi
