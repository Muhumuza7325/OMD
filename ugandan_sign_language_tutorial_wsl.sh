...
function handle_class_input() {
  if { [ -n "$last_class" ] || [ -z "$last_topic" ]; } && [ -f .ugandan_sign_language_surveyor ]; then
		while true; do
	    read -rp $'\n\nPlease enter '"${r}1 or 2 or 3 or 4${t}"' to go to your class or '"${r}x${t}"' to exit'$'\n\n> ' class
      if [ "$class" == "x" ]; then
        exit
      fi
		  # Check if .current_ugandan_sign_language_class is accidentally empty
		  if [ ! -s .current_ugandan_sign_language_class ]; then
		    # Echo 1 to .current_ugandan_sign_language_class
		    echo "1" > .current_ugandan_sign_language_class
		  fi
		  # Read the value from the file
		  current_ugandan_sign_language_class=$(<.current_ugandan_sign_language_class) 2>/dev/null
		  # Check if the value is equal to $class
		  if [ "$current_ugandan_sign_language_class" -lt "$class" 2>/dev/null ]; then
	      ...
	      rm -f ".ugandan_sign_language_topic_selected"
	      # Update the state file with the class
	      if [ "$class" != "x" ]; then
	        echo "$class" > .ugandan_sign_language_user_state 2>/dev/null
	      fi
				break
	  	fi
		done
...
function handle_s2_topic_input() {
  ...
  if [ -z "$last_topic" ] || [ -f .ugandan_sign_language_topic_selected ]; then
    read -rp $'\n\nChoose either topic '"${g}1 or 2 or 3 or 4 or 5${t}"' to learn'$'\nor enter '"${r}z${t}"' for an adventure or '"${r}r${t}"' to revise or '"${r}s${t}"' to get sample_items'$'\nor '"${r}a${t}"' to get an activity of integration or '"${r}q${t}"' to get a short answer question'$'\nor '"${r}n${t}"' to do your final class assignment and if necessary, gain access to the next class'$'\nor '"${r}p${t}"' to track academic progress or '"${r}x${t}"' to exit'$'\n\n1. Establishing and managing relationships '"${r}Term1${t}"''$'\n\n2. School environment '"${r}Term1${t}"''$'\n\n3. Traditional ceremonies '"${r}Term2${t}"''$'\n\n4. Occupations and careers '"${r}Term2${t}"''$'\n\n5. Public places '"${r}Term3${t}"''$'\n\n> ' topic
    touch .ugandan_sign_language_surveyor
    touch .ugandan_sign_language_topic_selected
    # Update the state file with the topic
    # Check if the state file exists, and the topic is not "x"
...
function handle_s1_topic_input() {
  ...
  if [ -z "$last_topic" ] || [ -f .ugandan_sign_language_topic_selected ]; then
    read -rp $'\n\nChoose either topic '"${g}1 or 2 or 3 or 4 or 5 or 6 or 7${t}"' to learn'$'\nor enter '"${r}z${t}"' for an adventure or '"${r}r${t}"' to revise or '"${r}s${t}"' to get sample_items'$'\nor '"${r}a${t}"' to get an activity of integration or '"${r}q${t}"' to get a short answer question'$'\nor '"${r}n${t}"' to do your final class assignment and if necessary, gain access to the next class'$'\nor '"${r}p${t}"' to track academic progress or '"${r}x${t}"' to exit'$'\n\n1. Introduction to Ugandan sign language '"${r}Term1${t}"''$'\n\n2. Number, time and dates '"${r}Term1${t}"''$'\n\n3. Family '"${r}Term1${t}"''$'\n\n4. Life at home '"${r}Term1${t}"''$'\n\n5. Plants in our area '"${r}Term2${t}"''$'\n\n6. Animals and birds '"${r}Term2${t}"''$'\n\n7. Personal and community hygiene '"${r}Term3${t}"''$'\n\n> ' topic
    touch .ugandan_sign_language_surveyor
    touch .ugandan_sign_language_topic_selected
    # Update the state file with the topic
    # Check if the state file exists, and the topic is not "x"
...
process_reminders_from_file() {
  ...
  prev_sentence=""
  echo_next=false
	echo -n > .ugandan_sign_language_reminder
    ...
    for sentence in "${sentences[@]}"; do
      if [[ $echo_next == true ]]; then
        ...
        for split_sentence in "${split_sentences[@]}"; do
					echo "$split_sentence" >> .ugandan_sign_language_reminder
        done
      echo_next=false
      fi
        ...
        echo "Did you know that $lower_cased_sentence" >> .ugandan_sign_language_reminder
        ...
        echo_next=true
  			...
  			echo "Do you recall that $lower_cased_sentence;" >> .ugandan_sign_language_reminder

			elif [[ "$sentence" =~ " → " ]]; then
  			echo "Hope you know that: $sentence;" >> .ugandan_sign_language_reminder

      elif [[ "$sentence" =~ " ↔ " ]]; then
        echo "Hope you know that: $sentence;" >> .ugandan_sign_language_reminder

      elif [[ "$sentence" =~ (Generally|general) ]]; then
        echo "Note: $sentence;" >> .ugandan_sign_language_reminder
...
process_random_reminder() {
  ...
  # Check if .ugandan_sign_language_reminder exists
  if [ -f .ugandan_sign_language_reminder ]; then
		# Remove empty lines from .ugandan_sign_language_reminder
  	sed -i '/^[[:space:]]*$/d' .ugandan_sign_language_reminder 2>/dev/null
    # Randomly select a non-empty sentence
    local reminder # Declare the variable
		reminder=$(awk -v RS=';' 'BEGIN{srand();}{gsub(/^[[:space:]]+|[[:space:]]+$/, ""); if (length > 0) a[++n]=$0}END{if (n > 0) print a[int(rand()*n)+1]}' .ugandan_sign_language_reminder)
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    for sentence in "${sentences[@]}"; do
        ...
        if [[ $sentence == *"Figure"* ]]; then
          ...
          # Change to the "Figures/Ugandan_sign_language" directory
          cd Figures/Ugandan_sign_language || { echo "Failed to change to Figures/Ugandan_sign_language"; return; }
        ...
        if [[ $sentence == *"Table"* ]]; then
          cd Tables/Ugandan_sign_language || { echo -e "\nFailed to change to Tables/Ugandan_sign_language \c"; return; }
          explorer.exe "$sentence" > /dev/null 2>&1 &
          cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        fi

        if [[ $sentence == *"Video"* ]]; then
          cd Videos/Ugandan_sign_language || { echo -e "\nFailed to change to Videos/Ugandan_sign_language \c"; return; }
          explorer.exe "$sentence" > /dev/null 2>&1 &
          cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        fi
...
contact_ai() {
  last_topic=$(awk -F' ' '{print $2}' .ugandan_sign_language_user_state)
...
while true; do
  ...
  # Prompt the user for input
  if [ -z "$(find ./Notes/Ugandan_sign_language -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
  ...
  if [[ -n "$user_input" ]]; then
		if [[ "$user_input" == "sh" ]]; then
		  Response="We are looking forward to receiving your economic support, thoughts, suggestions, and any files you feel should reach out to everyone of our children. Please remember to label the files you are to attach using the format: Your_name_School_Ugandan_sign_language_File_content (e.g., Muhumuza_Omega_Kasule_High_School_O_level_Chemistry_Answered_EOC1_Items.pdf). Thanks a lot for your contributions."  
		  ugandan_sign_language="$(basename "$0") - $(date +"%Y-%m-%d %H:%M:%S") - Thoughts, suggestions, and contributions"
		  encoded_ugandan_sign_language=$(echo "$ugandan_sign_language" | sed 's/ /%20/g; s/\n/%0A/g')
		  encoded_body=$(echo "$Response" | sed 's/ /%20/g; s/\n/%0A/g')
		  # Open the email in the browser with the encoded ugandan_sign_language and body
		  powershell.exe -Command "Start-Process 'https://mail.google.com/mail/?view=cm&to=2024omd256@gmail.com&su=${encoded_ugandan_sign_language}&body=${encoded_body}'"
			return
		fi
     ...
     if [[ "$user_input" == "sr" ]]; then
      ...
      explorer.exe Ugandan_sign_language > /dev/null 2>&1 &
    ...
    if [[ "$user_input" == "zz" ]]; then
      ...
      TEMP_FILE=$(mktemp) && curl -o "$TEMP_FILE" -L "https://github.com/Muhumuza7325/OMD/raw/main/ugandan_sign_language_tutorial_wsl.sh" && mv "$TEMP_FILE" ugandan_sign_language_tutorial_wsl.sh && chmod +x ugandan_sign_language_tutorial_wsl.sh && echo -e "\n\n${y}Code successfully updated.. You will have to restart a new session${t} \c" && sleep 4 && exit || (echo -e "\n\n${m}Error updating code!... Please check your internet connection and try again!${t} \c" && rm -f "$TEMP_FILE" && return)
    fi
    if [[ "$user_input" == "xx" ]]; then
      ...
      if [[ "$input" == "y" || "$input" == "Y" ]]; then
        ...
        curl -O -L "https://github.com/Muhumuza7325/OMD/raw/main/update_ugandan_sign_language.sh" || { echo -e "\n\n${m}Check your internet connection and try again!${t}" >&2; return; }
        mv update_ugandan_sign_language.sh .update_ugandan_sign_language.sh
        bash .update_ugandan_sign_language.sh
        return
      else
        return
      fi
    ...
    if [[ -n "$result" ]]; then
      ...
      if echo "$result" | grep "Figure"; then
        echo "$result" | grep -o '\bFigure[0-9]\+.*\.jpg\(\.[0-9]\+\)*\b' > Figures/Ugandan_sign_language/figures.txt
        sed -i -e '/^Figure/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Figures/Ugandan_sign_language/figures.txt
        # Change to the "Figures/Ugandan_sign_language" directory
        cd Figures/Ugandan_sign_language || { echo "Failed to change to Figures/Ugandan_sign_language"; return; }
        ...
        rm -f Figures/Ugandan_sign_language/figures.txt
      fi

      if echo "$result" | grep "Table"; then
        echo "$result" | grep -o '\bTable[0-9]\+\(\.[0-9]\+\)*\b' > Tables/Ugandan_sign_language/tables.txt
        sed -i -e '/^Table/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Tables/Ugandan_sign_language/tables.txt
        cd Tables/Ugandan_sign_language || { echo -e "\nFailed to change to Tables/Ugandan_sign_language \c"; return; }
        ...
        # Go back to the original directory
        cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        rm -f Tables/Ugandan_sign_language/tables.txt
      fi

      if echo "$result" | grep "Video"; then
        #echo "$result" > Videos/Ugandan_sign_language/videos.txt
        echo "$result" | grep -o '\bVideo[0-9]\+\(\.[0-9]\+\)*\b' > Videos/Ugandan_sign_language/videos.txt
        # Remove lines not starting with "Video" and any leading/trailing whitespaces
        sed -i -e '/^Video/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Videos/Ugandan_sign_language/videos.txt
        # Change to the "Videos/Ugandan_sign_language" directory
        cd Videos/Ugandan_sign_language || { echo -e "\nFailed to change to Videos/Ugandan_sign_language \c"; return; }
        ...
        cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        # Remove the temporary file
        rm -f Videos/Ugandan_sign_language/videos.txt
      fi
  ...
  if [ -f "$selected_file" ]; then
    ...
    if [[ -n "$selected_question" && "$selected_question" =~ [[:graph:]] ]]; then
      ...
      if echo "$selected_question" | grep "Figure"; then
        echo "$selected_question" | grep -o '\bFigure[0-9]\+[^;]*\b' > ../../Figures/Ugandan_sign_language/figures.txt
        sed -i -e '/^Figure/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' ../../Figures/Ugandan_sign_language/figures.txt
        # Change to the "Figures/Ugandan_sign_language" directory
        cd ../../Figures/Ugandan_sign_language || { echo "Failed to change to Figures/Ugandan_sign_language"; return; }
        # Specify the path to the text file containing figure names
        ...
        # Go back to the original directory
        cd ../../Revision/Ugandan_sign_language || { echo "Failed to change back to the targeted directory \c"; exit 1; }
        # Remove the temporary file
        rm -f ../../Figures/Ugandan_sign_language/figures.txt
      fi
...
process_question_answer() {
  ...
  if [ -f ../../../.school_name ]; then
    ...
    sed -i '/^[[:space:]]*$/d' ../../../."$school_name"_students_file.txt
    existing_class=$(awk '{print $1}' ../../../.ugandan_sign_language_user_state)
    existing_topic=$(awk '{print $2}' ../../../.ugandan_sign_language_user_state)
    echo ''
    ...
    if grep -q "Ugandan_sign_language" ../../../."$school_name"_students_file.txt; then
      sed -i -E 's/;/\n/g' ../../../."$school_name"_students_file.txt
      # Find the line with the word Ugandan_sign_language, replace the information below it with the already available information adding a comma existing_class_existing_topic [$percentage]
      if [ "$existing_topic" == "r" ]; then
        sed -i '/Ugandan_sign_language/{n;s/\(.*\)/\1, '"$existing_class"'_'"$input"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
      else
        (( existing_topic-- ))
        sed -i '/Ugandan_sign_language/{n;s/\(.*\)/\1, '"$existing_class"'_'"$existing_topic"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
      fi
    else
      if [ "$existing_topic" == "r" ]; then
        sed -i -E '/^School/ i\Ugandan_sign_language\n'"${existing_class} ${names}[${percentage}%]"'' ../../../."$school_name"_students_file.txt
        echo -e "Ugandan_sign_language\n"$existing_class"_'"$input"' "$names"["$percentage%"]" >> ../../../."$school_name"_students_file.txt
      else
        (( existing_topic-- ))
...
function resume_from_last_point() {
  if [ -f .ugandan_sign_language_user_state ]; then
    last_class=$(awk -F' ' '{print $1}' .ugandan_sign_language_user_state)
    last_topic=$(awk -F' ' '{print $2}' .ugandan_sign_language_user_state)
    if [ -n "$last_class" ] && [ -n "$last_topic" ]; then
      echo -e "\n       Resuming from ${r}S$last_class${t} : ${g}Topic '$last_topic'${t} \c"
      rm -f .ugandan_sign_language_surveyor
...
process_final_assignment() {
  # Check if user is.ugandan_sign_language_ready for the assignment
  # Check if .current_ugandan_sign_language_class is accidentally empty
  if [ ! -s .current_ugandan_sign_language_class ]; then
    # Echo 1 to .current_ugandan_sign_language_class
    echo "1" > .current_ugandan_sign_language_class
  fi
  # Read the value from the .current_ugandan_sign_language_class file
  current_ugandan_sign_language_class=$(<.current_ugandan_sign_language_class) 2>/dev/null
  # Check if the value in the .ugandan_sign_language_ready file is equal to $class
  # Read the value from the .ugandan_sign_language_ready file
  if [ ! -s .ugandan_sign_language_ready ]; then
    echo "0" > .ugandan_sign_language_ready
  fi
  how.ugandan_sign_language_ready=$(<.ugandan_sign_language_ready) 2>/dev/null
  # Check if the value is equal to $class
  if [ "$how.ugandan_sign_language_ready" -lt "$current_ugandan_sign_language_class" 2>/dev/null ]; then
    read -rp $'\n\nYou havent done all the topic assignments for your current ugandan_sign_language class\n\n'"${r}Proceeding from here will affect your very final score${y}"'. To go back and progress right, enter '"${y}ok${t}"'. Otherwise, press the Enter key to do the final class assignment: ' progress
    if [ "$progress" == "ok" ]; then
      return
    fi
  fi
    ...
    if ! [ "$total_questions" == 100 ]; then
      ...
      if [ "$elapsed_time" -gt 3600 ]; then
        ...
        if [ "$percentage" -lt 95 ]; then
          ...
          cd - > /dev/null 2>&1 || exit
          if ! [ -f .current_ugandan_sign_language_class ]; then
            # Echo the result to the file .current_class
            echo "2" > .current_ugandan_sign_language_class
            ...
            # Read the value from the file
            current_ugandan_sign_language_class=$(<.current_ugandan_sign_language_class) 2>/dev/null
            # Check if the new_class value is lt $current_ugandan_sign_language_class
            if [ "$new_class" -gt "$current_ugandan_sign_language_class" 2>/dev/null ]; then
		          # Echo the result to the file .current_class
		          echo "$new_class" > .current_ugandan_sign_language_class
            fi
...
get_sample_items() {
  ...
  # Save the current working directory
  pushd . > /dev/null
  cd Revision/Ugandan_sign_language || { echo "Directory not found"; return; }
  ...
  while true; do
      ...
      if [ ! -f "$last_echo_time_file" ]; then
    	...
    	if [ $time_diff -gt 3600 ]; then
      	# Echo the message and update the last echo time
	    	echo -e "\n\n\n${r}You are advised to not make any changes to the provided answers, instead, you can make copies that you can edit${t}\n\n${y}For a teacher willing to join us reach out to everyone of our children, please send us your questions and answers in a file labelled with your name, school, ugandan_sign_language, and file content (e.g., Muhumuza_Omega_Kasule_High_School_O_level_Chemistry_Answered_EOC1_Items.pdf) to our contacts${t}\n\n\nEmail: ${g}2024omd256@gmail.com${t} \c"
				echo $current_time > "$last_echo_time_file"
  		fi
      read -rp $'\n\n\nEnter '"${m}any character${t}"' for access to the file of answered items or simply press '"${r}Enter${t}"' to get items to attempt : ' input
     	if [[ -n $input ]]; then
     		explorer.exe .ugandan_sign_language_samples* > /dev/null 2>&1 &
      	clear
      	popd > /dev/null || exit
        return
      fi
      if [ -f .e_o_c_ugandan_sign_language.txt ]; then
        echo -e "\n\n${y}Below is the list of the elements of construct${t} \n"
        cat .e_o_c_ugandan_sign_language.txt
        ...
        if [[ -n $input ]]; then
          ...
          selected_file1=$(ls -a | grep -E "\.e_o_c_ugandan_sign_language_${input}\.txt")
          cat "$selected_file1"
          # Remove empty lines from the selected files
          find . -type f -name "*_samples_[0-9].txt" -exec sed -i '/^[[:space:]]*$/d' {} +
          selected_file=$(ls -a | grep -E "\.e_o_c_ugandan_sign_language_${input}_samples" | shuf -n 1)
        else
    ...
    rm -f .revise.txt .ugandan_sign_language_topic_selected 2>/dev/null
  done
  popd > /dev/null || exit
	return
}
...
while true; do
  ...
  if ls Students | grep -wq "$initials"; then
    ...
    echo -e "                  $initials\n" > "Students/$initials/Exercise/ugandan_sign_language_answered_ans.txt"
    echo -e "                  $initials\n" > "Students/$initials/Revision/ugandan_sign_language_covered_qns.txt"
    ...
    if [ -f ../../../.school_name ]; then
      ...
      if grep -q "Ugandan_sign_language" ../../../."$school_name"_students_file.txt; then
        ...
        if [ "$existing_topic" == "z" ]; then
          sed -i '/Ugandan_sign_language/{n;s/\(.*\)/\1, '"$existing_class"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
        else
          (( existing_topic-- ))
          sed -i '/Ugandan_sign_language/{n;s/\(.*\)/\1, '"$existing_class"'_'"$existing_topic"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
        fi
...
if [ ! -f .ugandan_sign_language_user_state ]; then
	touch .ugandan_sign_language_user_state
	touch .ugandan_sign_language_surveyor
	get_and_display_pattern
else
  if [ -f .omd_communication.txt ]; then
    ...
    process_random_reminder .ugandan_sign_language_reminder
  fi
	handle_resume_input
fi

if [ -z "$class" ] && [ -s ".ugandan_sign_language_user_state" ]; then
  get_and_display_pattern
fi
# Check for the presence of specific directories and a file
if ! [ -d "Notes/Ugandan_sign_language" ] || ! [ -d "Revision" ] || ! [ -d "Exercise" ] || ! [ -d "Videos" ] || ! [ -d "Figures" ] || ! [ -d "Tables" ]; then
  ...
  mkdir -p Notes Notes/Ugandan_sign_language Revision Revision/Ugandan_sign_language Revision/Ugandan_sign_language/{S1,S2,S3,S4} Exercise Exercise/Ugandan_sign_language Exercise/Ugandan_sign_language/{S1,S2,S3,S4} Videos Videos/Ugandan_sign_language Figures Figures/Ugandan_sign_language Tables Tables/Ugandan_sign_language
  touch Revision/ugandan_sign_language_covered_qns.txt Exercise/ugandan_sign_language_answered_ans.txt
  echo -e "\n"
  pwd
...
if [ -z "$(find ./Notes/Ugandan_sign_language -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
  ...
  if [[ -z "$user_input" ]]; then
    ...
    curl -O -L https://github.com/Muhumuza7325/OMD/raw/main/SIGN/1.1.introduction_to_ugandan_sign_language.txt || echo -e "\n\nError fetching material for this tutorial \c"
    ...
    cp 1.1.introduction_to_ugandan_sign_language.txt Notes/Ugandan_sign_language || echo -e "\n\nError copying 1.1.introduction_to_ugandan_sign_language.txt to the Ugandan_sign_language directory in the Notes directory \c"
    wait_for_a_key_press
  else
  ...
  rm -f 1.1.introduction_to_ugandan_sign_language.txt
fi
...
while true; do
  ...
  handle_class_input
  if [[ "$class" == "1" ]]; then
    if ! find . -maxdepth 1 -name '.s_ugandan_sign_language_1*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to the S1 Ugandan_sign_language class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
    ...
    do
      ...
      do
        ...
        then
          ...
          question_directory="Revision/Ugandan_sign_language/S1"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../ugandan_sign_language_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Ugandan_sign_language/S1"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../ugandan_sign_language_answered_ans.txt"
          # Call the function to process a random question
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_ugandan_sign_language_1_1" ]; then
                echo -e "\n\nYou chose to explore Introduction to Ugandan sign language ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/Ugandan_sign_language/1.1.introduction_to_ugandan_sign_language.txt" . || exit 1
              mv 1.1.introduction_to_ugandan_sign_language.txt .1.1.introduction_to_ugandan_sign_language.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.1.introduction_to_ugandan_sign_language.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.1.introduction_to_ugandan_sign_language.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.1.introduction_to_ugandan_sign_language.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.1.introduction_to_ugandan_sign_language.txt
              process_reminders_from_file .1.1.introduction_to_ugandan_sign_language.txt
              STATE_FILE=".s_ugandan_sign_language_1_1"
              process_file .1.1.introduction_to_ugandan_sign_language.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.1.introduction_to_ugandan_sign_language.txt
              sed -i '/^1$/!d' .s_ugandan_sign_language_1_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Ugandan_sign_language/S1"
              # Define the file extension
              file_extension=".1.introduction_to_ugandan_sign_language.qns.txt"
              # Define the revision file
              revision_file="../../ugandan_sign_language_covered_qns.txt"
              # Call the function to process a random question
              ...
              question_directory="Revision/Ugandan_sign_language/S1"
              # Define the file extension
              file_extension_question=".1.introduction_to_ugandan_sign_language.qns.txt"
              # Define the revision file
              revision_file="../../ugandan_sign_language_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".ugandan_sign_language.1.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Ugandan_sign_language/S1"
                # Define the file extension
                file_extension_answer=".1.introduction_to_ugandan_sign_language.ans.txt"
                # Define the exercise file
                exercise_file="../../ugandan_sign_language_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .ugandan_sign_language.1.1
              fi
              if ! [ -f ".s_ugandan_sign_language_1_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Number, time and dates ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Ugandan_sign_language/1.2.number_time_and_dates.txt" . || exit 1
              mv 1.2.number_time_and_dates.txt .1.2.number_time_and_dates.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.2.number_time_and_dates.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.2.number_time_and_dates.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.2.number_time_and_dates.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.2.number_time_and_dates.txt
              process_reminders_from_file .1.2.number_time_and_dates.txt
              STATE_FILE=".s_ugandan_sign_language_1_2"
              process_file .1.2.number_time_and_dates.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.2.number_time_and_dates.txt
              sed -i '/^1$/!d' .s_ugandan_sign_language_1_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Ugandan_sign_language/S1"
              # Define the file extension
              file_extension_question=".2.number_time_and_dates.qns.txt"
              # Define the revision file
              revision_file="../../ugandan_sign_language_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".ugandan_sign_language.1.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Ugandan_sign_language/S1"
                # Define the file extension
                file_extension_answer=".2.number_time_and_dates.ans.txt"
                # Define the exercise file
                exercise_file="../../ugandan_sign_language_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .ugandan_sign_language.1.2
              fi
              if ! [ -f ".s_ugandan_sign_language_1_3" ]; then
                echo -e "\n\nYou have made a choice to cover Family ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Ugandan_sign_language/1.3.family.txt" . || exit 1
              mv 1.3.family.txt .1.3.family.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.3.family.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.3.family.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.3.family.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.3.family.txt
              process_reminders_from_file .1.3.family.txt
              STATE_FILE=".s_ugandan_sign_language_1_3"
              process_file .1.3.family.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.3.family.txt
              sed -i '/^1$/!d' .s_ugandan_sign_language_1_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Ugandan_sign_language/S1"
              # Define the file extension
              file_extension_question=".3.family.qns.txt"
              # Define the revision file
              revision_file="../../ugandan_sign_language_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".ugandan_sign_language.1.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Ugandan_sign_language/S1"
                # Define the file extension
                file_extension_answer=".3.family.ans.txt"
                # Define the exercise file
                exercise_file="../../ugandan_sign_language_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .ugandan_sign_language.1.3
              fi
              if ! [ -f ".s_ugandan_sign_language_1_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Life at home ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/Ugandan_sign_language/1.4.life_at_home.txt" . || exit 1
              mv 1.4.life_at_home.txt .1.4.life_at_home.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.4.life_at_home.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.4.life_at_home.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.4.life_at_home.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.4.life_at_home.txt
              process_reminders_from_file .1.4.life_at_home.txt
              STATE_FILE=".s_ugandan_sign_language_1_4"
              process_file .1.4.life_at_home.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.4.life_at_home.txt
              sed -i '/^1$/!d' .s_ugandan_sign_language_1_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Ugandan_sign_language/S1"
              # Define the file extension
              file_extension=".4.life_at_home.qns.txt"
              # Define the revision file
              revision_file="../../ugandan_sign_language_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Ugandan_sign_language/S1"
              # Define the file extension
              file_extension_question=".4.life_at_home.qns.txt"
              # Define the revision file
              revision_file="../../ugandan_sign_language_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            5)
              if ! [ -f ".ugandan_sign_language.1.4" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Ugandan_sign_language/S1"
                # Define the file extension
                file_extension_answer=".4.life_at_home.ans.txt"
                # Define the exercise file
                exercise_file="../../ugandan_sign_language_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .ugandan_sign_language.1.4
              fi
              if ! [ -f ".s_ugandan_sign_language_1_5" ]; then
                echo -e "\n\nHere you are dear one... Stay organised as you explore Plants in our area ...\n\n${g}Just know we are not going to leave you alone${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Ugandan_sign_language/1.5.plants_in_our_area.txt" . || exit 1
              mv 1.5.plants_in_our_area.txt .1.5.plants_in_our_area.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.5.plants_in_our_area.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.5.plants_in_our_area.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.5.plants_in_our_area.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.5.plants_in_our_area.txt
              process_reminders_from_file .1.5.plants_in_our_area.txt
              STATE_FILE=".s_ugandan_sign_language_1_5"
              process_file .1.5.plants_in_our_area.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.5.plants_in_our_area.txt
              sed -i '/^1$/!d' .s_ugandan_sign_language_1_5
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Ugandan_sign_language/S1"
              # Define the file extension
              file_extension_question=".5.plants_in_our_area.qns.txt"
              # Define the revision file
              revision_file="../../ugandan_sign_language_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            6)
              if ! [ -f ".ugandan_sign_language.1.5" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Ugandan_sign_language/S1"
                # Define the file extension
                file_extension_answer=".5.plants_in_our_area.ans.txt"
                # Define the exercise file
                exercise_file="../../ugandan_sign_language_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .ugandan_sign_language.1.5
              fi
              if ! [ -f ".s_ugandan_sign_language_1_6" ]; then
                echo -e "\n\nYou have managed to make it to Animals and birds ...\n\n${g}Remember to pray always${t}\n\nThe fear of the Lord is the beginning of wisdom \c" && wait_for_a_key_press
              fi
              cp "Notes/Ugandan_sign_language/1.6.animals_and_birds.txt" . || exit 1
              mv 1.6.animals_and_birds.txt .1.6.animals_and_birds.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.6.animals_and_birds.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.6.animals_and_birds.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.6.animals_and_birds.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.6.animals_and_birds.txt
              process_reminders_from_file .1.6.animals_and_birds.txt
              STATE_FILE=".s_ugandan_sign_language_1_6"
              process_file .1.6.animals_and_birds.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.6.animals_and_birds.txt
              sed -i '/^1$/!d' .s_ugandan_sign_language_1_6
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Ugandan_sign_language/S1"
              # Define the file extension
              file_extension_question=".6.animals_and_birds.qns.txt"
              # Define the revision file
              revision_file="../../ugandan_sign_language_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            7)
              if ! [ -f ".ugandan_sign_language.1.6" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Ugandan_sign_language/S1"
                # Define the file extension
                file_extension_answer=".6.animals_and_birds.ans.txt"
                # Define the exercise file
                exercise_file="../../ugandan_sign_language_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .ugandan_sign_language.1.6
              fi
              if ! [ -f ".s_ugandan_sign_language_1_7" ]; then
                echo -e "\n\nFrom here, you will be proceeding with Personal and community hygiene ...\n\n${g}Please never ever forget that your education is your future${t}\n\nFocus dear \c" && wait_for_a_key_press
              fi
              cp "Notes/Ugandan_sign_language/1.7.personal_and_community_hygiene.txt" . || exit 1
              mv 1.7.personal_and_community_hygiene.txt .1.7.personal_and_community_hygiene.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.7.personal_and_community_hygiene.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.7.personal_and_community_hygiene.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.7.personal_and_community_hygiene.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.7.personal_and_community_hygiene.txt
              process_reminders_from_file .1.7.personal_and_community_hygiene.txt
              STATE_FILE=".s_ugandan_sign_language_1_7"
              process_file .1.7.personal_and_community_hygiene.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.7.personal_and_community_hygiene.txt
              sed -i '/^1$/!d' .s_ugandan_sign_language_1_7
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Ugandan_sign_language/S1"
              # Define the file extension
              file_extension_question=".7.personal_and_community_hygiene.qns.txt"
              # Define the revision file
              revision_file="../../ugandan_sign_language_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".ugandan_sign_language.1.7" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/Ugandan_sign_language/S1"
	              # Define the file extension
	              file_extension_answer=".7.personal_and_community_hygiene.ans.txt"
	              # Define the exercise file
	              exercise_file="../../ugandan_sign_language_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
								touch .ugandan_sign_language.1.7
								echo "1" > .ugandan_sign_language_ready
							fi
            ;;
    ...
    if ! find . -maxdepth 1 -name '.s_ugandan_sign_language_2*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to the S2 Ugandan_sign_language class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
    ...
    do
      ...
      do
        ...
        then
          attempts=0
          # Define the targeted directory
          question_directory="Revision/Ugandan_sign_language/S2"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../ugandan_sign_language_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Ugandan_sign_language/S2"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../ugandan_sign_language_answered_ans.txt"
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_ugandan_sign_language_2_1" ]; then
                echo -e "\n\nYou chose to explore Establishing and managing relationships ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/Ugandan_sign_language/2.1.establishing_and_managing_relationships.txt" . || exit 1
              ...
              STATE_FILE=".s_ugandan_sign_language_2_1"
              ...
              sed -i '/^1$/!d' .s_ugandan_sign_language_2_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Ugandan_sign_language/S2"
              # Define the file extension
            ...
            ;;
            2)
              if ! [ -f ".ugandan_sign_language.2.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Ugandan_sign_language/S2"
                # Define the file extension
                ...
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .ugandan_sign_language.2.1
              fi
              if ! [ -f ".s_ugandan_sign_language_2_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into School environment ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Ugandan_sign_language/2.2.school_environment.txt" . || exit 1
              ...
              STATE_FILE=".s_ugandan_sign_language_2_2"
              ...
              sed -i '/^1$/!d' .s_ugandan_sign_language_2_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Ugandan_sign_language/S2"
              # Define the file extension
            ...
            ;;
            3)
              if ! [ -f ".ugandan_sign_language.2.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Ugandan_sign_language/S2"
                # Define the file extension
                ...
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .ugandan_sign_language.2.2
              fi
              if ! [ -f ".s_ugandan_sign_language_2_3" ]; then
                echo -e "\n\nYou have made a choice to cover Traditional ceremonies ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Ugandan_sign_language/2.3.traditional_ceremonies.txt" . || exit 1
              mv 2.3.traditional_ceremonies.txt .2.3.traditional_ceremonies.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.3.traditional_ceremonies.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.3.traditional_ceremonies.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.3.traditional_ceremonies.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.3.traditional_ceremonies.txt
              process_reminders_from_file .2.3.traditional_ceremonies.txt
              STATE_FILE=".s_ugandan_sign_language_2_3"
              process_file .2.3.traditional_ceremonies.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.3.traditional_ceremonies.txt
              sed -i '/^1$/!d' .s_ugandan_sign_language_2_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Ugandan_sign_language/S2"
              # Define the file extension
              file_extension_question=".3.traditional_ceremonies.qns.txt"
              # Define the revision file
              revision_file="../../ugandan_sign_language_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".ugandan_sign_language.2.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Ugandan_sign_language/S2"
                # Define the file extension
                file_extension_answer=".3.traditional_ceremonies.ans.txt"
                # Define the exercise file
                exercise_file="../../ugandan_sign_language_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .ugandan_sign_language.2.3
              fi
              if ! [ -f ".s_ugandan_sign_language_2_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Occupations and careers ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/Ugandan_sign_language/2.4.occupations_and_careers.txt" . || exit 1
              mv 2.4.occupations_and_careers.txt .2.4.occupations_and_careers.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.4.occupations_and_careers.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.4.occupations_and_careers.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.4.occupations_and_careers.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.4.occupations_and_careers.txt
              process_reminders_from_file .2.4.occupations_and_careers.txt
              STATE_FILE=".s_ugandan_sign_language_2_4"
              process_file .2.4.occupations_and_careers.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.4.occupations_and_careers.txt
              sed -i '/^1$/!d' .s_ugandan_sign_language_2_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Ugandan_sign_language/S2"
              # Define the file extension
              file_extension=".4.occupations_and_careers.qns.txt"
              # Define the revision file
              revision_file="../../ugandan_sign_language_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Ugandan_sign_language/S2"
              # Define the file extension
              file_extension_question=".4.occupations_and_careers.qns.txt"
              # Define the revision file
              revision_file="../../ugandan_sign_language_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            5)
              if ! [ -f ".ugandan_sign_language.2.4" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Ugandan_sign_language/S2"
                # Define the file extension
                file_extension_answer=".4.occupations_and_careers.ans.txt"
                # Define the exercise file
                exercise_file="../../ugandan_sign_language_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .ugandan_sign_language.2.4
              fi
              if ! [ -f ".s_ugandan_sign_language_2_5" ]; then
                echo -e "\n\nHere you are dear one... Stay organised as you explore Public places ...\n\n${g}Just know we are not going to leave you alone${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Ugandan_sign_language/2.5.public_places.txt" . || exit 1
              mv 2.5.public_places.txt .2.5.public_places.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.5.public_places.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.5.public_places.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.5.public_places.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.5.public_places.txt
              process_reminders_from_file .2.5.public_places.txt
              STATE_FILE=".s_ugandan_sign_language_2_5"
              process_file .2.5.public_places.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.5.public_places.txt
              sed -i '/^1$/!d' .s_ugandan_sign_language_2_5
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Ugandan_sign_language/S2"
              # Define the file extension
              file_extension_question=".5.public_places.qns.txt"
              # Define the revision file
              revision_file="../../ugandan_sign_language_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".ugandan_sign_language.2.5" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/Ugandan_sign_language/S2"
	              # Define the file extension
	              file_extension_answer=".5.public_places.ans.txt"
	              # Define the exercise file
	              exercise_file="../../ugandan_sign_language_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
								touch .ugandan_sign_language.2.5
								echo "2" > .ugandan_sign_language_ready
							fi
            ;;
    ...
    if ! find . -maxdepth 1 -name '.s_ugandan_sign_language_3*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to the S3 Ugandan_sign_language class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
    ...
    do
      ...
      do
        ...
        then
          attempts=0
          # Define the targeted directory
          question_directory="Revision/Ugandan_sign_language/S3"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../ugandan_sign_language_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Ugandan_sign_language/S3"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../ugandan_sign_language_answered_ans.txt"
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_ugandan_sign_language_3_1" ]; then
                echo -e "\n\nYou chose to explore Games, sports and leisure ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/Ugandan_sign_language/3.1.games_sports_and_leisure.txt" . || exit 1
              ...
              STATE_FILE=".s_ugandan_sign_language_3_1"
              ...
              sed -i '/^1$/!d' .s_ugandan_sign_language_3_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Ugandan_sign_language/S3"
              # Define the file extension
            ...
            ;;
            2)
              if ! [ -f ".ugandan_sign_language.3.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Ugandan_sign_language/S3"
                # Define the file extension
                ...
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .ugandan_sign_language.3.1
              fi
              if ! [ -f ".s_ugandan_sign_language_3_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Indigenous tourism ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Ugandan_sign_language/3.2.indigenous_tourism.txt" . || exit 1
              ...
              STATE_FILE=".s_ugandan_sign_language_3_2"
              ...
              sed -i '/^1$/!d' .s_ugandan_sign_language_3_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Ugandan_sign_language/S3"
              # Define the file extension
            ...
            ;;
            3)
              if ! [ -f ".ugandan_sign_language.3.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Ugandan_sign_language/S3"
                # Define the file extension
                ...
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .ugandan_sign_language.3.2
              fi
              if ! [ -f ".s_ugandan_sign_language_3_3" ]; then
                echo -e "\n\nYou have made a choice to cover Clans ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Ugandan_sign_language/3.3.clans.txt" . || exit 1
              ...
              STATE_FILE=".s_ugandan_sign_language_3_3"
              ...
              rm -f .3.3.clans.txt
              sed -i '/^1$/!d' .s_ugandan_sign_language_3_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Ugandan_sign_language/S3"
              # Define the file extension
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".ugandan_sign_language.3.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Ugandan_sign_language/S3"
                # Define the file extension
                ...
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .ugandan_sign_language.3.3
              fi
              if ! [ -f ".s_ugandan_sign_language_3_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Wealth creation ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/Ugandan_sign_language/3.4.wealth_creation.txt" . || exit 1
              ...
              STATE_FILE=".s_ugandan_sign_language_3_4"
              ...
              sed -i '/^1$/!d' .s_ugandan_sign_language_3_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Ugandan_sign_language/S3"
              # Define the file extension
            ...
            ;;
            5)
              if ! [ -f ".ugandan_sign_language.3.4" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Ugandan_sign_language/S3"
                # Define the file extension
                ...
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .ugandan_sign_language.3.4
              fi
              if ! [ -f ".s_ugandan_sign_language_3_5" ]; then
                echo -e "\n\nHere you are dear one... Stay organised as you explore Environmental awareness ...\n\n${g}Just know we are not going to leave you alone${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Ugandan_sign_language/3.5.environmental_awareness.txt" . || exit 1
              ...
              STATE_FILE=".s_ugandan_sign_language_3_5"
              ...
              sed -i '/^1$/!d' .s_ugandan_sign_language_3_5
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Ugandan_sign_language/S3"
              # Define the file extension
            ...
            ;;
            6)
              if ! [ -f ".ugandan_sign_language.3.5" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Ugandan_sign_language/S3"
                # Define the file extension
                ...
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .ugandan_sign_language.3.5
              fi
              if ! [ -f ".s_ugandan_sign_language_3_6" ]; then
                echo -e "\n\nYou have managed to make it to Water ...\n\n${g}Remember to pray always${t}\n\nThe fear of the Lord is the beginning of wisdom \c" && wait_for_a_key_press
              fi
              cp "Notes/Ugandan_sign_language/3.6.water.txt" . || exit 1
              ...
              STATE_FILE=".s_ugandan_sign_language_3_6"
              ...
              rm -f .3.6.water.txt
              sed -i '/^1$/!d' .s_ugandan_sign_language_3_6
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Ugandan_sign_language/S3"
              # Define the file extension
              ...
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".ugandan_sign_language.3.6" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/Ugandan_sign_language/S3"
	              # Define the file extension
	              ...
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
								touch .ugandan_sign_language.3.6
								echo "3" > .ugandan_sign_language_ready
							fi
            ;;
    ...
    if ! find . -maxdepth 1 -name '.s_ugandan_sign_language_4*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to the S4 Ugandan_sign_language class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
      clear_and_center "There are ${r}5${t} topics to be covered. Your tasks will always expand or shrink to fit in the time you give them. For that reason, never procrastinate darling!"
    fi
    attempts=0
    ...
    do
      ...
      do
        ...
        then
          attempts=0
          # Define the targeted directory
          question_directory="Revision/Ugandan_sign_language/S4"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../ugandan_sign_language_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Ugandan_sign_language/S4"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../ugandan_sign_language_answered_ans.txt"
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_ugandan_sign_language_4_1" ]; then
                echo -e "\n\nYou chose to explore Migration and settlement ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/Ugandan_sign_language/4.1.migration_and_settlement.txt" . || exit 1
              ...
              STATE_FILE=".s_ugandan_sign_language_4_1"
              ...
              sed -i '/^1$/!d' .s_ugandan_sign_language_4_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Ugandan_sign_language/S4"
              # Define the file extension
            ...
            ;;
            2)
              if ! [ -f ".ugandan_sign_language.4.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Ugandan_sign_language/S4"
                # Define the file extension
                ...
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .ugandan_sign_language.4.1
              fi
              if ! [ -f ".s_ugandan_sign_language_4_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Traditional ceremonies (initiation, marriage and funeral rites) ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Ugandan_sign_language/4.2.traditional_ceremonies_initiation_marriage_and_funeral_rites.txt" . || exit 1
              mv 4.2.traditional_ceremonies_initiation_marriage_and_funeral_rites.txt .4.2.traditional_ceremonies_initiation_marriage_and_funeral_rites.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.2.traditional_ceremonies_initiation_marriage_and_funeral_rites.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.2.traditional_ceremonies_initiation_marriage_and_funeral_rites.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.2.traditional_ceremonies_initiation_marriage_and_funeral_rites.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.2.traditional_ceremonies_initiation_marriage_and_funeral_rites.txt
              process_reminders_from_file .4.2.traditional_ceremonies_initiation_marriage_and_funeral_rites.txt
              STATE_FILE=".s_ugandan_sign_language_4_2"
              process_file .4.2.traditional_ceremonies_initiation_marriage_and_funeral_rites.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.2.traditional_ceremonies_initiation_marriage_and_funeral_rites.txt
              sed -i '/^1$/!d' .s_ugandan_sign_language_4_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Ugandan_sign_language/S4"
              # Define the file extension
              file_extension_question=".2.traditional_ceremonies_initiation_marriage_and_funeral_rites.qns.txt"
              # Define the revision file
              revision_file="../../ugandan_sign_language_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".ugandan_sign_language.4.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Ugandan_sign_language/S4"
                # Define the file extension
                file_extension_answer=".2.traditional_ceremonies_initiation_marriage_and_funeral_rites.ans.txt"
                # Define the exercise file
                exercise_file="../../ugandan_sign_language_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .ugandan_sign_language.4.2
              fi
              if ! [ -f ".s_ugandan_sign_language_4_3" ]; then
                echo -e "\n\nYou have made a choice to cover Cultural values, morals and ethics ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Ugandan_sign_language/4.3.cultural_values_morals_and_ethics.txt" . || exit 1
              ...
              STATE_FILE=".s_ugandan_sign_language_4_3"
              ...
              sed -i '/^1$/!d' .s_ugandan_sign_language_4_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Ugandan_sign_language/S4"
              # Define the file extension
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".ugandan_sign_language.4.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Ugandan_sign_language/S4"
                # Define the file extension
                ...
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .ugandan_sign_language.4.3
              fi
              if ! [ -f ".s_ugandan_sign_language_4_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Leadership and citizenship ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/Ugandan_sign_language/4.4.leadership_and_citizenship.txt" . || exit 1
              ...
              STATE_FILE=".s_ugandan_sign_language_4_4"
              ...
              sed -i '/^1$/!d' .s_ugandan_sign_language_4_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Ugandan_sign_language/S4"
              # Define the file extension
            ...
            ;;
            5)
              if ! [ -f ".ugandan_sign_language.4.4" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Ugandan_sign_language/S4"
                # Define the file extension
                ...
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .ugandan_sign_language.4.4
              fi
              if ! [ -f ".s_ugandan_sign_language_4_5" ]; then
                echo -e "\n\nHere you are dear one... Stay organised as you explore Human rights ...\n\n${g}Just know we are not going to leave you alone${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Ugandan_sign_language/4.5.human_rights.txt" . || exit 1
              ...
              STATE_FILE=".s_ugandan_sign_language_4_5"
              ...
              sed -i '/^1$/!d' .s_ugandan_sign_language_4_5
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Ugandan_sign_language/S4"
              # Define the file extension
              ...
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".ugandan_sign_language.4.5" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/Ugandan_sign_language/S4"
	              # Define the file extension
	              ...
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
								touch .ugandan_sign_language.4.5
								echo "4" > .ugandan_sign_language_ready
							fi
            ;;
