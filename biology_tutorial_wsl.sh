  ...
  if { [ -n "$last_class" ] || [ -z "$last_topic" ]; } && [ -f .biology_surveyor ]; then
      ...
      if [ "$class" == "x" ]; then
        exit
		  ...
		  # Check if .current_biology_class is accidentally empty
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    for sentence in "${sentences[@]}"; do
        ...
        if [[ $sentence == *"Table"* ]]; then
          cd Tables/Biology || { echo -e "\nFailed to change to Tables/Biology \c"; return; }
          explorer.exe "$sentence" > /dev/null 2>&1 &
          cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        fi

        if [[ $sentence == *"Video"* ]]; then
          cd Videos/Biology || { echo -e "\nFailed to change to Videos/Biology \c"; return; }
          explorer.exe "$sentence" > /dev/null 2>&1 &
          cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        fi
...
while true; do
  ...
  # Prompt the user for input
  if [ -z "$(find ./Notes/Biology -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
  ...
  if [[ -n "$user_input" ]]; then
     ...
     if [[ "$user_input" == "sr" ]]; then
      ...
      explorer.exe Biology > /dev/null 2>&1 &
    ...
    if [[ "$user_input" == "zz" ]]; then
      ...
      TEMP_FILE=$(mktemp) && curl -o "$TEMP_FILE" -L "https://github.com/Muhumuza7325/OMD/raw/main/biology_tutorial_wsl.sh" && mv "$TEMP_FILE" biology_tutorial_wsl.sh && chmod +x biology_tutorial_wsl.sh && cp biology_tutorial_wsl.sh Students/Omd && echo -e "\n\n${y}Code successfully updated.. You will have to restart a new session${t} \c" && sleep 4 && exit || (echo -e "\n\n${m}Error updating code!... Please check your internet connection and try again!${t} \c" && rm -f "$TEMP_FILE" && return)
    fi
    ...
    if [[ -n "$result" ]]; then
      ...
      if echo "$result" | grep "Figure"; then
        echo "$result" | grep -o '\bFigure[0-9]\+.*\.jpg\(\.[0-9]\+\)*\b' > Figures/Biology/figures.txt
        ...
        rm -f Figures/Biology/figures.txt
      fi

      if echo "$result" | grep "Table"; then
        echo "$result" | grep -o '\bTable[0-9]\+\(\.[0-9]\+\)*\b' > Tables/Biology/tables.txt
        sed -i -e '/^Table/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Tables/Biology/tables.txt
        cd Tables/Biology || { echo -e "\nFailed to change to Tables/Biology \c"; return; }
        ...
        # Go back to the original directory
        cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        rm -f Tables/Biology/tables.txt
      fi

      if echo "$result" | grep "Video"; then
        #echo "$result" > Videos/Biology/videos.txt
        echo "$result" | grep -o '\bVideo[0-9]\+\(\.[0-9]\+\)*\b' > Videos/Biology/videos.txt
        # Remove lines not starting with "Video" and any leading/trailing whitespaces
        sed -i -e '/^Video/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Videos/Biology/videos.txt
        # Change to the "Videos/Biology" directory
        cd Videos/Biology || { echo -e "\nFailed to change to Videos/Biology \c"; return; }
        ...
        cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        # Remove the temporary file
        rm -f Videos/Biology/videos.txt
      fi
  ...
  if [ -f "$selected_file" ]; then
    ...
    if [[ -n "$selected_question" && "$selected_question" =~ [[:graph:]] ]]; then
      ...
      if echo "$selected_question" | grep "Figure"; then
        ...
        # Go back to the original directory
        cd ../../Revision/Biology || { echo "Failed to change back to the targeted directory \c"; exit 1; }
        # Remove the temporary file
        rm -f ../../Figures/Biology/figures.txt
      fi
...
process_final_assignment() {
  # Check if user is.biology_ready for the assignment
  # Check if .current_biology_class is accidentally empty
  if [ ! -s .current_biology_class ]; then
    # Echo 1 to .current_biology_class
    echo "1" > .current_biology_class
  fi
  # Read the value from the .current_biology_class file
  current_biology_class=$(<.current_biology_class) 2>/dev/null
	...
	while true; do
		if [ ! -f .revise.txt ]; then
			...
			if [ $time_diff -gt 3600 ]; then
				...
				echo -e "\n\n\n${r}You are advised to not make any changes to the provided answers, instead, you can make copies that you can edit${t}\n\n${y}For a teacher willing to join us reach out to everyone of our children, please send us your questions and answers in a file labelled with your name, school, biology, and file content (e.g., Muhumuza_Omega_Kasule_High_School_O_level_Chemistry_Answered_EOC1_Items.pdf) to our contacts${t}\n\n\nEmail: ${g}2024omd256@gmail.com${t} \c"
				echo $current_time > "$last_echo_time_file"
			fi
			read -rp $'\n\n\nEnter '"${m}any character${t}"' for access to the file of answered items or simply press '"${r}Enter${t}"' to get items to attempt : ' input
			if [[ -n $input ]]; then
				explorer.exe .biology_samples* > /dev/null 2>&1 &
				clear
				popd > /dev/null || exit
				return
			fi
...
if ! [ -d "Notes/Biology" ] || ! [ -d "Revision" ] || ! [ -d "Exercise" ] || ! [ -d "Videos" ] || ! [ -d "Figures" ] || ! [ -d "Tables" ]; then
        ...
        then
          ...
          case "$topic" in
            ...
            2)
              ...
              rm -f .2.2.soil_erosion_and_conservation.txt
          ...
          case "$topic" in
            ...
            4)
              ...
              if ! [ -f ".s_biology_1_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of The five kingdoms of living organisms ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/Biology/1.4.the_five_kingdoms_of_living_organisms.txt" . || exit 1
              mv 1.4.the_five_kingdoms_of_living_organisms.txt .1.4.the_five_kingdoms_of_living_organisms.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.4.the_five_kingdoms_of_living_organisms.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.4.the_five_kingdoms_of_living_organisms.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.4.the_five_kingdoms_of_living_organisms.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.4.the_five_kingdoms_of_living_organisms.txt
              process_reminders_from_file .1.4.the_five_kingdoms_of_living_organisms.txt
              STATE_FILE=".s_biology_1_4"
              process_file .1.4.the_five_kingdoms_of_living_organisms.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.4.the_five_kingdoms_of_living_organisms.txt
              sed -i '/^1$/!d' .s_biology_1_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Biology/S1"
              # Define the file extension
              file_extension=".4.the_five_kingdoms_of_living_organisms.qns.txt"
              # Define the revision file
              revision_file="../../biology_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Biology/S1"
              # Define the file extension
              file_extension_question=".4.the_five_kingdoms_of_living_organisms.qns.txt"
              # Define the revision file
              revision_file="../../biology_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ...
            5)
              if ! [ -f ".biology.1.4" ]; then
                ...
                answered_directory="Exercise/Biology/S1"
                # Define the file extension
                file_extension_answer=".4.the_five_kingdoms_of_living_organisms.ans.txt"
                # Define the exercise file
                exercise_file="../../biology_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
        ...
        then
          ...
          case "$topic" in
            ...
            2)
              ...
              rm -f .3.2.aerobic_and_anaerobic_respiration.txt
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_biology_4_1" ]; then
                echo -e "\n\nYou chose to explore Asexual reproduction in plants - vegetative reproduction ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
            ...
            2)
              ...
              rm -f .4.2.sexual_reproduction_in_plants.txt
