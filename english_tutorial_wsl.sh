...
function handle_class_input() {
  if { [ -n "$last_class" ] || [ -z "$last_topic" ]; } && [ -f .English_surveyor ]; then
		while true; do
	    read -rp $'\n\nPlease enter '"${r}1 or 2 or 3 or 4${t}"' to go to your class or '"${r}x${t}"' to exit'$'\n\n> ' class
      if [ "$class" == "x" ]; then
        exit
      fi
		  # Check if .current_English_class is accidentally empty
		  if [ ! -s .current_English_class ]; then
		    # Echo 1 to .current_English_class
		    echo "1" > .current_English_class
		  fi
		  # Read the value from the file
		  current_English_class=$(<.current_English_class) 2>/dev/null
		  # Check if the value is equal to $class
		  if [ "$current_English_class" -lt "$class" 2>/dev/null ]; then
...
function handle_s3_topic_input() {
  ...
  if [ -z "$last_topic" ] || [ -f .english_topic_selected ]; then
    read -rp $'\n\nChoose either topic '"${g}1 or 2 or 3 or 4 or 5 or 6 or 7 or 8${t}"' to learn'$'\nor enter '"${r}z${t}"' for an adventure or '"${r}r${t}"' to revise or '"${r}s${t}"' to get sample_items'$'\nor '"${r}a${t}"' to get an activity of integration or '"${r}q${t}"' to get a short answer question'$'\nor '"${r}n${t}"' to do your final class assignment and if necessary, gain access to the next class'$'\nor '"${r}p${t}"' to track academic progress or '"${r}x${t}"' to exit'$'\n\n1. Childhood memories '"${r}Term1${t}"''$'\n\n2. School clubs '"${r}Term1${t}"''$'\n\n3. Integrity '"${r}Term2${t}"''$'\n\n4. Identity crisis '"${r}Term2${t}"''$'\n\n5. Relationships and emotions '"${r}Term2${t}"''$'\n\n6. Patriotism '"${r}Terms 2&3${t}"''$'\n\n7. Further education '"${r}Term3${t}"''$'\n\n8. Banking or money '"${r}Term3${t}"''$'\n\n> ' topic
    touch .english_surveyor
    touch .english_topic_selected
    # Update the state file with the topic
    # Check if the state file exists, and the topic is not "x"
...
function handle_s2_topic_input() {
  ...
  if [ -z "$last_topic" ] || [ -f .english_topic_selected ]; then
    read -rp $'\n\nChoose either topic '"${g}1 or 2 or 3 or 4 or 5 or 6 or 7 or 8${t}"' to learn'$'\nor enter '"${r}z${t}"' for an adventure or '"${r}r${t}"' to revise or '"${r}s${t}"' to get sample_items'$'\nor '"${r}a${t}"' to get an activity of integration or '"${r}q${t}"' to get a short answer question'$'\nor '"${r}n${t}"' to do your final class assignment and if necessary, gain access to the next class'$'\nor '"${r}p${t}"' to track academic progress or '"${r}x${t}"' to exit'$'\n\n1. Modern communication technology '"${r}Term1${t}"''$'\n\n2. Celebrations '"${r}Term1${t}"''$'\n\n3. Parents and children '"${r}Term1${t}"''$'\n\n4. Anti-corruption '"${r}Term2${t}"''$'\n\n5. Human rights, gender and responsibilities '"${r}Term2${t}"''$'\n\n6. Tourism maps and giving directions '"${r}Terms 2&3${t}"''$'\n\n7. Leisure '"${r}Term3${t}"''$'\n\n8. Appearance and grooming '"${r}Term3${t}"''$'\n\n> ' topic
    touch .english_surveyor
    touch .english_topic_selected
    # Update the state file with the topic
    # Check if the state file exists, and the topic is not "x"
...
function handle_s1_topic_input() {
  ...
  if [ -z "$last_topic" ] || [ -f .english_topic_selected ]; then
    read -rp $'\n\nChoose either topic '"${g}1 or 2 or 3 or 4 or 5 or 6 or 7 or 8 or 9${t}"' to learn'$'\nor enter '"${r}z${t}"' for an adventure or '"${r}r${t}"' to revise or '"${r}s${t}"' to get sample_items'$'\nor '"${r}a${t}"' to get an activity of integration or '"${r}q${t}"' to get a short answer question'$'\nor '"${r}n${t}"' to do your final class assignment and if necessary, gain access to the next class'$'\nor '"${r}p${t}"' to track academic progress or '"${r}x${t}"' to exit'$'\n\n1. Personal life and family '"${r}Term1${t}"''$'\n\n2. Finding information '"${r}Term1${t}"''$'\n\n3. Food '"${r}Term1${t}"''$'\n\n4. At the scoreet '"${r}Term2${t}"''$'\n\n5. Children at work '"${r}Term2${t}"''$'\n\n6. Environment and pollution '"${r}Term2${t}"''$'\n\n7. Urban and rural life '"${r}Term3${t}"''$'\n\n8. Travel '"${r}Term3${t}"''$'\n\n9. Experience of secondary school '"${r}Term3${t}"''$'\n\n> ' topic
    touch .english_surveyor
    touch .english_topic_selected
    # Update the state file with the topic
    # Check if the state file exists, and the topic is not "x"
    ...
    for sentence in "${sentences[@]}"; do
      if [[ $echo_next == true ]]; then
        ...
        for split_sentence in "${split_sentences[@]}"; do
					echo "$split_sentence" >> .English_reminder
        done
      echo_next=false
      fi
        ...
        echo "Did you know that $lower_cased_sentence" >> .English_reminder
        ...
        echo_next=true
  			...
  			echo "Do you recall that $lower_cased_sentence;" >> .English_reminder

			elif [[ "$sentence" =~ " → " ]]; then
  			echo "Hope you know that: $sentence;" >> .English_reminder

      elif [[ "$sentence" =~ " ↔ " ]]; then
        echo "Hope you know that: $sentence;" >> .English_reminder

      elif [[ "$sentence" =~ (Generally|general) ]]; then
        echo "Note: $sentence;" >> .English_reminder
...
process_random_reminder() {
  ...
  # Check if .English_reminder exists
  if [ -f .English_reminder ]; then
		# Remove empty lines from .English_reminder
  	sed -i '/^[[:space:]]*$/d' .English_reminder 2>/dev/null
    # Randomly select a non-empty sentence
    local reminder # Declare the variable
		reminder=$(awk -v RS=';' 'BEGIN{srand();}{gsub(/^[[:space:]]+|[[:space:]]+$/, ""); if (length > 0) a[++n]=$0}END{if (n > 0) print a[int(rand()*n)+1]}' .English_reminder)
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    for sentence in "${sentences[@]}"; do
        ...
        if [[ $sentence == *"Figure"* ]]; then
          ...
          # Change to the "Figures/English" directory
          cd Figures/English || { echo "Failed to change to Figures/English"; return; }
        ...
        if [[ $sentence == *"Table"* ]]; then
          cd Tables/English || { echo -e "\nFailed to change to Tables/English \c"; return; }
          explorer.exe "$sentence" > /dev/null 2>&1 &
          cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        fi

        if [[ $sentence == *"Video"* ]]; then
          cd Videos/English || { echo -e "\nFailed to change to Videos/English \c"; return; }
          explorer.exe "$sentence" > /dev/null 2>&1 &
          cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        fi
...
contact_ai() {
  last_topic=$(awk -F' ' '{print $2}' .English_user_state)
...
while true; do
  ...
  if [[ -n "$user_input" ]]; then
     ...
     if [[ "$user_input" == "sr" ]]; then
      ...
      explorer.exe English > /dev/null 2>&1 &
    ...
    if [[ "$user_input" == "zz" ]]; then
      ...
      TEMP_FILE=$(mktemp) && curl -o "$TEMP_FILE" -L "https://github.com/Muhumuza7325/OMD/raw/main/English_tutorial_wsl.sh" && mv "$TEMP_FILE" English_tutorial_wsl.sh && chmod +x English_tutorial_wsl.sh && cp english_tutorial_wsl.sh Students/Omd && echo -e "\n\n${y}Code successfully updated.. You will have to restart a new session${t} \c" && sleep 4 && exit || (echo -e "\n\n${m}Error updating code!... Please check your internet connection and try again!${t} \c" && rm -f "$TEMP_FILE" && return)
    fi
    if [[ "$user_input" == "xx" ]]; then
      ...
      if [[ "$input" == "y" || "$input" == "Y" ]]; then
        ...
        curl -O -L "https://github.com/Muhumuza7325/OMD/raw/main/update_English.sh" || { echo -e "\n\n${m}Check your internet connection and try again!${t}" >&2; return; }
        mv update_English.sh .update_English.sh
        bash .update_English.sh
        return
      else
        return
      fi
    ...
    if [[ -n "$result" ]]; then
      ...
      if echo "$result" | grep "Figure"; then
        echo "$result" | grep -o '\bFigure[0-9]\+.*\.jpg\(\.[0-9]\+\)*\b' > Figures/English/figures.txt
        sed -i -e '/^Figure/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Figures/English/figures.txt
        # Change to the "Figures/English" directory
        cd Figures/English || { echo "Failed to change to Figures/English"; return; }
        ...
        rm -f Figures/English/figures.txt
      fi

      if echo "$result" | grep "Table"; then
        echo "$result" | grep -o '\bTable[0-9]\+\(\.[0-9]\+\)*\b' > Tables/English/tables.txt
        sed -i -e '/^Table/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Tables/English/tables.txt
        cd Tables/English || { echo -e "\nFailed to change to Tables/English \c"; return; }
        ...
        # Go back to the original directory
        cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        rm -f Tables/English/tables.txt
      fi

      if echo "$result" | grep "Video"; then
        #echo "$result" > Videos/English/videos.txt
        echo "$result" | grep -o '\bVideo[0-9]\+\(\.[0-9]\+\)*\b' > Videos/English/videos.txt
        # Remove lines not starting with "Video" and any leading/trailing whitespaces
        sed -i -e '/^Video/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Videos/English/videos.txt
        # Change to the "Videos/English" directory
        cd Videos/English || { echo -e "\nFailed to change to Videos/English \c"; return; }
        ...
        cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        # Remove the temporary file
        rm -f Videos/English/videos.txt
      fi
  ...
  if [ -f "$selected_file" ]; then
    ...
    if [[ -n "$selected_question" && "$selected_question" =~ [[:graph:]] ]]; then
      ...
      if echo "$selected_question" | grep "Figure"; then
        echo "$selected_question" | grep -o '\bFigure[0-9]\+[^;]*\b' > ../../Figures/English/figures.txt
        sed -i -e '/^Figure/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' ../../Figures/English/figures.txt
        # Change to the "Figures/English" directory
        cd ../../Figures/English || { echo "Failed to change to Figures/English"; return; }
        # Specify the path to the text file containing figure names
        ...
        # Go back to the original directory
        cd ../../Revision/English || { echo "Failed to change back to the targeted directory \c"; exit 1; }
        # Remove the temporary file
        rm -f ../../Figures/English/figures.txt
      fi
...
process_question_answer() {
  ...
  if [ -f ../../../.school_name ]; then
    ...
    sed -i '/^[[:space:]]*$/d' ../../../."$school_name"_students_file.txt
    existing_class=$(awk '{print $1}' ../../../.English_user_state)
    existing_topic=$(awk '{print $2}' ../../../.English_user_state)
    echo ''
    ...
    if grep -q "English" ../../../."$school_name"_students_file.txt; then
      sed -i -E 's/;/\n/g' ../../../."$school_name"_students_file.txt
      # Find the line with the word English, replace the information below it with the already available information adding a comma existing_class_existing_topic [$percentage]
      if [ "$existing_topic" == "r" ]; then
        sed -i '/English/{n;s/\(.*\)/\1, '"$existing_class"'_'"$input"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
      else
        (( existing_topic-- ))
        sed -i '/English/{n;s/\(.*\)/\1, '"$existing_class"'_'"$existing_topic"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
      fi
    else
      if [ "$existing_topic" == "r" ]; then
        sed -i -E '/^School/ i\English\n'"${existing_class} ${names}[${percentage}%]"'' ../../../."$school_name"_students_file.txt
...
function resume_from_last_point() {
  if [ -f .English_user_state ]; then
    last_class=$(awk -F' ' '{print $1}' .English_user_state)
    last_topic=$(awk -F' ' '{print $2}' .English_user_state)
    if [ -n "$last_class" ] && [ -n "$last_topic" ]; then
      echo -e "\n       Resuming from ${r}S$last_class${t} : ${g}Topic '$last_topic'${t} \c"
      rm -f .English_surveyor
...
process_final_assignment() {
  # Check if user is.English_ready for the assignment
  # Check if .current_English_class is accidentally empty
  if [ ! -s .current_English_class ]; then
    # Echo 1 to .current_English_class
    echo "1" > .current_English_class
  fi
  # Read the value from the .current_English_class file
  current_English_class=$(<.current_English_class) 2>/dev/null
  # Check if the value in the .English_ready file is equal to $class
  # Read the value from the .English_ready file
  if [ ! -s .English_ready ]; then
    echo "0" > .English_ready
  fi
  how.English_ready=$(<.English_ready) 2>/dev/null
  # Check if the value is equal to $class
  if [ "$how.English_ready" -lt "$current_English_class" 2>/dev/null ]; then
    read -rp $'\n\nYou havent done all the topic assignments for your current English class\n\n'"${r}Proceeding from here will affect your very final score${y}"'. To go back and progress right, enter '"${y}ok${t}"'. Otherwise, press the Enter key to do the final class assignment: ' progress
    if [ "$progress" == "ok" ]; then
      return
    fi
  fi
    ...
    if ! [ "$total_questions" == 100 ]; then
      ...
      if [ "$elapsed_time" -gt 3600 ]; then
        ...
        if [ "$percentage" -lt 95 ]; then
          ...
          cd - > /dev/null 2>&1 || exit
          if ! [ -f .current_English_class ]; then
            # Echo the result to the file .current_class
            echo "2" > .current_English_class
            ...
            # Read the value from the file
            current_English_class=$(<.current_English_class) 2>/dev/null
            # Check if the new_class value is lt $current_English_class
            if [ "$new_class" -gt "$current_English_class" 2>/dev/null ]; then
		          # Echo the result to the file .current_class
		          echo "$new_class" > .current_English_class
            fi
...
get_sample_items() {
	...
	# Save the current working directory
	pushd . > /dev/null
	cd Revision/English || { echo "Directory not found"; return; }
	...
	while true; do
		if [ ! -f .revise.txt ]; then
			...
			if [ $time_diff -gt 3600 ]; then
				...
				echo -e "\n\n\n${r}You are advised to not make any changes to the provided answers, instead, you can make copies that you can edit${t}\n\n${y}For a teacher willing to join us reach out to everyone of our children, please send us your questions and answers in a file labelled with your name, school, english, and file content (e.g., Muhumuza_Omega_Kasule_High_School_O_level_Chemistry_Answered_EOC1_Items.pdf) to our contacts${t}\n\n\nEmail: ${g}2024omd256@gmail.com${t} \c"
				echo $current_time > "$last_echo_time_file"
			fi
			read -rp $'\n\n\nEnter '"${m}any character${t}"' for access to the file of answered items or simply press '"${r}Enter${t}"' to get items to attempt : ' input
			if [[ -n $input ]]; then
				explorer.exe .english_samples* > /dev/null 2>&1 &
				clear
				popd > /dev/null || exit
				return
			fi
		...
		rm -f .revise.txt .english_topic_selected 2>/dev/null
	done
	popd > /dev/null || exit
	return
}
...
while true; do
  ...
  if ls Students | grep -wq "$initials"; then
    ...
    echo -e "                  $initials\n" > "Students/$initials/Exercise/English_answered_ans.txt"
    echo -e "                  $initials\n" > "Students/$initials/Revision/English_covered_qns.txt"
    ...
    if [ -f ../../../.school_name ]; then
      ...
      if grep -q "English" ../../../."$school_name"_students_file.txt; then
        ...
        if [ "$existing_topic" == "z" ]; then
          sed -i '/English/{n;s/\(.*\)/\1, '"$existing_class"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
        else
          (( existing_topic-- ))
          sed -i '/English/{n;s/\(.*\)/\1, '"$existing_class"'_'"$existing_topic"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
        fi
...
if [ ! -f .English_user_state ]; then
  ...
  if [ -f .omd_communication.txt ]; then
    ...
    process_random_reminder .English_reminder
  fi
	handle_resume_input
fi

if [ -z "$class" ] && [ -s ".English_user_state" ]; then
  get_and_display_pattern
fi
...
if ! [ -d "Notes/English" ] || ! [ -d "Revision" ] || ! [ -d "Exercise" ] || ! [ -d "Videos" ] || ! [ -d "Figures" ] || ! [ -d "Tables" ]; then
  ...
  mkdir -p Notes Notes/English Revision Revision/English Revision/English/{S1,S2,S3,S4} Exercise Exercise/English Exercise/English/{S1,S2,S3,S4} Videos Videos/English Figures Figures/English Tables Tables/English
  touch Revision/English_covered_qns.txt Exercise/English_answered_ans.txt
  echo -e "\n"
  pwd
...
if [ -z "$(find ./Notes/English -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
  ...
  if [[ -z "$user_input" ]]; then
    ...
    curl -O -L https://github.com/Muhumuza7325/OMD/raw/main/ENG/1.1.personal_life_and_family.txt || echo -e "\n\nError fetching material for this tutorial \c"
    ...
    cp 1.1.personal_life_and_family.txt Notes/English || echo -e "\n\nError copying 1.1.personal_life_and_family.txt to the English directory in the Notes directory \c"
    wait_for_a_key_press
  else
  ...
  rm -f 1.1.personal_life_and_family.txt
fi
...
while true; do
  ...
  handle_class_input
  if [[ "$class" == "1" ]]; then
    if ! find . -maxdepth 1 -name '.s_english_1*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to S1 English class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
    ...
    do
      ...
      do
        ...
        then
          ...
          question_directory="Revision/English/S1"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../english_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/English/S1"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../english_answered_ans.txt"
          # Call the function to process a random question
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_english_1_1" ]; then
                echo -e "\n\nYou chose to explore Personal life and family ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/English/1.1.personal_life_and_family.txt" . || exit 1
              mv 1.1.personal_life_and_family.txt .1.1.personal_life_and_family.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.1.personal_life_and_family.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.1.personal_life_and_family.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.1.personal_life_and_family.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.1.personal_life_and_family.txt
              process_reminders_from_file .1.1.personal_life_and_family.txt
              STATE_FILE=".s_english_1_1"
              process_file .1.1.personal_life_and_family.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.1.personal_life_and_family.txt
              sed -i '/^1$/!d' .s_english_1_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S1"
              # Define the file extension
              file_extension=".1.personal_life_and_family.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              ...
              question_directory="Revision/English/S1"
              # Define the file extension
              file_extension_question=".1.personal_life_and_family.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".english.1.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/English/S1"
                # Define the file extension
                file_extension_answer=".1.personal_life_and_family.ans.txt"
                # Define the exercise file
                exercise_file="../../english_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_english_1_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Finding information ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/English/1.2.finding_information.txt" . || exit 1
              mv 1.2.finding_information.txt .1.2.finding_information.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.2.finding_information.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.2.finding_information.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.2.finding_information.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.2.finding_information.txt
              process_reminders_from_file .1.2.finding_information.txt
              STATE_FILE=".s_english_1_2"
              process_file .1.2.finding_information.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.2.finding_information.txt
              sed -i '/^1$/!d' .s_english_1_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S1"
              # Define the file extension
              file_extension_question=".2.finding_information.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".english.1.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/English/S1"
                # Define the file extension
                file_extension_answer=".2.finding_information.ans.txt"
                # Define the exercise file
                exercise_file="../../english_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_english_1_3" ]; then
                echo -e "\n\nYou have made a choice to cover Food ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/English/1.3.food.txt" . || exit 1
              mv 1.3.food.txt .1.3.food.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.3.food.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.3.food.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.3.food.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.3.food.txt
              process_reminders_from_file .1.3.food.txt
              STATE_FILE=".s_english_1_3"
              process_file .1.3.food.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.3.food.txt
              sed -i '/^1$/!d' .s_english_1_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S1"
              # Define the file extension
              file_extension_question=".3.food.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".english.1.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/English/S1"
                # Define the file extension
                file_extension_answer=".3.food.ans.txt"
                # Define the exercise file
                exercise_file="../../english_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_english_1_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of At the scoreet ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/English/1.4.at_the_market.txt" . || exit 1
              mv 1.4.at_the_market.txt .1.4.at_the_market.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.4.at_the_market.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.4.at_the_market.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.4.at_the_market.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.4.at_the_market.txt
              process_reminders_from_file .1.4.at_the_market.txt
              STATE_FILE=".s_english_1_4"
              process_file .1.4.at_the_market.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.4.at_the_market.txt
              sed -i '/^1$/!d' .s_english_1_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S1"
              # Define the file extension
              file_extension=".4.at_the_market.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/English/S1"
              # Define the file extension
              file_extension_question=".4.at_the_market.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            5)
              if ! [ -f ".english.1.4" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/English/S1"
                # Define the file extension
                file_extension_answer=".4.at_the_market.ans.txt"
                # Define the exercise file
                exercise_file="../../english_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_english_1_5" ]; then
                echo -e "\n\nHere you are dear one... Stay organised as you explore Children at work ...\n\n${g}Just know we are not going to leave you alone${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/English/1.5.children_at_work.txt" . || exit 1
              mv 1.5.children_at_work.txt .1.5.children_at_work.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.5.children_at_work.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.5.children_at_work.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.5.children_at_work.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.5.children_at_work.txt
              process_reminders_from_file .1.5.children_at_work.txt
              STATE_FILE=".s_english_1_5"
              process_file .1.5.children_at_work.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.5.children_at_work.txt
              sed -i '/^1$/!d' .s_english_1_5
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S1"
              # Define the file extension
              file_extension_question=".5.children_at_work.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            6)
              if ! [ -f ".english.1.5" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/English/S1"
                # Define the file extension
                file_extension_answer=".5.children_at_work.ans.txt"
                # Define the exercise file
                exercise_file="../../english_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_english_1_6" ]; then
                echo -e "\n\nYou have managed to make it to Environment and pollution ...\n\n${g}Remember to pray always${t}\n\nThe fear of the Lord is the beginning of wisdom \c" && wait_for_a_key_press
              fi
              cp "Notes/English/1.6.environment_and_pollution.txt" . || exit 1
              mv 1.6.environment_and_pollution.txt .1.6.environment_and_pollution.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.6.environment_and_pollution.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.6.environment_and_pollution.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.6.environment_and_pollution.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.6.environment_and_pollution.txt
              process_reminders_from_file .1.6.environment_and_pollution.txt
              STATE_FILE=".s_english_1_6"
              process_file .1.6.environment_and_pollution.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.6.environment_and_pollution.txt
              sed -i '/^1$/!d' .s_english_1_6
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S1"
              # Define the file extension
              file_extension_question=".6.environment_and_pollution.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            7)
              if ! [ -f ".english.1.6" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/English/S1"
                # Define the file extension
                file_extension_answer=".6.environment_and_pollution.ans.txt"
                # Define the exercise file
                exercise_file="../../english_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_english_1_7" ]; then
                echo -e "\n\nFrom here, you will be proceeding with Urban and rural life ...\n\n${g}Please never ever forget that your education is your future${t}\n\nFocus dear \c" && wait_for_a_key_press
              fi
              cp "Notes/English/1.7.urban_and_rural_life.txt" . || exit 1
              mv 1.7.urban_and_rural_life.txt .1.7.urban_and_rural_life.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.7.urban_and_rural_life.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.7.urban_and_rural_life.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.7.urban_and_rural_life.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.7.urban_and_rural_life.txt
              process_reminders_from_file .1.7.urban_and_rural_life.txt
              STATE_FILE=".s_english_1_7"
              process_file .1.7.urban_and_rural_life.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.7.urban_and_rural_life.txt
              sed -i '/^1$/!d' .s_english_1_7
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S1"
              # Define the file extension
              file_extension_question=".7.urban_and_rural_life.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            8)
              if ! [ -f ".english.1.7" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/English/S1"
                # Define the file extension
                file_extension_answer=".7.urban_and_rural_life.ans.txt"
                # Define the exercise file
                exercise_file="../../english_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_english_1_8" ]; then
                echo -e "\n\nYou are to cover Travel ...\n\n${g}Please never ever settle for less${t}\n\nPromise yourself that you wont give up \c" && wait_for_a_key_press
              fi
              cp "Notes/English/1.8.travel.txt" . || exit 1
              mv 1.8.travel.txt .1.8.travel.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.8.travel.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.8.travel.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.8.travel.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.8.travel.txt
              process_reminders_from_file .1.8.travel.txt
              STATE_FILE=".s_english_1_8"
              process_file .1.8.travel.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.8.travel.txt
              sed -i '/^1$/!d' .s_english_1_8
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S1"
              # Define the file extension
              file_extension_question=".8.travel.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            9)
              if ! [ -f ".english.1.8" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/English/S1"
                # Define the file extension
                file_extension_answer=".8.travel.ans.txt"
                # Define the exercise file
                exercise_file="../../english_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_english_1_9" ]; then
                echo -e "\n\nI am so happy for you dear one. You are here to cover the the 9th topic (Experience of secondary school )...\n\n${g}Just never underestimate the value of a single second${t}\n\nThat extra one second maybe all you need to fully understand a given concept \c" && wait_for_a_key_press
              fi
              cp "Notes/English/1.9.experience_of_secondary_school.txt" . || exit 1
              mv 1.9.experience_of_secondary_school.txt .1.9.experience_of_secondary_school.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.9.experience_of_secondary_school.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.9.experience_of_secondary_school.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.9.experience_of_secondary_school.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.9.experience_of_secondary_school.txt
              process_reminders_from_file .1.9.experience_of_secondary_school.txt
              STATE_FILE=".s_english_1_9"
              process_file .1.9.experience_of_secondary_school.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.9.experience_of_secondary_school.txt
              sed -i '/^1$/!d' .s_english_1_9
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S1"
              # Define the file extension
              file_extension_question=".9.experience_of_secondary_school.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".english.1.9" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/English/S1"
	              # Define the file extension
	              file_extension_answer=".9.experience_of_secondary_school.ans.txt"
	              # Define the exercise file
	              exercise_file="../../english_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
								touch .english.1.9
								echo "1" > .english_ready
							fi
            ;;
    ...
    if ! find . -maxdepth 1 -name '.s_english_2*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to S2 English class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
    ...
    do
      ...
      do
        ...
        then
          attempts=0
          # Define the targeted directory
          question_directory="Revision/English/S2"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../english_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/English/S2"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../english_answered_ans.txt"
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_english_2_1" ]; then
                echo -e "\n\nYou chose to explore Modern communication technology ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/English/2.1.modern_communication_technology.txt" . || exit 1
              mv 2.1.modern_communication_technology.txt .2.1.modern_communication_technology.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.1.modern_communication_technology.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.1.modern_communication_technology.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.1.modern_communication_technology.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.1.modern_communication_technology.txt
              process_reminders_from_file .2.1.modern_communication_technology.txt
              STATE_FILE=".s_english_2_1"
              process_file .2.1.modern_communication_technology.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.1.modern_communication_technology.txt
              sed -i '/^1$/!d' .s_english_2_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S2"
              # Define the file extension
              file_extension=".1.modern_communication_technology.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/English/S2"
              # Define the file extension
              file_extension_question=".1.modern_communication_technology.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".english.2.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/English/S2"
                # Define the file extension
                file_extension_answer=".1.modern_communication_technology.ans.txt"
                # Define the exercise file
                exercise_file="../../english_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_english_2_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Celebrations ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/English/2.2.celebrations.txt" . || exit 1
              mv 2.2.celebrations.txt .2.2.celebrations.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.2.celebrations.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.2.celebrations.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.2.celebrations.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.2.celebrations.txt
              process_reminders_from_file .2.2.celebrations.txt
              STATE_FILE=".s_english_2_2"
              process_file .2.2.celebrations.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.2.celebrations.txt
              sed -i '/^1$/!d' .s_english_2_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S2"
              # Define the file extension
              file_extension_question=".2.celebrations.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".english.2.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/English/S2"
                # Define the file extension
                file_extension_answer=".2.celebrations.ans.txt"
                # Define the exercise file
                exercise_file="../../english_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_english_2_3" ]; then
                echo -e "\n\nYou have made a choice to cover Parents and children ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/English/2.3.parents_and_children.txt" . || exit 1
              mv 2.3.parents_and_children.txt .2.3.parents_and_children.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.3.parents_and_children.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.3.parents_and_children.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.3.parents_and_children.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.3.parents_and_children.txt
              process_reminders_from_file .2.3.parents_and_children.txt
              STATE_FILE=".s_english_2_3"
              process_file .2.3.parents_and_children.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.3.parents_and_children.txt
              sed -i '/^1$/!d' .s_english_2_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S2"
              # Define the file extension
              file_extension_question=".3.parents_and_children.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".english.2.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/English/S2"
                # Define the file extension
                file_extension_answer=".3.parents_and_children.ans.txt"
                # Define the exercise file
                exercise_file="../../english_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_english_2_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Anti-corruption ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/English/2.4.anti-corruption.txt" . || exit 1
              mv 2.4.anti-corruption.txt .2.4.anti-corruption.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.4.anti-corruption.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.4.anti-corruption.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.4.anti-corruption.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.4.anti-corruption.txt
              process_reminders_from_file .2.4.anti-corruption.txt
              STATE_FILE=".s_english_2_4"
              process_file .2.4.anti-corruption.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.4.anti-corruption.txt
              sed -i '/^1$/!d' .s_english_2_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S2"
              # Define the file extension
              file_extension=".4.anti-corruption.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/English/S2"
              # Define the file extension
              file_extension_question=".4.anti-corruption.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            5)
              if ! [ -f ".english.2.4" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/English/S2"
                # Define the file extension
                file_extension_answer=".4.anti-corruption.ans.txt"
                # Define the exercise file
                exercise_file="../../english_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_english_2_5" ]; then
                echo -e "\n\nHere you are dear one... Stay organised as you explore Human, rights, gender and responsibilities ...\n\n${g}Just know we are not going to leave you alone${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/English/2.5.human_rights_gender_and_responsibilities.txt" . || exit 1
              mv 2.5.human_rights_gender_and_responsibilities.txt .2.5.human_rights_gender_and_responsibilities.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.5.human_rights_gender_and_responsibilities.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.5.human_rights_gender_and_responsibilities.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.5.human_rights_gender_and_responsibilities.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.5.human_rights_gender_and_responsibilities.txt
              process_reminders_from_file .2.5.human_rights_gender_and_responsibilities.txt
              STATE_FILE=".s_english_2_5"
              process_file .2.5.human_rights_gender_and_responsibilities.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.5.human_rights_gender_and_responsibilities.txt
              sed -i '/^1$/!d' .s_english_2_5
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S2"
              # Define the file extension
              file_extension_question=".5.human_rights_gender_and_responsibilities.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            6)
              if ! [ -f ".english.2.5" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/English/S2"
                # Define the file extension
                file_extension_answer=".5.human_rights_gender_and_responsibilities.ans.txt"
                # Define the exercise file
                exercise_file="../../english_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_english_2_6" ]; then
                echo -e "\n\nYou have managed to make it to Tourism maps and giving directions ...\n\n${g}Remember to pray always${t}\n\nThe fear of the Lord is the beginning of wisdom \c" && wait_for_a_key_press
              fi
              cp "Notes/English/2.6.tourism_maps_and_giving_directions.txt" . || exit 1
              mv 2.6.tourism_maps_and_giving_directions.txt .2.6.tourism_maps_and_giving_directions.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.6.tourism_maps_and_giving_directions.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.6.tourism_maps_and_giving_directions.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.6.tourism_maps_and_giving_directions.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.6.tourism_maps_and_giving_directions.txt
              process_reminders_from_file .2.6.tourism_maps_and_giving_directions.txt
              STATE_FILE=".s_english_2_6"
              process_file .2.6.tourism_maps_and_giving_directions.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.6.tourism_maps_and_giving_directions.txt
              sed -i '/^1$/!d' .s_english_2_6
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S2"
              # Define the file extension
              file_extension_question=".6.tourism_maps_and_giving_directions.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            7)
              if ! [ -f ".english.2.6" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/English/S2"
                # Define the file extension
                file_extension_answer=".6.tourism_maps_and_giving_directions.ans.txt"
                # Define the exercise file
                exercise_file="../../english_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_english_2_7" ]; then
                echo -e "\n\nFrom here, you will be proceeding with Leisure ...\n\n${g}Please never ever forget that your education is your future${t}\n\nFocus dear \c" && wait_for_a_key_press
              fi
              cp "Notes/English/2.7.leisure.txt" . || exit 1
              mv 2.7.leisure.txt .2.7.leisure.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.7.leisure.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.7.leisure.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.7.leisure.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.7.leisure.txt
              process_reminders_from_file .2.7.leisure.txt
              STATE_FILE=".s_english_2_7"
              process_file .2.7.leisure.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.7.leisure.txt
              sed -i '/^1$/!d' .s_english_2_7
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S2"
              # Define the file extension
              file_extension_question=".7.leisure.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            8)
              if ! [ -f ".english.2.7" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/English/S2"
                # Define the file extension
                file_extension_answer=".7.leisure.ans.txt"
                # Define the exercise file
                exercise_file="../../english_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_english_2_8" ]; then
                echo -e "\n\nYou are to cover Appearance and grooming ...\n\n${g}Please never ever settle for less${t}\n\nPromise yourself that you wont give up \c" && wait_for_a_key_press
              fi
              cp "Notes/English/2.8.appearance_and_grooming.txt" . || exit 1
              mv 2.8.appearance_and_grooming.txt .2.8.appearance_and_grooming.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.8.appearance_and_grooming.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.8.appearance_and_grooming.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.8.appearance_and_grooming.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.8.appearance_and_grooming.txt
              process_reminders_from_file .2.8.appearance_and_grooming.txt
              STATE_FILE=".s_english_2_8"
              process_file .2.8.appearance_and_grooming.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.8.appearance_and_grooming.txt
              sed -i '/^1$/!d' .s_english_2_8
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S2"
              # Define the file extension
              file_extension_question=".8.appearance_and_grooming.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".english.2.8" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/English/S2"
	              # Define the file extension
	              file_extension_answer=".8.appearance_and_grooming.ans.txt"
	              # Define the exercise file
	              exercise_file="../../english_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
								touch .english.2.8
								echo "2" > .english_ready
							fi
            ;;
    ...
    if ! find . -maxdepth 1 -name '.s_english_3*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to S3 English class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
      clear_and_center "There are ${r}8${t} topics to be covered. Your tasks will always expand or shrink to fit in the time you give them. For that reason, never procrastinate darling!"
    fi
    attempts=0
    ...
    do
      ...
      do
        ...
        then
          attempts=0
          # Define the targeted directory
          question_directory="Revision/English/S3"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../english_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/English/S3"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../english_answered_ans.txt"
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_english_3_1" ]; then
                echo -e "\n\nYou chose to explore Childhood memories ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/English/3.1.childhood_memories.txt" . || exit 1
              mv 3.1.childhood_memories.txt .3.1.childhood_memories.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.1.childhood_memories.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.1.childhood_memories.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.1.childhood_memories.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.1.childhood_memories.txt
              process_reminders_from_file .3.1.childhood_memories.txt
              STATE_FILE=".s_english_3_1"
              process_file .3.1.childhood_memories.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.1.childhood_memories.txt
              sed -i '/^1$/!d' .s_english_3_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S3"
              # Define the file extension
              file_extension=".1.childhood_memories.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/English/S3"
              # Define the file extension
              file_extension_question=".1.childhood_memories.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".english.3.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/English/S3"
                # Define the file extension
                file_extension_answer=".1.childhood_memories.ans.txt"
                # Define the exercise file
                exercise_file="../../english_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_english_3_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into School clubs ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/English/3.2.school_clubs.txt" . || exit 1
              mv 3.2.school_clubs.txt .3.2.school_clubs.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.2.school_clubs.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.2.school_clubs.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.2.school_clubs.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.2.school_clubs.txt
              process_reminders_from_file .3.2.school_clubs.txt
              STATE_FILE=".s_english_3_2"
              process_file .3.2.school_clubs.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.2.school_clubs.txt
              sed -i '/^1$/!d' .s_english_3_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S3"
              # Define the file extension
              file_extension_question=".2.school_clubs.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".english.3.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/English/S3"
                # Define the file extension
                file_extension_answer=".2.school_clubs.ans.txt"
                # Define the exercise file
                exercise_file="../../english_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_english_3_3" ]; then
                echo -e "\n\nYou have made a choice to cover Integrity ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/English/3.3.integrity.txt" . || exit 1
              mv 3.3.integrity.txt .3.3.integrity.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.3.integrity.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.3.integrity.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.3.integrity.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.3.integrity.txt
              process_reminders_from_file .3.3.integrity.txt
              STATE_FILE=".s_english_3_3"
              process_file .3.3.integrity.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.3.integrity.txt
              sed -i '/^1$/!d' .s_english_3_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S3"
              # Define the file extension
              file_extension_question=".3.integrity.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".english.3.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/English/S3"
                # Define the file extension
                file_extension_answer=".3.integrity.ans.txt"
                # Define the exercise file
                exercise_file="../../english_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_english_3_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Identity crisis ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/English/3.4.identity_crisis.txt" . || exit 1
              mv 3.4.identity_crisis.txt .3.4.identity_crisis.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.4.identity_crisis.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.4.identity_crisis.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.4.identity_crisis.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.4.identity_crisis.txt
              process_reminders_from_file .3.4.identity_crisis.txt
              STATE_FILE=".s_english_3_4"
              process_file .3.4.identity_crisis.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.4.identity_crisis.txt
              sed -i '/^1$/!d' .s_english_3_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S3"
              # Define the file extension
              file_extension=".4.identity_crisis.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/English/S3"
              # Define the file extension
              file_extension_question=".4.identity_crisis.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            5)
              if ! [ -f ".english.3.4" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/English/S3"
                # Define the file extension
                file_extension_answer=".4.identity_crisis.ans.txt"
                # Define the exercise file
                exercise_file="../../english_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_english_3_5" ]; then
                echo -e "\n\nHere you are dear one... Stay organised as you explore Relationships and emotions ...\n\n${g}Just know we are not going to leave you alone${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/English/3.5.relationships_and_emotions.txt" . || exit 1
              mv 3.5.relationships_and_emotions.txt .3.5.relationships_and_emotions.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.5.relationships_and_emotions.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.5.relationships_and_emotions.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.5.relationships_and_emotions.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.5.relationships_and_emotions.txt
              process_reminders_from_file .3.5.relationships_and_emotions.txt
              STATE_FILE=".s_english_3_5"
              process_file .3.5.relationships_and_emotions.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.5.relationships_and_emotions.txt
              sed -i '/^1$/!d' .s_english_3_5
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S3"
              # Define the file extension
              file_extension_question=".5.relationships_and_emotions.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            6)
              if ! [ -f ".english.3.5" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/English/S3"
                # Define the file extension
                file_extension_answer=".5.relationships_and_emotions.ans.txt"
                # Define the exercise file
                exercise_file="../../english_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_english_3_6" ]; then
                echo -e "\n\nYou have managed to make it to Patriotism ...\n\n${g}Remember to pray always${t}\n\nThe fear of the Lord is the beginning of wisdom \c" && wait_for_a_key_press
              fi
              cp "Notes/English/3.6.patriotism.txt" . || exit 1
              mv 3.6.patriotism.txt .3.6.patriotism.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.6.patriotism.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.6.patriotism.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.6.patriotism.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.6.patriotism.txt
              process_reminders_from_file .3.6.patriotism.txt
              STATE_FILE=".s_english_3_6"
              process_file .3.6.patriotism.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.6.patriotism.txt
              sed -i '/^1$/!d' .s_english_3_6
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S3"
              # Define the file extension
              file_extension_question=".6.patriotism.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            7)
              if ! [ -f ".english.3.6" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/English/S3"
                # Define the file extension
                file_extension_answer=".6.patriotism.ans.txt"
                # Define the exercise file
                exercise_file="../../english_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_english_3_7" ]; then
                echo -e "\n\nFrom here, you will be proceeding with Further education ...\n\n${g}Please never ever forget that your education is your future${t}\n\nFocus dear \c" && wait_for_a_key_press
              fi
              cp "Notes/English/3.7.further_education.txt" . || exit 1
              mv 3.7.further_education.txt .3.7.further_education.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.7.further_education.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.7.further_education.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.7.further_education.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.7.further_education.txt
              process_reminders_from_file .3.7.further_education.txt
              STATE_FILE=".s_english_3_7"
              process_file .3.7.further_education.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.7.further_education.txt
              sed -i '/^1$/!d' .s_english_3_7
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S3"
              # Define the file extension
              file_extension_question=".7.further_education.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            8)
              if ! [ -f ".english.3.7" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/English/S3"
                # Define the file extension
                file_extension_answer=".7.further_education.ans.txt"
                # Define the exercise file
                exercise_file="../../english_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_english_3_8" ]; then
                echo -e "\n\nYou are to cover Banking or money ...\n\n${g}Please never ever settle for less${t}\n\nPromise yourself that you wont give up \c" && wait_for_a_key_press
              fi
              cp "Notes/English/3.8.banking_or_money.txt" . || exit 1
              mv 3.8.banking_or_money.txt .3.8.banking_or_money.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.8.banking_or_money.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.8.banking_or_money.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.8.banking_or_money.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.8.banking_or_money.txt
              process_reminders_from_file .3.8.banking_or_money.txt
              STATE_FILE=".s_english_3_8"
              process_file .3.8.banking_or_money.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.8.banking_or_money.txt
              sed -i '/^1$/!d' .s_english_3_8
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S3"
              # Define the file extension
              file_extension_question=".8.banking_or_money.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".english.3.8" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/English/S3"
	              # Define the file extension
	              file_extension_answer=".8.banking_or_money.ans.txt"
	              # Define the exercise file
	              exercise_file="../../english_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
								touch .english.3.8
								echo "3" > .english_ready
							fi
            ;;
    ...
    if ! find . -maxdepth 1 -name '.s_english_4*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to S4 English class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
    ...
    do
      ...
      do
        ...
        then
          attempts=0
          # Define the targeted directory
          question_directory="Revision/English/S4"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../english_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/English/S4"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../english_answered_ans.txt"
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_english_4_1" ]; then
                echo -e "\n\nYou chose to explore Leadership ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/English/4.1.leadership.txt" . || exit 1
              mv 4.1.leadership.txt .4.1.leadership.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.1.leadership.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.1.leadership.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.1.leadership.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.1.leadership.txt
              process_reminders_from_file .4.1.leadership.txt
              STATE_FILE=".s_english_4_1"
              process_file .4.1.leadership.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.1.leadership.txt
              sed -i '/^1$/!d' .s_english_4_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S4"
              # Define the file extension
              file_extension=".1.leadership.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/English/S4"
              # Define the file extension
              file_extension_question=".1.leadership.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".english.4.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/English/S4"
                # Define the file extension
                file_extension_answer=".1.leadership.ans.txt"
                # Define the exercise file
                exercise_file="../../english_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_english_4_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into The media ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/English/4.2.the_media.txt" . || exit 1
              mv 4.2.the_media.txt .4.2.the_media.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.2.the_media.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.2.the_media.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.2.the_media.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.2.the_media.txt
              process_reminders_from_file .4.2.the_media.txt
              STATE_FILE=".s_english_4_2"
              process_file .4.2.the_media.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.2.the_media.txt
              sed -i '/^1$/!d' .s_english_4_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S4"
              # Define the file extension
              file_extension_question=".2.the_media.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".english.4.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/English/S4"
                # Define the file extension
                file_extension_answer=".2.the_media.ans.txt"
                # Define the exercise file
                exercise_file="../../english_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_english_4_3" ]; then
                echo -e "\n\nYou have made a choice to cover Culture ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/English/4.3.culture.txt" . || exit 1
              mv 4.3.culture.txt .4.3.culture.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.3.culture.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.3.culture.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.3.culture.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.3.culture.txt
              process_reminders_from_file .4.3.culture.txt
              STATE_FILE=".s_english_4_3"
              process_file .4.3.culture.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.3.culture.txt
              sed -i '/^1$/!d' .s_english_4_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S4"
              # Define the file extension
              file_extension_question=".3.culture.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".english.4.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/English/S4"
                # Define the file extension
                file_extension_answer=".3.culture.ans.txt"
                # Define the exercise file
                exercise_file="../../english_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_english_4_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Choosing a career ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/English/4.4.choosing_a_career.txt" . || exit 1
              mv 4.4.choosing_a_career.txt .4.4.choosing_a_career.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.4.choosing_a_career.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.4.choosing_a_career.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.4.choosing_a_career.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.4.choosing_a_career.txt
              process_reminders_from_file .4.4.choosing_a_career.txt
              STATE_FILE=".s_english_4_4"
              process_file .4.4.choosing_a_career.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.4.choosing_a_career.txt
              sed -i '/^1$/!d' .s_english_4_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S4"
              # Define the file extension
              file_extension=".4.choosing_a_career.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/English/S4"
              # Define the file extension
              file_extension_question=".4.choosing_a_career.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            5)
              if ! [ -f ".english.4.4" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/English/S4"
                # Define the file extension
                file_extension_answer=".4.choosing_a_career.ans.txt"
                # Define the exercise file
                exercise_file="../../english_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_english_4_5" ]; then
                echo -e "\n\nHere you are dear one... Stay organised as you explore Applying for a job ...\n\n${g}Just know we are not going to leave you alone${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/English/4.5.applying_for_a_job.txt" . || exit 1
              mv 4.5.applying_for_a_job.txt .4.5.applying_for_a_job.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.5.applying_for_a_job.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.5.applying_for_a_job.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.5.applying_for_a_job.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.5.applying_for_a_job.txt
              process_reminders_from_file .4.5.applying_for_a_job.txt
              STATE_FILE=".s_english_4_5"
              process_file .4.5.applying_for_a_job.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.5.applying_for_a_job.txt
              sed -i '/^1$/!d' .s_english_4_5
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S4"
              # Define the file extension
              file_extension_question=".5.applying_for_a_job.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            6)
              if ! [ -f ".english.4.5" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/English/S4"
                # Define the file extension
                file_extension_answer=".5.applying_for_a_job.ans.txt"
                # Define the exercise file
                exercise_file="../../english_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_english_4_6" ]; then
                echo -e "\n\nYou have managed to make it to Globalisation ...\n\n${g}Remember to pray always${t}\n\nThe fear of the Lord is the beginning of wisdom \c" && wait_for_a_key_press
              fi
              cp "Notes/English/4.6.globalisation.txt" . || exit 1
              mv 4.6.globalisation.txt .4.6.globalisation.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.6.globalisation.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.6.globalisation.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.6.globalisation.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.6.globalisation.txt
              process_reminders_from_file .4.6.globalisation.txt
              STATE_FILE=".s_english_4_6"
              process_file .4.6.globalisation.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.6.globalisation.txt
              sed -i '/^1$/!d' .s_english_4_6
              attempts=0
              # Define the targeted directory
              question_directory="Revision/English/S4"
              # Define the file extension
              file_extension_question=".6.globalisation.qns.txt"
              # Define the revision file
              revision_file="../../english_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".english.4.6" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/English/S4"
	              # Define the file extension
	              file_extension_answer=".6.globalisation.ans.txt"
	              # Define the exercise file
	              exercise_file="../../english_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
								touch .english.4.6
								echo "4" > .english_ready
							fi
            ;;
