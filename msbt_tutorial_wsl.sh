...
# Function to handle user input for semester selection
function handle_semester_input() {
  if { [ -n "$last_semester" ] || [ -z "$last_course_unit" ]; } && [ -f .msbt_surveyor ]; then
		while true; do
	    read -rp $'\n\nPlease enter '"${r}1 or 2${t}"' to go to your current semester or '"${r}x${t}"' to exit'$'\n\n> ' semester
      if [ "$semester" == "x" ]; then
        exit
      fi
		  # Check if .current_msbt_semester is accidentally empty
		  if [ ! -s .current_msbt_semester ]; then
		    # Echo 1 to .current_msbt_semester
		    echo "1" > .current_msbt_semester
		  fi
		  # Read the value from the file
		  current_msbt_semester=$(<.current_msbt_semester) 2>/dev/null
		  # Check if the value is equal to $semester
		  if [ "$current_msbt_semester" -lt "$semester" 2>/dev/null ]; then
	      ...
	      rm -f "$msbt_course_unit_selected"
	      # Update the state file with the semester
	      if [ "$semester" != "x" ]; then
	        echo "$semester" > .msbt_user_state 2>/dev/null
	      fi
				break
	  	fi
		done
...
# Function to handle Sem1 Yr1 user input for course_unit selection
function handle_sem1_yr1_course_unit_input() {
  if [ -f .resume_to_semester ]; then
    rm -f .resume_to_semester
    rm -f .msbt_course_unit_selected
  fi
  if [ -z "$last_course_unit" ] || [ -f .msbt_course_unit_selected ]; then
    read -rp $'\n\nChoose either course_unit '"${g}1 or 2 or 3 or 4 or 5 or 6${t}"' to learn'$'\nor enter '"${r}z${t}"' for an adventure or '"${r}r${t}"' to revise or '"${r}s${t}"' to get sample_items'$'\nor '"${r}a${t}"' to get an assignment (course_work) or '"${r}q${t}"' to get a short answer question'$'\nor '"${r}n${t}"' to do your final semester assignment and if necessary, gain access to the next semester'$'\nor '"${r}p${t}"' to track academic progress or '"${r}x${t}"' to exit'$'\n\n1. Molecular Biology For Bioinformatics'$'\n\n2. Bioconductor and R'$'\n\n3. Biounix and Shell scripting'$'\n\n4. Online Bioinformatics and Sequence databases'$'\n\n5. Highthroughput sequencing and Analysis'$'\n\n6. Journal Club Series 1'$'\n\n> ' course_unit
    touch .msbt_surveyor
    touch .msbt_course_unit_selected
    # Update the state file with the course_unit
    # Check if the state file exists, and the course_unit is not "x"
    if [ -f .msbt_user_state ] && [ "$course_unit" != "x" ]; then
      # Get the current semester value from the state file
      existing_semester=$(awk '{print $1}' .msbt_user_state)
      # Update the state file with the course_unit, preserving the existing semester value
      echo "$existing_semester $course_unit" > .msbt_user_state 2>/dev/null
    fi
  fi
}
    ...
    for sentence in "${sentences[@]}"; do
      if [[ $echo_next == true ]]; then
        ...
        for split_sentence in "${split_sentences[@]}"; do
					echo "$split_sentence" >> .msbt_reminder
        done
      echo_next=false
      fi
        ...
        echo "Did you know that $lower_cased_sentence" >> .msbt_reminder
        ...
        echo_next=true
  			...
  			echo "Do you recall that $lower_cased_sentence;" >> .msbt_reminder

			elif [[ "$sentence" =~ " → " ]]; then
  			echo "Hope you know that: $sentence;" >> .msbt_reminder

      elif [[ "$sentence" =~ (Generally|general) ]]; then
        echo "Note: $sentence;" >> .msbt_reminder
...
process_random_reminder() {
  ...
  # Check if .msbt_reminder exists
  if [ -f .msbt_reminder ]; then
		# Remove empty lines from .msbt_reminder
  	sed -i '/^[[:space:]]*$/d' .msbt_reminder 2>/dev/null
    # Randomly select a non-empty sentence
    local reminder # Declare the variable
		reminder=$(awk -v RS=';' 'BEGIN{srand();}{gsub(/^[[:space:]]+|[[:space:]]+$/, ""); if (length > 0) a[++n]=$0}END{if (n > 0) print a[int(rand()*n)+1]}' .msbt_reminder)
...
PROGRESSIVE ASSESSMENT' > student
  ...
  if [ -f .institution_name ]; then
    read -r institution_name < .institution_name
    institution_name="${institution_name// /_}"
    touch ."$institution_name"_students_file.txt
    sed -i '/^[[:space:]]*$/d' ."$institution_name"_students_file.txt
  fi
  ...
  while true; do
    ...
    file=."$institution_name"_students_file.txt
    ...
    if $flag; then
    awk -v user_input="$matched" 'tolower($0) ~ tolower(user_input) { print }' ."$institution_name"_students_file.txt |
  ...
  echo -e ""
  if ! [ -f .institution_name ]; then
    echo -e "\nPlease Enter the name of your institution... This option is only available once \c"
    echo -e ""
    if replace_prompt "" replacement; then
      replacement=${replacement^^} # Convert to uppercase
      # Store the value in the institution_name file
      echo "$replacement" > .institution_name
    fi
  else
    read -r replacement < .institution_name
    current_datetime=$(date)
    echo -e "\nYou are at ${g}"$replacement"${t} and it is ${b}$current_datetime${t} \c"
    echo ""
  fi
  sed -i "s/Institution:             ?/Institution:             $replacement/" student
  current_datetime=$(date)
  sed -i "s/DateTime:            ?/Date + Time:          $current_datetime/" student

  if replace_prompt 'What is the '"client's"' current semester (just a single digit)?' replacement; then
    sed -i "s/Semester on joining:        ?/Semester on joining:        Sem$replacement/" student
  fi
  ...
  student_number=$(grep -c "^Institution" ."$institution_name"_students_file.txt)
  (( student_number++ ))
  ...
  wait_for_a_key_press
  sed -e ':a;N;$!ba;s/\n/;/g' .student.txt >> ."$institution_name"_students_file.txt
  sort -f ."$institution_name"_students_file.txt | uniq -i > .temmmm && mv .temmmm ."$institution_name"_students_file.txt
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    for sentence in "${sentences[@]}"; do
        ...
        if [[ $sentence == *"Figure"* ]]; then
          ...
          # Change to the "Figures/Msbt" directory
          cd Figures/Msbt || { echo "Failed to change to Figures/Msbt"; return; }
        ...
        if [[ $sentence == *"Table"* ]]; then
          cd Tables/Msbt || { echo -e "\nFailed to change to Tables/Msbt \c"; return; }
          explorer.exe "$sentence" > /dev/null 2>&1 &
          cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        fi

        if [[ $sentence == *"Video"* ]]; then
          cd Videos/Msbt || { echo -e "\nFailed to change to Videos/Msbt \c"; return; }
          explorer.exe "$sentence" > /dev/null 2>&1 &
          cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        fi
...
contact_ai() {
  last_course_unit=$(awk -F' ' '{print $2}' .msbt_user_state)
  if [ -f .connect_to_ai ]; then
    echo ""
    # Connect to internet
    geminichat
    rm -f .connect_to_ai
    touch .resume_to_semester
  fi
}
...
while true; do
  ...
  # Prompt the user for input
  if [ -z "$(find ./Notes/Msbt -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
  ...
  if [[ -n "$user_input" ]]; then
    ...
    if [[ "$user_input" == "zz" ]]; then
      ...
      TEMP_FILE=$(mktemp) && curl -o "$TEMP_FILE" -L "https://github.com/Muhumuza7325/OMD/raw/main/msbt_tutorial_wsl.sh" && mv "$TEMP_FILE" msbt_tutorial_wsl.sh && chmod +x msbt_tutorial_wsl.sh && cp msbt_tutorial.sh Students/Omd && echo -e "\n\n${y}Code successfully updated.. You will have to restart a new session${t} \c" && sleep 4 && exit || (echo -e "\n\n${m}Error updating code!... Please check your internet connection and try again!${t} \c" && rm -f "$TEMP_FILE" && return)
    fi
    if [[ "$user_input" == "xx" ]]; then
      ...
      if [[ "$input" == "y" || "$input" == "Y" ]]; then
        ...
        curl -O -L "https://github.com/Muhumuza7325/OMD/raw/main/update_msbt.sh" || { echo -e "\n\n${m}Check your internet connection and try again!${t}" >&2; return; }
        mv update_msbt.sh .update_msbt.sh
        bash .update_msbt.sh
        return
      else
        return
    ...
    if [[ -n "$result" ]]; then
      ...
      if echo "$result" | grep "Figure"; then
        echo "$result" | grep -o '\bFigure[0-9]\+.*\.jpg\(\.[0-9]\+\)*\b' > Figures/Msbt/figures.txt
      ...
      if echo "$result" | grep "Table"; then
        echo "$result" | grep -o '\bTable[0-9]\+\(\.[0-9]\+\)*\b' > Tables/Msbt/tables.txt
        sed -i -e '/^Table/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Tables/Msbt/tables.txt
        cd Tables/Msbt || { echo -e "\nFailed to change to Tables/Msbt \c"; return; }
        ...
        # Go back to the original directory
        cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        rm -f Tables/Msbt/tables.txt
      fi

      if echo "$result" | grep "Video"; then
        #echo "$result" > Videos/Msbt/videos.txt
        echo "$result" | grep -o '\bVideo[0-9]\+\(\.[0-9]\+\)*\b' > Videos/Msbt/videos.txt
        # Remove lines not starting with "Video" and any leading/trailing whitespaces
        sed -i -e '/^Video/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Videos/Msbt/videos.txt
        # Change to the "Videos/Msbt" directory
        cd Videos/Msbt || { echo -e "\nFailed to change to Videos/Msbt \c"; return; }
  ...
  if [ -f "$selected_file" ]; then
    ...
    if [[ -n "$selected_question" && "$selected_question" =~ [[:graph:]] ]]; then
      ...
      if echo "$selected_question" | grep "Figure"; then
        ...
        # Go back to the original directory
        cd ../../Revision/Msbt || { echo "Failed to change back to the targeted directory \c"; exit 1; }
        # Remove the temporary file
        rm -f ../../Figures/Msbt/figures.txt
      fi
...
process_question_answer() {
  ...
  if [ -f ../../../.institution_name ]; then
    ...
    sed -i '/^[[:space:]]*$/d' ../../../."$institution_name"_students_file.txt
    existing_semester=$(awk '{print $1}' ../../../.msbt_user_state)
    existing_course_unit=$(awk '{print $2}' ../../../.msbt_user_state)
    echo ''
    ...
    if grep -q "Msbt" ../../../."$institution_name"_students_file.txt; then
      sed -i -E 's/;/\n/g' ../../../."$institution_name"_students_file.txt
      # Find the line with the word Msbt, replace the information below it with the already available information adding a comma existing_semester_existing_course_unit [$percentage]
      if [ "$existing_course_unit" == "r" ]; then
        sed -i '/Msbt/{n;s/\(.*\)/\1, '"$existing_semester"'_'"$input"' '"$names"'['"$percentage%"']/}' ../../../."$institution_name"_students_file.txt
      else
        (( existing_course_unit-- ))
        sed -i '/Msbt/{n;s/\(.*\)/\1, '"$existing_semester"'_'"$existing_course_unit"' '"$names"'['"$percentage%"']/}' ../../../."$institution_name"_students_file.txt
      fi
    else
      if [ "$existing_course_unit" == "r" ]; then
        sed -i -E '/^Institution/ i\Msbt\n'"${existing_semester} ${names}[${percentage}%]"'' ../../../."$institution_name"_students_file.txt
        echo -e "Msbt\n"$existing_semester" "$names"["$percentage%"]" >> ../../../."$institution_name"_students_file.txt
      else
        (( existing_course_unit-- ))
        sed -i -E '/^Institution/ i\Msbt\n'"${existing_semester}_${existing_course_unit} ${names}[${percentage}%]"'' ../../../."$institution_name"_students_file.txt
        echo -e "Msbt\n"$existing_semester"_"$existing_course_unit" "$names"["$percentage%"]" >> ../../../."$institution_name"_students_file.txt
      fi
        sed -i '1,2s/.*//g' ../../../."$institution_name"_students_file.txt
        sed -i '/^[[:space:]]*$/d' ../../../."$institution_name"_students_file.txt
    fi
    sed -i -e ':a;N;$!ba;s/\n/;/g' ../../../."$institution_name"_students_file.txt
    sed -i -E 's/;Institution/\nInstitution/g' ../../../."$institution_name"_students_file.txt
    echo -e "\n\nThe obtained score has been recorded and allocated accordingly... \c"
  else
    echo -e "\n\nThe obtained score has not been recorded... Please visit course_unit options on the next visit and select the option to provide student details \c"
  fi
  wait_for_a_key_press
...
function resume_from_last_point() {
  if [ -f .msbt_user_state ]; then
    last_semester=$(awk -F' ' '{print $1}' .msbt_user_state)
    last_course_unit=$(awk -F' ' '{print $2}' .msbt_user_state)
    if [ -n "$last_semester" ] && [ -n "$last_course_unit" ]; then
      echo -e "\n       Resuming from ${r}S$last_semester${t} : ${g}Course unit '$last_course_unit'${t} \c"
      rm -f .msbt_surveyor
      clear_and_center "     ..........  Resumed from ${g}Course unit $last_course_unit${t} (${r}S$last_semester${t})  ............"
      return 0
    elif [ -n "$last_semester" ] && [ -z "$last_course_unit" ]; then
      echo -e "\nTaking you to ${r}semester selection${t} \c"
      wait_for_a_key_press
      return 0
    elif [ -z "$last_semester" ] && [ -z "$last_course_unit" ]; then
      return 0
    else
...
process_final_assignment() {
  # Check if user is.msbt_ready for the assignment
  # Check if .current_msbt_semester is accidentally empty
  if [ ! -s .current_msbt_semester ]; then
    # Echo 1 to .current_msbt_semester
    echo "1" > .current_msbt_semester
  fi
  # Read the value from the .current_msbt_semester file
  current_msbt_semester=$(<.current_msbt_semester) 2>/dev/null
  # Check if the value in the .msbt_ready file is equal to $semester
  # Read the value from the .msbt_ready file
  if [ ! -s .msbt_ready ]; then
    echo "0" > .msbt_ready
  fi
  how.msbt_ready=$(<.msbt_ready) 2>/dev/null
  # Check if the value is equal to $semester
  if [ "$how.msbt_ready" -lt "$current_msbt_semester" 2>/dev/null ]; then
    read -rp $'\n\nYou havent done all the course_unit assignments for your current msbt semester\n\n'"${r}Proceeding from here will affect your very final score${y}"'. To go back and progress right, enter '"${y}ok${t}"'. Otherwise, press the Enter key to do the final semester assignment: ' progress
    if [ "$progress" == "ok" ]; then
      return
    fi
  fi
  echo -e "\n\n${y}You are welcome to your final semester session.${t}\n\n${r}You will have to use a maximum of 3600s (1hr) to obtain a minimum of 95 scores by answering 100 questions, @ carrying 1 score!${t}\n\n${g}You must get that score to move on to the next semester. We wish you the very best dear...${t} \c"
  # Save the current working directory
  pushd . > /dev/null
  # Change to the specified directory
  cd "$answered_directory" || exit
    ...
    percentage=$((score * 100 / total_questions))
      if [ "$elapsed_time" -gt 3600 ]; then
        ...
        if [ "$percentage" -lt 95 ]; then
          ...
          cd - > /dev/null 2>&1 || exit
          if ! [ -f .current_msbt_semester ]; then
            # Echo the result to the file .current_semester
            echo "2" > .current_msbt_semester
            echo -e "\n\n${g}Congratulations!${t} You have successfully gained access to the next semester (S2).\n\nHowever, if a diferrent semester was expected, then something is wrong.\n\nFrom here, you will have to go back to go back to S2 and acess the next semesteres the right way \c"
            wait_for_a_key_press
          else
            # Add 1 to $semester value
            new_semester=$(($semester + 1))
            # Read the value from the file
            current_msbt_semester=$(<.current_msbt_semester) 2>/dev/null
            # Check if the new_semester value is lt $current_msbt_semester
            if [ "$new_semester" -gt "$current_msbt_semester" 2>/dev/null ]; then
		          # Echo the result to the file .current_semester
		          echo "$new_semester" > .current_msbt_semester
            fi
            echo -e "\n\n${g}Congratulations!${t} You have successfully gained access to the next semester! \c"
            wait_for_a_key_press
          fi
          echo -e "\n\nYou can now choose the next semester from the available options... ${y}Returning to your current session${t} \c"
          wait_for_a_key_press
          break 2
        fi
      fi
...
get_sample_items() {
	...
	rm -f .msbt_topic_selected
	# Save the current working directory
	pushd . > /dev/null
	cd Revision/Msbt || { echo "Directory not found"; return; }
	...
	while true; do
		if [ ! -f .revise.txt ]; then
			...
			if [ $time_diff -gt 3600 ]; then
				...
				echo -e "\n\n\n${r}You are advised to not make any changes to the provided answers, instead, you can make copies that you can edit${t}\n\n${y}For a teacher willing to join us reach out to everyone of our children, please send us your questions and answers in a file labelled with your name, school, msbt, and file content (e.g., Muhumuza_Omega_Kasule_High_School_O_level_Chemistry_Answered_EOC1_Items.pdf) to our contacts${t}\n\n\nEmail: ${g}2024omd256@gmail.com${t} \c"
				echo $current_time > "$last_echo_time_file"
			fi
			read -rp $'\n\n\nEnter '"${m}any character${t}"' for access to the file of answered items or simply press '"${r}Enter${t}"' to get items to attempt : ' input
			if [[ -n $input ]]; then
				explorer.exe .msbt_samples* > /dev/null 2>&1 &
				clear
				popd > /dev/null || exit
				return
			fi
		...
		rm -f .revise.txt .msbt_topic_selected 2>/dev/null
	done
	popd > /dev/null || exit
	return
}
...
while true; do
  ...
  if ls Students | grep -wq "$initials"; then
    ...
    echo -e "                  $initials\n" > "Students/$initials/Exercise/msbt_answered_ans.txt"
    echo -e "                  $initials\n" > "Students/$initials/Revision/msbt_covered_qns.txt"
...
process_question_answer_adv() {
  ...
  while true; do
    ...
    if [ -f ../../../.school_name ]; then
      ...
      existing_semester=$(awk '{print $1}' ../../../.msbt_user_state)
      existing_ course_unit=$(awk '{print $2}' ../../../.msbt_user_state)
      echo ''
      if replace_prompt 'By just pressing Enter, the obtained score will be allocated to every recorded student... If otherwise, enter your Initial(s) (space-separated) to label the score' replacement; then
        replacement=${replacement^^} # Convert to uppercase
        names="($replacement) "
      ...
      if grep -q "Msbt" ../../../."$school_name"_students_file.txt; then
        sed -i -E 's/;/\n/g' ../../../."$school_name"_students_file.txt
        # Find the line with the word Msbt, replace the information below it with the already available information adding a comma existing_semester_existing_ course_unit [$percentage]
        if [ "$existing_ course_unit" == "z" ]; then
          sed -i '/Msbt/{n;s/\(.*\)/\1, '"$existing_semester"'_'"$input"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
        else
          (( existing_ course_unit-- ))
          sed -i '/Msbt/{n;s/\(.*\)/\1, '"$existing_semester"'_'"$existing_ course_unit"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
        fi
      else
        if [ "$existing_ course_unit" == "z" ]; then
          sed -i -E '/^School/ i\Msbt\n'"${existing_semester} ${names}[${percentage}%]"'' ../../../."$school_name"_students_file.txt
          echo -e "Msbt\n"$existing_semester" "$names"["$percentage%"]" >> ../../../."$school_name"_students_file.txt
        else
          (( existing_ course_unit-- ))
          sed -i -E '/^School/ i\Msbt\n'"${existing_semester}_${existing_ course_unit} ${names}[${percentage}%]"'' ../../../."$school_name"_students_file.txt
          echo -e "Msbt\n"$existing_semester"_"$existing_ course_unit" "$names"["$percentage%"]" >> ../../../."$school_name"_students_file.txt
        fi
...
if [ ! -f .msbt_user_state ]; then
  ...
  if [ -f .omd_communication.txt ]; then
    ...
    process_random_reminder .msbt_reminder
  fi
	handle_resume_input
fi
...
if ! [ -d "Notes/Msbt" ] || ! [ -d "Revision" ] || ! [ -d "Exercise" ] || ! [ -d "Videos" ] || ! [ -d "Figures" ] || ! [ -d "Tables" ]; then
  ...
  wait_for_a_key_press 
  echo -e "\n\nAlways remember to press ${r}control and c${t} together or just close the terminal to exit a semester session\n\nIf nothing goes wrong, you will always be able to continue from where you stopped. In most cases, pressing the backspace key will connect you to an AI model and entering q will always return you from the model to your current session${b} \c"
  wait_for_a_key_press

  # Create additional directories and files
  mkdir -p Notes Notes/Msbt Revision Revision/Msbt Revision/Msbt/{Sem1_Yr1,Sem2_Yr1} Exercise Exercise/Msbt Exercise/Msbt/{Sem1_Yr1,Sem2_Yr1} Videos Videos/Msbt Figures Figures/Msbt Tables Tables/Msbt
  touch Revision/msbt_covered_qns.txt Exercise/msbt_answered_ans.txt
  echo -e "\n"
  pwd
  ...
  ls "$PWD"
  wait_for_a_key_press
  echo -e "\n\n${t}For this tutorial, you will require current learning material from your lecturer in your current folder or directory\n\nOtherwise follow the procedure below to obtain the material \c"
  echo
fi
...
if [ -z "$(find ./Notes/Msbt -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
  ...
  if [[ -z "$user_input" ]]; then
    ...
    curl -O -L https://github.com/Muhumuza7325/OMD/raw/main/MSBT/1.1.first_1.txt || echo -e "\n\nError fetching material for this tutorial \c"
    ...
    cp 1.1.first_1.txt Notes/Msbt || echo -e "\n\nError copying 1.1.first_1.txt to the Msbt directory in the Notes directory \c"
    wait_for_a_key_press
  else
  ...
  rm -f 1.1.first_1.txt
fi

while true; do

  handle_semester_input
  if [[ "$semester" == "1" ]]; then
    if ! find . -maxdepth 1 -name '.s_msbt_1*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to Sem1_Yr1 Msbt semester${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
      clear_and_center "There are ${r}6${t} course_units to be covered. Your tasks will always expand or shrink to fit in the time you give them. For that reason, never procrastinate darling!"
    fi
    attempts=0
    max_attempts=4
    while true
    do
      ...
      do
        handle_sem1_yr1_course_unit_input
        touch .msbt_course_unit_selected
        if [[ "$course_unit" == "x" ]]
        then
          quit
        ...
        then
          ...
          question_directory="Revision/Msbt/Sem1_Yr1"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../msbt_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Msbt/Sem1_Yr1"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../msbt_answered_ans.txt"
          # Call the function to process a random question
        ...
        then
          track_student_progress
        elif [[ ! "$course_unit" =~ ^[1-6]$ || -z "$course_unit" ]]
        then
          echo -e "\n\ncourse_unit ${r}$course_unit not available${t}... Please choose from the available options\c"
          wait_for_a_key_press
        else
          case "$course_unit" in
            1)
              if ! [ -f ".s_msbt_1_1" ]; then
                echo -e "\n\nYou chose to explore Molecular Biology For Bioinformatics...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/Msbt/1.1.molecular_biology_for_bioinformatics.txt" . || exit 1
              mv 1.1.molecular_biology_for_bioinformatics.txt .1.1.molecular_biology_for_bioinformatics.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.1.molecular_biology_for_bioinformatics.txt
              process_reminders_from_file .1.1.molecular_biology_for_bioinformatics.txt
              STATE_FILE=".s_msbt_1_1"
              process_file .1.1.molecular_biology_for_bioinformatics.txt
              contact_ai
              if [ -f .resume_to_semester ]; then
                break
              fi
              ...
              rm -f .1.1.molecular_biology_for_bioinformatics.txt
              sed -i '/1/!d' .s_msbt_1_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Msbt/Sem1_Yr1"
              # Define the file extension
              file_extension=".1.molecular_biology_for_bioinformatics.qns.txt"
              # Define the revision file
              revision_file="../../msbt_covered_qns.txt"
              # Call the function to process a random question
              ...
              question_directory="Revision/Msbt/Sem1_Yr1"
              # Define the file extension
              file_extension_question=".1.molecular_biology_for_bioinformatics.qns.txt"
              # Define the revision file
              revision_file="../../msbt_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ...
            2)
              if ! [ -f ".msbt.1.1" ]; then
                ...
                answered_directory="Exercise/Msbt/Sem1_Yr1"
                # Define the file extension
                file_extension_answer=".1.molecular_biology_for_bioinformatics.ans.txt"
                # Define the exercise file
                exercise_file="../../msbt_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_msbt_1_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Bioconductor and R...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Msbt/1.2.bioconductor_and_r.txt" . || exit 1
              mv 1.2.bioconductor_and_r.txt .1.2.bioconductor_and_r.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.2.bioconductor_and_r.txt
              sed -i "s/;\([^();]*\))/\1)/g" .1.2.bioconductor_and_r.txt
              sed -i 's/^\([:!?]\);/\1/' .1.2.bioconductor_and_r.txt
              process_reminders_from_file .1.2.bioconductor_and_r.txt
              STATE_FILE=".s_msbt_1_2"
              process_file .1.2.bioconductor_and_r.txt
              contact_ai
              if [ -f .resume_to_semester ]; then
                break
              fi
              ...
              rm -f .1.2.bioconductor_and_r.txt
              sed -i '/1/!d' .s_msbt_1_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Msbt/Sem1_Yr1"
              # Define the file extension
              file_extension_question=".2.bioconductor_and_r.qns.txt"
              # Define the revision file
              revision_file="../../msbt_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            3)
              if ! [ -f ".msbt.1.2" ]; then
                ...
                answered_directory="Exercise/Msbt/Sem1_Yr1"
                # Define the file extension
                file_extension_answer=".2.bioconductor_and_r.ans.txt"
                # Define the exercise file
                exercise_file="../../msbt_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_msbt_1_3" ]; then
                echo -e "\n\nYou have made a choice to cover Biounix and Shell scripting...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Msbt/1.3.biounix_and_shell_scripting.txt" . || exit 1
              mv 1.3.biounix_and_shell_scripting.txt .1.3.biounix_and_shell_scripting.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.3.biounix_and_shell_scripting.txt
              process_reminders_from_file .1.3.biounix_and_shell_scripting.txt
              STATE_FILE=".s_msbt_1_3"
              process_file .1.3.biounix_and_shell_scripting.txt
              contact_ai
              if [ -f .resume_to_semester ]; then
                break
              fi
              ...
              rm -f .1.3.biounix_and_shell_scripting.txt
              sed -i '/1/!d' .s_msbt_1_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Msbt/Sem1_Yr1"
              # Define the file extension
              file_extension_question=".3.biounix_and_shell_scripting.qns.txt"
              # Define the revision file
              revision_file="../../msbt_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            4)
              if ! [ -f ".msbt.1.3" ]; then
                ...
                answered_directory="Exercise/Msbt/Sem1_Yr1"
                # Define the file extension
                file_extension_answer=".3.biounix_and_shell_scripting.ans.txt"
                # Define the exercise file
                exercise_file="../../msbt_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_msbt_1_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Online Bioinformatics and Sequence databases...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/Msbt/1.4.online_bioinformatics_and_sequence_databases.txt" . || exit 1
              mv 1.4.online_bioinformatics_and_sequence_databases.txt .1.4.online_bioinformatics_and_sequence_databases.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.4.online_bioinformatics_and_sequence_databases.txt
              process_reminders_from_file .1.4.online_bioinformatics_and_sequence_databases.txt
              STATE_FILE=".s_msbt_1_4"
              process_file .1.4.online_bioinformatics_and_sequence_databases.txt
              contact_ai
              if [ -f .resume_to_semester ]; then
                break
              fi
              ...
              rm -f .1.4.online_bioinformatics_and_sequence_databases.txt
              sed -i '/1/!d' .s_msbt_1_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Msbt/Sem1_Yr1"
              # Define the file extension
              file_extension=".4.online_bioinformatics_and_sequence_databases.qns.txt"
              # Define the revision file
              revision_file="../../msbt_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Msbt/Sem1_Yr1"
              # Define the file extension
              file_extension_question=".4.online_bioinformatics_and_sequence_databases.qns.txt"
              # Define the revision file
              revision_file="../../msbt_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ...
            5)
              if ! [ -f ".msbt.1.4" ]; then
                ...
                answered_directory="Exercise/Msbt/Sem1_Yr1"
                # Define the file extension
                file_extension_answer=".4.online_bioinformatics_and_sequence_databases.ans.txt"
                # Define the exercise file
                exercise_file="../../msbt_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_msbt_1_5" ]; then
                echo -e "\n\nHere you are dear one... Stay organised as you explore Highthroughput sequencing and Analysis...\n\n${g}Just know we are not going to leave you alone${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Msbt/1.5.highthroughput_sequencing_and_analysis.txt" . || exit 1
              mv 1.5.highthroughput_sequencing_and_analysis.txt .1.5.highthroughput_sequencing_and_analysis.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.5.highthroughput_sequencing_and_analysis.txt
              process_reminders_from_file .1.5.highthroughput_sequencing_and_analysis.txt
              STATE_FILE=".s_msbt_1_5"
              process_file .1.5.highthroughput_sequencing_and_analysis.txt
              contact_ai
              if [ -f .resume_to_semester ]; then
                break
              fi
              ...
              rm -f .1.5.highthroughput_sequencing_and_analysis.txt
              sed -i '/1/!d' .s_msbt_1_5
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Msbt/Sem1_Yr1"
              # Define the file extension
              file_extension_question=".5.highthroughput_sequencing_and_analysis.qns.txt"
              # Define the revision file
              revision_file="../../msbt_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            6)
              if ! [ -f ".msbt.1.5" ]; then
                ...
                answered_directory="Exercise/Msbt/Sem1_Yr1"
                # Define the file extension
                file_extension_answer=".5.highthroughput_sequencing_and_analysis.ans.txt"
                # Define the exercise file
                exercise_file="../../msbt_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_msbt_1_6" ]; then
                echo -e "\n\nYou have managed to make it to Journal Club Series 1...\n\n${g}Remember to pray always${t}\n\nThe fear of the Lord is the beginning of wisdom \c" && wait_for_a_key_press
              fi
              cp "Notes/Msbt/1.6.journal_club_series_1.txt" . || exit 1
              mv 1.6.journal_club_series_1.txt .1.6.journal_club_series_1.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.6.journal_club_series_1.txt
              process_reminders_from_file .1.6.journal_club_series_1.txt
              STATE_FILE=".s_msbt_1_6"
              process_file .1.6.journal_club_series_1.txt
              contact_ai
              if [ -f .resume_to_semester ]; then
                break
              fi
              ...
              rm -f .1.6.journal_club_series_1.txt
              sed -i '/1/!d' .s_msbt_1_6
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Msbt/Sem1_Yr1"
              # Define the file extension
              file_extension_question=".6.journal_club_series_1.qns.txt"
              # Define the revision file
              revision_file="../../msbt_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              if ! [ -f ".msbt.1.6" ]; then
	              ...
	              answered_directory="Exercise/Msbt/Sem1_Yr1"
	              # Define the file extension
	              file_extension_answer=".6.journal_club_series_1.ans.txt"
	              # Define the exercise file
	              exercise_file="../../msbt_answered_ans.txt"
            ...
            # Additional cases for other course_units can be added here
            *)
              echo -e "\n\nInvalid course_unit number \c"
              continue
            ;;
          esac
  ...
  echo -e "\n\nLectures for your semester are still being developed.. Keep in touch \n"
  wait_for_a_key_press
