...
if ! [ -d "Notes" ] || ! [ -d "Revision" ] || ! [ -d "Exercise" ] || ! [ -d "Videos" ] || ! [ -d "Figures" ] || ! [ -d "Tables" ]; then
    ...
    cp ../nutrition_and_food_technology_tutorial .
    clear_and_center
    echo
fi

files=(Notes/Nutrition_and_food_technology/*.txt)
if [ ${#files[@]} -eq 0 ]; then
    ...
    rm -f ../nutrition_and_food_technology_tutorial 1.1.introduction_to_nutrition_and_food_technology.txt
fi
