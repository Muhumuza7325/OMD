...
function handle_class_input() {
  if { [ -n "$last_class" ] || [ -z "$last_topic" ]; } && [ -f .nutrition_and_food_technology_surveyor ]; then
		while true; do
	    read -rp $'\n\nPlease enter '"${r}1 or 2 or 3 or 4${t}"' to go to your class or '"${r}x${t}"' to exit'$'\n\n> ' class
      if [ "$class" == "x" ]; then
        exit
      fi
		  # Check if .current_nutrition_and_food_technology_class is accidentally empty
		  if [ ! -s .current_nutrition_and_food_technology_class ]; then
		    # Echo 1 to .current_nutrition_and_food_technology_class
		    echo "1" > .current_nutrition_and_food_technology_class
		  fi
		  # Read the value from the file
		  current_nutrition_and_food_technology_class=$(<.current_nutrition_and_food_technology_class) 2>/dev/null
		  # Check if the value is equal to $class
		  if [ "$current_nutrition_and_food_technology_class" -lt "$class" 2>/dev/null ]; then
	      ...
	      rm -f ".nutrition_and_food_technology_topic_selected"
	      # Update the state file with the class
	      if [ "$class" != "x" ]; then
	        echo "$class" > .nutrition_and_food_technology_user_state 2>/dev/null
	      fi
				break
	  	fi
		done
...
function handle_s4_topic_input() {
  ...
  if [ -z "$last_topic" ] || [ -f .nutrition_and_food_technology_topic_selected ]; then
    read -rp $'\n\nChoose either topic '"${g}1 or 2 or 3 or 4 or 5 or 6${t}"' to learn'$'\nor enter '"${r}z${t}"' for an adventure or '"${r}r${t}"' to revise or '"${r}s${t}"' to get sample_items'$'\nor '"${r}a${t}"' to get an activity of integration or '"${r}q${t}"' to get a short answer question'$'\nor '"${r}n${t}"' to do your final class assignment and if necessary, gain access to the next class'$'\nor '"${r}p${t}"' to track academic progress or '"${r}x${t}"' to exit'$'\n\n1. Nutrition and the life cycle '"${r}Term1${t}"''$'\n\n2. Management of nutritional related diseases and disorders '"${r}Term1${t}"''$'\n\n3. Processing meat, fish and poultry products '"${r}Term2${t}"''$'\n\n4. Importance of rechauffe and convenience foods in the diet '"${r}Term2${t}"''$'\n\n5. Table setting and food service '"${r}Term3${t}"''$'\n\n6. Consolidation of skills '"${r}Term3${t}"''$'\n\n> ' topic
    touch .nutrition_and_food_technology_surveyor
    touch .nutrition_and_food_technology_topic_selected
    # Update the state file with the topic
    # Check if the state file exists, and the topic is not "x"
    if [ -f .nutrition_and_food_technology_user_state ] && [ "$topic" != "x" ]; then
      # Get the current class value from the state file
      existing_class=$(awk '{print $1}' .nutrition_and_food_technology_user_state)
      # Update the state file with the topic, preserving the existing class value
      echo "$existing_class $topic" > .nutrition_and_food_technology_user_state 2>/dev/null
    fi
  fi
}
...
function handle_s3_topic_input() {
  ...
  if [ -z "$last_topic" ] || [ -f .nutrition_and_food_technology_topic_selected ]; then
    read -rp $'\n\nChoose either topic '"${g}1 or 2 or 3 or 4 or 5 or 6 or 7${t}"' to learn'$'\nor enter '"${r}z${t}"' for an adventure or '"${r}r${t}"' to revise or '"${r}s${t}"' to get sample_items'$'\nor '"${r}a${t}"' to get an activity of integration or '"${r}q${t}"' to get a short answer question'$'\nor '"${r}n${t}"' to do your final class assignment and if necessary, gain access to the next class'$'\nor '"${r}p${t}"' to track academic progress or '"${r}x${t}"' to exit'$'\n\n1. Stocks, soups, sauces and gravy '"${r}Term1${t}"''$'\n\n2. Sweets and puddings '"${r}Term1${t}"''$'\n\n3. Food preparation (basic meals) '"${r}Term1${t}"''$'\n\n4. Meals for special occasions '"${r}Term2${t}"''$'\n\n5. Milk and milk products '"${r}Term2${t}"''$'\n\n6. Yeast cookery '"${r}Term3${t}"''$'\n\n7. Confectioneries and pastries '"${r}Term3${t}"''$'\n\n> ' topic
    touch .nutrition_and_food_technology_surveyor
    touch .nutrition_and_food_technology_topic_selected
    # Update the state file with the topic
    # Check if the state file exists, and the topic is not "x"
...
function handle_s2_topic_input() {
  ...
  if [ -z "$last_topic" ] || [ -f .nutrition_and_food_technology_topic_selected ]; then
    read -rp $'\n\nChoose either topic '"${g}1 or 2 or 3 or 4 or 5 or 6 or 7${t}"' to learn'$'\nor enter '"${r}z${t}"' for an adventure or '"${r}r${t}"' to revise or '"${r}s${t}"' to get sample_items'$'\nor '"${r}a${t}"' to get an activity of integration or '"${r}q${t}"' to get a short answer question'$'\nor '"${r}n${t}"' to do your final class assignment and if necessary, gain access to the next class'$'\nor '"${r}p${t}"' to track academic progress or '"${r}x${t}"' to exit'$'\n\n1. Lipids '"${r}Term1${t}"''$'\n\n2. Vitamins '"${r}Term1${t}"''$'\n\n3. Water '"${r}Term1${t}"''$'\n\n4. Processing foods from vegetable seeds '"${r}Term2${t}"''$'\n\n5. Processing and preservation of fruits '"${r}Term2${t}"''$'\n\n6. Common foods '"${r}Term3${t}"''$'\n\n7. Meal planning '"${r}Term3${t}"''$'\n\n> ' topic
    touch .nutrition_and_food_technology_surveyor
    touch .nutrition_and_food_technology_topic_selected
    # Update the state file with the topic
    # Check if the state file exists, and the topic is not "x"
...
function handle_s1_topic_input() {
  ...
  if [ -z "$last_topic" ] || [ -f .nutrition_and_food_technology_topic_selected ]; then
    read -rp $'\n\nChoose either topic '"${g}1 or 2 or 3 or 4 or 5 or 6 or 7${t}"' to learn'$'\nor enter '"${r}z${t}"' for an adventure or '"${r}r${t}"' to revise or '"${r}s${t}"' to get sample_items'$'\nor '"${r}a${t}"' to get an activity of integration or '"${r}q${t}"' to get a short answer question'$'\nor '"${r}n${t}"' to do your final class assignment and if necessary, gain access to the next class'$'\nor '"${r}p${t}"' to track academic progress or '"${r}x${t}"' to exit'$'\n\n1. Introduction to nutrition and food technology '"${r}Term1${t}"''$'\n\n2. Kitchen equipment and planning '"${r}Term1${t}"''$'\n\n3. Safety in the home '"${r}Term1${t}"''$'\n\n4. Proteins '"${r}Term2${t}"''$'\n\n5. Carbohydrates '"${r}Term2${t}"''$'\n\n6. Mineral elements '"${r}Term3${t}"''$'\n\n7. Processing and preservation of vegetables '"${r}Term3${t}"''$'\n\n> ' topic
    touch .nutrition_and_food_technology_surveyor
    touch .nutrition_and_food_technology_topic_selected
    # Update the state file with the topic
    # Check if the state file exists, and the topic is not "x"
...
process_reminders_from_file() {
  ...
  prev_sentence=""
  echo_next=false
	echo -n > .nutrition_and_food_technology_reminder
    ...
    for sentence in "${sentences[@]}"; do
      if [[ $echo_next == true ]]; then
        ...
        for split_sentence in "${split_sentences[@]}"; do
					echo "$split_sentence" >> .nutrition_and_food_technology_reminder
        done
      echo_next=false
      fi
        ...
        echo "Did you know that $lower_cased_sentence" >> .nutrition_and_food_technology_reminder
        ...
        echo_next=true
  			...
  			echo "Do you recall that $lower_cased_sentence;" >> .nutrition_and_food_technology_reminder

			elif [[ "$sentence" =~ " → " ]]; then
  			echo "Hope you know that: $sentence;" >> .nutrition_and_food_technology_reminder

      elif [[ "$sentence" =~ " ↔ " ]]; then
        echo "Hope you know that: $sentence;" >> .nutrition_and_food_technology_reminder

      elif [[ "$sentence" =~ (Generally|general) ]]; then
        echo "Note: $sentence;" >> .nutrition_and_food_technology_reminder
...
process_random_reminder() {
  ...
  # Check if .nutrition_and_food_technology_reminder exists
  if [ -f .nutrition_and_food_technology_reminder ]; then
		# Remove empty lines from .nutrition_and_food_technology_reminder
  	sed -i '/^[[:space:]]*$/d' .nutrition_and_food_technology_reminder 2>/dev/null
    # Randomly select a non-empty sentence
    local reminder # Declare the variable
		reminder=$(awk -v RS=';' 'BEGIN{srand();}{gsub(/^[[:space:]]+|[[:space:]]+$/, ""); if (length > 0) a[++n]=$0}END{if (n > 0) print a[int(rand()*n)+1]}' .nutrition_and_food_technology_reminder)
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    for sentence in "${sentences[@]}"; do
        ...
        if [[ $sentence == *"Figure"* ]]; then
          ...
          # Change to the "Figures/Nutrition_and_food_technology" directory
          cd Figures/Nutrition_and_food_technology || { echo "Failed to change to Figures/Nutrition_and_food_technology"; return; }
        ...
        if [[ $sentence == *"Table"* ]]; then
          cd Tables/Nutrition_and_food_technology || { echo -e "\nFailed to change to Tables/Nutrition_and_food_technology \c"; return; }
          explorer.exe "$sentence" > /dev/null 2>&1 &
          cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        fi

        if [[ $sentence == *"Video"* ]]; then
          cd Videos/Nutrition_and_food_technology || { echo -e "\nFailed to change to Videos/Nutrition_and_food_technology \c"; return; }
          explorer.exe "$sentence" > /dev/null 2>&1 &
          cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        fi
...
contact_ai() {
  last_topic=$(awk -F' ' '{print $2}' .nutrition_and_food_technology_user_state)
...
while true; do
  ...
  # Prompt the user for input
  if [ -z "$(find ./Notes/Nutrition_and_food_technology -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
  ...
  if [[ -n "$user_input" ]]; then
		if [[ "$user_input" == "sh" ]]; then
		  Response="We are looking forward to receiving your economic support, thoughts, suggestions, and any files you feel should reach out to everyone of our children. Please remember to label the files you are to attach using the format: Your_name_School_Nutrition_and_food_technology_File_content (e.g., Muhumuza_Omega_Kasule_High_School_O_level_Chemistry_Answered_EOC1_Items.pdf). Thanks a lot for your contributions."  
		  nutrition_and_food_technology="$(basename "$0") - $(date +"%Y-%m-%d %H:%M:%S") - Thoughts, suggestions, and contributions"
		  encoded_nutrition_and_food_technology=$(echo "$nutrition_and_food_technology" | sed 's/ /%20/g; s/\n/%0A/g')
		  encoded_body=$(echo "$Response" | sed 's/ /%20/g; s/\n/%0A/g')
		  # Open the email in the browser with the encoded nutrition_and_food_technology and body
		  powershell.exe -Command "Start-Process 'https://mail.google.com/mail/?view=cm&to=2024omd256@gmail.com&su=${encoded_nutrition_and_food_technology}&body=${encoded_body}'"
			return
		fi
     ...
     if [[ "$user_input" == "sr" ]]; then
      ...
      explorer.exe Nutrition_and_food_technology > /dev/null 2>&1 &
    ...
    if [[ "$user_input" == "zz" ]]; then
      ...
      TEMP_FILE=$(mktemp) && curl -o "$TEMP_FILE" -L "https://github.com/Muhumuza7325/OMD/raw/main/nutrition_and_food_technology_tutorial_wsl.sh" && mv "$TEMP_FILE" nutrition_and_food_technology_tutorial_wsl.sh && chmod +x nutrition_and_food_technology_tutorial_wsl.sh && echo -e "\n\n${y}Code successfully updated.. You will have to restart a new session${t} \c" && sleep 4 && exit || (echo -e "\n\n${m}Error updating code!... Please check your internet connection and try again!${t} \c" && rm -f "$TEMP_FILE" && return)
    fi
    if [[ "$user_input" == "xx" ]]; then
      ...
      if [[ "$input" == "y" || "$input" == "Y" ]]; then
        ...
        curl -O -L "https://github.com/Muhumuza7325/OMD/raw/main/update_nutrition_and_food_technology.sh" || { echo -e "\n\n${m}Check your internet connection and try again!${t}" >&2; return; }
        mv update_nutrition_and_food_technology.sh .update_nutrition_and_food_technology.sh
        bash .update_nutrition_and_food_technology.sh
        return
      else
        return
      fi
    ...
    if [[ -n "$result" ]]; then
      ...
      if echo "$result" | grep "Figure"; then
        echo "$result" | grep -o '\bFigure[0-9]\+.*\.jpg\(\.[0-9]\+\)*\b' > Figures/Nutrition_and_food_technology/figures.txt
        sed -i -e '/^Figure/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Figures/Nutrition_and_food_technology/figures.txt
        # Change to the "Figures/Nutrition_and_food_technology" directory
        cd Figures/Nutrition_and_food_technology || { echo "Failed to change to Figures/Nutrition_and_food_technology"; return; }
        ...
        rm -f Figures/Nutrition_and_food_technology/figures.txt
      fi

      if echo "$result" | grep "Table"; then
        echo "$result" | grep -o '\bTable[0-9]\+\(\.[0-9]\+\)*\b' > Tables/Nutrition_and_food_technology/tables.txt
        sed -i -e '/^Table/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Tables/Nutrition_and_food_technology/tables.txt
        cd Tables/Nutrition_and_food_technology || { echo -e "\nFailed to change to Tables/Nutrition_and_food_technology \c"; return; }
        ...
        # Go back to the original directory
        cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        rm -f Tables/Nutrition_and_food_technology/tables.txt
      fi

      if echo "$result" | grep "Video"; then
        #echo "$result" > Videos/Nutrition_and_food_technology/videos.txt
        echo "$result" | grep -o '\bVideo[0-9]\+\(\.[0-9]\+\)*\b' > Videos/Nutrition_and_food_technology/videos.txt
        # Remove lines not starting with "Video" and any leading/trailing whitespaces
        sed -i -e '/^Video/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Videos/Nutrition_and_food_technology/videos.txt
        # Change to the "Videos/Nutrition_and_food_technology" directory
        cd Videos/Nutrition_and_food_technology || { echo -e "\nFailed to change to Videos/Nutrition_and_food_technology \c"; return; }
        ...
        cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        # Remove the temporary file
        rm -f Videos/Nutrition_and_food_technology/videos.txt
      fi
  ...
  if [ -f "$selected_file" ]; then
    ...
    if [[ -n "$selected_question" && "$selected_question" =~ [[:graph:]] ]]; then
      ...
      if echo "$selected_question" | grep "Figure"; then
        echo "$selected_question" | grep -o '\bFigure[0-9]\+[^;]*\b' > ../../Figures/Nutrition_and_food_technology/figures.txt
        sed -i -e '/^Figure/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' ../../Figures/Nutrition_and_food_technology/figures.txt
        # Change to the "Figures/Nutrition_and_food_technology" directory
        cd ../../Figures/Nutrition_and_food_technology || { echo "Failed to change to Figures/Nutrition_and_food_technology"; return; }
        # Specify the path to the text file containing figure names
        ...
        # Go back to the original directory
        cd ../../Revision/Nutrition_and_food_technology || { echo "Failed to change back to the targeted directory \c"; exit 1; }
        # Remove the temporary file
        rm -f ../../Figures/Nutrition_and_food_technology/figures.txt
      fi
...
process_question_answer() {
  ...
  if [ -f ../../../.school_name ]; then
    ...
    sed -i '/^[[:space:]]*$/d' ../../../."$school_name"_students_file.txt
    existing_class=$(awk '{print $1}' ../../../.nutrition_and_food_technology_user_state)
    existing_topic=$(awk '{print $2}' ../../../.nutrition_and_food_technology_user_state)
    echo ''
    ...
    if grep -q "Nutrition_and_food_technology" ../../../."$school_name"_students_file.txt; then
      sed -i -E 's/;/\n/g' ../../../."$school_name"_students_file.txt
      # Find the line with the word Nutrition_and_food_technology, replace the information below it with the already available information adding a comma existing_class_existing_topic [$percentage]
      if [ "$existing_topic" == "r" ]; then
        sed -i '/Nutrition_and_food_technology/{n;s/\(.*\)/\1, '"$existing_class"'_'"$input"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
      else
        (( existing_topic-- ))
        sed -i '/Nutrition_and_food_technology/{n;s/\(.*\)/\1, '"$existing_class"'_'"$existing_topic"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
      fi
    else
      if [ "$existing_topic" == "r" ]; then
        sed -i -E '/^School/ i\Nutrition_and_food_technology\n'"${existing_class} ${names}[${percentage}%]"'' ../../../."$school_name"_students_file.txt
        echo -e "Nutrition_and_food_technology\n"$existing_class"_'"$input"' "$names"["$percentage%"]" >> ../../../."$school_name"_students_file.txt
      else
        (( existing_topic-- ))
...
function resume_from_last_point() {
  if [ -f .nutrition_and_food_technology_user_state ]; then
    last_class=$(awk -F' ' '{print $1}' .nutrition_and_food_technology_user_state)
    last_topic=$(awk -F' ' '{print $2}' .nutrition_and_food_technology_user_state)
    if [ -n "$last_class" ] && [ -n "$last_topic" ]; then
      echo -e "\n       Resuming from ${r}S$last_class${t} : ${g}Topic '$last_topic'${t} \c"
      rm -f .nutrition_and_food_technology_surveyor
...
process_final_assignment() {
  # Check if user is.nutrition_and_food_technology_ready for the assignment
  # Check if .current_nutrition_and_food_technology_class is accidentally empty
  if [ ! -s .current_nutrition_and_food_technology_class ]; then
    # Echo 1 to .current_nutrition_and_food_technology_class
    echo "1" > .current_nutrition_and_food_technology_class
  fi
  # Read the value from the .current_nutrition_and_food_technology_class file
  current_nutrition_and_food_technology_class=$(<.current_nutrition_and_food_technology_class) 2>/dev/null
  # Check if the value in the .nutrition_and_food_technology_ready file is equal to $class
  # Read the value from the .nutrition_and_food_technology_ready file
  if [ ! -s .nutrition_and_food_technology_ready ]; then
    echo "0" > .nutrition_and_food_technology_ready
  fi
  how.nutrition_and_food_technology_ready=$(<.nutrition_and_food_technology_ready) 2>/dev/null
  # Check if the value is equal to $class
  if [ "$how.nutrition_and_food_technology_ready" -lt "$current_nutrition_and_food_technology_class" 2>/dev/null ]; then
    read -rp $'\n\nYou havent done all the topic assignments for your current nutrition_and_food_technology class\n\n'"${r}Proceeding from here will affect your very final score${y}"'. To go back and progress right, enter '"${y}ok${t}"'. Otherwise, press the Enter key to do the final class assignment: ' progress
    if [ "$progress" == "ok" ]; then
      return
    fi
  fi
    ...
    if ! [ "$total_questions" == 100 ]; then
      ...
      if [ "$elapsed_time" -gt 3600 ]; then
        ...
        if [ "$percentage" -lt 95 ]; then
          ...
          cd - > /dev/null 2>&1 || exit
          if ! [ -f .current_nutrition_and_food_technology_class ]; then
            # Echo the result to the file .current_class
            echo "2" > .current_nutrition_and_food_technology_class
            ...
            # Read the value from the file
            current_nutrition_and_food_technology_class=$(<.current_nutrition_and_food_technology_class) 2>/dev/null
            # Check if the new_class value is lt $current_nutrition_and_food_technology_class
            if [ "$new_class" -gt "$current_nutrition_and_food_technology_class" 2>/dev/null ]; then
		          # Echo the result to the file .current_class
		          echo "$new_class" > .current_nutrition_and_food_technology_class
            fi
...
get_sample_items() {
  ...
  # Save the current working directory
  pushd . > /dev/null
  cd Revision/Nutrition_and_food_technology || { echo "Directory not found"; return; }
  ...
  while true; do
      ...
      if [ ! -f "$last_echo_time_file" ]; then
    	...
    	if [ $time_diff -gt 3600 ]; then
      	# Echo the message and update the last echo time
	    	echo -e "\n\n\n${r}You are advised to not make any changes to the provided answers, instead, you can make copies that you can edit${t}\n\n${y}For a teacher willing to join us reach out to everyone of our children, please send us your questions and answers in a file labelled with your name, school, nutrition_and_food_technology, and file content (e.g., Muhumuza_Omega_Kasule_High_School_O_level_Chemistry_Answered_EOC1_Items.pdf) to our contacts${t}\n\n\nEmail: ${g}2024omd256@gmail.com${t} \c"
				echo $current_time > "$last_echo_time_file"
  		fi
      read -rp $'\n\n\nEnter '"${m}any character${t}"' for access to the file of answered items or simply press '"${r}Enter${t}"' to get items to attempt : ' input
     	if [[ -n $input ]]; then
     		explorer.exe .nutrition_and_food_technology_samples* > /dev/null 2>&1 &
      	clear
      	popd > /dev/null || exit
        return
      fi
      if [ -f .e_o_c_nutrition_and_food_technology.txt ]; then
        echo -e "\n\n${y}Below is the list of the elements of construct${t} \n"
        cat .e_o_c_nutrition_and_food_technology.txt
        ...
        if [[ -n $input ]]; then
          ...
          selected_file1=$(ls -a | grep -E "\.e_o_c_nutrition_and_food_technology_${input}\.txt")
          cat "$selected_file1"
          # Remove empty lines from the selected files
          find . -type f -name "*_samples_[0-9].txt" -exec sed -i '/^[[:space:]]*$/d' {} +
          selected_file=$(ls -a | grep -E "\.e_o_c_nutrition_and_food_technology_${input}_samples" | shuf -n 1)
        else
    ...
    rm -f .revise.txt .nutrition_and_food_technology_topic_selected 2>/dev/null
  done
  popd > /dev/null || exit
	return
}
...
while true; do
  ...
  if ls Students | grep -wq "$initials"; then
    ...
    echo -e "                  $initials\n" > "Students/$initials/Exercise/nutrition_and_food_technology_answered_ans.txt"
    echo -e "                  $initials\n" > "Students/$initials/Revision/nutrition_and_food_technology_covered_qns.txt"
    ...
    if [ -f ../../../.school_name ]; then
      ...
      if grep -q "Nutrition_and_food_technology" ../../../."$school_name"_students_file.txt; then
        ...
        if [ "$existing_topic" == "z" ]; then
          sed -i '/Nutrition_and_food_technology/{n;s/\(.*\)/\1, '"$existing_class"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
        else
          (( existing_topic-- ))
          sed -i '/Nutrition_and_food_technology/{n;s/\(.*\)/\1, '"$existing_class"'_'"$existing_topic"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
        fi
...
if [ ! -f .nutrition_and_food_technology_user_state ]; then
	touch .nutrition_and_food_technology_user_state
	touch .nutrition_and_food_technology_surveyor
	get_and_display_pattern
else
  if [ -f .omd_communication.txt ]; then
    ...
    process_random_reminder .nutrition_and_food_technology_reminder
  fi
	handle_resume_input
fi

if [ -z "$class" ] && [ -s ".nutrition_and_food_technology_user_state" ]; then
  get_and_display_pattern
fi
# Check for the presence of specific directories and a file
if ! [ -d "Notes/Nutrition_and_food_technology" ] || ! [ -d "Revision" ] || ! [ -d "Exercise" ] || ! [ -d "Videos" ] || ! [ -d "Figures" ] || ! [ -d "Tables" ]; then
  ...
  mkdir -p Notes Notes/Nutrition_and_food_technology Revision Revision/Nutrition_and_food_technology Revision/Nutrition_and_food_technology/{S1,S2,S3,S4} Exercise Exercise/Nutrition_and_food_technology Exercise/Nutrition_and_food_technology/{S1,S2,S3,S4} Videos Videos/Nutrition_and_food_technology Figures Figures/Nutrition_and_food_technology Tables Tables/Nutrition_and_food_technology
  touch Revision/nutrition_and_food_technology_covered_qns.txt Exercise/nutrition_and_food_technology_answered_ans.txt
  echo -e "\n"
  pwd
...
if [ -z "$(find ./Notes/Nutrition_and_food_technology -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
  ...
  if [[ -z "$user_input" ]]; then
    ...
    curl -O -L https://github.com/Muhumuza7325/OMD/raw/main/NAFT/1.1.introduction_to_nutrition_and_food_technology.txt || echo -e "\n\nError fetching material for this tutorial \c"
    ...
    cp 1.1.introduction_to_nutrition_and_food_technology.txt Notes/Nutrition_and_food_technology || echo -e "\n\nError copying 1.1.introduction_to_nutrition_and_food_technology.txt to the Nutrition_and_food_technology directory in the Notes directory \c"
    wait_for_a_key_press
  else
  ...
  rm -f 1.1.introduction_to_nutrition_and_food_technology.txt
fi
...
while true; do
  ...
  handle_class_input
  if [[ "$class" == "1" ]]; then
    if ! find . -maxdepth 1 -name '.s_nutrition_and_food_technology_1*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to the S1 Nutrition and food technology class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
    ...
    do
      ...
      do
        ...
        then
          ...
          question_directory="Revision/Nutrition_and_food_technology/S1"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../nutrition_and_food_technology_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Nutrition_and_food_technology/S1"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
          # Call the function to process a random question
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_nutrition_and_food_technology_1_1" ]; then
                echo -e "\n\nYou chose to explore Introduction to nutrition and food technology ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/Nutrition_and_food_technology/1.1.introduction_to_nutrition_and_food_technology.txt" . || exit 1
              mv 1.1.introduction_to_nutrition_and_food_technology.txt .1.1.introduction_to_nutrition_and_food_technology.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.1.introduction_to_nutrition_and_food_technology.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.1.introduction_to_nutrition_and_food_technology.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.1.introduction_to_nutrition_and_food_technology.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.1.introduction_to_nutrition_and_food_technology.txt
              process_reminders_from_file .1.1.introduction_to_nutrition_and_food_technology.txt
              STATE_FILE=".s_nutrition_and_food_technology_1_1"
              process_file .1.1.introduction_to_nutrition_and_food_technology.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.1.introduction_to_nutrition_and_food_technology.txt
              sed -i '/^1$/!d' .s_nutrition_and_food_technology_1_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Nutrition_and_food_technology/S1"
              # Define the file extension
              file_extension=".1.introduction_to_nutrition_and_food_technology.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              ...
              question_directory="Revision/Nutrition_and_food_technology/S1"
              # Define the file extension
              file_extension_question=".1.introduction_to_nutrition_and_food_technology.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".nutrition_and_food_technology.1.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Nutrition_and_food_technology/S1"
                # Define the file extension
                file_extension_answer=".1.introduction_to_nutrition_and_food_technology.ans.txt"
                # Define the exercise file
                exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .nutrition_and_food_technology.1.1
              fi
              if ! [ -f ".s_nutrition_and_food_technology_1_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Kitchen equipment and planning ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Nutrition_and_food_technology/1.2.kitchen_equipment_and_planning.txt" . || exit 1
              mv 1.2.kitchen_equipment_and_planning.txt .1.2.kitchen_equipment_and_planning.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.2.kitchen_equipment_and_planning.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.2.kitchen_equipment_and_planning.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.2.kitchen_equipment_and_planning.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.2.kitchen_equipment_and_planning.txt
              process_reminders_from_file .1.2.kitchen_equipment_and_planning.txt
              STATE_FILE=".s_nutrition_and_food_technology_1_2"
              process_file .1.2.kitchen_equipment_and_planning.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.2.kitchen_equipment_and_planning.txt
              sed -i '/^1$/!d' .s_nutrition_and_food_technology_1_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Nutrition_and_food_technology/S1"
              # Define the file extension
              file_extension_question=".2.kitchen_equipment_and_planning.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".nutrition_and_food_technology.1.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Nutrition_and_food_technology/S1"
                # Define the file extension
                file_extension_answer=".2.kitchen_equipment_and_planning.ans.txt"
                # Define the exercise file
                exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .nutrition_and_food_technology.1.2
              fi
              if ! [ -f ".s_nutrition_and_food_technology_1_3" ]; then
                echo -e "\n\nYou have made a choice to cover Safety in the home ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Nutrition_and_food_technology/1.3.safety_in_the_home.txt" . || exit 1
              mv 1.3.safety_in_the_home.txt .1.3.safety_in_the_home.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.3.safety_in_the_home.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.3.safety_in_the_home.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.3.safety_in_the_home.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.3.safety_in_the_home.txt
              process_reminders_from_file .1.3.safety_in_the_home.txt
              STATE_FILE=".s_nutrition_and_food_technology_1_3"
              process_file .1.3.safety_in_the_home.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.3.safety_in_the_home.txt
              sed -i '/^1$/!d' .s_nutrition_and_food_technology_1_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Nutrition_and_food_technology/S1"
              # Define the file extension
              file_extension_question=".3.safety_in_the_home.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".nutrition_and_food_technology.1.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Nutrition_and_food_technology/S1"
                # Define the file extension
                file_extension_answer=".3.safety_in_the_home.ans.txt"
                # Define the exercise file
                exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .nutrition_and_food_technology.1.3
              fi
              if ! [ -f ".s_nutrition_and_food_technology_1_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Proteins ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/Nutrition_and_food_technology/1.4.proteins.txt" . || exit 1
              mv 1.4.proteins.txt .1.4.proteins.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.4.proteins.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.4.proteins.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.4.proteins.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.4.proteins.txt
              process_reminders_from_file .1.4.proteins.txt
              STATE_FILE=".s_nutrition_and_food_technology_1_4"
              process_file .1.4.proteins.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.4.proteins.txt
              sed -i '/^1$/!d' .s_nutrition_and_food_technology_1_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Nutrition_and_food_technology/S1"
              # Define the file extension
              file_extension=".4.proteins.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Nutrition_and_food_technology/S1"
              # Define the file extension
              file_extension_question=".4.proteins.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            5)
              if ! [ -f ".nutrition_and_food_technology.1.4" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Nutrition_and_food_technology/S1"
                # Define the file extension
                file_extension_answer=".4.proteins.ans.txt"
                # Define the exercise file
                exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .nutrition_and_food_technology.1.4
              fi
              if ! [ -f ".s_nutrition_and_food_technology_1_5" ]; then
                echo -e "\n\nHere you are dear one... Stay organised as you explore Carbohydrates ...\n\n${g}Just know we are not going to leave you alone${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Nutrition_and_food_technology/1.5.carbohydrates.txt" . || exit 1
              mv 1.5.carbohydrates.txt .1.5.carbohydrates.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.5.carbohydrates.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.5.carbohydrates.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.5.carbohydrates.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.5.carbohydrates.txt
              process_reminders_from_file .1.5.carbohydrates.txt
              STATE_FILE=".s_nutrition_and_food_technology_1_5"
              process_file .1.5.carbohydrates.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.5.carbohydrates.txt
              sed -i '/^1$/!d' .s_nutrition_and_food_technology_1_5
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Nutrition_and_food_technology/S1"
              # Define the file extension
              file_extension_question=".5.carbohydrates.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            6)
              if ! [ -f ".nutrition_and_food_technology.1.5" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Nutrition_and_food_technology/S1"
                # Define the file extension
                file_extension_answer=".5.carbohydrates.ans.txt"
                # Define the exercise file
                exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .nutrition_and_food_technology.1.5
              fi
              if ! [ -f ".s_nutrition_and_food_technology_1_6" ]; then
                echo -e "\n\nYou have managed to make it to Mineral elements ...\n\n${g}Remember to pray always${t}\n\nThe fear of the Lord is the beginning of wisdom \c" && wait_for_a_key_press
              fi
              cp "Notes/Nutrition_and_food_technology/1.6.mineral_elements.txt" . || exit 1
              mv 1.6.mineral_elements.txt .1.6.mineral_elements.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.6.mineral_elements.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.6.mineral_elements.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.6.mineral_elements.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.6.mineral_elements.txt
              process_reminders_from_file .1.6.mineral_elements.txt
              STATE_FILE=".s_nutrition_and_food_technology_1_6"
              process_file .1.6.mineral_elements.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.6.mineral_elements.txt
              sed -i '/^1$/!d' .s_nutrition_and_food_technology_1_6
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Nutrition_and_food_technology/S1"
              # Define the file extension
              file_extension_question=".6.mineral_elements.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            7)
              if ! [ -f ".nutrition_and_food_technology.1.6" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Nutrition_and_food_technology/S1"
                # Define the file extension
                file_extension_answer=".6.mineral_elements.ans.txt"
                # Define the exercise file
                exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .nutrition_and_food_technology.1.6
              fi
              if ! [ -f ".s_nutrition_and_food_technology_1_7" ]; then
                echo -e "\n\nFrom here, you will be proceeding with Processing and preservation of vegetables ...\n\n${g}Please never ever forget that your education is your future${t}\n\nFocus dear \c" && wait_for_a_key_press
              fi
              cp "Notes/Nutrition_and_food_technology/1.7.processing_and_preservation_of_vegetables.txt" . || exit 1
              mv 1.7.processing_and_preservation_of_vegetables.txt .1.7.processing_and_preservation_of_vegetables.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.7.processing_and_preservation_of_vegetables.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.7.processing_and_preservation_of_vegetables.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.7.processing_and_preservation_of_vegetables.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.7.processing_and_preservation_of_vegetables.txt
              process_reminders_from_file .1.7.processing_and_preservation_of_vegetables.txt
              STATE_FILE=".s_nutrition_and_food_technology_1_7"
              process_file .1.7.processing_and_preservation_of_vegetables.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.7.processing_and_preservation_of_vegetables.txt
              sed -i '/^1$/!d' .s_nutrition_and_food_technology_1_7
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Nutrition_and_food_technology/S1"
              # Define the file extension
              file_extension_question=".7.processing_and_preservation_of_vegetables.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".nutrition_and_food_technology.1.7" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/Nutrition_and_food_technology/S1"
	              # Define the file extension
	              file_extension_answer=".7.processing_and_preservation_of_vegetables.ans.txt"
	              # Define the exercise file
	              exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
								touch .nutrition_and_food_technology.1.7
								echo "1" > .nutrition_and_food_technology_ready
							fi
            ;;
    ...
    if ! find . -maxdepth 1 -name '.s_nutrition_and_food_technology_2*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to the S2 Nutrition and Food technology class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
      clear_and_center "There are ${r}7${t} topics to be covered. Your tasks will always expand or shrink to fit in the time you give them. For that reason, never procrastinate darling!"
    fi
    attempts=0
    ...
    do
      ...
      do
        ...
        then
          attempts=0
          # Define the targeted directory
          question_directory="Revision/Nutrition_and_food_technology/S2"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../nutrition_and_food_technology_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Nutrition_and_food_technology/S2"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_nutrition_and_food_technology_2_1" ]; then
                echo -e "\n\nYou chose to explore Lipids ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/Nutrition_and_food_technology/2.1.lipids.txt" . || exit 1
              mv 2.1.lipids.txt .2.1.lipids.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.1.lipids.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.1.lipids.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.1.lipids.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.1.lipids.txt
              process_reminders_from_file .2.1.lipids.txt
              STATE_FILE=".s_nutrition_and_food_technology_2_1"
              process_file .2.1.lipids.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.1.lipids.txt
              sed -i '/^1$/!d' .s_nutrition_and_food_technology_2_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Nutrition_and_food_technology/S2"
              # Define the file extension
              file_extension=".1.lipids.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Nutrition_and_food_technology/S2"
              # Define the file extension
              file_extension_question=".1.lipids.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".nutrition_and_food_technology.2.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Nutrition_and_food_technology/S2"
                # Define the file extension
                file_extension_answer=".1.lipids.ans.txt"
                # Define the exercise file
                exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .nutrition_and_food_technology.2.1
              fi
              if ! [ -f ".s_nutrition_and_food_technology_2_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Vitamins ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Nutrition_and_food_technology/2.2.vitamins.txt" . || exit 1
              mv 2.2.vitamins.txt .2.2.vitamins.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.2.vitamins.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.2.vitamins.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.2.vitamins.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.2.vitamins.txt
              process_reminders_from_file .2.2.vitamins.txt
              STATE_FILE=".s_nutrition_and_food_technology_2_2"
              process_file .2.2.vitamins.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.2.vitamins.txt
              sed -i '/^1$/!d' .s_nutrition_and_food_technology_2_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Nutrition_and_food_technology/S2"
              # Define the file extension
              file_extension_question=".2.vitamins.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".nutrition_and_food_technology.2.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Nutrition_and_food_technology/S2"
                # Define the file extension
                file_extension_answer=".2.vitamins.ans.txt"
                # Define the exercise file
                exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .nutrition_and_food_technology.2.2
              fi
              if ! [ -f ".s_nutrition_and_food_technology_2_3" ]; then
                echo -e "\n\nYou have made a choice to cover Water ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Nutrition_and_food_technology/2.3.water.txt" . || exit 1
              mv 2.3.water.txt .2.3.water.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.3.water.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.3.water.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.3.water.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.3.water.txt
              process_reminders_from_file .2.3.water.txt
              STATE_FILE=".s_nutrition_and_food_technology_2_3"
              process_file .2.3.water.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.3.water.txt
              sed -i '/^1$/!d' .s_nutrition_and_food_technology_2_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Nutrition_and_food_technology/S2"
              # Define the file extension
              file_extension_question=".3.water.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".nutrition_and_food_technology.2.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Nutrition_and_food_technology/S2"
                # Define the file extension
                file_extension_answer=".3.water.ans.txt"
                # Define the exercise file
                exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .nutrition_and_food_technology.2.3
              fi
              if ! [ -f ".s_nutrition_and_food_technology_2_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Processing foods from vegetable seeds ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/Nutrition_and_food_technology/2.4.processing_foods_from_vegetable_seeds.txt" . || exit 1
              mv 2.4.processing_foods_from_vegetable_seeds.txt .2.4.processing_foods_from_vegetable_seeds.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.4.processing_foods_from_vegetable_seeds.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.4.processing_foods_from_vegetable_seeds.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.4.processing_foods_from_vegetable_seeds.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.4.processing_foods_from_vegetable_seeds.txt
              process_reminders_from_file .2.4.processing_foods_from_vegetable_seeds.txt
              STATE_FILE=".s_nutrition_and_food_technology_2_4"
              process_file .2.4.processing_foods_from_vegetable_seeds.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.4.processing_foods_from_vegetable_seeds.txt
              sed -i '/^1$/!d' .s_nutrition_and_food_technology_2_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Nutrition_and_food_technology/S2"
              # Define the file extension
              file_extension=".4.processing_foods_from_vegetable_seeds.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Nutrition_and_food_technology/S2"
              # Define the file extension
              file_extension_question=".4.processing_foods_from_vegetable_seeds.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            5)
              if ! [ -f ".nutrition_and_food_technology.2.4" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Nutrition_and_food_technology/S2"
                # Define the file extension
                file_extension_answer=".4.processing_foods_from_vegetable_seeds.ans.txt"
                # Define the exercise file
                exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .nutrition_and_food_technology.2.4
              fi
              if ! [ -f ".s_nutrition_and_food_technology_2_5" ]; then
                echo -e "\n\nHere you are dear one... Stay organised as you explore Processing and preservation of fruits ...\n\n${g}Just know we are not going to leave you alone${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Nutrition_and_food_technology/2.5.processing_and_preservation_of_fruits.txt" . || exit 1
              mv 2.5.processing_and_preservation_of_fruits.txt .2.5.processing_and_preservation_of_fruits.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.5.processing_and_preservation_of_fruits.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.5.processing_and_preservation_of_fruits.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.5.processing_and_preservation_of_fruits.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.5.processing_and_preservation_of_fruits.txt
              process_reminders_from_file .2.5.processing_and_preservation_of_fruits.txt
              STATE_FILE=".s_nutrition_and_food_technology_2_5"
              process_file .2.5.processing_and_preservation_of_fruits.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.5.processing_and_preservation_of_fruits.txt
              sed -i '/^1$/!d' .s_nutrition_and_food_technology_2_5
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Nutrition_and_food_technology/S2"
              # Define the file extension
              file_extension_question=".5.processing_and_preservation_of_fruits.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            6)
              if ! [ -f ".nutrition_and_food_technology.2.5" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Nutrition_and_food_technology/S2"
                # Define the file extension
                file_extension_answer=".5.processing_and_preservation_of_fruits.ans.txt"
                # Define the exercise file
                exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .nutrition_and_food_technology.2.5
              fi
              if ! [ -f ".s_nutrition_and_food_technology_2_6" ]; then
                echo -e "\n\nYou have managed to make it to Common foods ...\n\n${g}Remember to pray always${t}\n\nThe fear of the Lord is the beginning of wisdom \c" && wait_for_a_key_press
              fi
              cp "Notes/Nutrition_and_food_technology/2.6.common_foods.txt" . || exit 1
              mv 2.6.common_foods.txt .2.6.common_foods.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.6.common_foods.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.6.common_foods.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.6.common_foods.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.6.common_foods.txt
              process_reminders_from_file .2.6.common_foods.txt
              STATE_FILE=".s_nutrition_and_food_technology_2_6"
              process_file .2.6.common_foods.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.6.common_foods.txt
              sed -i '/^1$/!d' .s_nutrition_and_food_technology_2_6
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Nutrition_and_food_technology/S2"
              # Define the file extension
              file_extension_question=".6.common_foods.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            7)
              if ! [ -f ".nutrition_and_food_technology.2.6" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Nutrition_and_food_technology/S2"
                # Define the file extension
                file_extension_answer=".6.common_foods.ans.txt"
                # Define the exercise file
                exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .nutrition_and_food_technology.2.6
              fi
              if ! [ -f ".s_nutrition_and_food_technology_2_7" ]; then
                echo -e "\n\nFrom here, you will be proceeding with Meal planning ...\n\n${g}Please never ever forget that your education is your future${t}\n\nFocus dear \c" && wait_for_a_key_press
              fi
              cp "Notes/Nutrition_and_food_technology/2.7.meal_planning.txt" . || exit 1
              mv 2.7.meal_planning.txt .2.7.meal_planning.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.7.meal_planning.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.7.meal_planning.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.7.meal_planning.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.7.meal_planning.txt
              process_reminders_from_file .2.7.meal_planning.txt
              STATE_FILE=".s_nutrition_and_food_technology_2_7"
              process_file .2.7.meal_planning.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.7.meal_planning.txt
              sed -i '/^1$/!d' .s_nutrition_and_food_technology_2_7
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Nutrition_and_food_technology/S2"
              # Define the file extension
              file_extension_question=".7.meal_planning.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".nutrition_and_food_technology.2.7" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/Nutrition_and_food_technology/S2"
	              # Define the file extension
	              file_extension_answer=".7.meal_planning.ans.txt"
	              # Define the exercise file
	              exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
								touch .nutrition_and_food_technology.2.7
								echo "2" > .nutrition_and_food_technology_ready
							fi
            ;;
    ...
    if ! find . -maxdepth 1 -name '.s_nutrition_and_food_technology_3*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to the S3 Nutrition and food technology class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
      clear_and_center "There are ${r}7${t} topics to be covered. Your tasks will always expand or shrink to fit in the time you give them. For that reason, never procrastinate darling!"
    fi
    attempts=0
    ...
    do
      ...
      do
        ...
        then
          attempts=0
          # Define the targeted directory
          question_directory="Revision/Nutrition_and_food_technology/S3"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../nutrition_and_food_technology_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Nutrition_and_food_technology/S3"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_nutrition_and_food_technology_3_1" ]; then
                echo -e "\n\nYou chose to explore Stocks, soups, sauces and gravy ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/Nutrition_and_food_technology/3.1.stocks_soups_sauces_and_gravy.txt" . || exit 1
              mv 3.1.stocks_soups_sauces_and_gravy.txt .3.1.stocks_soups_sauces_and_gravy.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.1.stocks_soups_sauces_and_gravy.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.1.stocks_soups_sauces_and_gravy.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.1.stocks_soups_sauces_and_gravy.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.1.stocks_soups_sauces_and_gravy.txt
              process_reminders_from_file .3.1.stocks_soups_sauces_and_gravy.txt
              STATE_FILE=".s_nutrition_and_food_technology_3_1"
              process_file .3.1.stocks_soups_sauces_and_gravy.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.1.stocks_soups_sauces_and_gravy.txt
              sed -i '/^1$/!d' .s_nutrition_and_food_technology_3_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Nutrition_and_food_technology/S3"
              # Define the file extension
              file_extension=".1.stocks_soups_sauces_and_gravy.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Nutrition_and_food_technology/S3"
              # Define the file extension
              file_extension_question=".1.stocks_soups_sauces_and_gravy.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".nutrition_and_food_technology.3.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Nutrition_and_food_technology/S3"
                # Define the file extension
                file_extension_answer=".1.stocks_soups_sauces_and_gravy.ans.txt"
                # Define the exercise file
                exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .nutrition_and_food_technology.3.1
              fi
              if ! [ -f ".s_nutrition_and_food_technology_3_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Sweets and puddings ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Nutrition_and_food_technology/3.2.sweets_and_puddings.txt" . || exit 1
              mv 3.2.sweets_and_puddings.txt .3.2.sweets_and_puddings.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.2.sweets_and_puddings.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.2.sweets_and_puddings.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.2.sweets_and_puddings.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.2.sweets_and_puddings.txt
              process_reminders_from_file .3.2.sweets_and_puddings.txt
              STATE_FILE=".s_nutrition_and_food_technology_3_2"
              process_file .3.2.sweets_and_puddings.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.2.sweets_and_puddings.txt
              sed -i '/^1$/!d' .s_nutrition_and_food_technology_3_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Nutrition_and_food_technology/S3"
              # Define the file extension
              file_extension_question=".2.sweets_and_puddings.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".nutrition_and_food_technology.3.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Nutrition_and_food_technology/S3"
                # Define the file extension
                file_extension_answer=".2.sweets_and_puddings.ans.txt"
                # Define the exercise file
                exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .nutrition_and_food_technology.3.2
              fi
              if ! [ -f ".s_nutrition_and_food_technology_3_3" ]; then
                echo -e "\n\nYou have made a choice to cover Food preparation (basic meals) ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Nutrition_and_food_technology/3.3.food_preparation_basic_meals.txt" . || exit 1
              mv 3.3.food_preparation_basic_meals.txt .3.3.food_preparation_basic_meals.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.3.food_preparation_basic_meals.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.3.food_preparation_basic_meals.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.3.food_preparation_basic_meals.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.3.food_preparation_basic_meals.txt
              process_reminders_from_file .3.3.food_preparation_basic_meals.txt
              STATE_FILE=".s_nutrition_and_food_technology_3_3"
              process_file .3.3.food_preparation_basic_meals.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.3.food_preparation_basic_meals.txt
              sed -i '/^1$/!d' .s_nutrition_and_food_technology_3_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Nutrition_and_food_technology/S3"
              # Define the file extension
              file_extension_question=".3.food_preparation_basic_meals.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".nutrition_and_food_technology.3.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Nutrition_and_food_technology/S3"
                # Define the file extension
                file_extension_answer=".3.food_preparation_basic_meals.ans.txt"
                # Define the exercise file
                exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .nutrition_and_food_technology.3.3
              fi
              if ! [ -f ".s_nutrition_and_food_technology_3_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Meals for special occasions ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/Nutrition_and_food_technology/3.4.meals_for_special_occasions.txt" . || exit 1
              mv 3.4.meals_for_special_occasions.txt .3.4.meals_for_special_occasions.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.4.meals_for_special_occasions.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.4.meals_for_special_occasions.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.4.meals_for_special_occasions.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.4.meals_for_special_occasions.txt
              process_reminders_from_file .3.4.meals_for_special_occasions.txt
              STATE_FILE=".s_nutrition_and_food_technology_3_4"
              process_file .3.4.meals_for_special_occasions.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.4.meals_for_special_occasions.txt
              sed -i '/^1$/!d' .s_nutrition_and_food_technology_3_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Nutrition_and_food_technology/S3"
              # Define the file extension
              file_extension=".4.meals_for_special_occasions.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Nutrition_and_food_technology/S3"
              # Define the file extension
              file_extension_question=".4.meals_for_special_occasions.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            5)
              if ! [ -f ".nutrition_and_food_technology.3.4" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Nutrition_and_food_technology/S3"
                # Define the file extension
                file_extension_answer=".4.meals_for_special_occasions.ans.txt"
                # Define the exercise file
                exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .nutrition_and_food_technology.3.4
              fi
              if ! [ -f ".s_nutrition_and_food_technology_3_5" ]; then
                echo -e "\n\nHere you are dear one... Stay organised as you explore Milk and milk products ...\n\n${g}Just know we are not going to leave you alone${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Nutrition_and_food_technology/3.5.milk_and_milk_products.txt" . || exit 1
              mv 3.5.milk_and_milk_products.txt .3.5.milk_and_milk_products.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.5.milk_and_milk_products.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.5.milk_and_milk_products.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.5.milk_and_milk_products.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.5.milk_and_milk_products.txt
              process_reminders_from_file .3.5.milk_and_milk_products.txt
              STATE_FILE=".s_nutrition_and_food_technology_3_5"
              process_file .3.5.milk_and_milk_products.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.5.milk_and_milk_products.txt
              sed -i '/^1$/!d' .s_nutrition_and_food_technology_3_5
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Nutrition_and_food_technology/S3"
              # Define the file extension
              file_extension_question=".5.milk_and_milk_products.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            6)
              if ! [ -f ".nutrition_and_food_technology.3.5" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Nutrition_and_food_technology/S3"
                # Define the file extension
                file_extension_answer=".5.milk_and_milk_products.ans.txt"
                # Define the exercise file
                exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .nutrition_and_food_technology.3.5
              fi
              if ! [ -f ".s_nutrition_and_food_technology_3_6" ]; then
                echo -e "\n\nYou have managed to make it to Yeast cookery ...\n\n${g}Remember to pray always${t}\n\nThe fear of the Lord is the beginning of wisdom \c" && wait_for_a_key_press
              fi
              cp "Notes/Nutrition_and_food_technology/3.6.yeast_cookery.txt" . || exit 1
              mv 3.6.yeast_cookery.txt .3.6.yeast_cookery.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.6.yeast_cookery.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.6.yeast_cookery.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.6.yeast_cookery.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.6.yeast_cookery.txt
              process_reminders_from_file .3.6.yeast_cookery.txt
              STATE_FILE=".s_nutrition_and_food_technology_3_6"
              process_file .3.6.yeast_cookery.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.6.yeast_cookery.txt
              sed -i '/^1$/!d' .s_nutrition_and_food_technology_3_6
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Nutrition_and_food_technology/S3"
              # Define the file extension
              file_extension_question=".6.yeast_cookery.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            7)
              if ! [ -f ".nutrition_and_food_technology.3.6" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Nutrition_and_food_technology/S3"
                # Define the file extension
                file_extension_answer=".6.yeast_cookery.ans.txt"
                # Define the exercise file
                exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .nutrition_and_food_technology.3.6
              fi
              if ! [ -f ".s_nutrition_and_food_technology_3_7" ]; then
                echo -e "\n\nFrom here, you will be proceeding with Confectioneries and pastries ...\n\n${g}Please never ever forget that your education is your future${t}\n\nFocus dear \c" && wait_for_a_key_press
              fi
              cp "Notes/Nutrition_and_food_technology/3.7.confectioneries_and_pastries.txt" . || exit 1
              mv 3.7.confectioneries_and_pastries.txt .3.7.confectioneries_and_pastries.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.7.confectioneries_and_pastries.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.7.confectioneries_and_pastries.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.7.confectioneries_and_pastries.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.7.confectioneries_and_pastries.txt
              process_reminders_from_file .3.7.confectioneries_and_pastries.txt
              STATE_FILE=".s_nutrition_and_food_technology_3_7"
              process_file .3.7.confectioneries_and_pastries.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.7.confectioneries_and_pastries.txt
              sed -i '/^1$/!d' .s_nutrition_and_food_technology_3_7
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Nutrition_and_food_technology/S3"
              # Define the file extension
              file_extension_question=".7.confectioneries_and_pastries.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".nutrition_and_food_technology.3.7" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/Nutrition_and_food_technology/S3"
	              # Define the file extension
	              file_extension_answer=".7.confectioneries_and_pastries.ans.txt"
	              # Define the exercise file
	              exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
								touch .nutrition_and_food_technology.3.7
								echo "3" > .nutrition_and_food_technology_ready
							fi
            ;;
    ...
    if ! find . -maxdepth 1 -name '.s_nutrition_and_food_technology_4*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to the S4 Nutrition and food technology class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
    ...
    do
      ...
      do
        ...
        then
          attempts=0
          # Define the targeted directory
          question_directory="Revision/Nutrition_and_food_technology/S4"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../nutrition_and_food_technology_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Nutrition_and_food_technology/S4"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_nutrition_and_food_technology_4_1" ]; then
                echo -e "\n\nYou chose to explore Nutrition and the life cycle ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/Nutrition_and_food_technology/4.1.nutrition_and_the_life_cycle.txt" . || exit 1
              mv 4.1.nutrition_and_the_life_cycle.txt .4.1.nutrition_and_the_life_cycle.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.1.nutrition_and_the_life_cycle.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.1.nutrition_and_the_life_cycle.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.1.nutrition_and_the_life_cycle.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.1.nutrition_and_the_life_cycle.txt
              process_reminders_from_file .4.1.nutrition_and_the_life_cycle.txt
              STATE_FILE=".s_nutrition_and_food_technology_4_1"
              process_file .4.1.nutrition_and_the_life_cycle.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.1.nutrition_and_the_life_cycle.txt
              sed -i '/^1$/!d' .s_nutrition_and_food_technology_4_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Nutrition_and_food_technology/S4"
              # Define the file extension
              file_extension=".1.nutrition_and_the_life_cycle.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Nutrition_and_food_technology/S4"
              # Define the file extension
              file_extension_question=".1.nutrition_and_the_life_cycle.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".nutrition_and_food_technology.4.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Nutrition_and_food_technology/S4"
                # Define the file extension
                file_extension_answer=".1.nutrition_and_the_life_cycle.ans.txt"
                # Define the exercise file
                exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .nutrition_and_food_technology.4.1
              fi
              if ! [ -f ".s_nutrition_and_food_technology_4_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Management of nutritional related diseases and disorders ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Nutrition_and_food_technology/4.2.management_of_nutritional_related_diseases_and_disorders.txt" . || exit 1
              mv 4.2.management_of_nutritional_related_diseases_and_disorders.txt .4.2.management_of_nutritional_related_diseases_and_disorders.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.2.management_of_nutritional_related_diseases_and_disorders.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.2.management_of_nutritional_related_diseases_and_disorders.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.2.management_of_nutritional_related_diseases_and_disorders.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.2.management_of_nutritional_related_diseases_and_disorders.txt
              process_reminders_from_file .4.2.management_of_nutritional_related_diseases_and_disorders.txt
              STATE_FILE=".s_nutrition_and_food_technology_4_2"
              process_file .4.2.management_of_nutritional_related_diseases_and_disorders.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.2.management_of_nutritional_related_diseases_and_disorders.txt
              sed -i '/^1$/!d' .s_nutrition_and_food_technology_4_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Nutrition_and_food_technology/S4"
              # Define the file extension
              file_extension_question=".2.management_of_nutritional_related_diseases_and_disorders.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".nutrition_and_food_technology.4.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Nutrition_and_food_technology/S4"
                # Define the file extension
                file_extension_answer=".2.management_of_nutritional_related_diseases_and_disorders.ans.txt"
                # Define the exercise file
                exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .nutrition_and_food_technology.4.2
              fi
              if ! [ -f ".s_nutrition_and_food_technology_4_3" ]; then
                echo -e "\n\nYou have made a choice to cover Processing meat, fish and poultry products ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Nutrition_and_food_technology/4.3.processing_meat_fish_and_poultry_products.txt" . || exit 1
              mv 4.3.processing_meat_fish_and_poultry_products.txt .4.3.processing_meat_fish_and_poultry_products.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.3.processing_meat_fish_and_poultry_products.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.3.processing_meat_fish_and_poultry_products.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.3.processing_meat_fish_and_poultry_products.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.3.processing_meat_fish_and_poultry_products.txt
              process_reminders_from_file .4.3.processing_meat_fish_and_poultry_products.txt
              STATE_FILE=".s_nutrition_and_food_technology_4_3"
              process_file .4.3.processing_meat_fish_and_poultry_products.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.3.processing_meat_fish_and_poultry_products.txt
              sed -i '/^1$/!d' .s_nutrition_and_food_technology_4_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Nutrition_and_food_technology/S4"
              # Define the file extension
              file_extension_question=".3.processing_meat_fish_and_poultry_products.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".nutrition_and_food_technology.4.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Nutrition_and_food_technology/S4"
                # Define the file extension
                file_extension_answer=".3.processing_meat_fish_and_poultry_products.ans.txt"
                # Define the exercise file
                exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .nutrition_and_food_technology.4.3
              fi
              if ! [ -f ".s_nutrition_and_food_technology_4_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Importance of rechauffe and convenience foods in the diet ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/Nutrition_and_food_technology/4.4.importance_of_rechauffe_and_convenience_foods_in_the_diet.txt" . || exit 1
              mv 4.4.importance_of_rechauffe_and_convenience_foods_in_the_diet.txt .4.4.importance_of_rechauffe_and_convenience_foods_in_the_diet.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.4.importance_of_rechauffe_and_convenience_foods_in_the_diet.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.4.importance_of_rechauffe_and_convenience_foods_in_the_diet.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.4.importance_of_rechauffe_and_convenience_foods_in_the_diet.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.4.importance_of_rechauffe_and_convenience_foods_in_the_diet.txt
              process_reminders_from_file .4.4.importance_of_rechauffe_and_convenience_foods_in_the_diet.txt
              STATE_FILE=".s_nutrition_and_food_technology_4_4"
              process_file .4.4.importance_of_rechauffe_and_convenience_foods_in_the_diet.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.4.importance_of_rechauffe_and_convenience_foods_in_the_diet.txt
              sed -i '/^1$/!d' .s_nutrition_and_food_technology_4_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Nutrition_and_food_technology/S4"
              # Define the file extension
              file_extension=".4.importance_of_rechauffe_and_convenience_foods_in_the_diet.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Nutrition_and_food_technology/S4"
              # Define the file extension
              file_extension_question=".4.importance_of_rechauffe_and_convenience_foods_in_the_diet.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            5)
              if ! [ -f ".nutrition_and_food_technology.4.4" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Nutrition_and_food_technology/S4"
                # Define the file extension
                file_extension_answer=".4.importance_of_rechauffe_and_convenience_foods_in_the_diet.ans.txt"
                # Define the exercise file
                exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .nutrition_and_food_technology.4.4
              fi
              if ! [ -f ".s_nutrition_and_food_technology_4_5" ]; then
                echo -e "\n\nHere you are dear one... Stay organised as you explore Table setting and food service ...\n\n${g}Just know we are not going to leave you alone${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Nutrition_and_food_technology/4.5.table_setting_and_food_service.txt" . || exit 1
              mv 4.5.table_setting_and_food_service.txt .4.5.table_setting_and_food_service.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.5.table_setting_and_food_service.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.5.table_setting_and_food_service.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.5.table_setting_and_food_service.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.5.table_setting_and_food_service.txt
              process_reminders_from_file .4.5.table_setting_and_food_service.txt
              STATE_FILE=".s_nutrition_and_food_technology_4_5"
              process_file .4.5.table_setting_and_food_service.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.5.table_setting_and_food_service.txt
              sed -i '/^1$/!d' .s_nutrition_and_food_technology_4_5
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Nutrition_and_food_technology/S4"
              # Define the file extension
              file_extension_question=".5.table_setting_and_food_service.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            6)
              if ! [ -f ".nutrition_and_food_technology.4.5" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Nutrition_and_food_technology/S4"
                # Define the file extension
                file_extension_answer=".5.table_setting_and_food_service.ans.txt"
                # Define the exercise file
                exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
                touch .nutrition_and_food_technology.4.5
              fi
              if ! [ -f ".s_nutrition_and_food_technology_4_6" ]; then
                echo -e "\n\nYou have managed to make it to Consolidation of skills ...\n\n${g}Remember to pray always${t}\n\nThe fear of the Lord is the beginning of wisdom \c" && wait_for_a_key_press
              fi
              cp "Notes/Nutrition_and_food_technology/4.6.consolidation_of_skills.txt" . || exit 1
              mv 4.6.consolidation_of_skills.txt .4.6.consolidation_of_skills.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.6.consolidation_of_skills.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.6.consolidation_of_skills.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.6.consolidation_of_skills.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.6.consolidation_of_skills.txt
              process_reminders_from_file .4.6.consolidation_of_skills.txt
              STATE_FILE=".s_nutrition_and_food_technology_4_6"
              process_file .4.6.consolidation_of_skills.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.6.consolidation_of_skills.txt
              sed -i '/^1$/!d' .s_nutrition_and_food_technology_4_6
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Nutrition_and_food_technology/S4"
              # Define the file extension
              file_extension_question=".6.consolidation_of_skills.qns.txt"
              # Define the revision file
              revision_file="../../nutrition_and_food_technology_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".nutrition_and_food_technology.4.6" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/Nutrition_and_food_technology/S4"
	              # Define the file extension
	              file_extension_answer=".6.consolidation_of_skills.ans.txt"
	              # Define the exercise file
	              exercise_file="../../nutrition_and_food_technology_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
								touch .nutrition_and_food_technology.4.6
								echo "4" > .nutrition_and_food_technology_ready
							fi
            ;;
