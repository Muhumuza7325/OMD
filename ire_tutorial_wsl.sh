...
function handle_class_input() {
  if { [ -n "$last_class" ] || [ -z "$last_topic" ]; } && [ -f .ire_surveyor ]; then
		while true; do
	    read -rp $'\n\nPlease enter '"${r}1 or 2 or 3 or 4${t}"' to go to your class or '"${r}x${t}"' to exit'$'\n\n> ' class
      if [ "$class" == "x" ]; then
        exit
      fi
		  # Check if .current_ire_class is accidentally empty
		  if [ ! -s .current_ire_class ]; then
		    # Echo 1 to .current_ire_class
		    echo "1" > .current_ire_class
		  fi
		  # Read the value from the file
		  current_ire_class=$(<.current_ire_class) 2>/dev/null
		  # Check if the value is equal to $class
		  if [ "$current_ire_class" -lt "$class" 2>/dev/null ]; then
...
function handle_s4_topic_input() {
  ...
  if [ -z "$last_topic" ] || [ -f .ire_topic_selected ]; then
...
function handle_s1_topic_input() {
  ...
  if [ -z "$last_topic" ] || [ -f .ire_topic_selected ]; then
    read -rp $'\n\nChoose either topic '"${g}1 or 2 or 3${t}"' to learn'$'\nor enter '"${r}z${t}"' for an adventure or '"${r}r${t}"' to revise or '"${r}s${t}"' to get sample_items'$'\nor '"${r}a${t}"' to get an activity of integration or '"${r}q${t}"' to get a short answer question'$'\nor '"${r}n${t}"' to do your final class assignment and if necessary, gain access to the next class'$'\nor '"${r}p${t}"' to track academic progress or '"${r}x${t}"' to exit'$'\n\n1. Worship '"${r}Term1${t}"''$'\n\n2. Islamic rituals and celebrations '"${r}Term2${t}"''$'\n\n3. Islam and values in Christianity and African traditional religion '"${r}Term3${t}"''$'\n\n> ' topic
    touch .ire_surveyor
    touch .ire_topic_selected
    # Update the state file with the topic
    # Check if the state file exists, and the topic is not "x"
    ...
    for sentence in "${sentences[@]}"; do
      if [[ $echo_next == true ]]; then
        ...
        for split_sentence in "${split_sentences[@]}"; do
					echo "$split_sentence" >> .ire_reminder
        done
      echo_next=false
      fi
        ...
        echo "Did you know that $lower_cased_sentence" >> .ire_reminder
        ...
        echo_next=true
  			...
  			echo "Do you recall that $lower_cased_sentence;" >> .ire_reminder

			elif [[ "$sentence" =~ " → " ]]; then
  			echo "Hope you know that: $sentence;" >> .ire_reminder

      elif [[ "$sentence" =~ " ↔ " ]]; then
        echo "Hope you know that: $sentence;" >> .ire_reminder

      elif [[ "$sentence" =~ (Generally|general) ]]; then
        echo "Note: $sentence;" >> .ire_reminder
...
process_random_reminder() {
  ...
  # Check if .ire_reminder exists
  if [ -f .ire_reminder ]; then
		# Remove empty lines from .ire_reminder
  	sed -i '/^[[:space:]]*$/d' .ire_reminder 2>/dev/null
    # Randomly select a non-empty sentence
    local reminder # Declare the variable
		reminder=$(awk -v RS=';' 'BEGIN{srand();}{gsub(/^[[:space:]]+|[[:space:]]+$/, ""); if (length > 0) a[++n]=$0}END{if (n > 0) print a[int(rand()*n)+1]}' .ire_reminder)
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    for sentence in "${sentences[@]}"; do
        ...
        if [[ $sentence == *"Figure"* ]]; then
          ...
          # Change to the "Figures/Ire" directory
          cd Figures/Ire || { echo "Failed to change to Figures/Ire"; return; }
        ...
        if [[ $sentence == *"Table"* ]]; then
          cd Tables/Ire || { echo -e "\nFailed to change to Tables/Ire \c"; return; }
          explorer.exe "$sentence" > /dev/null 2>&1 &
          cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        fi

        if [[ $sentence == *"Video"* ]]; then
          cd Videos/Ire || { echo -e "\nFailed to change to Videos/Ire \c"; return; }
          explorer.exe "$sentence" > /dev/null 2>&1 &
          cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        fi
...
contact_ai() {
  last_topic=$(awk -F' ' '{print $2}' .ire_user_state)
...
while true; do
  ...
  # Prompt the user for input
  if [ -z "$(find ./Notes/Ire -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
  ...
  if [[ -n "$user_input" ]]; then
		if [[ "$user_input" == "sh" ]]; then
		  Response="We are looking forward to receiving your economic support, thoughts, suggestions, and any files you feel should reach out to everyone of our children. Please remember to label the files you are to attach using the format: Your_name_School_Ire_File_content (e.g., Muhumuza_Omega_Kasule_High_School_O_level_Chemistry_Answered_EOC1_Items.pdf). Thanks a lot for your contributions."  
		  ire="$(basename "$0") - $(date +"%Y-%m-%d %H:%M:%S") - Thoughts, suggestions, and contributions"
		  encoded_ire=$(echo "$ire" | sed 's/ /%20/g; s/\n/%0A/g')
		  encoded_body=$(echo "$Response" | sed 's/ /%20/g; s/\n/%0A/g')
		  # Open the email in the browser with the encoded ire and body
		  powershell.exe -Command "Start-Process 'https://mail.google.com/mail/?view=cm&to=2024omd256@gmail.com&su=${encoded_ire}&body=${encoded_body}'"
			return
		fi
    ...
    if [[ "$user_input" == "xx" ]]; then
      ...
      if [[ "$input" == "y" || "$input" == "Y" ]]; then
        ...
        curl -O -L "https://github.com/Muhumuza7325/OMD/raw/main/update_ire.sh" || { echo -e "\n\n${m}Check your internet connection and try again!${t}" >&2; return; }
        mv update_ire.sh .update_ire.sh
        bash .update_ire.sh
        return
      else
        return
    ...
    if [[ -n "$result" ]]; then
      ...
      if echo "$result" | grep "Figure"; then
        echo "$result" | grep -o '\bFigure[0-9]\+.*\.jpg\(\.[0-9]\+\)*\b' > Figures/Ire/figures.txt
        sed -i -e '/^Figure/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Figures/Ire/figures.txt
        # Change to the "Figures/Ire" directory
        cd Figures/Ire || { echo "Failed to change to Figures/Ire"; return; }
        ...
        rm -f Figures/Ire/figures.txt
      fi

      if echo "$result" | grep "Table"; then
        echo "$result" | grep -o '\bTable[0-9]\+\(\.[0-9]\+\)*\b' > Tables/Ire/tables.txt
        sed -i -e '/^Table/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Tables/Ire/tables.txt
        cd Tables/Ire || { echo -e "\nFailed to change to Tables/Ire \c"; return; }
        ...
        # Go back to the original directory
        cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        rm -f Tables/Ire/tables.txt
      fi

      if echo "$result" | grep "Video"; then
        #echo "$result" > Videos/Ire/videos.txt
        echo "$result" | grep -o '\bVideo[0-9]\+\(\.[0-9]\+\)*\b' > Videos/Ire/videos.txt
        # Remove lines not starting with "Video" and any leading/trailing whitespaces
        sed -i -e '/^Video/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Videos/Ire/videos.txt
        # Change to the "Videos/Ire" directory
        cd Videos/Ire || { echo -e "\nFailed to change to Videos/Ire \c"; return; }
  ...
  if [ -f "$selected_file" ]; then
    ...
    if [[ -n "$selected_question" && "$selected_question" =~ [[:graph:]] ]]; then
      ...
      if echo "$selected_question" | grep "Figure"; then
        echo "$selected_question" | grep -o '\bFigure[0-9]\+[^;]*\b' > ../../Figures/Ire/figures.txt
        sed -i -e '/^Figure/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' ../../Figures/Ire/figures.txt
        # Change to the "Figures/Ire" directory
        cd ../../Figures/Ire || { echo "Failed to change to Figures/Ire"; return; }
        # Specify the path to the text file containing figure names
        ...
        # Go back to the original directory
        cd ../../Revision/Ire || { echo "Failed to change back to the targeted directory \c"; exit 1; }
        # Remove the temporary file
        rm -f ../../Figures/Ire/figures.txt
      fi
...
process_question_answer() {
  ...
  if [ -f ../../../.school_name ]; then
    ...
    sed -i '/^[[:space:]]*$/d' ../../../."$school_name"_students_file.txt
    existing_class=$(awk '{print $1}' ../../../.ire_user_state)
    existing_topic=$(awk '{print $2}' ../../../.ire_user_state)
    echo ''
    ...
    if grep -q "Ire" ../../../."$school_name"_students_file.txt; then
      sed -i -E 's/;/\n/g' ../../../."$school_name"_students_file.txt
      # Find the line with the word Ire, replace the information below it with the already available information adding a comma existing_class_existing_topic [$percentage]
      if [ "$existing_topic" == "r" ]; then
        sed -i '/Ire/{n;s/\(.*\)/\1, '"$existing_class"'_'"$input"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
      else
        (( existing_topic-- ))
        sed -i '/Ire/{n;s/\(.*\)/\1, '"$existing_class"'_'"$existing_topic"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
      fi
    else
      if [ "$existing_topic" == "r" ]; then
        sed -i -E '/^School/ i\Ire\n'"${existing_class} ${names}[${percentage}%]"'' ../../../."$school_name"_students_file.txt
...
function resume_from_last_point() {
  if [ -f .ire_user_state ]; then
    last_class=$(awk -F' ' '{print $1}' .ire_user_state)
    last_topic=$(awk -F' ' '{print $2}' .ire_user_state)
...
process_final_assignment() {
  # Check if user is.ire_ready for the assignment
  # Check if .current_ire_class is accidentally empty
  if [ ! -s .current_ire_class ]; then
    # Echo 1 to .current_ire_class
    echo "1" > .current_ire_class
  fi
  # Read the value from the .current_ire_class file
  current_ire_class=$(<.current_ire_class) 2>/dev/null
  # Check if the value in the .ire_ready file is equal to $class
  # Read the value from the .ire_ready file
  if [ ! -s .ire_ready ]; then
    echo "0" > .ire_ready
  fi
  how.ire_ready=$(<.ire_ready) 2>/dev/null
  # Check if the value is equal to $class
  if [ "$how.ire_ready" -lt "$current_ire_class" 2>/dev/null ]; then
    read -rp $'\n\nYou havent done all the topic assignments for your current ire class\n\n'"${r}Proceeding from here will affect your very final score${y}"'. To go back and progress right, enter '"${y}ok${t}"'. Otherwise, press the Enter key to do the final class assignment: ' progress
    if [ "$progress" == "ok" ]; then
      return
    fi
  fi
    ...
    if ! [ "$total_questions" == 100 ]; then
      ...
      if [ "$elapsed_time" -gt 3600 ]; then
        ...
        if [ "$percentage" -lt 95 ]; then
          ...
          cd - > /dev/null 2>&1 || exit
          if ! [ -f .current_ire_class ]; then
            # Echo the result to the file .current_class
            echo "2" > .current_ire_class
            ...
            # Read the value from the file
            current_ire_class=$(<.current_ire_class) 2>/dev/null
            # Check if the new_class value is lt $current_ire_class
            if [ "$new_class" -gt "$current_ire_class" 2>/dev/null ]; then
		          # Echo the result to the file .current_class
		          echo "$new_class" > .current_ire_class
            fi
...
get_sample_items() {
  ...
  # Save the current working directory
  pushd . > /dev/null
  cd Revision/Ire || { echo "Directory not found"; return; }
  ...
  while true; do
      ...
      if [ ! -f "$last_echo_time_file" ]; then
    	...
    	if [ $time_diff -gt 3600 ]; then
      	# Echo the message and update the last echo time
	    	echo -e "\n\n\n${r}You are advised to not make any changes to the provided answers, instead, you can make copies that you can edit${t}\n\n${y}For a teacher willing to join us reach out to everyone of our children, please send us your questions and answers in a file labelled with your name, school, ire, and file content (e.g., Muhumuza_Omega_Kasule_High_School_O_level_Chemistry_Answered_EOC1_Items.pdf) to our contacts${t}\n\n\nEmail: ${g}2024omd256@gmail.com${t} \c"
				echo $current_time > "$last_echo_time_file"
  		fi
      read -rp $'\n\n\nEnter '"${m}any character${t}"' for access to the file of answered items or simply press '"${r}Enter${t}"' to get items to attempt : ' input
     	if [[ -n $input ]]; then
     		explorer.exe .ire_samples* > /dev/null 2>&1 &
      	clear
      	popd > /dev/null || exit
        return
      fi
      if [ -f .e_o_c_ire.txt ]; then
        ...
        if [[ -n $input ]]; then
          ...
          selected_file1=$(ls -a | grep -E "\.e_o_c_ire_${input}\.txt")
          cat "$selected_file1"
          # Remove empty lines from the selected files
          find . -type f -name "*_samples_[0-9].txt" -exec sed -i '/^[[:space:]]*$/d' {} +
          selected_file=$(ls -a | grep -E "\.e_o_c_ire_${input}_samples" | shuf -n 1)
        else
    ...
    rm -f .revise.txt .ire_topic_selected 2>/dev/null
  done
  popd > /dev/null || exit
	return
}
...
while true; do
  ...
  if ls Students | grep -wq "$initials"; then
    ...
    echo -e "                  $initials\n" > "Students/$initials/Exercise/ire_answered_ans.txt"
    echo -e "                  $initials\n" > "Students/$initials/Revision/ire_covered_qns.txt"
    ...
    if [ -f ../../../.school_name ]; then
      ...
      if grep -q "Ire" ../../../."$school_name"_students_file.txt; then
        ...
        if [ "$existing_topic" == "z" ]; then
          sed -i '/Ire/{n;s/\(.*\)/\1, '"$existing_class"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
        else
          (( existing_topic-- ))
          sed -i '/Ire/{n;s/\(.*\)/\1, '"$existing_class"'_'"$existing_topic"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
        fi
...
if [ ! -f .ire_user_state ]; then
  ...
  if [ -f .omd_communication.txt ]; then
    ...
    process_random_reminder .ire_reminder
  fi
	handle_resume_input
fi

if [ -z "$class" ] && [ -s ".ire_user_state" ]; then
  get_and_display_pattern
fi
# Check for the presence of specific directories and a file
if ! [ -d "Notes/Ire" ] || ! [ -d "Revision" ] || ! [ -d "Exercise" ] || ! [ -d "Videos" ] || ! [ -d "Figures" ] || ! [ -d "Tables" ]; then
  ...
  mkdir -p Notes Notes/Ire Revision Revision/Ire Revision/Ire/{S1,S2,S3,S4} Exercise Exercise/Ire Exercise/Ire/{S1,S2,S3,S4} Videos Videos/Ire Figures Figures/Ire Tables Tables/Ire
  touch Revision/ire_covered_qns.txt Exercise/ire_answered_ans.txt
  echo -e "\n"
  pwd
...
if [ -z "$(find ./Notes/Ire -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
  ...
  if [[ -z "$user_input" ]]; then
    ...
    curl -O -L https://github.com/Muhumuza7325/OMD/raw/main/IRE/1.1.worship.txt || echo -e "\n\nError fetching material for this tutorial \c"
    ...
    cp 1.1.worship.txt Notes/Ire || echo -e "\n\nError copying 1.1.worship.txt to the Ire directory in the Notes directory \c"
    wait_for_a_key_press
  else
...
while true; do
  ...
  handle_class_input
  if [[ "$class" == "1" ]]; then
    if ! find . -maxdepth 1 -name '.s_ire_1*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to S1 IRE class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
    ...
    do
      ...
      do
        ...
        then
          ...
          question_directory="Revision/Ire/S1"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../ire_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Ire/S1"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../ire_answered_ans.txt"
          # Call the function to process a random question
        ...
        then
          ...
          case "$topic" in
            1)
              ...
              cp "Notes/Ire/1.1.worship.txt" . || exit 1
            ...
            2)
              ...
              if ! [ -f ".s_ire_1_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Islamic rituals and celebrations ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Ire/1.2.islamic_rituals_and_celebrations.txt" . || exit 1
              mv 1.2.islamic_rituals_and_celebrations.txt .1.2.islamic_rituals_and_celebrations.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.2.islamic_rituals_and_celebrations.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.2.islamic_rituals_and_celebrations.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.2.islamic_rituals_and_celebrations.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.2.islamic_rituals_and_celebrations.txt
              process_reminders_from_file .1.2.islamic_rituals_and_celebrations.txt
              STATE_FILE=".s_ire_1_2"
              process_file .1.2.islamic_rituals_and_celebrations.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.2.islamic_rituals_and_celebrations.txt
              sed -i '/^1$/!d' .s_ire_1_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Ire/S1"
              # Define the file extension
              file_extension_question=".2.islamic_rituals_and_celebrations.qns.txt"
              # Define the revision file
              revision_file="../../ire_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            3)
              if ! [ -f ".ire.1.2" ]; then
                ...
                answered_directory="Exercise/Ire/S1"
                # Define the file extension
                file_extension_answer=".2.islamic_rituals_and_celebrations.ans.txt"
                # Define the exercise file
                exercise_file="../../ire_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_ire_1_3" ]; then
                echo -e "\n\nYou have made a choice to cover Islam and values, in, christianity, and, African, traditional, religion ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Ire/1.3.islam_and_values_in_christianity_and_african_traditional_religion.txt" . || exit 1
              mv 1.3.islam_and_values_in_christianity_and_african_traditional_religion.txt .1.3.islam_and_values_in_christianity_and_african_traditional_religion.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.3.islam_and_values_in_christianity_and_african_traditional_religion.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.3.islam_and_values_in_christianity_and_african_traditional_religion.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.3.islam_and_values_in_christianity_and_african_traditional_religion.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.3.islam_and_values_in_christianity_and_african_traditional_religion.txt
              process_reminders_from_file .1.3.islam_and_values_in_christianity_and_african_traditional_religion.txt
              STATE_FILE=".s_ire_1_3"
              process_file .1.3.islam_and_values_in_christianity_and_african_traditional_religion.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.3.islam_and_values_in_christianity_and_african_traditional_religion.txt
              sed -i '/^1$/!d' .s_ire_1_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Ire/S1"
              # Define the file extension
              file_extension_question=".3.islam_and_values_in_christianity_and_african_traditional_religion.qns.txt"
              # Define the revision file
              revision_file="../../ire_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              if ! [ -f ".ire.1.3" ]; then
	              ...
	              answered_directory="Exercise/Ire/S1"
	              # Define the file extension
	              file_extension_answer=".3.islam_and_values_in_christianity_and_african_traditional_religion.ans.txt"
	              # Define the exercise file
	              exercise_file="../../ire_answered_ans.txt"
    ...
    if ! find . -maxdepth 1 -name '.s_ire_2*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to S2 IRE class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
      clear_and_center "There are ${r}3${t} topics to be covered. Your tasks will always expand or shrink to fit in the time you give them. For that reason, never procrastinate darling!"
    fi
    attempts=0
    ...
    do
      ...
      do
        ...
        then
          attempts=0
          # Define the targeted directory
          question_directory="Revision/Ire/S2"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../ire_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Ire/S2"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../ire_answered_ans.txt"
        ...
        then
          ...
          case "$topic" in
            1)
              ...
              cp "Notes/Ire/2.1.respect_for_human_life.txt" . || exit 1
            ...
            2)
              ...
              cp "Notes/Ire/2.2.marriage.txt" . || exit 1
            ...
            3)
              ...
              cp "Notes/Ire/2.3.family.txt" . || exit 1
    ...
    if ! find . -maxdepth 1 -name '.s_ire_3*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to S3 IRE class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
      clear_and_center "There are ${r}3${t} topics to be covered. Your tasks will always expand or shrink to fit in the time you give them. For that reason, never procrastinate darling!"
    fi
    attempts=0
    ...
    do
      ...
      do
        ...
        then
          attempts=0
          # Define the targeted directory
          question_directory="Revision/Ire/S3"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../ire_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Ire/S3"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../ire_answered_ans.txt"
        ...
        then
          ...
          case "$topic" in
            1)
              ...
              cp "Notes/Ire/3.1.work.txt" . || exit 1
            ...
            2)
              ...
              cp "Notes/Ire/3.2.wealth_and_development.txt" . || exit 1
            ...
            3)
              ...
              cp "Notes/Ire/3.3.leisure.txt" . || exit 1
    ...
    if ! find . -maxdepth 1 -name '.s_ire_4*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to S4 IRE class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
      clear_and_center "There are ${r}3${t} topics to be covered. Your tasks will always expand or shrink to fit in the time you give them. For that reason, never procrastinate darling!"
    fi
    attempts=0
    ...
    do
      ...
      do
        ...
        then
          attempts=0
          # Define the targeted directory
          question_directory="Revision/Ire/S4"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../ire_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Ire/S4"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../ire_answered_ans.txt"
        ...
        then
          ...
          case "$topic" in
            1)
              ...
              cp "Notes/Ire/4.1.peace.txt" . || exit 1
            ...
            2)
              ...
              cp "Notes/Ire/4.2.justice.txt" . || exit 1
            ...
            3)
              ...
              cp "Notes/Ire/4.3.conflict_resolution.txt" . || exit 1
