...
function handle_class_input() {
  if { [ -n "$last_class" ] || [ -z "$last_topic" ]; } && [ -f .arabic_surveyor ]; then
		while true; do
	    read -rp $'\n\nPlease enter '"${r}1 or 2 or 3 or 4${t}"' to go to your class or '"${r}x${t}"' to exit'$'\n\n> ' class
      if [ "$class" == "x" ]; then
        exit
      fi
		  # Check if .current_arabic_class is accidentally empty
		  if [ ! -s .current_arabic_class ]; then
		    # Echo 1 to .current_arabic_class
		    echo "1" > .current_arabic_class
		  fi
		  # Read the value from the file
		  current_arabic_class=$(<.current_arabic_class) 2>/dev/null
		  # Check if the value is equal to $class
		  if [ "$current_arabic_class" -lt "$class" 2>/dev/null ]; then
	      ...
	      rm -f ".arabic_topic_selected"
	      # Update the state file with the class
	      if [ "$class" != "x" ]; then
	        echo "$class" > .arabic_user_state 2>/dev/null
	      fi
				break
	  	fi
		done
...
function handle_s3_topic_input() {
  ...
  if [ -z "$last_topic" ] || [ -f .arabic_topic_selected ]; then
    read -rp $'\n\nChoose either topic '"${g}1 or 2 or 3 or 4${t}"' to learn'$'\nor enter '"${r}z${t}"' for an adventure or '"${r}r${t}"' to revise or '"${r}s${t}"' to get sample_items'$'\nor '"${r}a${t}"' to get an activity of integration or '"${r}q${t}"' to get a short answer question'$'\nor '"${r}n${t}"' to do your final class assignment and if necessary, gain access to the next class'$'\nor '"${r}p${t}"' to track academic progress or '"${r}x${t}"' to exit'$'\n\n1. Going shopping '"${r}Term1${t}"''$'\n\n2. Marketing a product '"${r}Term2${t}"''$'\n\n3. In the restaurant '"${r}Term2${t}"''$'\n\n4. In vegetables and fruits market '"${r}Term3${t}"''$'\n\n> ' topic
    touch .arabic_surveyor
    touch .arabic_topic_selected
    # Update the state file with the topic
    # Check if the state file exists, and the topic is not "x"
...
function handle_s2_topic_input() {
  ...
  if [ -z "$last_topic" ] || [ -f .arabic_topic_selected ]; then
    read -rp $'\n\nChoose either topic '"${g}1 or 2 or 3 or 4${t}"' to learn'$'\nor enter '"${r}z${t}"' for an adventure or '"${r}r${t}"' to revise or '"${r}s${t}"' to get sample_items'$'\nor '"${r}a${t}"' to get an activity of integration or '"${r}q${t}"' to get a short answer question'$'\nor '"${r}n${t}"' to do your final class assignment and if necessary, gain access to the next class'$'\nor '"${r}p${t}"' to track academic progress or '"${r}x${t}"' to exit'$'\n\n1. My home '"${r}Term1${t}"''$'\n\n2. My family '"${r}Term2${t}"''$'\n\n3. My home town '"${r}Term2${t}"''$'\n\n4. Free time activities and family parties '"${r}Term3${t}"''$'\n\n> ' topic
    touch .arabic_surveyor
    touch .arabic_topic_selected
    # Update the state file with the topic
    # Check if the state file exists, and the topic is not "x"
...
function handle_s1_topic_input() {
  ...
  if [ -z "$last_topic" ] || [ -f .arabic_topic_selected ]; then
    read -rp $'\n\nChoose either topic '"${g}1 or 2 or 3 or 4${t}"' to learn'$'\nor enter '"${r}z${t}"' for an adventure or '"${r}r${t}"' to revise or '"${r}s${t}"' to get sample_items'$'\nor '"${r}a${t}"' to get an activity of integration or '"${r}q${t}"' to get a short answer question'$'\nor '"${r}n${t}"' to do your final class assignment and if necessary, gain access to the next class'$'\nor '"${r}p${t}"' to track academic progress or '"${r}x${t}"' to exit'$'\n\n1. School compound '"${r}Term1${t}"''$'\n\n2. In my class '"${r}Term2${t}"''$'\n\n3. Objects in the classroom '"${r}Term2${t}"''$'\n\n4. Visit to a sick bay '"${r}Term3${t}"''$'\n\n> ' topic
    touch .arabic_surveyor
    touch .arabic_topic_selected
    # Update the state file with the topic
    # Check if the state file exists, and the topic is not "x"
    ...
    for sentence in "${sentences[@]}"; do
      if [[ $echo_next == true ]]; then
        ...
        for split_sentence in "${split_sentences[@]}"; do
					echo "$split_sentence" >> .arabic_reminder
        done
      echo_next=false
      fi
        ...
        echo "Did you know that $lower_cased_sentence" >> .arabic_reminder
        ...
        echo_next=true
  			...
  			echo "Do you recall that $lower_cased_sentence;" >> .arabic_reminder

			elif [[ "$sentence" =~ " → " ]]; then
  			echo "Hope you know that: $sentence;" >> .arabic_reminder

      elif [[ "$sentence" =~ " ↔ " ]]; then
        echo "Hope you know that: $sentence;" >> .arabic_reminder

      elif [[ "$sentence" =~ (Generally|general) ]]; then
        echo "Note: $sentence;" >> .arabic_reminder
...
process_random_reminder() {
  ...
  # Check if .arabic_reminder exists
  if [ -f .arabic_reminder ]; then
		# Remove empty lines from .arabic_reminder
  	sed -i '/^[[:space:]]*$/d' .arabic_reminder 2>/dev/null
    # Randomly select a non-empty sentence
    local reminder # Declare the variable
		reminder=$(awk -v RS=';' 'BEGIN{srand();}{gsub(/^[[:space:]]+|[[:space:]]+$/, ""); if (length > 0) a[++n]=$0}END{if (n > 0) print a[int(rand()*n)+1]}' .arabic_reminder)
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    for sentence in "${sentences[@]}"; do
        ...
        if [[ $sentence == *"Figure"* ]]; then
          ...
          # Change to the "Figures/Arabic" directory
          cd Figures/Arabic || { echo "Failed to change to Figures/Arabic"; return; }
        ...
        if [[ $sentence == *"Table"* ]]; then
          cd Tables/Arabic || { echo -e "\nFailed to change to Tables/Arabic \c"; return; }
          explorer.exe "$sentence" > /dev/null 2>&1 &
          cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        fi

        if [[ $sentence == *"Video"* ]]; then
          cd Videos/Arabic || { echo -e "\nFailed to change to Videos/Arabic \c"; return; }
          explorer.exe "$sentence" > /dev/null 2>&1 &
          cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        fi
...
contact_ai() {
  last_topic=$(awk -F' ' '{print $2}' .arabic_user_state)
...
while true; do
  ...
  # Prompt the user for input
  if [ -z "$(find ./Notes/Arabic -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
  ...
  if [[ -n "$user_input" ]]; then
		if [[ "$user_input" == "sh" ]]; then
		  Response="We are looking forward to receiving your economic support, thoughts, suggestions, and any files you feel should reach out to everyone of our children. Please remember to label the files you are to attach using the format: Your_name_School_Arabic_File_content (e.g., Muhumuza_Omega_Kasule_High_School_O_level_Chemistry_Answered_EOC1_Items.pdf). Thanks a lot for your contributions."  
		  arabic="$(basename "$0") - $(date +"%Y-%m-%d %H:%M:%S") - Thoughts, suggestions, and contributions"
		  encoded_arabic=$(echo "$arabic" | sed 's/ /%20/g; s/\n/%0A/g')
		  encoded_body=$(echo "$Response" | sed 's/ /%20/g; s/\n/%0A/g')
		  # Open the email in the browser with the encoded arabic and body
		  powershell.exe -Command "Start-Process 'https://mail.google.com/mail/?view=cm&to=2024omd256@gmail.com&su=${encoded_arabic}&body=${encoded_body}'"
			return
		fi
     ...
     if [[ "$user_input" == "sr" ]]; then
      ...
      explorer.exe Arabic > /dev/null 2>&1 &
    ...
    if [[ "$user_input" == "xx" ]]; then
      ...
      if [[ "$input" == "y" || "$input" == "Y" ]]; then
        ...
        curl -O -L "https://github.com/Muhumuza7325/OMD/raw/main/update_arabic.sh" || { echo -e "\n\n${m}Check your internet connection and try again!${t}" >&2; return; }
        mv update_arabic.sh .update_arabic.sh
        bash .update_arabic.sh
        return
      else
        return
      fi
    ...
    if [[ -n "$result" ]]; then
      ...
      if echo "$result" | grep "Figure"; then
        echo "$result" | grep -o '\bFigure[0-9]\+.*\.jpg\(\.[0-9]\+\)*\b' > Figures/Arabic/figures.txt
        sed -i -e '/^Figure/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Figures/Arabic/figures.txt
        # Change to the "Figures/Arabic" directory
        cd Figures/Arabic || { echo "Failed to change to Figures/Arabic"; return; }
        ...
        rm -f Figures/Arabic/figures.txt
      fi

      if echo "$result" | grep "Table"; then
        echo "$result" | grep -o '\bTable[0-9]\+\(\.[0-9]\+\)*\b' > Tables/Arabic/tables.txt
        sed -i -e '/^Table/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Tables/Arabic/tables.txt
        cd Tables/Arabic || { echo -e "\nFailed to change to Tables/Arabic \c"; return; }
        ...
        # Go back to the original directory
        cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        rm -f Tables/Arabic/tables.txt
      fi

      if echo "$result" | grep "Video"; then
        #echo "$result" > Videos/Arabic/videos.txt
        echo "$result" | grep -o '\bVideo[0-9]\+\(\.[0-9]\+\)*\b' > Videos/Arabic/videos.txt
        # Remove lines not starting with "Video" and any leading/trailing whitespaces
        sed -i -e '/^Video/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Videos/Arabic/videos.txt
        # Change to the "Videos/Arabic" directory
        cd Videos/Arabic || { echo -e "\nFailed to change to Videos/Arabic \c"; return; }
  ...
  if [ -f "$selected_file" ]; then
    ...
    if [[ -n "$selected_question" && "$selected_question" =~ [[:graph:]] ]]; then
      ...
      if echo "$selected_question" | grep "Figure"; then
        echo "$selected_question" | grep -o '\bFigure[0-9]\+[^;]*\b' > ../../Figures/Arabic/figures.txt
        sed -i -e '/^Figure/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' ../../Figures/Arabic/figures.txt
        # Change to the "Figures/Arabic" directory
        cd ../../Figures/Arabic || { echo "Failed to change to Figures/Arabic"; return; }
        # Specify the path to the text file containing figure names
        ...
        # Go back to the original directory
        cd ../../Revision/Arabic || { echo "Failed to change back to the targeted directory \c"; exit 1; }
        # Remove the temporary file
        rm -f ../../Figures/Arabic/figures.txt
      fi
...
process_question_answer() {
  ...
  if [ -f ../../../.school_name ]; then
    ...
    sed -i '/^[[:space:]]*$/d' ../../../."$school_name"_students_file.txt
    existing_class=$(awk '{print $1}' ../../../.arabic_user_state)
    existing_topic=$(awk '{print $2}' ../../../.arabic_user_state)
    echo ''
    ...
    if grep -q "Arabic" ../../../."$school_name"_students_file.txt; then
      sed -i -E 's/;/\n/g' ../../../."$school_name"_students_file.txt
      # Find the line with the word Arabic, replace the information below it with the already available information adding a comma existing_class_existing_topic [$percentage]
      if [ "$existing_topic" == "r" ]; then
        sed -i '/Arabic/{n;s/\(.*\)/\1, '"$existing_class"'_'"$input"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
      else
        (( existing_topic-- ))
        sed -i '/Arabic/{n;s/\(.*\)/\1, '"$existing_class"'_'"$existing_topic"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
      fi
    else
      if [ "$existing_topic" == "r" ]; then
        sed -i -E '/^School/ i\Arabic\n'"${existing_class} ${names}[${percentage}%]"'' ../../../."$school_name"_students_file.txt
...
function resume_from_last_point() {
  if [ -f .arabic_user_state ]; then
    last_class=$(awk -F' ' '{print $1}' .arabic_user_state)
    last_topic=$(awk -F' ' '{print $2}' .arabic_user_state)
    if [ -n "$last_class" ] && [ -n "$last_topic" ]; then
      echo -e "\n       Resuming from ${r}S$last_class${t} : ${g}Topic '$last_topic'${t} \c"
      rm -f .arabic_surveyor
...
process_final_assignment() {
  # Check if user is.arabic_ready for the assignment
  # Check if .current_arabic_class is accidentally empty
  if [ ! -s .current_arabic_class ]; then
    # Echo 1 to .current_arabic_class
    echo "1" > .current_arabic_class
  fi
  # Read the value from the .current_arabic_class file
  current_arabic_class=$(<.current_arabic_class) 2>/dev/null
  # Check if the value in the .arabic_ready file is equal to $class
  # Read the value from the .arabic_ready file
  if [ ! -s .arabic_ready ]; then
    echo "0" > .arabic_ready
  fi
  how.arabic_ready=$(<.arabic_ready) 2>/dev/null
  # Check if the value is equal to $class
  if [ "$how.arabic_ready" -lt "$current_arabic_class" 2>/dev/null ]; then
    read -rp $'\n\nYou havent done all the topic assignments for your current arabic class\n\n'"${r}Proceeding from here will affect your very final score${y}"'. To go back and progress right, enter '"${y}ok${t}"'. Otherwise, press the Enter key to do the final class assignment: ' progress
    if [ "$progress" == "ok" ]; then
      return
    fi
  fi
    ...
    if ! [ "$total_questions" == 100 ]; then
      ...
      if [ "$elapsed_time" -gt 3600 ]; then
        ...
        if [ "$percentage" -lt 95 ]; then
          ...
          cd - > /dev/null 2>&1 || exit
          if ! [ -f .current_arabic_class ]; then
            # Echo the result to the file .current_class
            echo "2" > .current_arabic_class
            ...
            # Read the value from the file
            current_arabic_class=$(<.current_arabic_class) 2>/dev/null
            # Check if the new_class value is lt $current_arabic_class
            if [ "$new_class" -gt "$current_arabic_class" 2>/dev/null ]; then
		          # Echo the result to the file .current_class
		          echo "$new_class" > .current_arabic_class
            fi
...
get_sample_items() {
  ...
  # Save the current working directory
  pushd . > /dev/null
  cd Revision/Arabic || { echo "Directory not found"; return; }
  ...
  while true; do
      ...
      if [ ! -f "$last_echo_time_file" ]; then
    	...
    	if [ $time_diff -gt 3600 ]; then
      	# Echo the message and update the last echo time
	    	echo -e "\n\n\n${r}You are advised to not make any changes to the provided answers, instead, you can make copies that you can edit${t}\n\n${y}For a teacher willing to join us reach out to everyone of our children, please send us your questions and answers in a file labelled with your name, school, arabic, and file content (e.g., Muhumuza_Omega_Kasule_High_School_O_level_Chemistry_Answered_EOC1_Items.pdf) to our contacts${t}\n\n\nEmail: ${g}2024omd256@gmail.com${t} \c"
				echo $current_time > "$last_echo_time_file"
  		fi
      read -rp $'\n\n\nEnter '"${m}any character${t}"' for access to the file of answered items or simply press '"${r}Enter${t}"' to get items to attempt : ' input
     	if [[ -n $input ]]; then
     		explorer.exe .arabic_samples* > /dev/null 2>&1 &
      	clear
      	popd > /dev/null || exit
        return
      fi
      if [ -f .e_o_c_arabic.txt ]; then
        echo -e "\n\n${y}Below is the list of the elements of construct${t} \n"
        cat .e_o_c_arabic.txt
        ...
        if [[ -n $input ]]; then
          ...
          selected_file1=$(ls -a | grep -E "\.e_o_c_arabic_${input}\.txt")
          cat "$selected_file1"
          # Remove empty lines from the selected files
          find . -type f -name "*_samples_[0-9].txt" -exec sed -i '/^[[:space:]]*$/d' {} +
          selected_file=$(ls -a | grep -E "\.e_o_c_arabic_${input}_samples" | shuf -n 1)
        else
    ...
    rm -f .revise.txt .arabic_topic_selected 2>/dev/null
  done
  popd > /dev/null || exit
	return
}
...
while true; do
  ...
  if ls Students | grep -wq "$initials"; then
    ...
    echo -e "                  $initials\n" > "Students/$initials/Exercise/arabic_answered_ans.txt"
    echo -e "                  $initials\n" > "Students/$initials/Revision/arabic_covered_qns.txt"
    ...
    if [ -f ../../../.school_name ]; then
      ...
      if grep -q "Arabic" ../../../."$school_name"_students_file.txt; then
        ...
        if [ "$existing_topic" == "z" ]; then
          sed -i '/Arabic/{n;s/\(.*\)/\1, '"$existing_class"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
        else
          (( existing_topic-- ))
          sed -i '/Arabic/{n;s/\(.*\)/\1, '"$existing_class"'_'"$existing_topic"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
        fi
...
if [ ! -f .arabic_user_state ]; then
  ...
  if [ -f .omd_communication.txt ]; then
    ...
    process_random_reminder .arabic_reminder
  fi
	handle_resume_input
fi

if [ -z "$class" ] && [ -s ".arabic_user_state" ]; then
  get_and_display_pattern
fi
# Check for the presence of specific directories and a file
if ! [ -d "Notes/Arabic" ] || ! [ -d "Revision" ] || ! [ -d "Exercise" ] || ! [ -d "Videos" ] || ! [ -d "Figures" ] || ! [ -d "Tables" ]; then
  ...
  mkdir -p Notes Notes/Arabic Revision Revision/Arabic Revision/Arabic/{S1,S2,S3,S4} Exercise Exercise/Arabic Exercise/Arabic/{S1,S2,S3,S4} Videos Videos/Arabic Figures Figures/Arabic Tables Tables/Arabic
  touch Revision/arabic_covered_qns.txt Exercise/arabic_answered_ans.txt
  echo -e "\n"
  pwd
...
if [ -z "$(find ./Notes/Arabic -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
  ...
  if [[ -z "$user_input" ]]; then
    ...
    curl -O -L https://github.com/Muhumuza7325/OMD/raw/main/ARABIC/1.1.school_compound.txt || echo -e "\n\nError fetching material for this tutorial \c"
    ...
    cp 1.1.school_compound.txt Notes/Arabic || echo -e "\n\nError copying 1.1.school_compound.txt to the Arabic directory in the Notes directory \c"
    wait_for_a_key_press
  else
...
while true; do
  ...
  handle_class_input
  if [[ "$class" == "1" ]]; then
    if ! find . -maxdepth 1 -name '.s_arabic_1*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to the S1 Arabic class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
    ...
    do
      ...
      do
        ...
        then
          ...
          question_directory="Revision/Arabic/S1"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../arabic_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Arabic/S1"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../arabic_answered_ans.txt"
          # Call the function to process a random question
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_arabic_1_1" ]; then
                echo -e "\n\nYou chose to explore School compound ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/Arabic/1.1.school_compound.txt" . || exit 1
              mv 1.1.school_compound.txt .1.1.school_compound.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.1.school_compound.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.1.school_compound.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.1.school_compound.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.1.school_compound.txt
              process_reminders_from_file .1.1.school_compound.txt
              STATE_FILE=".s_arabic_1_1"
              process_file .1.1.school_compound.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.1.school_compound.txt
              sed -i '/^1$/!d' .s_arabic_1_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Arabic/S1"
              # Define the file extension
              file_extension=".1.school_compound.qns.txt"
              # Define the revision file
              revision_file="../../arabic_covered_qns.txt"
              # Call the function to process a random question
              ...
              question_directory="Revision/Arabic/S1"
              # Define the file extension
              file_extension_question=".1.school_compound.qns.txt"
              # Define the revision file
              revision_file="../../arabic_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".arabic.1.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Arabic/S1"
                # Define the file extension
                file_extension_answer=".1.school_compound.ans.txt"
                # Define the exercise file
                exercise_file="../../arabic_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_arabic_1_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into In my class ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Arabic/1.2.in_my_class.txt" . || exit 1
              mv 1.2.in_my_class.txt .1.2.in_my_class.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.2.in_my_class.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.2.in_my_class.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.2.in_my_class.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.2.in_my_class.txt
              process_reminders_from_file .1.2.in_my_class.txt
              STATE_FILE=".s_arabic_1_2"
              process_file .1.2.in_my_class.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.2.in_my_class.txt
              sed -i '/^1$/!d' .s_arabic_1_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Arabic/S1"
              # Define the file extension
              file_extension_question=".2.in_my_class.qns.txt"
              # Define the revision file
              revision_file="../../arabic_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".arabic.1.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Arabic/S1"
                # Define the file extension
                file_extension_answer=".2.in_my_class.ans.txt"
                # Define the exercise file
                exercise_file="../../arabic_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_arabic_1_3" ]; then
                echo -e "\n\nYou have made a choice to cover Objects in the classroom ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Arabic/1.3.objects_in_the_classroom.txt" . || exit 1
              mv 1.3.objects_in_the_classroom.txt .1.3.objects_in_the_classroom.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.3.objects_in_the_classroom.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.3.objects_in_the_classroom.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.3.objects_in_the_classroom.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.3.objects_in_the_classroom.txt
              process_reminders_from_file .1.3.objects_in_the_classroom.txt
              STATE_FILE=".s_arabic_1_3"
              process_file .1.3.objects_in_the_classroom.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.3.objects_in_the_classroom.txt
              sed -i '/^1$/!d' .s_arabic_1_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Arabic/S1"
              # Define the file extension
              file_extension_question=".3.objects_in_the_classroom.qns.txt"
              # Define the revision file
              revision_file="../../arabic_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".arabic.1.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Arabic/S1"
                # Define the file extension
                file_extension_answer=".3.objects_in_the_classroom.ans.txt"
                # Define the exercise file
                exercise_file="../../arabic_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_arabic_1_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Visit to a sick bay ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/Arabic/1.4.visit_to_a_sick_bay.txt" . || exit 1
              mv 1.4.visit_to_a_sick_bay.txt .1.4.visit_to_a_sick_bay.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.4.visit_to_a_sick_bay.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.4.visit_to_a_sick_bay.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.4.visit_to_a_sick_bay.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.4.visit_to_a_sick_bay.txt
              process_reminders_from_file .1.4.visit_to_a_sick_bay.txt
              STATE_FILE=".s_arabic_1_4"
              process_file .1.4.visit_to_a_sick_bay.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.4.visit_to_a_sick_bay.txt
              sed -i '/^1$/!d' .s_arabic_1_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Arabic/S1"
              # Define the file extension
              file_extension=".4.visit_to_a_sick_bay.qns.txt"
              # Define the revision file
              revision_file="../../arabic_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Arabic/S1"
              # Define the file extension
              file_extension_question=".4.visit_to_a_sick_bay.qns.txt"
              # Define the revision file
              revision_file="../../arabic_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".arabic.1.4" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/Arabic/S1"
	              # Define the file extension
	              file_extension_answer=".4.visit_to_a_sick_bay.ans.txt"
	              # Define the exercise file
	              exercise_file="../../arabic_answered_ans.txt"
    ...
    if ! find . -maxdepth 1 -name '.s_arabic_2*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to the S2 Arabic class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
      clear_and_center "There are ${r}4${t} topics to be covered. Your tasks will always expand or shrink to fit in the time you give them. For that reason, never procrastinate darling!"
    fi
    attempts=0
    ...
    do
      ...
      do
        ...
        then
          attempts=0
          # Define the targeted directory
          question_directory="Revision/Arabic/S2"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../arabic_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Arabic/S2"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../arabic_answered_ans.txt"
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_arabic_2_1" ]; then
                echo -e "\n\nYou chose to explore My home ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/Arabic/2.1.my_home.txt" . || exit 1
              mv 2.1.my_home.txt .2.1.my_home.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.1.my_home.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.1.my_home.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.1.my_home.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.1.my_home.txt
              process_reminders_from_file .2.1.my_home.txt
              STATE_FILE=".s_arabic_2_1"
              process_file .2.1.my_home.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.1.my_home.txt
              sed -i '/^1$/!d' .s_arabic_2_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Arabic/S2"
              # Define the file extension
              file_extension=".1.my_home.qns.txt"
              # Define the revision file
              revision_file="../../arabic_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Arabic/S2"
              # Define the file extension
              file_extension_question=".1.my_home.qns.txt"
              # Define the revision file
              revision_file="../../arabic_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".arabic.2.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Arabic/S2"
                # Define the file extension
                file_extension_answer=".1.my_home.ans.txt"
                # Define the exercise file
                exercise_file="../../arabic_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_arabic_2_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into My family ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Arabic/2.2.my_family.txt" . || exit 1
              mv 2.2.my_family.txt .2.2.my_family.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.2.my_family.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.2.my_family.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.2.my_family.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.2.my_family.txt
              process_reminders_from_file .2.2.my_family.txt
              STATE_FILE=".s_arabic_2_2"
              process_file .2.2.my_family.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.2.my_family.txt
              sed -i '/^1$/!d' .s_arabic_2_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Arabic/S2"
              # Define the file extension
              file_extension_question=".2.my_family.qns.txt"
              # Define the revision file
              revision_file="../../arabic_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".arabic.2.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Arabic/S2"
                # Define the file extension
                file_extension_answer=".2.my_family.ans.txt"
                # Define the exercise file
                exercise_file="../../arabic_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_arabic_2_3" ]; then
                echo -e "\n\nYou have made a choice to cover My home town ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Arabic/2.3.my_home_town.txt" . || exit 1
              mv 2.3.my_home_town.txt .2.3.my_home_town.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.3.my_home_town.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.3.my_home_town.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.3.my_home_town.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.3.my_home_town.txt
              process_reminders_from_file .2.3.my_home_town.txt
              STATE_FILE=".s_arabic_2_3"
              process_file .2.3.my_home_town.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.3.my_home_town.txt
              sed -i '/^1$/!d' .s_arabic_2_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Arabic/S2"
              # Define the file extension
              file_extension_question=".3.my_home_town.qns.txt"
              # Define the revision file
              revision_file="../../arabic_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".arabic.2.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Arabic/S2"
                # Define the file extension
                file_extension_answer=".3.my_home_town.ans.txt"
                # Define the exercise file
                exercise_file="../../arabic_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_arabic_2_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Free time activities and family parties ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/Arabic/2.4.free_time_activities_and_family_parties.txt" . || exit 1
              mv 2.4.free_time_activities_and_family_parties.txt .2.4.free_time_activities_and_family_parties.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.4.free_time_activities_and_family_parties.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.4.free_time_activities_and_family_parties.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.4.free_time_activities_and_family_parties.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.4.free_time_activities_and_family_parties.txt
              process_reminders_from_file .2.4.free_time_activities_and_family_parties.txt
              STATE_FILE=".s_arabic_2_4"
              process_file .2.4.free_time_activities_and_family_parties.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.4.free_time_activities_and_family_parties.txt
              sed -i '/^1$/!d' .s_arabic_2_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Arabic/S2"
              # Define the file extension
              file_extension=".4.free_time_activities_and_family_parties.qns.txt"
              # Define the revision file
              revision_file="../../arabic_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Arabic/S2"
              # Define the file extension
              file_extension_question=".4.free_time_activities_and_family_parties.qns.txt"
              # Define the revision file
              revision_file="../../arabic_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".arabic.2.4" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/Arabic/S2"
	              # Define the file extension
	              file_extension_answer=".4.free_time_activities_and_family_parties.ans.txt"
	              # Define the exercise file
	              exercise_file="../../arabic_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
    ...
    if ! find . -maxdepth 1 -name '.s_arabic_3*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to the S3 Arabic class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
      clear_and_center "There are ${r}4${t} topics to be covered. Your tasks will always expand or shrink to fit in the time you give them. For that reason, never procrastinate darling!"
    fi
    attempts=0
    ...
    do
      ...
      do
        ...
        then
          attempts=0
          # Define the targeted directory
          question_directory="Revision/Arabic/S3"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../arabic_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Arabic/S3"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../arabic_answered_ans.txt"
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_arabic_3_1" ]; then
                echo -e "\n\nYou chose to explore Going shopping ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/Arabic/3.1.going_shopping.txt" . || exit 1
              mv 3.1.going_shopping.txt .3.1.going_shopping.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.1.going_shopping.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.1.going_shopping.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.1.going_shopping.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.1.going_shopping.txt
              process_reminders_from_file .3.1.going_shopping.txt
              STATE_FILE=".s_arabic_3_1"
              process_file .3.1.going_shopping.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.1.going_shopping.txt
              sed -i '/^1$/!d' .s_arabic_3_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Arabic/S3"
              # Define the file extension
              file_extension=".1.going_shopping.qns.txt"
              # Define the revision file
              revision_file="../../arabic_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Arabic/S3"
              # Define the file extension
              file_extension_question=".1.going_shopping.qns.txt"
              # Define the revision file
              revision_file="../../arabic_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".arabic.3.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Arabic/S3"
                # Define the file extension
                file_extension_answer=".1.going_shopping.ans.txt"
                # Define the exercise file
                exercise_file="../../arabic_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_arabic_3_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Marketing a product ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Arabic/3.2.marketing_a_product.txt" . || exit 1
              mv 3.2.marketing_a_product.txt .3.2.marketing_a_product.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.2.marketing_a_product.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.2.marketing_a_product.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.2.marketing_a_product.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.2.marketing_a_product.txt
              process_reminders_from_file .3.2.marketing_a_product.txt
              STATE_FILE=".s_arabic_3_2"
              process_file .3.2.marketing_a_product.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.2.marketing_a_product.txt
              sed -i '/^1$/!d' .s_arabic_3_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Arabic/S3"
              # Define the file extension
              file_extension_question=".2.marketing_a_product.qns.txt"
              # Define the revision file
              revision_file="../../arabic_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".arabic.3.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Arabic/S3"
                # Define the file extension
                file_extension_answer=".2.marketing_a_product.ans.txt"
                # Define the exercise file
                exercise_file="../../arabic_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_arabic_3_3" ]; then
                echo -e "\n\nYou have made a choice to cover In the restaurant ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Arabic/3.3.in_the_restaurant.txt" . || exit 1
              mv 3.3.in_the_restaurant.txt .3.3.in_the_restaurant.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.3.in_the_restaurant.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.3.in_the_restaurant.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.3.in_the_restaurant.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.3.in_the_restaurant.txt
              process_reminders_from_file .3.3.in_the_restaurant.txt
              STATE_FILE=".s_arabic_3_3"
              process_file .3.3.in_the_restaurant.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.3.in_the_restaurant.txt
              sed -i '/^1$/!d' .s_arabic_3_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Arabic/S3"
              # Define the file extension
              file_extension_question=".3.in_the_restaurant.qns.txt"
              # Define the revision file
              revision_file="../../arabic_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".arabic.3.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Arabic/S3"
                # Define the file extension
                file_extension_answer=".3.in_the_restaurant.ans.txt"
                # Define the exercise file
                exercise_file="../../arabic_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_arabic_3_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of In vegetables and fruits market ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/Arabic/3.4.in_vegetables_and_fruits_market.txt" . || exit 1
              mv 3.4.in_vegetables_and_fruits_market.txt .3.4.in_vegetables_and_fruits_market.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.4.in_vegetables_and_fruits_market.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.4.in_vegetables_and_fruits_market.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.4.in_vegetables_and_fruits_market.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.4.in_vegetables_and_fruits_market.txt
              process_reminders_from_file .3.4.in_vegetables_and_fruits_market.txt
              STATE_FILE=".s_arabic_3_4"
              process_file .3.4.in_vegetables_and_fruits_market.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.4.in_vegetables_and_fruits_market.txt
              sed -i '/^1$/!d' .s_arabic_3_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Arabic/S3"
              # Define the file extension
              file_extension=".4.in_vegetables_and_fruits_market.qns.txt"
              # Define the revision file
              revision_file="../../arabic_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Arabic/S3"
              # Define the file extension
              file_extension_question=".4.in_vegetables_and_fruits_market.qns.txt"
              # Define the revision file
              revision_file="../../arabic_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".arabic.3.4" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/Arabic/S3"
	              # Define the file extension
	              file_extension_answer=".4.in_vegetables_and_fruits_market.ans.txt"
	              # Define the exercise file
	              exercise_file="../../arabic_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
    ...
    if ! find . -maxdepth 1 -name '.s_arabic_4*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to the S4 Arabic class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
      clear_and_center "There are ${r}4${t} topics to be covered. Your tasks will always expand or shrink to fit in the time you give them. For that reason, never procrastinate darling!"
    fi
    attempts=0
    ...
    do
      ...
      do
        ...
        then
          attempts=0
          # Define the targeted directory
          question_directory="Revision/Arabic/S4"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../arabic_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Arabic/S4"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../arabic_answered_ans.txt"
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_arabic_4_1" ]; then
                echo -e "\n\nYou chose to explore Transport ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/Arabic/4.1.transport.txt" . || exit 1
              mv 4.1.transport.txt .4.1.transport.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.1.transport.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.1.transport.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.1.transport.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.1.transport.txt
              process_reminders_from_file .4.1.transport.txt
              STATE_FILE=".s_arabic_4_1"
              process_file .4.1.transport.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.1.transport.txt
              sed -i '/^1$/!d' .s_arabic_4_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Arabic/S4"
              # Define the file extension
              file_extension=".1.transport.qns.txt"
              # Define the revision file
              revision_file="../../arabic_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Arabic/S4"
              # Define the file extension
              file_extension_question=".1.transport.qns.txt"
              # Define the revision file
              revision_file="../../arabic_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".arabic.4.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Arabic/S4"
                # Define the file extension
                file_extension_answer=".1.transport.ans.txt"
                # Define the exercise file
                exercise_file="../../arabic_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_arabic_4_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Travelling abroad ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Arabic/4.2.travelling_abroad.txt" . || exit 1
              mv 4.2.travelling_abroad.txt .4.2.travelling_abroad.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.2.travelling_abroad.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.2.travelling_abroad.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.2.travelling_abroad.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.2.travelling_abroad.txt
              process_reminders_from_file .4.2.travelling_abroad.txt
              STATE_FILE=".s_arabic_4_2"
              process_file .4.2.travelling_abroad.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.2.travelling_abroad.txt
              sed -i '/^1$/!d' .s_arabic_4_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Arabic/S4"
              # Define the file extension
              file_extension_question=".2.travelling_abroad.qns.txt"
              # Define the revision file
              revision_file="../../arabic_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".arabic.4.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Arabic/S4"
                # Define the file extension
                file_extension_answer=".2.travelling_abroad.ans.txt"
                # Define the exercise file
                exercise_file="../../arabic_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_arabic_4_3" ]; then
                echo -e "\n\nYou have made a choice to cover Future plans for further education ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Arabic/4.3.future_plans_for_further_education.txt" . || exit 1
              mv 4.3.future_plans_for_further_education.txt .4.3.future_plans_for_further_education.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.3.future_plans_for_further_education.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.3.future_plans_for_further_education.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.3.future_plans_for_further_education.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.3.future_plans_for_further_education.txt
              process_reminders_from_file .4.3.future_plans_for_further_education.txt
              STATE_FILE=".s_arabic_4_3"
              process_file .4.3.future_plans_for_further_education.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.3.future_plans_for_further_education.txt
              sed -i '/^1$/!d' .s_arabic_4_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Arabic/S4"
              # Define the file extension
              file_extension_question=".3.future_plans_for_further_education.qns.txt"
              # Define the revision file
              revision_file="../../arabic_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".arabic.4.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Arabic/S4"
                # Define the file extension
                file_extension_answer=".3.future_plans_for_further_education.ans.txt"
                # Define the exercise file
                exercise_file="../../arabic_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_arabic_4_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Challenges in high educational institutions ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/Arabic/4.4.challenges_in_high_educational_institutions.txt" . || exit 1
              mv 4.4.challenges_in_high_educational_institutions.txt .4.4.challenges_in_high_educational_institutions.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.4.challenges_in_high_educational_institutions.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.4.challenges_in_high_educational_institutions.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.4.challenges_in_high_educational_institutions.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.4.challenges_in_high_educational_institutions.txt
              process_reminders_from_file .4.4.challenges_in_high_educational_institutions.txt
              STATE_FILE=".s_arabic_4_4"
              process_file .4.4.challenges_in_high_educational_institutions.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.4.challenges_in_high_educational_institutions.txt
              sed -i '/^1$/!d' .s_arabic_4_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Arabic/S4"
              # Define the file extension
              file_extension=".4.challenges_in_high_educational_institutions.qns.txt"
              # Define the revision file
              revision_file="../../arabic_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Arabic/S4"
              # Define the file extension
              file_extension_question=".4.challenges_in_high_educational_institutions.qns.txt"
              # Define the revision file
              revision_file="../../arabic_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".arabic.4.4" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/Arabic/S4"
	              # Define the file extension
	              file_extension_answer=".4.challenges_in_high_educational_institutions.ans.txt"
	              # Define the exercise file
	              exercise_file="../../arabic_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
