...
function handle_class_input() {
    if { [ -n "$last_class" ] || [ -z "$last_topic" ]; } && [ -f .mathematics_surveyor ]; then
		while true; do
		    ...
		    # Check if .current_mathematics_class is accidentally empty
    ...
    while IFS= read -r line || [ -n "$line" ]; do
        ...
        for sentence in "${sentences[@]}"; do
                ...
                if [[ $sentence == *"Table"* ]]; then
                    cd Tables/Mathematics || { echo -e "\nFailed to change to Tables/Mathematics \c"; return; }
                    explorer.exe "$sentence" > /dev/null 2>&1 &
                    cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
                fi

                if [[ $sentence == *"Video"* ]]; then
                    cd Videos/Mathematics || { echo -e "\nFailed to change to Videos/Mathematics \c"; return; }
                    explorer.exe "$sentence" > /dev/null 2>&1 &
                    cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
                fi
    ...
    if [[ -n "$user_input" ]]; then
		if [[ "$user_input" == "sh" ]]; then
         ...
         if [[ "$user_input" == "sr" ]]; then
            ...
            explorer.exe Mathematics > /dev/null 2>&1 &
        ...
        if [[ "$user_input" == "zz" ]]; then
            ...
            TEMP_FILE=$(mktemp) && curl -o "$TEMP_FILE" -L "https://github.com/Muhumuza7325/OMD/raw/main/mathematics_tutorial_wsl.sh" && mv "$TEMP_FILE" mathematics_tutorial_wsl.sh && chmod +x mathematics_tutorial_wsl.sh && cp mathematics_tutorial_wsl.sh Students/Omd && echo -e "\n\n${y}Code successfully updated.. You will have to restart a new session${t} \c" && sleep 4 && exit || (echo -e "\n\n${m}Error updating code!... Please check your internet connection and try again!${t} \c" && rm -f "$TEMP_FILE" && return)
        fi
        ...
        if [[ -n "$result" ]]; then
            ...
            if echo "$result" | grep "Figure"; then
                echo "$result" | grep -o '\bFigure[0-9]\+.*\.jpg\(\.[0-9]\+\)*\b' > Figures/Mathematics/figures.txt
                ...
                rm -f Figures/Mathematics/figures.txt
            fi

            if echo "$result" | grep "Table"; then
                echo "$result" | grep -o '\bTable[0-9]\+\(\.[0-9]\+\)*\b' > Tables/Mathematics/tables.txt
                sed -i -e '/^Table/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Tables/Mathematics/tables.txt
                cd Tables/Mathematics || { echo -e "\nFailed to change to Tables/Mathematics \c"; return; }
                ...
                # Go back to the original directory
                cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
                rm -f Tables/Mathematics/tables.txt
            fi

            if echo "$result" | grep "Video"; then
                #echo "$result" > Videos/Mathematics/videos.txt
                echo "$result" | grep -o '\bVideo[0-9]\+\(\.[0-9]\+\)*\b' > Videos/Mathematics/videos.txt
                # Remove lines not starting with "Video" and any leading/trailing whitespaces
                sed -i -e '/^Video/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Videos/Mathematics/videos.txt
                # Change to the "Videos/Mathematics" directory
                cd Videos/Mathematics || { echo -e "\nFailed to change to Videos/Mathematics \c"; return; }
                ...
                cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
                # Remove the temporary file
                rm -f Videos/Mathematics/videos.txt
            fi
    ...
    if [ -f "$selected_file" ]; then
        ...
        if [[ -n "$selected_question" && "$selected_question" =~ [[:graph:]] ]]; then
            ...
            if echo "$selected_question" | grep "Figure"; then
                ...
                # Go back to the original directory
                cd ../../Revision/Mathematics || { echo "Failed to change back to the targeted directory \c"; exit 1; }
                # Remove the temporary file
                rm -f ../../Figures/Mathematics/figures.txt
            fi
...
process_final_assignment() {
    # Check if user is.mathematics_ready for the assignment
    # Check if .current_mathematics_class is accidentally empty
    if [ ! -s .current_mathematics_class ]; then
        # Echo 1 to .current_mathematics_class
        echo "1" > .current_mathematics_class
    fi
    # Read the value from the .current_mathematics_class file
    current_mathematics_class=$(<.current_mathematics_class) 2>/dev/null
            ...
            if [ "$elapsed_time" -gt 3600 ]; then
                ...
                if [ "$percentage" -lt 95 ]; then
                    ...
                    if ! [ -f .current_mathematics_class ]; then
                        ...
                        echo "3" > .current_mathematics_class
                        echo -e "\n\n${g}Congratulations!${t}  You have successfully gained access to the next class (S3).\n\nHowever, if a diferrent class was expected, then something is wrong.\n\nFrom here, you will have to go back to go back to S3 and acess the next classes the right way \c"
                        wait_for_a_key_press
                    else
    ...
    while true; do
	    if [ ! -f .revise.txt ]; then
            ...
            read -rp $'\n\n\nEnter '"${m}any character${t}"' for access to the file of answered items or simply press '"${r}Enter${t}"' to get items to attempt : ' input
          	if [[ -n $input ]]; then
          		explorer.exe .mathematics_samples* > /dev/null 2>&1 &
            	clear
            	popd > /dev/null || exit
                return
            fi
            if [ -f .e_o_c_mathematics.txt ]; then
                echo -e "\n\n${y}Below is the list of the elements of construct${t} \n"
                cat .e_o_c_mathematics.txt
                ...
                if [[ -n $input ]]; then
                    ...
                    selected_file1=$(ls -a | grep -E "\.e_o_c_mathematics_${input}\.txt")
                    cat "$selected_file1"
                    # Remove empty lines from the selected files
                    find . -type f -name "*_samples_[0-9].txt" -exec sed -i '/^[[:space:]]*$/d' {} +
                    selected_file=$(ls -a | grep -E "\.e_o_c_mathematics_${input}_samples" | shuf -n 1)
                else
...
if ! [ -d "Notes" ] || ! [ -d "Revision" ] || ! [ -d "Exercise" ] || ! [ -d "Videos" ] || ! [ -d "Figures" ] || ! [ -d "Tables" ]; then
    ...
    mkdir -p Notes Notes/Mathematics Revision Revision/Mathematics Revision/Mathematics/{S1,S3,S3,S4} Exercise Exercise/Mathematics Exercise/Mathematics/{S1,S3,S3,S4} Videos Videos/Mathematics Figures Figures/Mathematics Tables Tables/Mathematics
...
if [ -z "$(find . -mindepth 3 -maxdepth 3 -type f -name "*.txt" 2>/dev/null)" ]; then
    ...
    if [[ -z "$user_input" ]]; then
        ...
        curl -O -L https://github.com/Muhumuza7325/OMD/raw/main/1.1.number_bases.txt || echo -e "\n\nError fetching material for this tutorial \c"
