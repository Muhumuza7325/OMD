  ...
  if { [ -n "$last_class" ] || [ -z "$last_topic" ]; } && [ -f .english_surveyor ]; then
      ...
      if [ "$class" == "x" ]; then
        exit
		  ...
		  # Check if .current english class is accidentally empty
...
process_file() {
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    for sentence in "${sentences[@]}"; do
          ...
          # Open the file
          eog "Figures/English/$modified_sentence" > /dev/null 2>&1 &
        fi
        if [[ $sentence == *"Table"* ]]; then
          libreoffice "Tables/English/$sentence" > /dev/null 2>&1 &
        fi
        if [[ $sentence == *"Video"* ]]; then
          # Open the video file
          vlc --one-instance --playlist-enqueue "Videos/English/${video_prefix}"* > /dev/null 2>&1 &
	      fi
...
while true; do
  ...
  if [[ -n "$user_input" ]]; then
     ...
     if [[ "$user_input" == "sr" ]]; then
      xdg-open Resources/English > /dev/null 2>&1 &
      return
     fi
    ...
    if [[ "$user_input" == "zz" ]]; then
      ...
      TEMP_FILE=$(mktemp) && curl -o "$TEMP_FILE" -L "https://github.com/Muhumuza7325/OMD/raw/main/english_tutorial.sh" && mv "$TEMP_FILE" english_tutorial.sh && chmod +x english_tutorial.sh && cp english_tutorial.sh Students/Omd && echo -e "\n\n${y}Code successfully updated.. You will have to restart a new session${t} \c" && sleep 4 && exit || (echo -e "\n\n${m}Error updating code!... Please check your internet connection and try again!${t} \c" && rm -f "$TEMP_FILE" && return)
    fi
    ...
    if [[ -n "$result" ]]; then
      ...
      if echo "$result" | grep "Figure"; then
        ...
        while IFS= read -r figure_prefix || [ -n "$figure_prefix" ]; do
          ...
          eog "Figures/English/${edited_figure_prefix}"* > /dev/null 2>&1 &
					sleep 5
        done < "$text_file"
      ...
      if echo "$result" | grep "Table"; then
        ...
        while IFS= read -r table_prefix || [ -n "$table_prefix" ]; do
          ...
          # Open the file
          libreoffice "Tables/English/${edited_table_prefix}"* > /dev/null 2>&1 &
					sleep 10
        done < "$text_file"
        # Remove the temporary file
        rm -f tables.txt
      ...
      if echo "$result" | grep "Video"; then
        ...
        while IFS= read -r video_prefix || [ -n "$video_prefix" ]; do
          ...
          vlc --one-instance --playlist-enqueue "../Videos/English/${edited_video_prefix}"* > /dev/null 2>&1 &					
          sleep 10
        done < "$text_file"
        # Remove the temporary file
				rm -f videos.txt
...
process_final_assignment() {
  ...
  # Check if .current english class is accidentally empty
  if [ ! -s .current_english_class ]; then
    # Echo 1 to .current english class
    echo "1" > .current_english_class
  fi
  # Read the value from the .current english class file
  current_english_class=$(<.current_english_class) 2>/dev/null
...
get_sample_items() {
	...
	while true; do
		if [ ! -f .revise.txt ]; then
			...
			if [[ -n $input ]]; then
				libreoffice .english_samples* > /dev/null 2>&1 &
				clear
				popd > /dev/null || exit
				return
			fi
					...
					while IFS= read -r line || [ -n "$line" ]; do
						...
						for sentence in "${sentences[@]}"; do
							if [[ -n "$sentence" && "$sentence" =~ [[:graph:]] ]]; then
								...
								if [[ $sentence == *"Video"* ]]; then
									# Open the video file
									vlc --one-instance --playlist-enqueue "../../../Videos/English/$sentence" > /dev/null 2>&1 &
								fi
								if [ $((sentence_count % 5)) -eq 0 ]; then
									# Clear and center for every 5th sentence
									clear_and_center "${y}$sentence${t} \c"
