...
function handle_class_input() {
  if { [ -n "$last_class" ] || [ -z "$last_topic" ]; } && [ -f .chinese_surveyor ]; then
		while true; do
	    read -rp $'\n\nPlease enter '"${r}1 or 2 or 3 or 4${t}"' to go to your class or '"${r}x${t}"' to exit'$'\n\n> ' class
      if [ "$class" == "x" ]; then
        exit
      fi
		  # Check if .current_chinese_class is accidentally empty
		  if [ ! -s .current_chinese_class ]; then
		    # Echo 1 to .current_chinese_class
		    echo "1" > .current_chinese_class
		  fi
		  # Read the value from the file
		  current_chinese_class=$(<.current_chinese_class) 2>/dev/null
		  # Check if the value is equal to $class
		  if [ "$current_chinese_class" -lt "$class" 2>/dev/null ]; then
	      ...
	      rm -f ".chinese_topic_selected"
	      # Update the state file with the class
	      if [ "$class" != "x" ]; then
	        echo "$class" > .chinese_user_state 2>/dev/null
	      fi
				break
	  	fi
		done
...
function handle_s3_topic_input() {
  ...
  if [ -z "$last_topic" ] || [ -f .chinese_topic_selected ]; then
    read -rp $'\n\nChoose either topic '"${g}1 or 2 or 3 or 4 or 5 or 6 or 7 or 8${t}"' to learn'$'\nor enter '"${r}z${t}"' for an adventure or '"${r}r${t}"' to revise or '"${r}s${t}"' to get sample_items'$'\nor '"${r}a${t}"' to get an activity of integration or '"${r}q${t}"' to get a short answer question'$'\nor '"${r}n${t}"' to do your final class assignment and if necessary, gain access to the next class'$'\nor '"${r}p${t}"' to track academic progress or '"${r}x${t}"' to exit'$'\n\n1. Shopping '"${r}Term1${t}"''$'\n\n2. The environment '"${r}Term1${t}"''$'\n\n3. Health '"${r}Term2${t}"''$'\n\n4. Fashion '"${r}Term3${t}"''$'\n\n5. Entertainment '"${r}Term2${t}"''$'\n\n6. Media '"${r}Term3${t}"''$'\n\n7. Travel '"${r}Term3${t}"''$'\n\n8. Custom '"${r}Term3${t}"''$'\n\n> ' topic
    touch .chinese_surveyor
    touch .chinese_topic_selected
    # Update the state file with the topic
    # Check if the state file exists, and the topic is not "x"
...
function handle_s2_topic_input() {
  ...
  if [ -z "$last_topic" ] || [ -f .chinese_topic_selected ]; then
    read -rp $'\n\nChoose either topic '"${g}1 or 2 or 3 or 4 or 5 or 6 or 7 or 8${t}"' to learn'$'\nor enter '"${r}z${t}"' for an adventure or '"${r}r${t}"' to revise or '"${r}s${t}"' to get sample_items'$'\nor '"${r}a${t}"' to get an activity of integration or '"${r}q${t}"' to get a short answer question'$'\nor '"${r}n${t}"' to do your final class assignment and if necessary, gain access to the next class'$'\nor '"${r}p${t}"' to track academic progress or '"${r}x${t}"' to exit'$'\n\n1. Market '"${r}Term1${t}"''$'\n\n2. Weather '"${r}Term1${t}"''$'\n\n3. Job and careers '"${r}Term1${t}"''$'\n\n4. Hobbies '"${r}Term1${t}"''$'\n\n5. Transport '"${r}Term1${t}"''$'\n\n6. Travel and tourism '"${r}Term2${t}"''$'\n\n7. Friends '"${r}Term2${t}"''$'\n\n8. My house '"${r}Term3${t}"''$'\n\n> ' topic
    touch .chinese_surveyor
    touch .chinese_topic_selected
    # Update the state file with the topic
    # Check if the state file exists, and the topic is not "x"
...
function handle_s1_topic_input() {
  ...
  if [ -z "$last_topic" ] || [ -f .chinese_topic_selected ]; then
    read -rp $'\n\nChoose either topic '"${g}1 or 2 or 3 or 4 or 5 or 6 or 7 or 8${t}"' to learn'$'\nor enter '"${r}z${t}"' for an adventure or '"${r}r${t}"' to revise or '"${r}s${t}"' to get sample_items'$'\nor '"${r}a${t}"' to get an activity of integration or '"${r}q${t}"' to get a short answer question'$'\nor '"${r}n${t}"' to do your final class assignment and if necessary, gain access to the next class'$'\nor '"${r}p${t}"' to track academic progress or '"${r}x${t}"' to exit'$'\n\n1. Introduction to Chinese pronounciation '"${r}Term1${t}"''$'\n\n2. Greetings '"${r}Term1${t}"''$'\n\n3. Introduction of self and others '"${r}Term2${t}"''$'\n\n4. Identification '"${r}Term2${t}"''$'\n\n5. My family '"${r}Terms 2&3${t}"''$'\n\n6. Food '"${r}Term3${t}"''$'\n\n7. School life '"${r}Term3${t}"''$'\n\n8. Telling time '"${r}Term3${t}"''$'\n\n> ' topic
    touch .chinese_surveyor
    touch .chinese_topic_selected
    # Update the state file with the topic
    # Check if the state file exists, and the topic is not "x"
    ...
    for sentence in "${sentences[@]}"; do
      if [[ $echo_next == true ]]; then
        ...
        for split_sentence in "${split_sentences[@]}"; do
					echo "$split_sentence" >> .chinese_reminder
        done
      echo_next=false
      fi
        ...
        echo "Did you know that $lower_cased_sentence" >> .chinese_reminder
        ...
        echo_next=true
  			...
  			echo "Do you recall that $lower_cased_sentence;" >> .chinese_reminder

			elif [[ "$sentence" =~ " → " ]]; then
  			echo "Hope you know that: $sentence;" >> .chinese_reminder

      elif [[ "$sentence" =~ " ↔ " ]]; then
        echo "Hope you know that: $sentence;" >> .chinese_reminder

      elif [[ "$sentence" =~ (Generally|general) ]]; then
        echo "Note: $sentence;" >> .chinese_reminder
...
process_random_reminder() {
  ...
  # Check if .chinese_reminder exists
  if [ -f .chinese_reminder ]; then
		# Remove empty lines from .chinese_reminder
  	sed -i '/^[[:space:]]*$/d' .chinese_reminder 2>/dev/null
    # Randomly select a non-empty sentence
    local reminder # Declare the variable
		reminder=$(awk -v RS=';' 'BEGIN{srand();}{gsub(/^[[:space:]]+|[[:space:]]+$/, ""); if (length > 0) a[++n]=$0}END{if (n > 0) print a[int(rand()*n)+1]}' .chinese_reminder)
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    for sentence in "${sentences[@]}"; do
        ...
        if [[ $sentence == *"Figure"* ]]; then
          ...
          # Change to the "Figures/Chinese" directory
          cd Figures/Chinese || { echo "Failed to change to Figures/Chinese"; return; }
        ...
        if [[ $sentence == *"Table"* ]]; then
          cd Tables/Chinese || { echo -e "\nFailed to change to Tables/Chinese \c"; return; }
          explorer.exe "$sentence" > /dev/null 2>&1 &
          cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        fi

        if [[ $sentence == *"Video"* ]]; then
          cd Videos/Chinese || { echo -e "\nFailed to change to Videos/Chinese \c"; return; }
          explorer.exe "$sentence" > /dev/null 2>&1 &
          cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        fi
...
contact_ai() {
  last_topic=$(awk -F' ' '{print $2}' .chinese_user_state)
...
while true; do
  ...
  # Prompt the user for input
  if [ -z "$(find ./Notes/Chinese -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
  ...
  if [[ -n "$user_input" ]]; then
     ...
     if [[ "$user_input" == "sr" ]]; then
      ...
      explorer.exe Chinese > /dev/null 2>&1 &
    ...
    if [[ "$user_input" == "zz" ]]; then
      ...
      TEMP_FILE=$(mktemp) && curl -o "$TEMP_FILE" -L "https://github.com/Muhumuza7325/OMD/raw/main/chinese_tutorial_wsl.sh" && mv "$TEMP_FILE" chinese_tutorial_wsl.sh && chmod +x chinese_tutorial_wsl.sh && cp chinese_tutorial_wsl.sh Students/Omd && echo -e "\n\n${y}Code successfully updated.. You will have to restart a new session${t} \c" && sleep 4 && exit || (echo -e "\n\n${m}Error updating code!... Please check your internet connection and try again!${t} \c" && rm -f "$TEMP_FILE" && return)
    fi
    if [[ "$user_input" == "xx" ]]; then
      ...
      if [[ "$input" == "y" || "$input" == "Y" ]]; then
        ...
        curl -O -L "https://github.com/Muhumuza7325/OMD/raw/main/update_chinese.sh" || { echo -e "\n\n${m}Check your internet connection and try again!${t}" >&2; return; }
        mv update_chinese.sh .update_chinese.sh
        bash .update_chinese.sh
        return
      else
        return
      fi
    ...
    if [[ -n "$result" ]]; then
      ...
      if echo "$result" | grep "Figure"; then
        echo "$result" | grep -o '\bFigure[0-9]\+.*\.jpg\(\.[0-9]\+\)*\b' > Figures/Chinese/figures.txt
        sed -i -e '/^Figure/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Figures/Chinese/figures.txt
        # Change to the "Figures/Chinese" directory
        cd Figures/Chinese || { echo "Failed to change to Figures/Chinese"; return; }
        ...
        rm -f Figures/Chinese/figures.txt
      fi

      if echo "$result" | grep "Table"; then
        echo "$result" | grep -o '\bTable[0-9]\+\(\.[0-9]\+\)*\b' > Tables/Chinese/tables.txt
        sed -i -e '/^Table/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Tables/Chinese/tables.txt
        cd Tables/Chinese || { echo -e "\nFailed to change to Tables/Chinese \c"; return; }
        ...
        # Go back to the original directory
        cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        rm -f Tables/Chinese/tables.txt
      fi

      if echo "$result" | grep "Video"; then
        #echo "$result" > Videos/Chinese/videos.txt
        echo "$result" | grep -o '\bVideo[0-9]\+\(\.[0-9]\+\)*\b' > Videos/Chinese/videos.txt
        # Remove lines not starting with "Video" and any leading/trailing whitespaces
        sed -i -e '/^Video/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Videos/Chinese/videos.txt
        # Change to the "Videos/Chinese" directory
        cd Videos/Chinese || { echo -e "\nFailed to change to Videos/Chinese \c"; return; }
        ...
        cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        # Remove the temporary file
        rm -f Videos/Chinese/videos.txt
      fi
  ...
  if [ -f "$selected_file" ]; then
    ...
    if [[ -n "$selected_question" && "$selected_question" =~ [[:graph:]] ]]; then
      ...
      if echo "$selected_question" | grep "Figure"; then
        echo "$selected_question" | grep -o '\bFigure[0-9]\+[^;]*\b' > ../../Figures/Chinese/figures.txt
        sed -i -e '/^Figure/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' ../../Figures/Chinese/figures.txt
        # Change to the "Figures/Chinese" directory
        cd ../../Figures/Chinese || { echo "Failed to change to Figures/Chinese"; return; }
        # Specify the path to the text file containing figure names
        ...
        # Go back to the original directory
        cd ../../Revision/Chinese || { echo "Failed to change back to the targeted directory \c"; exit 1; }
        # Remove the temporary file
        rm -f ../../Figures/Chinese/figures.txt
      fi
...
process_question_answer() {
  ...
  if [ -f ../../../.school_name ]; then
    ...
    sed -i '/^[[:space:]]*$/d' ../../../."$school_name"_students_file.txt
    existing_class=$(awk '{print $1}' ../../../.chinese_user_state)
    existing_topic=$(awk '{print $2}' ../../../.chinese_user_state)
    echo ''
    ...
    if grep -q "Chinese" ../../../."$school_name"_students_file.txt; then
      sed -i -E 's/;/\n/g' ../../../."$school_name"_students_file.txt
      # Find the line with the word Chinese, replace the information below it with the already available information adding a comma existing_class_existing_topic [$percentage]
      if [ "$existing_topic" == "r" ]; then
        sed -i '/Chinese/{n;s/\(.*\)/\1, '"$existing_class"'_'"$input"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
      else
        (( existing_topic-- ))
        sed -i '/Chinese/{n;s/\(.*\)/\1, '"$existing_class"'_'"$existing_topic"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
      fi
    else
      if [ "$existing_topic" == "r" ]; then
        sed -i -E '/^School/ i\Chinese\n'"${existing_class} ${names}[${percentage}%]"'' ../../../."$school_name"_students_file.txt
...
function resume_from_last_point() {
  if [ -f .chinese_user_state ]; then
    last_class=$(awk -F' ' '{print $1}' .chinese_user_state)
    last_topic=$(awk -F' ' '{print $2}' .chinese_user_state)
    if [ -n "$last_class" ] && [ -n "$last_topic" ]; then
      echo -e "\n       Resuming from ${r}S$last_class${t} : ${g}Topic '$last_topic'${t} \c"
      rm -f .chinese_surveyor
...
process_final_assignment() {
  # Check if user is.chinese_ready for the assignment
  # Check if .current_chinese_class is accidentally empty
  if [ ! -s .current_chinese_class ]; then
    # Echo 1 to .current_chinese_class
    echo "1" > .current_chinese_class
  fi
  # Read the value from the .current_chinese_class file
  current_chinese_class=$(<.current_chinese_class) 2>/dev/null
  # Check if the value in the .chinese_ready file is equal to $class
  # Read the value from the .chinese_ready file
  if [ ! -s .chinese_ready ]; then
    echo "0" > .chinese_ready
  fi
  how.chinese_ready=$(<.chinese_ready) 2>/dev/null
  # Check if the value is equal to $class
  if [ "$how.chinese_ready" -lt "$current_chinese_class" 2>/dev/null ]; then
    read -rp $'\n\nYou havent done all the topic assignments for your current chinese class\n\n'"${r}Proceeding from here will affect your very final score${y}"'. To go back and progress right, enter '"${y}ok${t}"'. Otherwise, press the Enter key to do the final class assignment: ' progress
    if [ "$progress" == "ok" ]; then
      return
    fi
  fi
    ...
    if ! [ "$total_questions" == 100 ]; then
      ...
      if [ "$elapsed_time" -gt 3600 ]; then
        ...
        if [ "$percentage" -lt 95 ]; then
          ...
          cd - > /dev/null 2>&1 || exit
          if ! [ -f .current_chinese_class ]; then
            # Echo the result to the file .current_class
            echo "2" > .current_chinese_class
            ...
            # Read the value from the file
            current_chinese_class=$(<.current_chinese_class) 2>/dev/null
            # Check if the new_class value is lt $current_chinese_class
            if [ "$new_class" -gt "$current_chinese_class" 2>/dev/null ]; then
		          # Echo the result to the file .current_class
		          echo "$new_class" > .current_chinese_class
            fi
...
get_sample_items() {
	...
	# Save the current working directory
	pushd . > /dev/null
	cd Revision/Chinese || { echo "Directory not found"; return; }
	...
	while true; do
		if [ ! -f .revise.txt ]; then
			...
			if [ $time_diff -gt 3600 ]; then
				...
				echo -e "\n\n\n${r}You are advised to not make any changes to the provided answers, instead, you can make copies that you can edit${t}\n\n${y}For a teacher willing to join us reach out to everyone of our children, please send us your questions and answers in a file labelled with your name, school, chinese, and file content (e.g., Muhumuza_Omega_Kasule_High_School_O_level_Chemistry_Answered_EOC1_Items.pdf) to our contacts${t}\n\n\nEmail: ${g}2024omd256@gmail.com${t} \c"
				echo $current_time > "$last_echo_time_file"
			fi
			read -rp $'\n\n\nEnter '"${m}any character${t}"' for access to the file of answered items or simply press '"${r}Enter${t}"' to get items to attempt : ' input
			if [[ -n $input ]]; then
				explorer.exe .chinese_samples* > /dev/null 2>&1 &
				clear
				popd > /dev/null || exit
				return
			fi
		...
		rm -f .revise.txt .chinese_topic_selected 2>/dev/null
	done
	popd > /dev/null || exit
	return
}
...
while true; do
  ...
  if ls Students | grep -wq "$initials"; then
    ...
    echo -e "                  $initials\n" > "Students/$initials/Exercise/chinese_answered_ans.txt"
    echo -e "                  $initials\n" > "Students/$initials/Revision/chinese_covered_qns.txt"
    ...
    if [ -f ../../../.school_name ]; then
      ...
      if grep -q "Chinese" ../../../."$school_name"_students_file.txt; then
        ...
        if [ "$existing_topic" == "z" ]; then
          sed -i '/Chinese/{n;s/\(.*\)/\1, '"$existing_class"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
        else
          (( existing_topic-- ))
          sed -i '/Chinese/{n;s/\(.*\)/\1, '"$existing_class"'_'"$existing_topic"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
        fi
...
if [ ! -f .chinese_user_state ]; then
  ...
  if [ -f .omd_communication.txt ]; then
    ...
    process_random_reminder .chinese_reminder
  fi
	handle_resume_input
fi

if [ -z "$class" ] && [ -s ".chinese_user_state" ]; then
  get_and_display_pattern
fi
# Check for the presence of specific directories and a file
if ! [ -d "Notes/Chinese" ] || ! [ -d "Revision" ] || ! [ -d "Exercise" ] || ! [ -d "Videos" ] || ! [ -d "Figures" ] || ! [ -d "Tables" ]; then
  ...
  mkdir -p Notes Notes/Chinese Revision Revision/Chinese Revision/Chinese/{S1,S2,S3,S4} Exercise Exercise/Chinese Exercise/Chinese/{S1,S2,S3,S4} Videos Videos/Chinese Figures Figures/Chinese Tables Tables/Chinese
  touch Revision/chinese_covered_qns.txt Exercise/chinese_answered_ans.txt
  echo -e "\n"
  pwd
...
if [ -z "$(find ./Notes/Chinese -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
  ...
  if [[ -z "$user_input" ]]; then
    ...
    curl -O -L https://github.com/Muhumuza7325/OMD/raw/main/CHINESE/1.1.introduction_to_chinese_pronounciation.txt || echo -e "\n\nError fetching material for this tutorial \c"
    ...
    cp 1.1.introduction_to_chinese_pronounciation.txt Notes/Chinese || echo -e "\n\nError copying 1.1.introduction_to_chinese_pronounciation.txt to the Chinese directory in the Notes directory \c"
    wait_for_a_key_press
  else
  ...
  rm -f 1.1.introduction_to_chinese_pronounciation.txt
fi
...
while true; do
  ...
  handle_class_input
  if [[ "$class" == "1" ]]; then
    if ! find . -maxdepth 1 -name '.s_chinese_1*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to S1 Chinese class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
    ...
    do
      ...
      do
        ...
        then
          ...
          question_directory="Revision/Chinese/S1"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../chinese_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Chinese/S1"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../chinese_answered_ans.txt"
          # Call the function to process a random question
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_chinese_1_1" ]; then
                echo -e "\n\nYou chose to explore Introduction to Chinese pronounciation ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/Chinese/1.1.introduction_to_chinese_pronounciation.txt" . || exit 1
              mv 1.1.introduction_to_chinese_pronounciation.txt .1.1.introduction_to_chinese_pronounciation.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.1.introduction_to_chinese_pronounciation.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.1.introduction_to_chinese_pronounciation.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.1.introduction_to_chinese_pronounciation.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.1.introduction_to_chinese_pronounciation.txt
              process_reminders_from_file .1.1.introduction_to_chinese_pronounciation.txt
              STATE_FILE=".s_chinese_1_1"
              process_file .1.1.introduction_to_chinese_pronounciation.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.1.introduction_to_chinese_pronounciation.txt
              sed -i '/^1$/!d' .s_chinese_1_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chinese/S1"
              # Define the file extension
              file_extension=".1.introduction_to_chinese_pronounciation.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              ...
              question_directory="Revision/Chinese/S1"
              # Define the file extension
              file_extension_question=".1.introduction_to_chinese_pronounciation.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".chinese.1.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chinese/S1"
                # Define the file extension
                file_extension_answer=".1.introduction_to_chinese_pronounciation.ans.txt"
                # Define the exercise file
                exercise_file="../../chinese_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_chinese_1_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Greetings ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Chinese/1.2.greetings.txt" . || exit 1
              mv 1.2.greetings.txt .1.2.greetings.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.2.greetings.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.2.greetings.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.2.greetings.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.2.greetings.txt
              process_reminders_from_file .1.2.greetings.txt
              STATE_FILE=".s_chinese_1_2"
              process_file .1.2.greetings.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.2.greetings.txt
              sed -i '/^1$/!d' .s_chinese_1_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chinese/S1"
              # Define the file extension
              file_extension_question=".2.greetings.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".chinese.1.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chinese/S1"
                # Define the file extension
                file_extension_answer=".2.greetings.ans.txt"
                # Define the exercise file
                exercise_file="../../chinese_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_chinese_1_3" ]; then
                echo -e "\n\nYou have made a choice to cover Introduction, of, self and others ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Chinese/1.3.introduction_of_self_and_others.txt" . || exit 1
              mv 1.3.introduction_of_self_and_others.txt .1.3.introduction_of_self_and_others.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.3.introduction_of_self_and_others.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.3.introduction_of_self_and_others.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.3.introduction_of_self_and_others.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.3.introduction_of_self_and_others.txt
              process_reminders_from_file .1.3.introduction_of_self_and_others.txt
              STATE_FILE=".s_chinese_1_3"
              process_file .1.3.introduction_of_self_and_others.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.3.introduction_of_self_and_others.txt
              sed -i '/^1$/!d' .s_chinese_1_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chinese/S1"
              # Define the file extension
              file_extension_question=".3.introduction_of_self_and_others.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".chinese.1.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chinese/S1"
                # Define the file extension
                file_extension_answer=".3.introduction_of_self_and_others.ans.txt"
                # Define the exercise file
                exercise_file="../../chinese_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_chinese_1_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Identification ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/Chinese/1.4.identification.txt" . || exit 1
              mv 1.4.identification.txt .1.4.identification.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.4.identification.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.4.identification.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.4.identification.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.4.identification.txt
              process_reminders_from_file .1.4.identification.txt
              STATE_FILE=".s_chinese_1_4"
              process_file .1.4.identification.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.4.identification.txt
              sed -i '/^1$/!d' .s_chinese_1_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chinese/S1"
              # Define the file extension
              file_extension=".4.identification.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Chinese/S1"
              # Define the file extension
              file_extension_question=".4.identification.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            5)
              if ! [ -f ".chinese.1.4" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chinese/S1"
                # Define the file extension
                file_extension_answer=".4.identification.ans.txt"
                # Define the exercise file
                exercise_file="../../chinese_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_chinese_1_5" ]; then
                echo -e "\n\nHere you are dear one... Stay organised as you explore My family ...\n\n${g}Just know we are not going to leave you alone${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Chinese/1.5.my_family.txt" . || exit 1
              mv 1.5.my_family.txt .1.5.my_family.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.5.my_family.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.5.my_family.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.5.my_family.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.5.my_family.txt
              process_reminders_from_file .1.5.my_family.txt
              STATE_FILE=".s_chinese_1_5"
              process_file .1.5.my_family.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.5.my_family.txt
              sed -i '/^1$/!d' .s_chinese_1_5
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chinese/S1"
              # Define the file extension
              file_extension_question=".5.my_family.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            6)
              if ! [ -f ".chinese.1.5" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chinese/S1"
                # Define the file extension
                file_extension_answer=".5.my_family.ans.txt"
                # Define the exercise file
                exercise_file="../../chinese_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_chinese_1_6" ]; then
                echo -e "\n\nYou have managed to make it to Food ...\n\n${g}Remember to pray always${t}\n\nThe fear of the Lord is the beginning of wisdom \c" && wait_for_a_key_press
              fi
              cp "Notes/Chinese/1.6.food.txt" . || exit 1
              mv 1.6.food.txt .1.6.food.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.6.food.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.6.food.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.6.food.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.6.food.txt
              process_reminders_from_file .1.6.food.txt
              STATE_FILE=".s_chinese_1_6"
              process_file .1.6.food.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.6.food.txt
              sed -i '/^1$/!d' .s_chinese_1_6
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chinese/S1"
              # Define the file extension
              file_extension_question=".6.food.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            7)
              if ! [ -f ".chinese.1.6" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chinese/S1"
                # Define the file extension
                file_extension_answer=".6.food.ans.txt"
                # Define the exercise file
                exercise_file="../../chinese_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_chinese_1_7" ]; then
                echo -e "\n\nFrom here, you will be proceeding with School life ...\n\n${g}Please never ever forget that your education is your future${t}\n\nFocus dear \c" && wait_for_a_key_press
              fi
              cp "Notes/Chinese/1.7.school_life.txt" . || exit 1
              mv 1.7.school_life.txt .1.7.school_life.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.7.school_life.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.7.school_life.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.7.school_life.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.7.school_life.txt
              process_reminders_from_file .1.7.school_life.txt
              STATE_FILE=".s_chinese_1_7"
              process_file .1.7.school_life.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.7.school_life.txt
              sed -i '/^1$/!d' .s_chinese_1_7
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chinese/S1"
              # Define the file extension
              file_extension_question=".7.school_life.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            8)
              if ! [ -f ".chinese.1.7" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chinese/S1"
                # Define the file extension
                file_extension_answer=".7.school_life.ans.txt"
                # Define the exercise file
                exercise_file="../../chinese_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_chinese_1_8" ]; then
                echo -e "\n\nYou are to cover Telling time ...\n\n${g}Please never ever settle for less${t}\n\nPromise yourself that you wont give up \c" && wait_for_a_key_press
              fi
              cp "Notes/Chinese/1.8.telling_time.txt" . || exit 1
              mv 1.8.telling_time.txt .1.8.telling_time.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.8.telling_time.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.8.telling_time.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.8.telling_time.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.8.telling_time.txt
              process_reminders_from_file .1.8.telling_time.txt
              STATE_FILE=".s_chinese_1_8"
              process_file .1.8.telling_time.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.8.telling_time.txt
              sed -i '/^1$/!d' .s_chinese_1_8
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chinese/S1"
              # Define the file extension
              file_extension_question=".8.telling_time.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".chinese.1.8" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/Chinese/S1"
	              # Define the file extension
	              file_extension_answer=".8.telling_time.ans.txt"
	              # Define the exercise file
	              exercise_file="../../chinese_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
								touch .chinese.1.8
								echo "1" > .chinese_ready
							fi
            ;;
    ...
    if ! find . -maxdepth 1 -name '.s_chinese_2*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to S2 Chinese class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
      clear_and_center "There are ${r}8${t} topics to be covered. Your tasks will always expand or shrink to fit in the time you give them. For that reason, never procrastinate darling!"
    fi
    attempts=0
    ...
    do
      ...
      do
        ...
        then
          attempts=0
          # Define the targeted directory
          question_directory="Revision/Chinese/S2"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../chinese_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Chinese/S2"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../chinese_answered_ans.txt"
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_chinese_2_1" ]; then
                echo -e "\n\nYou chose to explore Market ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/Chinese/2.1.market.txt" . || exit 1
              mv 2.1.market.txt .2.1.market.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.1.market.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.1.market.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.1.market.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.1.market.txt
              process_reminders_from_file .2.1.market.txt
              STATE_FILE=".s_chinese_2_1"
              process_file .2.1.market.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.1.market.txt
              sed -i '/^1$/!d' .s_chinese_2_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chinese/S2"
              # Define the file extension
              file_extension=".1.market.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Chinese/S2"
              # Define the file extension
              file_extension_question=".1.market.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".chinese.2.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chinese/S2"
                # Define the file extension
                file_extension_answer=".1.market.ans.txt"
                # Define the exercise file
                exercise_file="../../chinese_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_chinese_2_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Weather ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Chinese/2.2.weather.txt" . || exit 1
              mv 2.2.weather.txt .2.2.weather.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.2.weather.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.2.weather.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.2.weather.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.2.weather.txt
              process_reminders_from_file .2.2.weather.txt
              STATE_FILE=".s_chinese_2_2"
              process_file .2.2.weather.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.2.weather.txt
              sed -i '/^1$/!d' .s_chinese_2_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chinese/S2"
              # Define the file extension
              file_extension_question=".2.weather.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".chinese.2.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chinese/S2"
                # Define the file extension
                file_extension_answer=".2.weather.ans.txt"
                # Define the exercise file
                exercise_file="../../chinese_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_chinese_2_3" ]; then
                echo -e "\n\nYou have made a choice to cover Job and careers ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Chinese/2.3.job_and_careers.txt" . || exit 1
              mv 2.3.job_and_careers.txt .2.3.job_and_careers.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.3.job_and_careers.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.3.job_and_careers.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.3.job_and_careers.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.3.job_and_careers.txt
              process_reminders_from_file .2.3.job_and_careers.txt
              STATE_FILE=".s_chinese_2_3"
              process_file .2.3.job_and_careers.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.3.job_and_careers.txt
              sed -i '/^1$/!d' .s_chinese_2_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chinese/S2"
              # Define the file extension
              file_extension_question=".3.job_and_careers.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".chinese.2.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chinese/S2"
                # Define the file extension
                file_extension_answer=".3.job_and_careers.ans.txt"
                # Define the exercise file
                exercise_file="../../chinese_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_chinese_2_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Hobbies ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/Chinese/2.4.hobbies.txt" . || exit 1
              mv 2.4.hobbies.txt .2.4.hobbies.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.4.hobbies.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.4.hobbies.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.4.hobbies.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.4.hobbies.txt
              process_reminders_from_file .2.4.hobbies.txt
              STATE_FILE=".s_chinese_2_4"
              process_file .2.4.hobbies.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.4.hobbies.txt
              sed -i '/^1$/!d' .s_chinese_2_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chinese/S2"
              # Define the file extension
              file_extension=".4.hobbies.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Chinese/S2"
              # Define the file extension
              file_extension_question=".4.hobbies.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            5)
              if ! [ -f ".chinese.2.4" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chinese/S2"
                # Define the file extension
                file_extension_answer=".4.hobbies.ans.txt"
                # Define the exercise file
                exercise_file="../../chinese_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_chinese_2_5" ]; then
                echo -e "\n\nHere you are dear one... Stay organised as you explore Transport ...\n\n${g}Just know we are not going to leave you alone${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Chinese/2.5.transport.txt" . || exit 1
              mv 2.5.transport.txt .2.5.transport.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.5.transport.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.5.transport.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.5.transport.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.5.transport.txt
              process_reminders_from_file .2.5.transport.txt
              STATE_FILE=".s_chinese_2_5"
              process_file .2.5.transport.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.5.transport.txt
              sed -i '/^1$/!d' .s_chinese_2_5
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chinese/S2"
              # Define the file extension
              file_extension_question=".5.transport.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            6)
              if ! [ -f ".chinese.2.5" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chinese/S2"
                # Define the file extension
                file_extension_answer=".5.transport.ans.txt"
                # Define the exercise file
                exercise_file="../../chinese_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_chinese_2_6" ]; then
                echo -e "\n\nYou have managed to make it to Travel and tourism ...\n\n${g}Remember to pray always${t}\n\nThe fear of the Lord is the beginning of wisdom \c" && wait_for_a_key_press
              fi
              cp "Notes/Chinese/2.6.travel_and_tourism.txt" . || exit 1
              mv 2.6.travel_and_tourism.txt .2.6.travel_and_tourism.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.6.travel_and_tourism.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.6.travel_and_tourism.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.6.travel_and_tourism.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.6.travel_and_tourism.txt
              process_reminders_from_file .2.6.travel_and_tourism.txt
              STATE_FILE=".s_chinese_2_6"
              process_file .2.6.travel_and_tourism.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.6.travel_and_tourism.txt
              sed -i '/^1$/!d' .s_chinese_2_6
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chinese/S2"
              # Define the file extension
              file_extension_question=".6.travel_and_tourism.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            7)
              if ! [ -f ".chinese.2.6" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chinese/S2"
                # Define the file extension
                file_extension_answer=".6.travel_and_tourism.ans.txt"
                # Define the exercise file
                exercise_file="../../chinese_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_chinese_2_7" ]; then
                echo -e "\n\nFrom here, you will be proceeding with Friends ...\n\n${g}Please never ever forget that your education is your future${t}\n\nFocus dear \c" && wait_for_a_key_press
              fi
              cp "Notes/Chinese/2.7.friends.txt" . || exit 1
              mv 2.7.friends.txt .2.7.friends.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.7.friends.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.7.friends.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.7.friends.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.7.friends.txt
              process_reminders_from_file .2.7.friends.txt
              STATE_FILE=".s_chinese_2_7"
              process_file .2.7.friends.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.7.friends.txt
              sed -i '/^1$/!d' .s_chinese_2_7
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chinese/S2"
              # Define the file extension
              file_extension_question=".7.friends.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            8)
              if ! [ -f ".chinese.2.7" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chinese/S2"
                # Define the file extension
                file_extension_answer=".7.friends.ans.txt"
                # Define the exercise file
                exercise_file="../../chinese_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_chinese_2_8" ]; then
                echo -e "\n\nYou are to cover My house ...\n\n${g}Please never ever settle for less${t}\n\nPromise yourself that you wont give up \c" && wait_for_a_key_press
              fi
              cp "Notes/Chinese/2.8.my_house.txt" . || exit 1
              mv 2.8.my_house.txt .2.8.my_house.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.8.my_house.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.8.my_house.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.8.my_house.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.8.my_house.txt
              process_reminders_from_file .2.8.my_house.txt
              STATE_FILE=".s_chinese_2_8"
              process_file .2.8.my_house.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.8.my_house.txt
              sed -i '/^1$/!d' .s_chinese_2_8
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chinese/S2"
              # Define the file extension
              file_extension_question=".8.my_house.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".chinese.2.8" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/Chinese/S2"
	              # Define the file extension
	              file_extension_answer=".8.my_house.ans.txt"
	              # Define the exercise file
	              exercise_file="../../chinese_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
								touch .chinese.2.8
								echo "2" > .chinese_ready
							fi
            ;;
    ...
    if ! find . -maxdepth 1 -name '.s_chinese_3*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to S3 Chinese class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
      clear_and_center "There are ${r}8${t} topics to be covered. Your tasks will always expand or shrink to fit in the time you give them. For that reason, never procrastinate darling!"
    fi
    attempts=0
    ...
    do
      ...
      do
        ...
        then
          attempts=0
          # Define the targeted directory
          question_directory="Revision/Chinese/S3"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../chinese_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Chinese/S3"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../chinese_answered_ans.txt"
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_chinese_3_1" ]; then
                echo -e "\n\nYou chose to explore Shopping ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/Chinese/3.1.shopping.txt" . || exit 1
              mv 3.1.shopping.txt .3.1.shopping.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.1.shopping.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.1.shopping.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.1.shopping.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.1.shopping.txt
              process_reminders_from_file .3.1.shopping.txt
              STATE_FILE=".s_chinese_3_1"
              process_file .3.1.shopping.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.1.shopping.txt
              sed -i '/^1$/!d' .s_chinese_3_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chinese/S3"
              # Define the file extension
              file_extension=".1.shopping.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Chinese/S3"
              # Define the file extension
              file_extension_question=".1.shopping.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".chinese.3.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chinese/S3"
                # Define the file extension
                file_extension_answer=".1.shopping.ans.txt"
                # Define the exercise file
                exercise_file="../../chinese_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_chinese_3_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into The environment ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Chinese/3.2.the_environment.txt" . || exit 1
              mv 3.2.the_environment.txt .3.2.the_environment.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.2.the_environment.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.2.the_environment.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.2.the_environment.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.2.the_environment.txt
              process_reminders_from_file .3.2.the_environment.txt
              STATE_FILE=".s_chinese_3_2"
              process_file .3.2.the_environment.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.2.the_environment.txt
              sed -i '/^1$/!d' .s_chinese_3_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chinese/S3"
              # Define the file extension
              file_extension_question=".2.the_environment.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".chinese.3.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chinese/S3"
                # Define the file extension
                file_extension_answer=".2.the_environment.ans.txt"
                # Define the exercise file
                exercise_file="../../chinese_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_chinese_3_3" ]; then
                echo -e "\n\nYou have made a choice to cover Health ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Chinese/3.3.health.txt" . || exit 1
              mv 3.3.health.txt .3.3.health.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.3.health.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.3.health.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.3.health.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.3.health.txt
              process_reminders_from_file .3.3.health.txt
              STATE_FILE=".s_chinese_3_3"
              process_file .3.3.health.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.3.health.txt
              sed -i '/^1$/!d' .s_chinese_3_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chinese/S3"
              # Define the file extension
              file_extension_question=".3.health.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".chinese.3.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chinese/S3"
                # Define the file extension
                file_extension_answer=".3.health.ans.txt"
                # Define the exercise file
                exercise_file="../../chinese_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_chinese_3_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Fashion ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/Chinese/3.4.fashion.txt" . || exit 1
              mv 3.4.fashion.txt .3.4.fashion.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.4.fashion.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.4.fashion.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.4.fashion.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.4.fashion.txt
              process_reminders_from_file .3.4.fashion.txt
              STATE_FILE=".s_chinese_3_4"
              process_file .3.4.fashion.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.4.fashion.txt
              sed -i '/^1$/!d' .s_chinese_3_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chinese/S3"
              # Define the file extension
              file_extension=".4.fashion.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Chinese/S3"
              # Define the file extension
              file_extension_question=".4.fashion.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            5)
              if ! [ -f ".chinese.3.4" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chinese/S3"
                # Define the file extension
                file_extension_answer=".4.fashion.ans.txt"
                # Define the exercise file
                exercise_file="../../chinese_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_chinese_3_5" ]; then
                echo -e "\n\nHere you are dear one... Stay organised as you explore Entertainment ...\n\n${g}Just know we are not going to leave you alone${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Chinese/3.5.entertainment.txt" . || exit 1
              mv 3.5.entertainment.txt .3.5.entertainment.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.5.entertainment.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.5.entertainment.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.5.entertainment.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.5.entertainment.txt
              process_reminders_from_file .3.5.entertainment.txt
              STATE_FILE=".s_chinese_3_5"
              process_file .3.5.entertainment.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.5.entertainment.txt
              sed -i '/^1$/!d' .s_chinese_3_5
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chinese/S3"
              # Define the file extension
              file_extension_question=".5.entertainment.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            6)
              if ! [ -f ".chinese.3.5" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chinese/S3"
                # Define the file extension
                file_extension_answer=".5.entertainment.ans.txt"
                # Define the exercise file
                exercise_file="../../chinese_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_chinese_3_6" ]; then
                echo -e "\n\nYou have managed to make it to Media ...\n\n${g}Remember to pray always${t}\n\nThe fear of the Lord is the beginning of wisdom \c" && wait_for_a_key_press
              fi
              cp "Notes/Chinese/3.6.media.txt" . || exit 1
              mv 3.6.media.txt .3.6.media.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.6.media.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.6.media.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.6.media.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.6.media.txt
              process_reminders_from_file .3.6.media.txt
              STATE_FILE=".s_chinese_3_6"
              process_file .3.6.media.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.6.media.txt
              sed -i '/^1$/!d' .s_chinese_3_6
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chinese/S3"
              # Define the file extension
              file_extension_question=".6.media.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            7)
              if ! [ -f ".chinese.3.6" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chinese/S3"
                # Define the file extension
                file_extension_answer=".6.media.ans.txt"
                # Define the exercise file
                exercise_file="../../chinese_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_chinese_3_7" ]; then
                echo -e "\n\nFrom here, you will be proceeding with Travel ...\n\n${g}Please never ever forget that your education is your future${t}\n\nFocus dear \c" && wait_for_a_key_press
              fi
              cp "Notes/Chinese/3.7.travel.txt" . || exit 1
              mv 3.7.travel.txt .3.7.travel.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.7.travel.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.7.travel.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.7.travel.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.7.travel.txt
              process_reminders_from_file .3.7.travel.txt
              STATE_FILE=".s_chinese_3_7"
              process_file .3.7.travel.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.7.travel.txt
              sed -i '/^1$/!d' .s_chinese_3_7
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chinese/S3"
              # Define the file extension
              file_extension_question=".7.travel.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            8)
              if ! [ -f ".chinese.3.7" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chinese/S3"
                # Define the file extension
                file_extension_answer=".7.travel.ans.txt"
                # Define the exercise file
                exercise_file="../../chinese_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_chinese_3_8" ]; then
                echo -e "\n\nYou are to cover Custom ...\n\n${g}Please never ever settle for less${t}\n\nPromise yourself that you wont give up \c" && wait_for_a_key_press
              fi
              cp "Notes/Chinese/3.8.custom.txt" . || exit 1
              mv 3.8.custom.txt .3.8.custom.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.8.custom.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.8.custom.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.8.custom.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.8.custom.txt
              process_reminders_from_file .3.8.custom.txt
              STATE_FILE=".s_chinese_3_8"
              process_file .3.8.custom.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.8.custom.txt
              sed -i '/^1$/!d' .s_chinese_3_8
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chinese/S3"
              # Define the file extension
              file_extension_question=".8.custom.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".chinese.3.8" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/Chinese/S3"
	              # Define the file extension
	              file_extension_answer=".8.custom.ans.txt"
	              # Define the exercise file
	              exercise_file="../../chinese_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
								touch .chinese.3.8
								echo "3" > .chinese_ready
							fi
            ;;
    ...
    if ! find . -maxdepth 1 -name '.s_chinese_4*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to S4 Chinese class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
    ...
    do
      ...
      do
        ...
        then
          attempts=0
          # Define the targeted directory
          question_directory="Revision/Chinese/S4"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../chinese_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Chinese/S4"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../chinese_answered_ans.txt"
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_chinese_4_1" ]; then
                echo -e "\n\nYou chose to explore Personal profile ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/Chinese/4.1.personal_profile.txt" . || exit 1
              mv 4.1.personal_profile.txt .4.1.personal_profile.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.1.personal_profile.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.1.personal_profile.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.1.personal_profile.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.1.personal_profile.txt
              process_reminders_from_file .4.1.personal_profile.txt
              STATE_FILE=".s_chinese_4_1"
              process_file .4.1.personal_profile.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.1.personal_profile.txt
              sed -i '/^1$/!d' .s_chinese_4_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chinese/S4"
              # Define the file extension
              file_extension=".1.personal_profile.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Chinese/S4"
              # Define the file extension
              file_extension_question=".1.personal_profile.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".chinese.4.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chinese/S4"
                # Define the file extension
                file_extension_answer=".1.personal_profile.ans.txt"
                # Define the exercise file
                exercise_file="../../chinese_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_chinese_4_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Recommendation ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Chinese/4.2.recommendation.txt" . || exit 1
              mv 4.2.recommendation.txt .4.2.recommendation.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.2.recommendation.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.2.recommendation.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.2.recommendation.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.2.recommendation.txt
              process_reminders_from_file .4.2.recommendation.txt
              STATE_FILE=".s_chinese_4_2"
              process_file .4.2.recommendation.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.2.recommendation.txt
              sed -i '/^1$/!d' .s_chinese_4_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chinese/S4"
              # Define the file extension
              file_extension_question=".2.recommendation.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".chinese.4.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chinese/S4"
                # Define the file extension
                file_extension_answer=".2.recommendation.ans.txt"
                # Define the exercise file
                exercise_file="../../chinese_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_chinese_4_3" ]; then
                echo -e "\n\nYou have made a choice to cover Environment Part 2 ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Chinese/4.3.environment_part_2.txt" . || exit 1
              mv 4.3.environment_part_2.txt .4.3.environment_part_2.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.3.environment_part_2.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.3.environment_part_2.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.3.environment_part_2.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.3.environment_part_2.txt
              process_reminders_from_file .4.3.environment_part_2.txt
              STATE_FILE=".s_chinese_4_3"
              process_file .4.3.environment_part_2.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.3.environment_part_2.txt
              sed -i '/^1$/!d' .s_chinese_4_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chinese/S4"
              # Define the file extension
              file_extension_question=".3.environment_part_2.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".chinese.4.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chinese/S4"
                # Define the file extension
                file_extension_answer=".3.environment_part_2.ans.txt"
                # Define the exercise file
                exercise_file="../../chinese_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_chinese_4_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of House and furniture ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/Chinese/4.4.house_and_furniture.txt" . || exit 1
              mv 4.4.house_and_furniture.txt .4.4.house_and_furniture.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.4.house_and_furniture.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.4.house_and_furniture.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.4.house_and_furniture.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.4.house_and_furniture.txt
              process_reminders_from_file .4.4.house_and_furniture.txt
              STATE_FILE=".s_chinese_4_4"
              process_file .4.4.house_and_furniture.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.4.house_and_furniture.txt
              sed -i '/^1$/!d' .s_chinese_4_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chinese/S4"
              # Define the file extension
              file_extension=".4.house_and_furniture.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Chinese/S4"
              # Define the file extension
              file_extension_question=".4.house_and_furniture.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            5)
              if ! [ -f ".chinese.4.4" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chinese/S4"
                # Define the file extension
                file_extension_answer=".4.house_and_furniture.ans.txt"
                # Define the exercise file
                exercise_file="../../chinese_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_chinese_4_5" ]; then
                echo -e "\n\nHere you are dear one... Stay organised as you explore School life ...\n\n${g}Just know we are not going to leave you alone${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Chinese/4.5.school_life.txt" . || exit 1
              mv 4.5.school_life.txt .4.5.school_life.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.5.school_life.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.5.school_life.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.5.school_life.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.5.school_life.txt
              process_reminders_from_file .4.5.school_life.txt
              STATE_FILE=".s_chinese_4_5"
              process_file .4.5.school_life.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.5.school_life.txt
              sed -i '/^1$/!d' .s_chinese_4_5
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chinese/S4"
              # Define the file extension
              file_extension_question=".5.school_life.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            6)
              if ! [ -f ".chinese.4.5" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Chinese/S4"
                # Define the file extension
                file_extension_answer=".5.school_life.ans.txt"
                # Define the exercise file
                exercise_file="../../chinese_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_chinese_4_6" ]; then
                echo -e "\n\nYou have managed to make it to Health Part 2 ...\n\n${g}Remember to pray always${t}\n\nThe fear of the Lord is the beginning of wisdom \c" && wait_for_a_key_press
              fi
              cp "Notes/Chinese/4.6.health_part_2.txt" . || exit 1
              mv 4.6.health_part_2.txt .4.6.health_part_2.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.6.health_part_2.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.6.health_part_2.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.6.health_part_2.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.6.health_part_2.txt
              process_reminders_from_file .4.6.health_part_2.txt
              STATE_FILE=".s_chinese_4_6"
              process_file .4.6.health_part_2.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.6.health_part_2.txt
              sed -i '/^1$/!d' .s_chinese_4_6
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Chinese/S4"
              # Define the file extension
              file_extension_question=".6.health_part_2.qns.txt"
              # Define the revision file
              revision_file="../../chinese_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".chinese.4.6" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/Chinese/S4"
	              # Define the file extension
	              file_extension_answer=".6.health_part_2.ans.txt"
	              # Define the exercise file
	              exercise_file="../../chinese_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
								touch .chinese.4.6
								echo "4" > .chinese_ready
							fi
            ;;
