...
if ! [ -d "Notes" ] || ! [ -d "Revision" ] || ! [ -d "Exercise" ] || ! [ -d "Videos" ] || ! [ -d "Figures" ] || ! [ -d "Tables" ]; then
    ...
    cp ../agriculture_advanced_tutorial .
    clear_and_center
    echo
fi

files=(Notes/Agriculture_advanced/*.txt)
if [ ${#files[@]} -eq 0 ]; then
    ...
    rm -f ../agriculture_advanced_tutorial 1.1.agricultural_biology_in_crops_and_animals.txt
fi
