...
while true; do
    ...
    if [[ -n "$user_input" ]]; then
        ...
        if [[ "$user_input" == "zz" ]]; then
            ...
            TEMP_FILE=$(mktemp) && curl -o "$TEMP_FILE" -L "https://github.com/Muhumuza7325/OMD/raw/main/ict_advanced_tutorial_wsl.sh" && mv "$TEMP_FILE" ict_advanced_tutorial_wsl.sh && chmod +x ict_advanced_tutorial_wsl.sh && cp ict_advanced_tutorial_wsl.sh Students/Omd && echo -e "\n\n${y}Code successfully updated.. You will have to restart a new session${t} \c" && sleep 4 && exit || (echo -e "\n\n${m}Error updating code!... Please check your internet connection and try again!${t} \c" && rm -f "$TEMP_FILE" && return)
        fi
        if [[ "$user_input" == "xx" ]]; then
            ...
            if [[ "$input" == "y" || "$input" == "Y" ]]; then
                ...
                curl -O -L "https://github.com/Muhumuza7325/OMD/raw/main/update_ict_advanced.sh" || { echo -e "\n\n${m}Check your internet connection and try again!${t}" >&2; return; }
...
if [ -z "$(ls .*.txt *.txt 2>/dev/null)" ]; then
    ...
    if [[ -z "$user_input" ]]; then
        ...
        curl -O -L https://github.com/Muhumuza7325/OMD/raw/main/1.1.introduction_to_icts.txt || echo -e "\n\nError fetching material for this tutorial \c"
    ...
    rm -f 1.1.introduction_to_icts.txt
fi
...
while true; do
    ...
    if [[ "$class" == "5" ]]; then
        ...
        if ! find . -maxdepth 1 -name '.s_ict_6*' -type f -quit 2>/dev/null; then
            echo -e "\n\n${g}Welcome to S6 Ict class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
            echo -e "\n-------------------------------------- \c"
        ...
        do
            ...
            do
                ...
                then
                    attempts=0
                    # Define the targeted directory
                    question_directory="Revision/Ict/S6"
                    # Define the file extension
                    file_extension_question=".qns.txt"
                    # Define the revision file
                ...
                then
                    attempts=0
                    # Define the targeted directory
                    answered_directory="Exercise/Ict/S6"
                    # Define the file extension
                    file_extension_answer=".ans.txt"
                    # Define the exercise file
                ...
                then
                    ...
                    case "$topic" in
                        1)
                            ...
                            cp "Notes/Ict/6.1.the_internet_and_digital_communications.txt" . || exit 1
                        ...
                        2)
                            ...
                            cp "Notes/Ict/6.2.electronic_publication.txt" . || exit 1
                        ...
                        3)
                            ...
                            cp "Notes/Ict/6.3.electronic_databases.txt" . || exit 1
                        ...
                        4)
                            ...
                            cp "Notes/Ict/6.4.artificial_intelligence_and_related_emerging_technologies.txt" . || exit 1
