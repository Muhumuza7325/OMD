...
get_sample_items() {
    ...
    while true; do
	    if [ ! -f .revise.txt ]; then
            ...
            if [ -f .e_o_c_physics_advanced.txt ]; then
                echo -e "\n\n${y}Below is the list of the elements of construct${t} \n"
                cat .e_o_c_physics_advanced.txt
                ...
                if [[ -n $input ]]; then
                    ...
                    selected_file1=$(ls -a | grep -E "\.e_o_c_physics_advanced_${input}\.txt")
                    cat "$selected_file1"
                    # Remove empty lines from the selected files
                    find . -type f -name "*_samples_[0-9].txt" -exec sed -i '/^[[:space:]]*$/d' {} +
                    selected_file=$(ls -a | grep -E "\.e_o_c_physics_advanced_${input}_samples" | shuf -n 1)
                else
...
if ! [ -d "Notes" ] || ! [ -d "Revision" ] || ! [ -d "Exercise" ] || ! [ -d "Videos" ] || ! [ -d "Figures" ] || ! [ -d "Tables" ]; then
    ...
    cp ../physics_advanced_tutorial .
    clear_and_center
    echo
fi

files=(Notes/Physics_advanced/*.txt)
if [ ${#files[@]} -eq 0 ]; then
    ...
    rm -f ../physics_advanced_tutorial 1.1.measurement_and_dimensions_of_physical_quantities.txt
fi
