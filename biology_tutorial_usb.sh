...
while true; do
  ...
  if [[ -n "$user_input" ]]; then
    ...
    if [[ "$user_input" == "zz" ]]; then
      ...
      TEMP_FILE=$(mktemp) && curl -o "$TEMP_FILE" -L "https://github.com/Muhumuza7325/OMD/raw/main/biology_tutorial_usb.sh" && mv "$TEMP_FILE" biology_tutorial_usb.sh && chmod +x biology_tutorial_usb.sh && cp biology_tutorial_usb.sh Students/Omd && echo -e "\n\n${y}Code successfully updated.. You will have to restart a new session${t} \c" && sleep 4 && exit || (echo -e "\n\n${m}Error updating code!... Please check your internet connection and try again!${t} \c" && rm -f "$TEMP_FILE" && return)
    fi
...
if ! [ -d "Notes/Biology" ] || ! [ -d "Revision" ] || ! [ -d "Exercise" ] || ! [ -d "Figures" ] || ! [ -d "Tables" ]; then
