...
function handle_class_input() {
  if { [ -n "$last_class" ] || [ -z "$last_topic" ]; } && [ -f .cre_surveyor ]; then
		while true; do
	    read -rp $'\n\nPlease enter '"${r}1 or 2 or 3 or 4${t}"' to go to your class or '"${r}x${t}"' to exit'$'\n\n> ' class
      if [ "$class" == "x" ]; then
        exit
      fi
		  # Check if .current_cre_class is accidentally empty
		  if [ ! -s .current_cre_class ]; then
		    # Echo 1 to .current_cre_class
		    echo "1" > .current_cre_class
		  fi
		  # Read the value from the file
		  current_cre_class=$(<.current_cre_class) 2>/dev/null
		  # Check if the value is equal to $class
		  if [ "$current_cre_class" -lt "$class" 2>/dev/null ]; then
	      ...
	      rm -f ".cre_topic_selected"
	      # Update the state file with the class
	      if [ "$class" != "x" ]; then
	        echo "$class" > .cre_user_state 2>/dev/null
	      fi
				break
	  	fi
		done
...
function handle_s3_topic_input() {
  ...
  if [ -z "$last_topic" ] || [ -f .cre_topic_selected ]; then
    read -rp $'\n\nChoose either topic '"${g}1 or 2 or 3${t}"' to learn'$'\nor enter '"${r}z${t}"' for an adventure or '"${r}r${t}"' to revise or '"${r}s${t}"' to get sample_items'$'\nor '"${r}a${t}"' to get an activity of integration or '"${r}q${t}"' to get a short answer question'$'\nor '"${r}n${t}"' to do your final class assignment and if necessary, gain access to the next class'$'\nor '"${r}p${t}"' to track academic progress or '"${r}x${t}"' to exit'$'\n\n1. Work '"${r}Term1${t}"''$'\n\n2. Wealth and development '"${r}Term2${t}"''$'\n\n3. Leisure '"${r}Term3${t}"''$'\n\n> ' topic
    touch .cre_surveyor
    touch .cre_topic_selected
    # Update the state file with the topic
    # Check if the state file exists, and the topic is not "x"
...
function handle_s2_topic_input() {
  ...
  if [ -z "$last_topic" ] || [ -f .cre_topic_selected ]; then
    read -rp $'\n\nChoose either topic '"${g}1 or 2 or 3${t}"' to learn'$'\nor enter '"${r}z${t}"' for an adventure or '"${r}r${t}"' to revise or '"${r}s${t}"' to get sample_items'$'\nor '"${r}a${t}"' to get an activity of integration or '"${r}q${t}"' to get a short answer question'$'\nor '"${r}n${t}"' to do your final class assignment and if necessary, gain access to the next class'$'\nor '"${r}p${t}"' to track academic progress or '"${r}x${t}"' to exit'$'\n\n1. Respect for human life '"${r}Term1${t}"''$'\n\n2. Marriage '"${r}Term2${t}"''$'\n\n3. Family '"${r}Term3${t}"''$'\n\n> ' topic
    touch .cre_surveyor
    touch .cre_topic_selected
    # Update the state file with the topic
    # Check if the state file exists, and the topic is not "x"
...
function handle_s1_topic_input() {
  ...
  if [ -z "$last_topic" ] || [ -f .cre_topic_selected ]; then
    read -rp $'\n\nChoose either topic '"${g}1 or 2 or 3${t}"' to learn'$'\nor enter '"${r}z${t}"' for an adventure or '"${r}r${t}"' to revise or '"${r}s${t}"' to get sample_items'$'\nor '"${r}a${t}"' to get an activity of integration or '"${r}q${t}"' to get a short answer question'$'\nor '"${r}n${t}"' to do your final class assignment and if necessary, gain access to the next class'$'\nor '"${r}p${t}"' to track academic progress or '"${r}x${t}"' to exit'$'\n\n1. Worship '"${r}Term1${t}"''$'\n\n2. Christian rituals and celebrations '"${r}Term2${t}"''$'\n\n3. Values in christianity, islam and African traditional religion '"${r}Term3${t}"''$'\n\n> ' topic
    touch .cre_surveyor
    touch .cre_topic_selected
    # Update the state file with the topic
    # Check if the state file exists, and the topic is not "x"
    ...
    for sentence in "${sentences[@]}"; do
      if [[ $echo_next == true ]]; then
        ...
        for split_sentence in "${split_sentences[@]}"; do
					echo "$split_sentence" >> .cre_reminder
        done
      echo_next=false
      fi
        ...
        echo "Did you know that $lower_cased_sentence" >> .cre_reminder
        ...
        echo_next=true
  			...
  			echo "Do you recall that $lower_cased_sentence;" >> .cre_reminder

			elif [[ "$sentence" =~ " → " ]]; then
  			echo "Hope you know that: $sentence;" >> .cre_reminder

      elif [[ "$sentence" =~ " ↔ " ]]; then
        echo "Hope you know that: $sentence;" >> .cre_reminder

      elif [[ "$sentence" =~ (Generally|general) ]]; then
        echo "Note: $sentence;" >> .cre_reminder
...
process_random_reminder() {
  ...
  # Check if .cre_reminder exists
  if [ -f .cre_reminder ]; then
		# Remove empty lines from .cre_reminder
  	sed -i '/^[[:space:]]*$/d' .cre_reminder 2>/dev/null
    # Randomly select a non-empty sentence
    local reminder # Declare the variable
		reminder=$(awk -v RS=';' 'BEGIN{srand();}{gsub(/^[[:space:]]+|[[:space:]]+$/, ""); if (length > 0) a[++n]=$0}END{if (n > 0) print a[int(rand()*n)+1]}' .cre_reminder)
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    for sentence in "${sentences[@]}"; do
        ...
        if [[ $sentence == *"Figure"* ]]; then
          ...
          # Change to the "Figures/Cre" directory
          cd Figures/Cre || { echo "Failed to change to Figures/Cre"; return; }
        ...
        if [[ $sentence == *"Table"* ]]; then
          cd Tables/Cre || { echo -e "\nFailed to change to Tables/Cre \c"; return; }
          explorer.exe "$sentence" > /dev/null 2>&1 &
          cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        fi

        if [[ $sentence == *"Video"* ]]; then
          cd Videos/Cre || { echo -e "\nFailed to change to Videos/Cre \c"; return; }
          explorer.exe "$sentence" > /dev/null 2>&1 &
          cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        fi
...
contact_ai() {
  last_topic=$(awk -F' ' '{print $2}' .cre_user_state)
...
while true; do
  ...
  # Prompt the user for input
  if [ -z "$(find ./Notes/Cre -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
  ...
  if [[ -n "$user_input" ]]; then
		if [[ "$user_input" == "sh" ]]; then
		  Response="We are looking forward to receiving your economic support, thoughts, suggestions, and any files you feel should reach out to everyone of our children. Please remember to label the files you are to attach using the format: Your_name_School_Cre_File_content (e.g., Muhumuza_Omega_Kasule_High_School_O_level_Chemistry_Answered_EOC1_Items.pdf). Thanks a lot for your contributions."  
		  cre="$(basename "$0") - $(date +"%Y-%m-%d %H:%M:%S") - Thoughts, suggestions, and contributions"
		  encoded_cre=$(echo "$cre" | sed 's/ /%20/g; s/\n/%0A/g')
		  encoded_body=$(echo "$Response" | sed 's/ /%20/g; s/\n/%0A/g')
		  # Open the email in the browser with the encoded cre and body
		  powershell.exe -Command "Start-Process 'https://mail.google.com/mail/?view=cm&to=2024omd256@gmail.com&su=${encoded_cre}&body=${encoded_body}'"
			return
		fi
    ...
    if [[ "$user_input" == "xx" ]]; then
      ...
      if [[ "$input" == "y" || "$input" == "Y" ]]; then
        ...
        curl -O -L "https://github.com/Muhumuza7325/OMD/raw/main/update_cre.sh" || { echo -e "\n\n${m}Check your internet connection and try again!${t}" >&2; return; }
        mv update_cre.sh .update_cre.sh
        bash .update_cre.sh
        return
      else
        return
    ...
    if [[ -n "$result" ]]; then
      ...
      if echo "$result" | grep "Figure"; then
        echo "$result" | grep -o '\bFigure[0-9]\+.*\.jpg\(\.[0-9]\+\)*\b' > Figures/Cre/figures.txt
        sed -i -e '/^Figure/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Figures/Cre/figures.txt
        # Change to the "Figures/Cre" directory
        cd Figures/Cre || { echo "Failed to change to Figures/Cre"; return; }
        ...
        rm -f Figures/Cre/figures.txt
      fi

      if echo "$result" | grep "Table"; then
        echo "$result" | grep -o '\bTable[0-9]\+\(\.[0-9]\+\)*\b' > Tables/Cre/tables.txt
        sed -i -e '/^Table/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Tables/Cre/tables.txt
        cd Tables/Cre || { echo -e "\nFailed to change to Tables/Cre \c"; return; }
        ...
        # Go back to the original directory
        cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        rm -f Tables/Cre/tables.txt
      fi

      if echo "$result" | grep "Video"; then
        #echo "$result" > Videos/Cre/videos.txt
        echo "$result" | grep -o '\bVideo[0-9]\+\(\.[0-9]\+\)*\b' > Videos/Cre/videos.txt
        # Remove lines not starting with "Video" and any leading/trailing whitespaces
        sed -i -e '/^Video/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Videos/Cre/videos.txt
        # Change to the "Videos/Cre" directory
        cd Videos/Cre || { echo -e "\nFailed to change to Videos/Cre \c"; return; }
  ...
  if [ -f "$selected_file" ]; then
    ...
    if [[ -n "$selected_question" && "$selected_question" =~ [[:graph:]] ]]; then
      ...
      if echo "$selected_question" | grep "Figure"; then
        echo "$selected_question" | grep -o '\bFigure[0-9]\+[^;]*\b' > ../../Figures/Cre/figures.txt
        sed -i -e '/^Figure/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' ../../Figures/Cre/figures.txt
        # Change to the "Figures/Cre" directory
        cd ../../Figures/Cre || { echo "Failed to change to Figures/Cre"; return; }
        # Specify the path to the text file containing figure names
        ...
        # Go back to the original directory
        cd ../../Revision/Cre || { echo "Failed to change back to the targeted directory \c"; exit 1; }
        # Remove the temporary file
        rm -f ../../Figures/Cre/figures.txt
      fi
...
process_question_answer() {
  ...
  if [ -f ../../../.school_name ]; then
    ...
    sed -i '/^[[:space:]]*$/d' ../../../."$school_name"_students_file.txt
    existing_class=$(awk '{print $1}' ../../../.cre_user_state)
    existing_topic=$(awk '{print $2}' ../../../.cre_user_state)
    echo ''
    ...
    if grep -q "Cre" ../../../."$school_name"_students_file.txt; then
      sed -i -E 's/;/\n/g' ../../../."$school_name"_students_file.txt
      # Find the line with the word Cre, replace the information below it with the already available information adding a comma existing_class_existing_topic [$percentage]
      if [ "$existing_topic" == "r" ]; then
        sed -i '/Cre/{n;s/\(.*\)/\1, '"$existing_class"'_'"$input"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
      else
        (( existing_topic-- ))
        sed -i '/Cre/{n;s/\(.*\)/\1, '"$existing_class"'_'"$existing_topic"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
      fi
    else
      if [ "$existing_topic" == "r" ]; then
        sed -i -E '/^School/ i\Cre\n'"${existing_class} ${names}[${percentage}%]"'' ../../../."$school_name"_students_file.txt
...
function resume_from_last_point() {
  if [ -f .cre_user_state ]; then
    last_class=$(awk -F' ' '{print $1}' .cre_user_state)
    last_topic=$(awk -F' ' '{print $2}' .cre_user_state)
...
process_final_assignment() {
  # Check if user is.cre_ready for the assignment
  # Check if .current_cre_class is accidentally empty
  if [ ! -s .current_cre_class ]; then
    # Echo 1 to .current_cre_class
    echo "1" > .current_cre_class
  fi
  # Read the value from the .current_cre_class file
  current_cre_class=$(<.current_cre_class) 2>/dev/null
  # Check if the value in the .cre_ready file is equal to $class
  # Read the value from the .cre_ready file
  if [ ! -s .cre_ready ]; then
    echo "0" > .cre_ready
  fi
  how.cre_ready=$(<.cre_ready) 2>/dev/null
  # Check if the value is equal to $class
  if [ "$how.cre_ready" -lt "$current_cre_class" 2>/dev/null ]; then
    read -rp $'\n\nYou havent done all the topic assignments for your current cre class\n\n'"${r}Proceeding from here will affect your very final score${y}"'. To go back and progress right, enter '"${y}ok${t}"'. Otherwise, press the Enter key to do the final class assignment: ' progress
    if [ "$progress" == "ok" ]; then
      return
    fi
  fi
    ...
    if ! [ "$total_questions" == 100 ]; then
      ...
      if [ "$elapsed_time" -gt 3600 ]; then
        ...
        if [ "$percentage" -lt 95 ]; then
          ...
          cd - > /dev/null 2>&1 || exit
          if ! [ -f .current_cre_class ]; then
            # Echo the result to the file .current_class
            echo "2" > .current_cre_class
            ...
            # Read the value from the file
            current_cre_class=$(<.current_cre_class) 2>/dev/null
            # Check if the new_class value is lt $current_cre_class
            if [ "$new_class" -gt "$current_cre_class" 2>/dev/null ]; then
		          # Echo the result to the file .current_class
		          echo "$new_class" > .current_cre_class
            fi
...
get_sample_items() {
  ...
  # Save the current working directory
  pushd . > /dev/null
  cd Revision/Cre || { echo "Directory not found"; return; }
  ...
  while true; do
      ...
      if [ ! -f "$last_echo_time_file" ]; then
    	...
    	if [ $time_diff -gt 3600 ]; then
      	# Echo the message and update the last echo time
	    	echo -e "\n\n\n${r}You are advised to not make any changes to the provided answers, instead, you can make copies that you can edit${t}\n\n${y}For a teacher willing to join us reach out to everyone of our children, please send us your questions and answers in a file labelled with your name, school, cre, and file content (e.g., Muhumuza_Omega_Kasule_High_School_O_level_Chemistry_Answered_EOC1_Items.pdf) to our contacts${t}\n\n\nEmail: ${g}2024omd256@gmail.com${t} \c"
				echo $current_time > "$last_echo_time_file"
  		fi
      read -rp $'\n\n\nEnter '"${m}any character${t}"' for access to the file of answered items or simply press '"${r}Enter${t}"' to get items to attempt : ' input
     	if [[ -n $input ]]; then
     		explorer.exe .cre_samples* > /dev/null 2>&1 &
      	clear
      	popd > /dev/null || exit
        return
      fi
      if [ -f .e_o_c_cre.txt ]; then
        ...
        if [[ -n $input ]]; then
          ...
          selected_file1=$(ls -a | grep -E "\.e_o_c_cre_${input}\.txt")
          cat "$selected_file1"
          # Remove empty lines from the selected files
          find . -type f -name "*_samples_[0-9].txt" -exec sed -i '/^[[:space:]]*$/d' {} +
          selected_file=$(ls -a | grep -E "\.e_o_c_cre_${input}_samples" | shuf -n 1)
        else
    ...
    rm -f .revise.txt .cre_topic_selected 2>/dev/null
  done
  popd > /dev/null || exit
	return
}
...
while true; do
  ...
  if ls Students | grep -wq "$initials"; then
    ...
    echo -e "                  $initials\n" > "Students/$initials/Exercise/cre_answered_ans.txt"
    echo -e "                  $initials\n" > "Students/$initials/Revision/cre_covered_qns.txt"
    ...
    if [ -f ../../../.school_name ]; then
      ...
      if grep -q "Cre" ../../../."$school_name"_students_file.txt; then
        ...
        if [ "$existing_topic" == "z" ]; then
          sed -i '/Cre/{n;s/\(.*\)/\1, '"$existing_class"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
        else
          (( existing_topic-- ))
          sed -i '/Cre/{n;s/\(.*\)/\1, '"$existing_class"'_'"$existing_topic"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
        fi
...
if [ ! -f .cre_user_state ]; then
  ...
  if [ -f .omd_communication.txt ]; then
    ...
    process_random_reminder .cre_reminder
  fi
	handle_resume_input
fi

if [ -z "$class" ] && [ -s ".cre_user_state" ]; then
  get_and_display_pattern
fi
# Check for the presence of specific directories and a file
if ! [ -d "Notes/Cre" ] || ! [ -d "Revision" ] || ! [ -d "Exercise" ] || ! [ -d "Videos" ] || ! [ -d "Figures" ] || ! [ -d "Tables" ]; then
  ...
  mkdir -p Notes Notes/Cre Revision Revision/Cre Revision/Cre/{S1,S2,S3,S4} Exercise Exercise/Cre Exercise/Cre/{S1,S2,S3,S4} Videos Videos/Cre Figures Figures/Cre Tables Tables/Cre
  touch Revision/cre_covered_qns.txt Exercise/cre_answered_ans.txt
  echo -e "\n"
  pwd
...
if [ -z "$(find ./Notes/Cre -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
  ...
  if [[ -z "$user_input" ]]; then
    ...
    curl -O -L https://github.com/Muhumuza7325/OMD/raw/main/CRE/1.1.worship.txt || echo -e "\n\nError fetching material for this tutorial \c"
    ...
    cp 1.1.worship.txt Notes/Cre || echo -e "\n\nError copying 1.1.worship.txt to the Cre directory in the Notes directory \c"
    wait_for_a_key_press
  else
...
while true; do
  ...
  handle_class_input
  if [[ "$class" == "1" ]]; then
    if ! find . -maxdepth 1 -name '.s_cre_1*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to S1 CRE class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
    ...
    do
      ...
      do
        ...
        then
          ...
          question_directory="Revision/Cre/S1"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../cre_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Cre/S1"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../cre_answered_ans.txt"
          # Call the function to process a random question
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_cre_1_1" ]; then
                echo -e "\n\nYou chose to explore Worship ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/Cre/1.1.worship.txt" . || exit 1
              mv 1.1.worship.txt .1.1.worship.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.1.worship.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.1.worship.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.1.worship.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.1.worship.txt
              process_reminders_from_file .1.1.worship.txt
              STATE_FILE=".s_cre_1_1"
              process_file .1.1.worship.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              question_directory="Revision/Cre/S1"
              # Define the file extension
              file_extension=".1.worship.qns.txt"
              # Define the revision file
              revision_file="../../cre_covered_qns.txt"
              # Call the function to process a random question
              ...
              question_directory="Revision/Cre/S1"
              # Define the file extension
              file_extension_question=".1.worship.qns.txt"
              # Define the revision file
              revision_file="../../cre_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ...
            2)
              if ! [ -f ".cre.1.1" ]; then
                ...
                answered_directory="Exercise/Cre/S1"
                # Define the file extension
                file_extension_answer=".1.worship.ans.txt"
                # Define the exercise file
                exercise_file="../../cre_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_cre_1_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Christian rituals and celebrations ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Cre/1.2.christian_rituals_and_celebrations.txt" . || exit 1
              mv 1.2.christian_rituals_and_celebrations.txt .1.2.christian_rituals_and_celebrations.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.2.christian_rituals_and_celebrations.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.2.christian_rituals_and_celebrations.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.2.christian_rituals_and_celebrations.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.2.christian_rituals_and_celebrations.txt
              process_reminders_from_file .1.2.christian_rituals_and_celebrations.txt
              STATE_FILE=".s_cre_1_2"
              process_file .1.2.christian_rituals_and_celebrations.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.2.christian_rituals_and_celebrations.txt
              sed -i '/^1$/!d' .s_cre_1_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Cre/S1"
              # Define the file extension
              file_extension_question=".2.christian_rituals_and_celebrations.qns.txt"
              # Define the revision file
              revision_file="../../cre_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            3)
              if ! [ -f ".cre.1.2" ]; then
                ...
                answered_directory="Exercise/Cre/S1"
                # Define the file extension
                file_extension_answer=".2.christian_rituals_and_celebrations.ans.txt"
                # Define the exercise file
                exercise_file="../../cre_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_cre_1_3" ]; then
                echo -e "\n\nYou have made a choice to cover Values in Christianity, Islam and African traditional religion ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Cre/1.3.values_in_christianity_islam_and_african_traditional_religion.txt" . || exit 1
              mv 1.3.values_in_christianity_islam_and_african_traditional_religion.txt .1.3.values_in_christianity_islam_and_african_traditional_religion.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .1.3.values_in_christianity_islam_and_african_traditional_religion.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .1.3.values_in_christianity_islam_and_african_traditional_religion.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .1.3.values_in_christianity_islam_and_african_traditional_religion.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .1.3.values_in_christianity_islam_and_african_traditional_religion.txt
              process_reminders_from_file .1.3.values_in_christianity_islam_and_african_traditional_religion.txt
              STATE_FILE=".s_cre_1_3"
              process_file .1.3.values_in_christianity_islam_and_african_traditional_religion.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .1.3.values_in_christianity_islam_and_african_traditional_religion.txt
              sed -i '/^1$/!d' .s_cre_1_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Cre/S1"
              # Define the file extension
              file_extension_question=".3.values_in_christianity_islam_and_african_traditional_religion.qns.txt"
              # Define the revision file
              revision_file="../../cre_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              if ! [ -f ".cre.1.3" ]; then
	              ...
	              answered_directory="Exercise/Cre/S1"
	              # Define the file extension
	              file_extension_answer=".3.values_in_christianity_islam_and_african_traditional_religion.ans.txt"
	              # Define the exercise file
	              exercise_file="../../cre_answered_ans.txt"
    ...
    if ! find . -maxdepth 1 -name '.s_cre_2*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to S2 CRE class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
      clear_and_center "There are ${r}3${t} topics to be covered. Your tasks will always expand or shrink to fit in the time you give them. For that reason, never procrastinate darling!"
    fi
    attempts=0
    ...
    do
      ...
      do
        ...
        then
          attempts=0
          # Define the targeted directory
          question_directory="Revision/Cre/S2"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../cre_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Cre/S2"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../cre_answered_ans.txt"
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_cre_2_1" ]; then
                echo -e "\n\nYou chose to explore Respect for human life ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/Cre/2.1.respect_for_human_life.txt" . || exit 1
              mv 2.1.respect_for_human_life.txt .2.1.respect_for_human_life.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.1.respect_for_human_life.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.1.respect_for_human_life.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.1.respect_for_human_life.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.1.respect_for_human_life.txt
              process_reminders_from_file .2.1.respect_for_human_life.txt
              STATE_FILE=".s_cre_2_1"
              process_file .2.1.respect_for_human_life.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.1.respect_for_human_life.txt
              sed -i '/^1$/!d' .s_cre_2_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Cre/S2"
              # Define the file extension
              file_extension=".1.respect_for_human_life.qns.txt"
              # Define the revision file
              revision_file="../../cre_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Cre/S2"
              # Define the file extension
              file_extension_question=".1.respect_for_human_life.qns.txt"
              # Define the revision file
              revision_file="../../cre_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ...
            2)
              if ! [ -f ".cre.2.1" ]; then
                ...
                answered_directory="Exercise/Cre/S2"
                # Define the file extension
                file_extension_answer=".1.respect_for_human_life.ans.txt"
                # Define the exercise file
                exercise_file="../../cre_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_cre_2_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Marriage ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Cre/2.2.marriage.txt" . || exit 1
              mv 2.2.marriage.txt .2.2.marriage.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.2.marriage.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.2.marriage.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.2.marriage.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.2.marriage.txt
              process_reminders_from_file .2.2.marriage.txt
              STATE_FILE=".s_cre_2_2"
              process_file .2.2.marriage.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .2.2.marriage.txt
              sed -i '/^1$/!d' .s_cre_2_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Cre/S2"
              # Define the file extension
              file_extension_question=".2.marriage.qns.txt"
              # Define the revision file
              revision_file="../../cre_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            3)
              if ! [ -f ".cre.2.2" ]; then
                ...
                answered_directory="Exercise/Cre/S2"
                # Define the file extension
                file_extension_answer=".2.marriage.ans.txt"
                # Define the exercise file
                exercise_file="../../cre_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              cp "Notes/Cre/2.3.family.txt" . || exit 1
              mv 2.3.family.txt .2.3.family.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .2.3.family.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .2.3.family.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .2.3.family.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .2.3.family.txt
              process_reminders_from_file .2.3.family.txt
              STATE_FILE=".s_cre_2_3"
              process_file .2.3.family.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
    ...
    if ! find . -maxdepth 1 -name '.s_cre_3*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to S3 CRE class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
      clear_and_center "There are ${r}3${t} topics to be covered. Your tasks will always expand or shrink to fit in the time you give them. For that reason, never procrastinate darling!"
    fi
    attempts=0
    ...
    do
      ...
      do
        ...
        then
          attempts=0
          # Define the targeted directory
          question_directory="Revision/Cre/S3"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../cre_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Cre/S3"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../cre_answered_ans.txt"
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_cre_3_1" ]; then
                echo -e "\n\nYou chose to explore Work ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/Cre/3.1.work.txt" . || exit 1
              mv 3.1.work.txt .3.1.work.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.1.work.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.1.work.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.1.work.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.1.work.txt
              process_reminders_from_file .3.1.work.txt
              STATE_FILE=".s_cre_3_1"
              process_file .3.1.work.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              question_directory="Revision/Cre/S3"
              # Define the file extension
              file_extension_question=".1.work.qns.txt"
              # Define the revision file
              revision_file="../../cre_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ...
            2)
              if ! [ -f ".cre.3.1" ]; then
                ...
                answered_directory="Exercise/Cre/S3"
                # Define the file extension
                file_extension_answer=".1.work.ans.txt"
                # Define the exercise file
                exercise_file="../../cre_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_cre_3_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Wealth and development ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Cre/3.2.wealth_and_development.txt" . || exit 1
              mv 3.2.wealth_and_development.txt .3.2.wealth_and_development.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.2.wealth_and_development.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.2.wealth_and_development.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.2.wealth_and_development.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.2.wealth_and_development.txt
              process_reminders_from_file .3.2.wealth_and_development.txt
              STATE_FILE=".s_cre_3_2"
              process_file .3.2.wealth_and_development.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.2.wealth_and_development.txt
              sed -i '/^1$/!d' .s_cre_3_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Cre/S3"
              # Define the file extension
              file_extension_question=".2.wealth_and_development.qns.txt"
              # Define the revision file
              revision_file="../../cre_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            3)
              if ! [ -f ".cre.3.2" ]; then
                ...
                answered_directory="Exercise/Cre/S3"
                # Define the file extension
                file_extension_answer=".2.wealth_and_development.ans.txt"
                # Define the exercise file
                exercise_file="../../cre_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_cre_3_3" ]; then
                echo -e "\n\nYou have made a choice to cover Leisure ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Cre/3.3.leisure.txt" . || exit 1
              mv 3.3.leisure.txt .3.3.leisure.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .3.3.leisure.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .3.3.leisure.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .3.3.leisure.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .3.3.leisure.txt
              process_reminders_from_file .3.3.leisure.txt
              STATE_FILE=".s_cre_3_3"
              process_file .3.3.leisure.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .3.3.leisure.txt
              sed -i '/^1$/!d' .s_cre_3_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Cre/S3"
              # Define the file extension
              file_extension_question=".3.leisure.qns.txt"
              # Define the revision file
              revision_file="../../cre_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              if ! [ -f ".cre.3.3" ]; then
	              ...
	              answered_directory="Exercise/Cre/S3"
	              # Define the file extension
	              file_extension_answer=".3.leisure.ans.txt"
	              # Define the exercise file
	              exercise_file="../../cre_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
    ...
    if ! find . -maxdepth 1 -name '.s_cre_4*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to S4 CRE class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
      clear_and_center "There are ${r}3${t} topics to be covered. Your tasks will always expand or shrink to fit in the time you give them. For that reason, never procrastinate darling!"
    fi
    attempts=0
    ...
    do
      ...
      do
        ...
        then
          attempts=0
          # Define the targeted directory
          question_directory="Revision/Cre/S4"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../cre_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Cre/S4"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../cre_answered_ans.txt"
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_cre_4_1" ]; then
                echo -e "\n\nYou chose to explore Peace ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/Cre/4.1.peace.txt" . || exit 1
              mv 4.1.peace.txt .4.1.peace.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.1.peace.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.1.peace.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.1.peace.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.1.peace.txt
              process_reminders_from_file .4.1.peace.txt
              STATE_FILE=".s_cre_4_1"
              process_file .4.1.peace.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              question_directory="Revision/Cre/S4"
              # Define the file extension
              file_extension=".1.peace.qns.txt"
              # Define the revision file
              revision_file="../../cre_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Cre/S4"
              # Define the file extension
              file_extension_question=".1.peace.qns.txt"
              # Define the revision file
              revision_file="../../cre_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ...
            2)
              if ! [ -f ".cre.4.1" ]; then
                ...
                answered_directory="Exercise/Cre/S4"
                # Define the file extension
                file_extension_answer=".1.peace.ans.txt"
                # Define the exercise file
                exercise_file="../../cre_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_cre_4_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Justice ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Cre/4.2.justice.txt" . || exit 1
              mv 4.2.justice.txt .4.2.justice.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.2.justice.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.2.justice.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.2.justice.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.2.justice.txt
              process_reminders_from_file .4.2.justice.txt
              STATE_FILE=".s_cre_4_2"
              process_file .4.2.justice.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.2.justice.txt
              sed -i '/^1$/!d' .s_cre_4_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Cre/S4"
              # Define the file extension
              file_extension_question=".2.justice.qns.txt"
              # Define the revision file
              revision_file="../../cre_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            3)
              if ! [ -f ".cre.4.2" ]; then
                ...
                answered_directory="Exercise/Cre/S4"
                # Define the file extension
                file_extension_answer=".2.justice.ans.txt"
                # Define the exercise file
                exercise_file="../../cre_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_cre_4_3" ]; then
                echo -e "\n\nYou have made a choice to cover Conflict resolution ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Cre/4.3.conflict_resolution.txt" . || exit 1
              mv 4.3.conflict_resolution.txt .4.3.conflict_resolution.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .4.3.conflict_resolution.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .4.3.conflict_resolution.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .4.3.conflict_resolution.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .4.3.conflict_resolution.txt
              process_reminders_from_file .4.3.conflict_resolution.txt
              STATE_FILE=".s_cre_4_3"
              process_file .4.3.conflict_resolution.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .4.3.conflict_resolution.txt
              sed -i '/^1$/!d' .s_cre_4_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Cre/S4"
              # Define the file extension
              file_extension_question=".3.conflict_resolution.qns.txt"
              # Define the revision file
              revision_file="../../cre_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              if ! [ -f ".cre.4.3" ]; then
	              ...
	              answered_directory="Exercise/Cre/S4"
	              # Define the file extension
	              file_extension_answer=".3.conflict_resolution.ans.txt"
	              # Define the exercise file
	              exercise_file="../../cre_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
