  ...
  if { [ -n "$last_class" ] || [ -z "$last_topic" ]; } && [ -f .physics_surveyor ]; then
      ...
      if [ "$class" == "x" ]; then
        exit
		  ...
		  # Check if .current physics class is accidentally empty
...
process_file() {
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    for sentence in "${sentences[@]}"; do
          ...
          # Open the file
          eog "Figures/Physics/$modified_sentence" > /dev/null 2>&1 &
        fi
        if [[ $sentence == *"Table"* ]]; then
          libreoffice "Tables/Physics/$sentence" > /dev/null 2>&1 &
        fi
        if [[ $sentence == *"Video"* ]]; then
          # Open the video file
          vlc --one-instance --playlist-enqueue "Videos/Physics/${video_prefix}"* > /dev/null 2>&1 &
	      fi
...
while true; do
  ...
  # Prompt the user for input
  if [ -z "$(find ./Notes/Physics -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
  ...
  if [[ -n "$user_input" ]]; then
     ...
     if [[ "$user_input" == "sr" ]]; then
      xdg-open Resources/Physics > /dev/null 2>&1 &
      return
     fi
    ...
    if [[ "$user_input" == "zz" ]]; then
      echo -e "\n"
      TEMP_FILE=$(mktemp) && curl -o "$TEMP_FILE" -L "https://github.com/Muhumuza7325/OMD/raw/main/physics_tutorial.sh" && mv "$TEMP_FILE" physics_tutorial.sh && chmod +x physics_tutorial.sh && cp physics_tutorial.sh Students/Omd && echo -e "\n\n${y}Code successfully updated.. You will have to restart a new session${t} \c" && sleep 4 && exit || (echo -e "\n\n${m}Error updating code!... Please check your internet connection and try again!${t} \c" && rm -f "$TEMP_FILE" && return)
    fi
    ...
    if [[ -n "$result" ]]; then
      ...
      if echo "$result" | grep "Figure"; then
        ...
        while IFS= read -r figure_prefix || [ -n "$figure_prefix" ]; do
          ...
          eog "Figures/Physics/${edited_figure_prefix}"* > /dev/null 2>&1 &
					sleep 5
        done < "$text_file"
      ...
      if echo "$result" | grep "Table"; then
        ...
        while IFS= read -r table_prefix || [ -n "$table_prefix" ]; do
          ...
          # Open the file
          libreoffice "Tables/Physics/${edited_table_prefix}"* > /dev/null 2>&1 &
					sleep 10
        done < "$text_file"
        # Remove the temporary file
        rm -f tables.txt
      ...
      if echo "$result" | grep "Video"; then
        ...
        while IFS= read -r video_prefix || [ -n "$video_prefix" ]; do
          ...
          vlc --one-instance --playlist-enqueue "../Videos/Physics/${edited_video_prefix}"* > /dev/null 2>&1 &					
          sleep 10
        done < "$text_file"
        # Remove the temporary file
				rm -f videos.txt
...
process_final_assignment() {
  ...
  # Check if .current physics class is accidentally empty
  if [ ! -s .current_physics_class ]; then
    # Echo 1 to .current physics class
    echo "1" > .current_physics_class
  fi
  # Read the value from the .current physics class file
  current_physics_class=$(<.current_physics_class) 2>/dev/null
	...
	while true; do
		if [ ! -f .revise.txt ]; then
			...
			if [ $time_diff -gt 3600 ]; then
				...
				echo -e "\n\n\n${r}You are advised to not make any changes to the provided answers, instead, you can make copies that you can edit${t}\n\n${y}For a teacher willing to join us reach out to everyone of our children, please send us your questions and answers in a file labelled with your name, school, physics, and file content (e.g., Muhumuza_Omega_Kasule_High_School_O_level_Chemistry_Answered_EOC1_Items.pdf) to our contacts${t}\n\n\nEmail: ${g}2024omd256@gmail.com${t} \c"
				echo $current_time > "$last_echo_time_file"
			fi
			read -rp $'\n\n\nEnter '"${m}any character${t}"' for access to the file of answered items or simply press '"${r}Enter${t}"' to get items to attempt : ' input
			if [[ -n $input ]]; then
				libreoffice .physics_samples* > /dev/null 2>&1 &
				clear
				popd > /dev/null || exit
				return
			fi
					...
					while IFS= read -r line || [ -n "$line" ]; do
						...
						for sentence in "${sentences[@]}"; do
							if [[ -n "$sentence" && "$sentence" =~ [[:graph:]] ]]; then
								...
								if [[ $sentence == *"Video"* ]]; then
									# Open the video file
									vlc --one-instance --playlist-enqueue "../../../Videos/Physics/$sentence" > /dev/null 2>&1 &
								fi
								if [ $((sentence_count % 5)) -eq 0 ]; then
									# Clear and center for every 5th sentence
									clear_and_center "${y}$sentence${t} \c"
...
if ! [ -d "Notes/Physics" ] || ! [ -d "Revision" ] || ! [ -d "Exercise" ] || ! [ -d "Figures" ] || ! [ -d "Tables" ]; then
          ...
          case "$topic" in
            ...
            4)
              ...
              if ! [ -f ".s_physics_2_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Mechanical, properties, of, materials and hookes, law ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
          ...
          case "$topic" in
            ...
            2)
              ...
              if ! [ -f ".s_physics_4_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Voltage resistance and ohms law ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
