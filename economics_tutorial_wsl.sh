...
function handle_class_input() {
  if { [ -n "$last_class" ] || [ -z "$last_topic" ]; } && [ -f .economics_surveyor ]; then
		while true; do
	    read -rp $'\n\nPlease enter '"${r}5 or 6${t}"' to go to your class or '"${r}x${t}"' to exit'$'\n\n> ' class
      if [ "$class" == "x" ]; then
        exit
      fi
		  # Check if .current_economics_class is accidentally empty
		  if [ ! -s .current_economics_class ]; then
		    # echo 5 to .current_economics_class
		    echo "5" > .current_economics_class
		  fi
		  # Read the value from the file
		  current_economics_class=$(<.current_economics_class) 2>/dev/null
		  # Check if the value is equal to $class
		  if [ "$current_economics_class" -lt "$class" 2>/dev/null ]; then
	      ...
	      rm -f ".economics_topic_selected"
	      # Update the state file with the class
	      if [ "$class" != "x" ]; then
	        echo "$class" > .economics_user_state 2>/dev/null
	      fi
				break
	  	fi
		done
...
function handle_s5_topic_input() {
  ...
  if [ -z "$last_topic" ] || [ -f .economics_topic_selected ]; then
    read -rp $'\n\nChoose either topic '"${g}1 or 2 or 3 or 4 or 5${t}"' to learn'$'\nor enter '"${r}z${t}"' for an adventure or '"${r}r${t}"' to revise or '"${r}s${t}"' to get sample_items'$'\nor '"${r}a${t}"' to get an activity of integration or '"${r}q${t}"' to get a short answer question'$'\nor '"${r}n${t}"' to do your final class assignment and if necessary, gain access to the next class'$'\nor '"${r}p${t}"' to track academic progress or '"${r}x${t}"' to exit'$'\n\n1. Introduction to economics '"${r}Term1${t}"''$'\n\n2. Price theory '"${r}Term1${t}"''$'\n\n3. Production and market structures '"${r}Term2${t}"''$'\n\n4. National income '"${r}Term3${t}"''$'\n\n5. Economic growth, economic development and development theories '"${r}Term3${t}"''$'\n\n> ' topic
    touch .economics_surveyor
    touch .economics_topic_selected
    # Update the state file with the topic
    # Check if the state file exists, and the topic is not "x"
    ...
    for sentence in "${sentences[@]}"; do
      if [[ $echo_next == true ]]; then
        ...
        for split_sentence in "${split_sentences[@]}"; do
					echo "$split_sentence" >> .economics_reminder
        done
      echo_next=false
      fi
        ...
        echo "Did you know that $lower_cased_sentence" >> .economics_reminder
        ...
        echo_next=true
  			...
  			echo "Do you recall that $lower_cased_sentence;" >> .economics_reminder

			elif [[ "$sentence" =~ " → " ]]; then
  			echo "Hope you know that: $sentence;" >> .economics_reminder

      elif [[ "$sentence" =~ " ↔ " ]]; then
        echo "Hope you know that: $sentence;" >> .economics_reminder

      elif [[ "$sentence" =~ (Generally|general) ]]; then
        echo "Note: $sentence;" >> .economics_reminder
...
process_random_reminder() {
  ...
  # Check if .economics_reminder exists
  if [ -f .economics_reminder ]; then
		# Remove empty lines from .economics_reminder
  	sed -i '/^[[:space:]]*$/d' .economics_reminder 2>/dev/null
    # Randomly select a non-empty sentence
    local reminder # Declare the variable
		reminder=$(awk -v RS=';' 'BEGIN{srand();}{gsub(/^[[:space:]]+|[[:space:]]+$/, ""); if (length > 0) a[++n]=$0}END{if (n > 0) print a[int(rand()*n)+1]}' .economics_reminder)
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    for sentence in "${sentences[@]}"; do
        ...
        if [[ $sentence == *"Figure"* ]]; then
          ...
          # Change to the "Figures/Economics" directory
          cd Figures/Economics || { echo "Failed to change to Figures/Economics"; return; }
        ...
        if [[ $sentence == *"Table"* ]]; then
          cd Tables/Economics || { echo -e "\nFailed to change to Tables/Economics \c"; return; }
          explorer.exe "$sentence" > /dev/null 2>&1 &
          cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        fi

        if [[ $sentence == *"Video"* ]]; then
          cd Videos/Economics || { echo -e "\nFailed to change to Videos/Economics \c"; return; }
          explorer.exe "$sentence" > /dev/null 2>&1 &
          cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        fi
...
contact_ai() {
  last_topic=$(awk -F' ' '{print $2}' .economics_user_state)
...
while true; do
  ...
  # Prompt the user for input
  if [ -z "$(find ./Notes/Economics -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
  ...
  if [[ -n "$user_input" ]]; then
     ...
     if [[ "$user_input" == "sr" ]]; then
      ...
      explorer.exe Economics > /dev/null 2>&1 &
    ...
    if [[ "$user_input" == "xx" ]]; then
      ...
      if [[ "$input" == "y" || "$input" == "Y" ]]; then
        ...
        curl -O -L "https://github.com/Muhumuza7325/OMD/raw/main/update_economics.sh" || { echo -e "\n\n${m}Check your internet connection and try again!${t}" >&2; return; }
        mv update_economics.sh .update_economics.sh
        bash .update_economics.sh
        return
      else
        return
      fi
    ...
    if [[ -n "$result" ]]; then
      ...
      if echo "$result" | grep "Figure"; then
        echo "$result" | grep -o '\bFigure[0-9]\+.*\.jpg\(\.[0-9]\+\)*\b' > Figures/Economics/figures.txt
        sed -i -e '/^Figure/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Figures/Economics/figures.txt
        # Change to the "Figures/Economics" directory
        cd Figures/Economics || { echo "Failed to change to Figures/Economics"; return; }
        ...
        rm -f Figures/Economics/figures.txt
      fi

      if echo "$result" | grep "Table"; then
        echo "$result" | grep -o '\bTable[0-9]\+\(\.[0-9]\+\)*\b' > Tables/Economics/tables.txt
        sed -i -e '/^Table/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Tables/Economics/tables.txt
        cd Tables/Economics || { echo -e "\nFailed to change to Tables/Economics \c"; return; }
        ...
        # Go back to the original directory
        cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        rm -f Tables/Economics/tables.txt
      fi

      if echo "$result" | grep "Video"; then
        #echo "$result" > Videos/Economics/videos.txt
        echo "$result" | grep -o '\bVideo[0-9]\+\(\.[0-9]\+\)*\b' > Videos/Economics/videos.txt
        # Remove lines not starting with "Video" and any leading/trailing whitespaces
        sed -i -e '/^Video/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' Videos/Economics/videos.txt
        # Change to the "Videos/Economics" directory
        cd Videos/Economics || { echo -e "\nFailed to change to Videos/Economics \c"; return; }
        ...
        cd ../.. || { echo -e "\nFailed to change back to the original directory \c"; exit 1; }
        # Remove the temporary file
        rm -f Videos/Economics/videos.txt
      fi
  ...
  if [ -f "$selected_file" ]; then
    ...
    if [[ -n "$selected_question" && "$selected_question" =~ [[:graph:]] ]]; then
      ...
      if echo "$selected_question" | grep "Figure"; then
        echo "$selected_question" | grep -o '\bFigure[0-9]\+[^;]*\b' > ../../Figures/Economics/figures.txt
        sed -i -e '/^Figure/!d' -e '/^[[:space:]]*$/d' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' ../../Figures/Economics/figures.txt
        # Change to the "Figures/Economics" directory
        cd ../../Figures/Economics || { echo "Failed to change to Figures/Economics"; return; }
        # Specify the path to the text file containing figure names
        ...
        # Go back to the original directory
        cd ../../Revision/Economics || { echo "Failed to change back to the targeted directory \c"; exit 1; }
        # Remove the temporary file
        rm -f ../../Figures/Economics/figures.txt
      fi
...
process_question_answer() {
  ...
  if [ -f ../../../.school_name ]; then
    ...
    sed -i '/^[[:space:]]*$/d' ../../../."$school_name"_students_file.txt
    existing_class=$(awk '{print $1}' ../../../.economics_user_state)
    existing_topic=$(awk '{print $2}' ../../../.economics_user_state)
    echo ''
    ...
    if grep -q "Economics" ../../../."$school_name"_students_file.txt; then
      sed -i -E 's/;/\n/g' ../../../."$school_name"_students_file.txt
      # Find the line with the word Economics, replace the information below it with the already available information adding a comma existing_class_existing_topic [$percentage]
      if [ "$existing_topic" == "r" ]; then
        sed -i '/Economics/{n;s/\(.*\)/\1, '"$existing_class"'_'"$input"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
      else
        (( existing_topic-- ))
        sed -i '/Economics/{n;s/\(.*\)/\1, '"$existing_class"'_'"$existing_topic"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
      fi
    else
      if [ "$existing_topic" == "r" ]; then
        sed -i -E '/^School/ i\Economics\n'"${existing_class} ${names}[${percentage}%]"'' ../../../."$school_name"_students_file.txt
...
function resume_from_last_point() {
  if [ -f .economics_user_state ]; then
    last_class=$(awk -F' ' '{print $1}' .economics_user_state)
    last_topic=$(awk -F' ' '{print $2}' .economics_user_state)
    if [ -n "$last_class" ] && [ -n "$last_topic" ]; then
      echo -e "\n       Resuming from ${r}S$last_class${t} : ${g}Topic '$last_topic'${t} \c"
      rm -f .economics_surveyor
...
process_final_assignment() {
  # Check if user is.economics_ready for the assignment
  # Check if .current_economics_class is accidentally empty
  if [ ! -s .current_economics_class ]; then
    # echo 5 to .current_economics_class
    echo "5" > .current_economics_class
  fi
  # Read the value from the .current_economics_class file
  current_economics_class=$(<.current_economics_class) 2>/dev/null
  # Check if the value in the .economics_ready file is equal to $class
  # Read the value from the .economics_ready file
  if [ ! -s .economics_ready ]; then
    echo "0" > .economics_ready
  fi
  how.economics_ready=$(<.economics_ready) 2>/dev/null
  # Check if the value is equal to $class
  if [ "$how.economics_ready" -lt "$current_economics_class" 2>/dev/null ]; then
    read -rp $'\n\nYou havent done all the topic assignments for your current economics class\n\n'"${r}Proceeding from here will affect your very final score${y}"'. To go back and progress right, enter '"${y}ok${t}"'. Otherwise, press the Enter key to do the final class assignment: ' progress
    if [ "$progress" == "ok" ]; then
      return
    fi
  fi
    ...
    if ! [ "$total_questions" == 100 ]; then
      ...
      if [ "$elapsed_time" -gt 3600 ]; then
        ...
        if [ "$percentage" -lt 95 ]; then
          ...
          cd - > /dev/null 2>&1 || exit
          if ! [ -f .current_economics_class ]; then
            # Echo the result to the file .current_class
            echo "2" > .current_economics_class
            ...
            # Read the value from the file
            current_economics_class=$(<.current_economics_class) 2>/dev/null
            # Check if the new_class value is lt $current_economics_class
            if [ "$new_class" -gt "$current_economics_class" 2>/dev/null ]; then
		          # Echo the result to the file .current_class
		          echo "$new_class" > .current_economics_class
            fi
...
get_sample_items() {
  ...
  # Save the current working directory
  pushd . > /dev/null
  cd Revision/Economics || { echo "Directory not found"; return; }
  ...
  while true; do
      ...
      if [ ! -f "$last_echo_time_file" ]; then
    	...
    	if [ $time_diff -gt 3600 ]; then
      	# Echo the message and update the last echo time
	    	echo -e "\n\n\n${r}You are advised to not make any changes to the provided answers, instead, you can make copies that you can edit${t}\n\n${y}For a teacher willing to join us reach out to everyone of our children, please send us your questions and answers in a file labelled with your name, school, economics, and file content (e.g., Muhumuza_Omega_Kasule_High_School_O_level_Chemistry_Answered_EOC1_Items.pdf) to our contacts${t}\n\n\nEmail: ${g}2024omd256@gmail.com${t} \c"
				echo $current_time > "$last_echo_time_file"
  		fi
      read -rp $'\n\n\nEnter '"${m}any character${t}"' for access to the file of answered items or simply press '"${r}Enter${t}"' to get items to attempt : ' input
     	if [[ -n $input ]]; then
     		explorer.exe .economics_samples* > /dev/null 2>&1 &
      	clear
      	popd > /dev/null || exit
        return
      fi
      if [ -f .e_o_c_economics.txt ]; then
        echo -e "\n\n${y}Below is the list of the elements of construct${t} \n"
        cat .e_o_c_economics.txt
        ...
        if [[ -n $input ]]; then
          ...
          selected_file1=$(ls -a | grep -E "\.e_o_c_economics_${input}\.txt")
          cat "$selected_file1"
          # Remove empty lines from the selected files
          find . -type f -name "*_samples_[0-9].txt" -exec sed -i '/^[[:space:]]*$/d' {} +
          selected_file=$(ls -a | grep -E "\.e_o_c_economics_${input}_samples" | shuf -n 1)
        else
    ...
    rm -f .revise.txt .economics_topic_selected 2>/dev/null
  done
  popd > /dev/null || exit
	return
}
...
while true; do
  ...
  if ls Students | grep -wq "$initials"; then
    ...
    echo -e "                  $initials\n" > "Students/$initials/Exercise/economics_answered_ans.txt"
    echo -e "                  $initials\n" > "Students/$initials/Revision/economics_covered_qns.txt"
    ...
    if [ -f ../../../.school_name ]; then
      ...
      if grep -q "Economics" ../../../."$school_name"_students_file.txt; then
        ...
        if [ "$existing_topic" == "z" ]; then
          sed -i '/Economics/{n;s/\(.*\)/\1, '"$existing_class"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
        else
          (( existing_topic-- ))
          sed -i '/Economics/{n;s/\(.*\)/\1, '"$existing_class"'_'"$existing_topic"' '"$names"'['"$percentage%"']/}' ../../../."$school_name"_students_file.txt
        fi
...
if [ ! -f .economics_user_state ]; then
  ...
  if [ -f .omd_communication.txt ]; then
    ...
    process_random_reminder .economics_reminder
  fi
	handle_resume_input
fi

if [ -z "$class" ] && [ -s ".economics_user_state" ]; then
  get_and_display_pattern
fi
# Check for the presence of specific directories and a file
if ! [ -d "Notes/Economics" ] || ! [ -d "Revision" ] || ! [ -d "Exercise" ] || ! [ -d "Videos" ] || ! [ -d "Figures" ] || ! [ -d "Tables" ]; then
  ...
  mkdir -p Notes Notes/Economics Revision Revision/Economics Revision/Economics/{S5,S6} Exercise Exercise/Economics Exercise/Economics/{S5,S6} Videos Videos/Economics Figures Figures/Economics Tables Tables/Economics
  touch Revision/economics_covered_qns.txt Exercise/economics_answered_ans.txt
  echo -e "\n"
  pwd
...
if [ -z "$(find ./Notes/Economics -mindepth 1 -maxdepth 1 -type f -name "*.txt" 2>/dev/null)" ]; then
  ...
  if [[ -z "$user_input" ]]; then
    ...
    curl -O -L https://github.com/Muhumuza7325/OMD/raw/main/ECON/1.1.introduction_to_economics.txt || echo -e "\n\nError fetching material for this tutorial \c"
    ...
    cp 1.1.introduction_to_economics.txt Notes/Economics || echo -e "\n\nError copying 1.1.introduction_to_economics.txt to the Economics directory in the Notes directory \c"
    wait_for_a_key_press
  else
  ...
  rm -f 1.1.introduction_to_economics.txt
fi
...
while true; do
  ...
  handle_class_input
  if [[ "$class" == "5" ]]; then
    if ! find . -maxdepth 1 -name '.s_economics_5*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to the S5 Economics class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
    ...
    do
      ...
      do
        ...
        then
          ...
          question_directory="Revision/Economics/S5"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../economics_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Economics/S5"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../economics_answered_ans.txt"
          # Call the function to process a random question
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_economics_5_1" ]; then
                echo -e "\n\nYou chose to explore Introduction to economics ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/Economics/5.1.introduction_to_economics.txt" . || exit 1
              mv 5.1.introduction_to_economics.txt .5.1.introduction_to_economics.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .5.1.introduction_to_economics.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .5.1.introduction_to_economics.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .5.1.introduction_to_economics.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .5.1.introduction_to_economics.txt
              process_reminders_from_file .5.1.introduction_to_economics.txt
              STATE_FILE=".s_economics_5_1"
              process_file .5.1.introduction_to_economics.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .5.1.introduction_to_economics.txt
              sed -i '/^1$/!d' .s_economics_5_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Economics/S5"
              # Define the file extension
              file_extension=".1.introduction_to_economics.qns.txt"
              # Define the revision file
              revision_file="../../economics_covered_qns.txt"
              # Call the function to process a random question
              ...
              question_directory="Revision/Economics/S5"
              # Define the file extension
              file_extension_question=".1.introduction_to_economics.qns.txt"
              # Define the revision file
              revision_file="../../economics_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".economics.5.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Economics/S5"
                # Define the file extension
                file_extension_answer=".1.introduction_to_economics.ans.txt"
                # Define the exercise file
                exercise_file="../../economics_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_economics_5_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Price theory ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Economics/5.2.price_theory.txt" . || exit 1
              mv 5.2.price_theory.txt .5.2.price_theory.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .5.2.price_theory.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .5.2.price_theory.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .5.2.price_theory.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .5.2.price_theory.txt
              process_reminders_from_file .5.2.price_theory.txt
              STATE_FILE=".s_economics_5_2"
              process_file .5.2.price_theory.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .5.2.price_theory.txt
              sed -i '/^1$/!d' .s_economics_5_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Economics/S5"
              # Define the file extension
              file_extension_question=".2.price_theory.qns.txt"
              # Define the revision file
              revision_file="../../economics_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".economics.5.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Economics/S5"
                # Define the file extension
                file_extension_answer=".2.price_theory.ans.txt"
                # Define the exercise file
                exercise_file="../../economics_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_economics_5_3" ]; then
                echo -e "\n\nYou have made a choice to cover Production and market structures ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Economics/5.3.production_and_market_structures.txt" . || exit 1
              mv 5.3.production_and_market_structures.txt .5.3.production_and_market_structures.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .5.3.production_and_market_structures.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .5.3.production_and_market_structures.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .5.3.production_and_market_structures.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .5.3.production_and_market_structures.txt
              process_reminders_from_file .5.3.production_and_market_structures.txt
              STATE_FILE=".s_economics_5_3"
              process_file .5.3.production_and_market_structures.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .5.3.production_and_market_structures.txt
              sed -i '/^1$/!d' .s_economics_5_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Economics/S5"
              # Define the file extension
              file_extension_question=".3.production_and_market_structures.qns.txt"
              # Define the revision file
              revision_file="../../economics_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".economics.5.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Economics/S5"
                # Define the file extension
                file_extension_answer=".3.production_and_market_structures.ans.txt"
                # Define the exercise file
                exercise_file="../../economics_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_economics_5_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of National income ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/Economics/5.4.national_income.txt" . || exit 1
              mv 5.4.national_income.txt .5.4.national_income.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .5.4.national_income.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .5.4.national_income.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .5.4.national_income.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .5.4.national_income.txt
              process_reminders_from_file .5.4.national_income.txt
              STATE_FILE=".s_economics_5_4"
              process_file .5.4.national_income.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .5.4.national_income.txt
              sed -i '/^1$/!d' .s_economics_5_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Economics/S5"
              # Define the file extension
              file_extension=".4.national_income.qns.txt"
              # Define the revision file
              revision_file="../../economics_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Economics/S5"
              # Define the file extension
              file_extension_question=".4.national_income.qns.txt"
              # Define the revision file
              revision_file="../../economics_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            5)
              if ! [ -f ".economics.5.4" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Economics/S5"
                # Define the file extension
                file_extension_answer=".4.national_income.ans.txt"
                # Define the exercise file
                exercise_file="../../economics_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_economics_5_5" ]; then
                echo -e "\n\nHere you are dear one... Stay organised as you explore Economic growth, Economic development and Development theories ...\n\n${g}Just know we are not going to leave you alone${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Economics/5.5.economic_growth_economic_development_and_development_theories.txt" . || exit 1
              mv 5.5.economic_growth_economic_development_and_development_theories.txt .5.5.economic_growth_economic_development_and_development_theories.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .5.5.economic_growth_economic_development_and_development_theories.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .5.5.economic_growth_economic_development_and_development_theories.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .5.5.economic_growth_economic_development_and_development_theories.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .5.5.economic_growth_economic_development_and_development_theories.txt
              process_reminders_from_file .5.5.economic_growth_economic_development_and_development_theories.txt
              STATE_FILE=".s_economics_5_5"
              process_file .5.5.economic_growth_economic_development_and_development_theories.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .5.5.economic_growth_economic_development_and_development_theories.txt
              sed -i '/^1$/!d' .s_economics_5_5
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Economics/S5"
              # Define the file extension
              file_extension_question=".5.economic_growth_economic_development_and_development_theories.qns.txt"
              # Define the revision file
              revision_file="../../economics_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".economics.5.5" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/Economics/S5"
	              # Define the file extension
	              file_extension_answer=".5.economic_growth_economic_development_and_development_theories.ans.txt"
	              # Define the exercise file
	              exercise_file="../../economics_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
								touch .economics.5.5
								echo "5" > .economics_ready
							fi
            ;;
    ...
    if ! find . -maxdepth 1 -name '.s_economics_6*' -type f -quit 2>/dev/null; then
      echo -e "\n\n${g}Welcome to the S6 Economics class${t}\n\n${y}Together, we are going to get you started${t} \c" && wait_for_a_key_press
      echo -e "\n-------------------------------------- \c"
    ...
    do
      ...
      do
        ...
        then
          attempts=0
          # Define the targeted directory
          question_directory="Revision/Economics/S6"
          # Define the file extension
          file_extension_question=".qns.txt"
          # Define the revision file
          revision_file="../../economics_covered_qns.txt"
        ...
        then
          attempts=0
          # Define the targeted directory
          answered_directory="Exercise/Economics/S6"
          # Define the file extension
          file_extension_answer=".ans.txt"
          # Define the exercise file
          exercise_file="../../economics_answered_ans.txt"
        ...
        then
          ...
          case "$topic" in
            1)
              if ! [ -f ".s_economics_6_1" ]; then
                echo -e "\n\nYou chose to explore Economic development strategies ...\n\nThank you for choosing to educate yourself!\n\nWe adore you ${g}darling${t} and wish you the very best! \c" && wait_for_a_key_press
              fi
              cp "Notes/Economics/6.1.economic_development_strategies.txt" . || exit 1
              mv 6.1.economic_development_strategies.txt .6.1.economic_development_strategies.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .6.1.economic_development_strategies.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .6.1.economic_development_strategies.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .6.1.economic_development_strategies.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .6.1.economic_development_strategies.txt
              process_reminders_from_file .6.1.economic_development_strategies.txt
              STATE_FILE=".s_economics_6_1"
              process_file .6.1.economic_development_strategies.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .6.1.economic_development_strategies.txt
              sed -i '/^1$/!d' .s_economics_6_1
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Economics/S6"
              # Define the file extension
              file_extension=".1.economic_development_strategies.qns.txt"
              # Define the revision file
              revision_file="../../economics_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Economics/S6"
              # Define the file extension
              file_extension_question=".1.economic_development_strategies.qns.txt"
              # Define the revision file
              revision_file="../../economics_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            2)
              if ! [ -f ".economics.6.1" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Economics/S6"
                # Define the file extension
                file_extension_answer=".1.economic_development_strategies.ans.txt"
                # Define the exercise file
                exercise_file="../../economics_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_economics_6_2" ]; then
                echo -e "\n\nYou happen to have decided to delve into Population, labour, labour unions and employment ...\n\nOnce again we treasure you ${g}dear one${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Economics/6.2.population_labour_labour_unions_and_employment.txt" . || exit 1
              mv 6.2.population_labour_labour_unions_and_employment.txt .6.2.population_labour_labour_unions_and_employment.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .6.2.population_labour_labour_unions_and_employment.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .6.2.population_labour_labour_unions_and_employment.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .6.2.population_labour_labour_unions_and_employment.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .6.2.population_labour_labour_unions_and_employment.txt
              process_reminders_from_file .6.2.population_labour_labour_unions_and_employment.txt
              STATE_FILE=".s_economics_6_2"
              process_file .6.2.population_labour_labour_unions_and_employment.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .6.2.population_labour_labour_unions_and_employment.txt
              sed -i '/^1$/!d' .s_economics_6_2
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Economics/S6"
              # Define the file extension
              file_extension_question=".2.population_labour_labour_unions_and_employment.qns.txt"
              # Define the revision file
              revision_file="../../economics_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            3)
              if ! [ -f ".economics.6.2" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Economics/S6"
                # Define the file extension
                file_extension_answer=".2.population_labour_labour_unions_and_employment.ans.txt"
                # Define the exercise file
                exercise_file="../../economics_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_economics_6_3" ]; then
                echo -e "\n\nYou have made a choice to cover Money, banking and inflation ...\n\nWe are so exited to have you with us ${g}darling${t}\n\nRemember that hard work forever pays \c" && wait_for_a_key_press
              fi
              cp "Notes/Economics/6.3.money_banking_and_inflation.txt" . || exit 1
              mv 6.3.money_banking_and_inflation.txt .6.3.money_banking_and_inflation.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .6.3.money_banking_and_inflation.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .6.3.money_banking_and_inflation.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .6.3.money_banking_and_inflation.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .6.3.money_banking_and_inflation.txt
              process_reminders_from_file .6.3.money_banking_and_inflation.txt
              STATE_FILE=".s_economics_6_3"
              process_file .6.3.money_banking_and_inflation.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .6.3.money_banking_and_inflation.txt
              sed -i '/^1$/!d' .s_economics_6_3
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Economics/S6"
              # Define the file extension
              file_extension_question=".3.money_banking_and_inflation.qns.txt"
              # Define the revision file
              revision_file="../../economics_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
						;;
            4)
              if ! [ -f ".economics.6.3" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Economics/S6"
                # Define the file extension
                file_extension_answer=".3.money_banking_and_inflation.ans.txt"
                # Define the exercise file
                exercise_file="../../economics_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_economics_6_4" ]; then
                echo -e "\n\nYou did qualify to probe into the realm of Public finance and fiscal policy ...\n\nWe do treasure you ${g}darling${t}. Just never forget, that no matter how prepared you are, to win gold, you have to follow instructions! \c" && wait_for_a_key_press
              fi
              cp "Notes/Economics/6.4.public_finance_and_fiscal_policy.txt" . || exit 1
              mv 6.4.public_finance_and_fiscal_policy.txt .6.4.public_finance_and_fiscal_policy.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .6.4.public_finance_and_fiscal_policy.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .6.4.public_finance_and_fiscal_policy.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .6.4.public_finance_and_fiscal_policy.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .6.4.public_finance_and_fiscal_policy.txt
              process_reminders_from_file .6.4.public_finance_and_fiscal_policy.txt
              STATE_FILE=".s_economics_6_4"
              process_file .6.4.public_finance_and_fiscal_policy.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .6.4.public_finance_and_fiscal_policy.txt
              sed -i '/^1$/!d' .s_economics_6_4
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Economics/S6"
              # Define the file extension
              file_extension=".4.public_finance_and_fiscal_policy.qns.txt"
              # Define the revision file
              revision_file="../../economics_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension" "$revision_file"
              ...
              question_directory="Revision/Economics/S6"
              # Define the file extension
              file_extension_question=".4.public_finance_and_fiscal_policy.qns.txt"
              # Define the revision file
              revision_file="../../economics_covered_qns.txt"
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
            ;;
            5)
              if ! [ -f ".economics.6.4" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Economics/S6"
                # Define the file extension
                file_extension_answer=".4.public_finance_and_fiscal_policy.ans.txt"
                # Define the exercise file
                exercise_file="../../economics_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_economics_6_5" ]; then
                echo -e "\n\nHere you are dear one... Stay organised as you explore International trade ...\n\n${g}Just know we are not going to leave you alone${t}\n\nWe promise to always be right here for you \c" && wait_for_a_key_press
              fi
              cp "Notes/Economics/6.5.international_trade.txt" . || exit 1
              mv 6.5.international_trade.txt .6.5.international_trade.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .6.5.international_trade.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .6.5.international_trade.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .6.5.international_trade.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .6.5.international_trade.txt
              process_reminders_from_file .6.5.international_trade.txt
              STATE_FILE=".s_economics_6_5"
              process_file .6.5.international_trade.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .6.5.international_trade.txt
              sed -i '/^1$/!d' .s_economics_6_5
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Economics/S6"
              # Define the file extension
              file_extension_question=".5.international_trade.qns.txt"
              # Define the revision file
              revision_file="../../economics_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
            ...
            ;;
            6)
              if ! [ -f ".economics.6.5" ]; then
                attempts=0
                # Define the targeted directory
                answered_directory="Exercise/Economics/S6"
                # Define the file extension
                file_extension_answer=".5.international_trade.ans.txt"
                # Define the exercise file
                exercise_file="../../economics_answered_ans.txt"
                # Call the function to process a random question
                process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
              ...
              if ! [ -f ".s_economics_6_6" ]; then
                echo -e "\n\nYou have managed to make it to Economic development planning ...\n\n${g}Remember to pray always${t}\n\nThe fear of the Lord is the beginning of wisdom \c" && wait_for_a_key_press
              fi
              cp "Notes/Economics/6.6.economic_development_planning.txt" . || exit 1
              mv 6.6.economic_development_planning.txt .6.6.economic_development_planning.txt || exit 1
              sed -i -e 's/\.\( \+\)/;/g' -e '/https:/! s/\([!?:]\)/\1;/g' -e 's/\([;]\) /\1/g' .6.6.economic_development_planning.txt
              sed -i 's/;\([:!?]\);/\;\1/g' .6.6.economic_development_planning.txt
              sed -i 's/;\([0-9]*\);/;\1. /g' .6.6.economic_development_planning.txt
              sed -i -E 's/(\([^)]*);/\1/g; s/(\[[^]]*);/\1/g; s/(\{[^}]*);/\1/g' .6.6.economic_development_planning.txt
              process_reminders_from_file .6.6.economic_development_planning.txt
              STATE_FILE=".s_economics_6_6"
              process_file .6.6.economic_development_planning.txt
              contact_ai
              if [ -f .resume_to_class ]; then
                break
              fi
              ...
              rm -f .6.6.economic_development_planning.txt
              sed -i '/^1$/!d' .s_economics_6_6
              attempts=0
              # Define the targeted directory
              question_directory="Revision/Economics/S6"
              # Define the file extension
              file_extension_question=".6.economic_development_planning.qns.txt"
              # Define the revision file
              revision_file="../../economics_covered_qns.txt"
              # Call the function to process a random question
              process_random_short_answer_question "$question_directory" "$file_extension_question" "$revision_file"
              ...
              # Call the function to process a random question
              process_random_aoi "$question_directory" "$file_extension_question" "$revision_file"
              if ! [ -f ".economics.6.6" ]; then
	              attempts=0
	              # Define the targeted directory
	              answered_directory="Exercise/Economics/S6"
	              # Define the file extension
	              file_extension_answer=".6.economic_development_planning.ans.txt"
	              # Define the exercise file
	              exercise_file="../../economics_answered_ans.txt"
	              # Call the function to process a random answer
	              process_question_answer "$answered_directory" "$file_extension_answer" "$exercise_file"
								touch .economics.6.6
								echo "6" > .economics_ready
							fi
            ;;
